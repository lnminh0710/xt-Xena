    ...
    var ClientPrintJob = (function () {
        ...
        ClientPrintJob.prototype._genPFGArrayAsync = function (printFileGroup) {
            ...
            return new Promise(function (resolve, reject) {
                if (!zip)
                    reject(
                        "zip.js, zip-ext.js, and deflate.js files from https://github.com/gildas-lormeau/zip.js project are missing."
                    );
                else {
                    ...
                    zip.createWriter(
                        ...
                        function (zipWriter) {
                            ...
                            if (printFileGroup.length != 0) addPrintFile2Zip(0);
                        },
                        function (e) {
                            reject(e);
                        }
        ...
        ClientPrintJob.prototype._genPCArrayAsync = function (
            printerCommands,
            binPrinterCommands,
            printerCopies
        ) {
            ...
            return new Promise(function (resolve, reject) {
                try {
                    ...
                    if (
                        binPrinterCommands != null &&
                        binPrinterCommands.length > 0
                    )
                        resolve(new Blob([pcc_copies, binPrinterCommands]));
                    if (printerCommands.length != 0) {
                        var binPC = new Uint8Array(
                            _this._str2UTF8Array(printerCommands)
                        );
        ...
        ClientPrintJob.prototype._str2UTF8Array = function (str) {
            ...
            for (var i = 0; i < str.length; i++) {
                ...
                if (
                    ...
                    utf8.push(0xbf);
                    i += 2;
                } else if (charcode < 0x100) utf8.push(charcode);
        ...
        ClientPrintJob.prototype._genPrinterArrayAsync = function (
            clientPrinter
        ) {
            var _this = this;
            return new Promise(function (resolve, reject) {
                try {
                    ...
                    var toRet = new Uint8Array(
                        _this._str2UTF8Array(clientPrinter.serialize())
                    );
                    resolve(toRet);
                } catch (e) {
                    reject(e);
        ...
        ClientPrintJob.prototype._generateDataAsync = function () {
            ...
            return new Promise(function (resolve, reject) {
                ...
                Promise.race([
                    _this._genPCArrayAsync(
                        _this._printerCommands,
                        _this.binaryPrinterCommands,
                        _this._printerCommandsCopies
                    ),
                    _this._genPFGArrayAsync(_this._printFileGroup),
                ])
                    .then(function (file_data) {
                        _this
                            ._genPrinterArrayAsync(_this._clientPrinter)
                            .then(function (printer_data) {
                                var idx1 = _this._intToByteArray(
                                    file_data.size
                                );
                                var idx2 = _this._intToByteArray(
                                    file_data.size + printer_data.length
                                );
                                resolve(
                                    new Blob([
                                        header,
    ...
    var ClientPrintJobGroup = (function () {
        ...
        ClientPrintJobGroup.prototype._generateMiniJob = function (cj) {
            ...
            return new Promise(function (ok, error) {
                Promise.race([
                    cj._genPCArrayAsync(
                        cj.printerCommands,
                        cj.binaryPrinterCommands,
                        cj.printerCommandsCopies
                    ),
        ...
        ClientPrintJobGroup.prototype._generateDataAsync = function () {
            ...
            return new Promise(function (resolve, reject) {
                ...
                var jobs_qty = new Uint8Array(
                    _this._intToArray(_this.jobs.length)
                );
                var licenseTD = JSPM.JSPrintManager.license_url;
                var promises = [];
...
(function (JSPM) {
    ...
    var InstalledPrinter = (function () {
        ...
        InstalledPrinter.prototype.bool2str = function (
            value,
            true_val,
            false_val
        ) {
        ...
        InstalledPrinter.prototype.serialize = function () {
            ...
            return (
                ...
                this._name +
                SEP +
                this.bool2str(this._printDefault, "1", "0") +
                SEP +
                this._tray +
                SEP +
                this._paper
    ...
    var NetworkPrinter = (function () {
        function NetworkPrinter(port, ipAddress, dnsName) {
            ...
            if (ipAddress) this._networkIPAddress = ipAddress;
            if (dnsName) this._dnsName = dnsName;
        ...
        NetworkPrinter.prototype.serialize = function () {
            ...
            if (!(this.dnsName || this.ipAddress))
                throw "You have to specify an IP address or a DNS name";
            return (
                this.Id + this.dnsName + SEP + this.ipAddress + SEP + this.port
            );
        };
...
(function (JSPM) {
    ...
    (function (WSStatus) {
        ...
        WSStatus[(WSStatus["WaitingForUserResponse"] = 3)] =
    ...
    (function (PrintOrientation) {
        ...
        (PrintOrientation =
            JSPM.PrintOrientation || (JSPM.PrintOrientation = {}))
    );
})(JSPM || (JSPM = {}));
(function (JSPM) {
    var Serial;
    (function (Serial) {
        ...
        (function (Handshake) {
            ...
            Handshake[(Handshake["RequestToSendXOnXOff"] = 2)] =
    ...
    var JSPMWebSocket = (function () {
        ...
        JSPMWebSocket.prototype._onMessage = function (e, job_list) {
            try {
                ...
                if (!job) throw "Job " + json_data.id + "doesn't exist";
        ...
        JSPMWebSocket.prototype._pingPong = function () {
            ...
            setInterval(function (_) {
                if (_this._status != JSPM.WSStatus.Open) return;
                _this.send("ping");
            }, 30000);
        };
        JSPMWebSocket.prototype._onClose = function (e, __this) {
            var _this = this;
            if (e.code == 403) this._status = JSPM.WSStatus.BlackListed;
            else {
        ...
        JSPMWebSocket.prototype._send = function (data, params, ok, err) {
            ...
            if (data instanceof Blob) {
                var job_id = new Uint8Array(
                    ("id" + id).split("").map(function (a) {
                ...
                try {
                    ...
                    _this._ws.onopen = function (i) {
                        ...
                        if (JSPM.JSPrintManager.cache_license_at_start) {
                            JSPM.JSPrintManager.cacheLicense(
        ...
        JSPMWebSocket.prototype.send = function (data, params) {
            ...
            return new Promise(function (ok, err) {
                if (_this._status == JSPM.WSStatus.Closed)
                    ...
                    err(
                        "The site is blacklisted and the connection was closed"
                    );
...
(function (JSPM) {
    var JSPrintManager = (function () {
        ...
        JSPrintManager.start = function (secure, host, port) {
            ...
            if (!this.WS)
                this.WS = new JSPM.JSPMWebSocket(
                    host,
                    port,
                    secure,
                    this.auto_reconnect
...
(function (JSPM) {
    var PrintFilePDF = (function (_super) {
        ...
        function PrintFilePDF(fileContent, fileContentType, fileName, copies) {
            var _this =
                _super.call(
                    ...
                    fileContent,
                    fileContentType,
                    fileName.substring(0, fileName.lastIndexOf(".")) + ".wpdf",
                    copies
                ) || this;
        ...
        PrintFilePDF.prototype.isValidRange = function (range) {
            ...
            if (test[0].length != range.length) return false;
            return true;
        };
                    ...
                    case JSPM.FileSourceType.Base64:
                        {
                            try {
                                ...
                                err(
                                    "Error trying to decode the base64 data.\n" +
                                        e
                                );
                            }
                        }
                    ...
                    case JSPM.FileSourceType.Text:
                        {
                            try {
                                var bytes = new Uint8Array(
                                    _this.fileContent.length
                                );
                                for (
                                    var i = 0;
                                    i < _this.fileContent.length;
                                    i++
                                ) {
                                ...
                                err(
                                    "Error trying to decode the text data.\n" +
                                        e
                                );
                            }
                        }
            ...
            return new Promise(function (ok, err) {
                ...
                _this
                    ...
                    .then(function (file_content) {
                        var params_arr = new Uint8Array(
                            (
                                _this.bool2str(_this.printAsGrayscale) +
        ...
        function PrintFileTXT(fileContent, fileName, copies) {
            var _this =
                _super.call(
                    ...
                    fileName.substring(0, fileName.lastIndexOf(".")) + ".wtxt",
                    copies
                ) || this;
            _this.textContent = "";
        ...
        PrintFileTXT.prototype.serialize = function () {
            ...
            return new Promise(function (ok, err) {
                ...
                var params =
                    ...
                    _this.fontSize +
                    SEP +
                    _this.bool2str(_this.fontBold) +
                    SEP +
                    _this.bool2str(_this.fontItalic) +
                    SEP +
                    _this.bool2str(_this.fontUnderline) +
                    SEP +
                    _this.bool2str(_this.fontStrikethrough) +
                    SEP +
                    _this.fontColor +
                    SEP +
                    _this.marginLeft +
