...
export function tabSummaryReducer(state = initialState, action: CustomAction): TabSummaryState {
    let feature = baseReducer.getFeature(action, state, initialSubTabSummaryState);
    ...
    switch (action.type) {
        ...
        case TabSummaryActions.UNCHECK_TAB_HEADER_TABLE_FILTER_LIST: {
            state = baseReducer.updateStateData(action, feature, state, {
                tabHeaderTableFilterList: uncheckAllTabHeaderTableFilterList(feature.tabHeaderTableFilterList)
            });
            return Object.assign({}, state);
        }
