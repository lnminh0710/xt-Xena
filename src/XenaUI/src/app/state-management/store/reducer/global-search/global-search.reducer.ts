...
export function persistStateReducer(
  _reducer: ActionReducer<GlobalSearchState>
) {
...
export function updateStateReducer(_reducer: ActionReducer<GlobalSearchState>) {
  return (state: GlobalSearchState | undefined, action: Action) => {
    if (
      action.type ===
      GlobalSearchActions.UPDATE_GLOBAL_SEARCH_STATE_FROM_LOCAL_STORAGE
    ) {
...
export function globalSearchStateReducer(
  state = initialState,
  action: CustomAction
): GlobalSearchState {
  return updateStateReducer(persistStateReducer(globalSearchReducer))(
    state,
    action
  );
}
