...
export function propertyPanelStateReducer(state = initialState, action: CustomAction): PropertyPanelState {
    let feature = baseReducer.getFeature(action, state, initialSubPropertyPanelState);
        ...
        case PropertyPanelActions.UPDATE_PROPERTIES: {
            ...
            state = baseReducer.updateStateData(action, feature, state, {
                requestUpdateProperties: {
                    widgetData: payload ? new LightWidgetDetail(payload.widgetData) : null,
...
export function persistPropertyPanelStateReducer(_reducer: ActionReducer<PropertyPanelState>) {
    return (state: PropertyPanelState | undefined, action: Action) => {
        ...
        const isGlobalProperties = nextState['features'][moduleName] && nextState['features'][moduleName].isGlobal;
        switch (action.type) {
            case PropertyPanelActions.REQUEST_APPLY:
            case PropertyPanelActions.REQUEST_SAVE:
            case PropertyPanelActions.REQUEST_SAVE_GLOBAL:
            case PropertyPanelActions.UPDATE_PROPERTIES:
                if (location.pathname != '/widget') {
                    ...
                    const requestSaveGlobal = nextState['features'][moduleName].requestSaveGlobal;
                    const requestUpdateProperties = nextState['features'][moduleName].requestUpdateProperties;
                    ...
                    if (requestUpdateProperties && requestUpdateProperties.widgetProperties) {
                        nextState['features'][moduleName].properties = {
                            ...Uti.filterValueWhenSaveWidget(requestUpdateProperties.widgetProperties),
                            moduleName
                        };
                    }
                    localStorage.setItem(LocalStorageKey.buildKey(LocalStorageKey.LocalStorageWidgetPropertyKey, nextState['browserTabId']), JSON.stringify(nextState));
                }
                break;
            case PropertyPanelActions.UPDATE_TEMP_PROPERTIES:
                if (location.pathname != '/widget') {
                    nextState['browserTabId'] = Uti.defineBrowserTabId();
                    nextState['features'][moduleName].tempProperties =  {...Uti.filterValueWhenSaveWidget(nextState['features'][moduleName].tempProperties), moduleName};
                    localStorage.setItem(LocalStorageKey.buildKey(LocalStorageKey.LocalStorageWidgetTempPropertyKey, nextState['browserTabId']), JSON.stringify(nextState));
                }
                break;
            case PropertyPanelActions.UPDATE_ORIGINAL_PROPERTIES:
                if (location.pathname != '/widget') {
                    nextState['browserTabId'] = Uti.defineBrowserTabId();
                    localStorage.setItem(LocalStorageKey.buildKey(LocalStorageKey.LocalStorageWidgetOriginalPropertyKey, nextState['browserTabId']), JSON.stringify({value: Uti.getTempId()}));
                }
                break;
        }
        return nextState;
...
export function updatePropertyPanelStateReducer(_reducer: ActionReducer<PropertyPanelState>) {
    return (state: PropertyPanelState | undefined, action: Action) => {
        if (action.type === PropertyPanelActions.UPDATE_PROPERTY_STATE_FROM_LOCAL_STORAGE) {
...
export function propertyPanelReducer(state = initialState, action: CustomAction): PropertyPanelState {
    return updatePropertyPanelStateReducer(persistPropertyPanelStateReducer(propertyPanelStateReducer))(state, action);
};
