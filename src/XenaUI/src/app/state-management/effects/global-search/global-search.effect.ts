...
export class GlobalSearchEffects {
        ...
        .map((action: any) => {
            ...
            localStorage.setItem(
                LocalStorageKey.buildKey(
                    LocalStorageKey.LocalStorageGSStepKey,
    ...
    onChange = Observable.fromEvent<StorageEvent>(window, "storage")
        .filter((evt) => {
            return (
                evt.key ==
                    LocalStorageKey.buildKey(
        ...
        .map((evt) => {
            ...
            if (browserTabId && browserTabId != Uti.defineBrowserTabId())
                return;

            switch (type) {
                case GlobalSearchActions.SEARCH_KEYWORD:
                    return {
                        type: GlobalSearchActions.SEARCH_KEYWORD_STORAGE,
                        payload,
                        browserTabId,
                    };
                case GlobalSearchActions.CHANGE_MODULE_TAB:
                    return {
                        type: GlobalSearchActions.CHANGE_MODULE_TAB_STORAGE,
                        payload,
                        browserTabId,
                    };
                case GlobalSearchActions.CLOSE_MODULE_TAB:
                    return {
                        type: GlobalSearchActions.CLOSE_MODULE_TAB_STORAGE,
                        payload,
                        browserTabId,
                    };
                case GlobalSearchActions.ACTIVE_MODULE_TAB:
                    return {
                        type: GlobalSearchActions.ACTIVE_MODULE_TAB_STORAGE,
                        payload,
                        browserTabId,
                    };
                case GlobalSearchActions.CLOSE_POPUP:
                    return {
                        type: GlobalSearchActions.CLOSE_POPUP_STORAGE,
                        payload,
                        browserTabId,
                    };
                case GlobalSearchActions.MENU_CONTEXT_ACTION:
                    return {
                        type: GlobalSearchActions.MENU_CONTEXT_ACTION_STORAGE,
                        payload,
                        browserTabId,
                    };
                case GlobalSearchActions.ROW_DOUBLE_CLICK:
                    return {
                        type: GlobalSearchActions.ROW_DOUBLE_CLICK_STORAGE,
                        payload,
                        browserTabId,
                    };
                default:
