...
export class WidgetTemplateSettingEffects {
    ...
    @Effect() loadWidgetTemplateSetting$ = this.update$
        .ofType<CustomAction>(
            WidgetTemplateActions.LOAD_All_WIDGET_TEMPLATE_SETTING_BY_MODULE_ID
        )
        .map((action) => {
        ...
        .switchMap((moduleId: number) =>
            this.widgetTemplateService.getAllWidgetTemplateByModuleId(
                moduleId.toString()
            )
        )
        .map((widgetTemplateSettings: WidgetTemplateSettingModel[]) => {
            return this.widgetTemplateActions.loadWidgetTemplateSettingSuccess(
