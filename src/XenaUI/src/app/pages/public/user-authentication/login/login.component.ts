...
export class LoginComponent implements OnInit {
    ...
    this.httpClient
      ...
      .then((data: any) => {
        ...
        this.authenticationService
          .login(this.prepareLoginData(data && data.ip ? data.ip : ''))
          .subscribe(
      ...
      .catch((error) => {
        this.authenticationService.login(this.prepareLoginData()).subscribe(
          (data) => this.loginSuccess(data.item),
          (error) => this.loginError(error)
        );
      });
  ...
  private prepareLoginData(ip = ''): any {
    ...
    if (
      this.compareUserRemember() &&
      this.checkUserRememberKey() &&
      !this.isPasswordChanged
    ) {
      const dataUser = JSON.parse(
        this._commonService.decrypt(valueUserRemember)
      );
      return new User({
  ...
  private checkRememberLogin() {
    if (this.checkUserRememberKey()) {
      const valueUserRemember = localStorage.getItem(
        this.consts.userRememberKey
      );
      const dataUser = JSON.parse(
        this._commonService.decrypt(valueUserRemember)
      );
  ...
  private checkUserRememberKey() {
    ...
    if (valueUserRemember) {
      const valueRemember =
        valueUserRemember === 'null' ? null : valueUserRemember;
  ...
  private compareUserRemember() {
    ...
    if (!!valueUserRemember) {
      ...
      if (dataUser.loginName === this.model.loginName) return true;
    }
    return false;
  }
  ...
  private loginSuccess(userAuthentication: any) {
    ...
    if (
      ...
      if (
        (!this.checkUserRememberKey() && this.model.remember) ||
  ...
  private haveMessage(userAuthentication: UserAuthentication): boolean {
    if (
      !userAuthentication.message_type ||
      isNaN(parseInt(userAuthentication.message_type))
    )
      return;
    localStorage.removeItem(this.consts.localStorageCurrentUser);
    localStorage.setItem(
      this.consts.localStorageCurrentUserExpire,
      ...
      case 2: {
        if (this.model && this.model.loginName)
          this.router.navigate([
            this.consts.accountExpireUrl + '/' + this.model.loginName,
          ]);
        break;
      }
      // Account is denied
  ...
  private saveNearExpireMessageSuccess(
    ...
    this.currentUserGlobalSetting = globalSettingModels.find(
      (x) =>
        x.globalName ==
        this.globalSettingConstant.settingUserNoticeMessage +
          this.currentUserInfo.loginName
    );
    if (
      ...
      let currentUserNoticeMessage = JSON.parse(
        this.currentUserGlobalSetting.jsonSettings
      ) as Message[];
      ...
      this.currentUserGlobalSetting.jsonSettings = JSON.stringify(
        currentUserNoticeMessage
      );
    }

    this.globalSettingSer
      .saveGlobalSetting(this.currentUserGlobalSetting)
      .subscribe(
