...
export class AccountExpireComponent implements OnInit {
	...
	ngOnInit() {
        ...
        this.route.params.subscribe(params => this.loginName = params[this.consts.urlPramLoginName])
        this.loginUrl = this.consts.loginUrl;
	}

	submit() {
		if (!this.model.postMessage) { return; }
		this.authenticationService.sendExpireMessage(this.loginName, this.model.postMessage)
			.subscribe(
