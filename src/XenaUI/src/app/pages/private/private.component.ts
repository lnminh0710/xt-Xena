    ...
    implements OnInit, OnDestroy
{
    ...
    constructor(
        ...
        this.toggleDashboardLoadingState = store.select(
            (state) =>
        ...
        this.countDownClock =
            this.consts.defaultCountDownTime * 60 +
            this.consts.remainCountDownTime;
        ...
        this.layoutInfoModelSubscription = this.layoutInfoModel.subscribe(
            (layoutInfoState: SubLayoutInfoState) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.makeSpaceForTabButton =
                        layoutInfoState.makeSpaceForTabButton.isActive;
    ...
    private subscribeIsGlobalState() {
        this.isGlobalStateSubscription = this.isGlobalState.subscribe(
    ...
    private subscribePropertiesParentDataState() {
        this.propertiesParentDataStateSubscription =
            this.propertiesParentDataState.subscribe(
                (propertiesParentDataState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        this.propertiesParentData =
                            propertiesParentDataState || {};
                    });
                }
            );
    }
    ...
    private subscribePropertiesState() {
        this.propertiesStateSubscription = this.propertiesState.subscribe(
            (propertiesState: WidgetPropertyModel[]) => {
                this.appErrorHandler.executeAction(() => {
                    if (propertiesState) {
                        if (propertiesState["moduleName"])
                            return this.properties;
                        setTimeout(() => {
                            this.properties = this.isGlobal
                                ? propertiesState
                                : this.widgetUtils.mapProperty(propertiesState);
                        });
                    } else {
                        this.properties = [];
                    }
    ...
    private subscribeShowFeedbackClickedState() {
        this.showFeedbackClickedStateSubscription = this.dispatcher
        ...
        this.feedbackClickedDataStateSubscription = this.dispatcher
            .filter((action: CustomAction) => {
    ...
    private subscribeToggleDashboardLoadingState() {
        this.toggleDashboardLoadingStateSubscription =
            this.toggleDashboardLoadingState.subscribe(
                (toggleDashboardLoadingState: any) => {
                    this.appErrorHandler.executeAction(() => {
    ...
    public onPropertyPanelClose(event) {
        ...
        this.store.dispatch(
            this.additionalInformationActions.backToPreviousState(this.ofModule)
        );
    }
    ...
    public onPropertyPanelSave(event) {
        if (!this.isGlobal) {
            this.store.dispatch(
                this.propertyPanelActions.requestSave(event, this.ofModule)
            );
        } else {
            this.store.dispatch(
                this.propertyPanelActions.requestSaveGlobal(
                    this.properties,
                    ModuleList.Base
                )
            );
        ...
        if (event && this.isGlobal) {
            ...
            this.store.dispatch(
                this.propertyPanelActions.requestUpdateGlobalProperty(
                    event.widgetProperties,
                    ModuleList.Base
                )
            );
        }
        this.store.dispatch(
            this.propertyPanelActions.updateProperties(event, this.ofModule)
        );
    }

    public onPropertyPanelApply(event) {
        this.store.dispatch(
            this.propertyPanelActions.requestApply(event, this.ofModule)
        );
    }

    public handleToggleWidgetTemplate(event) {
        this.store.dispatch(
            this.tabButtonActions.toggle(!event, this.ofModule)
        );
    }

    private updateContentStyle(layoutInfo: SubLayoutInfoState) {
        let contentStyle = `calc(100vw - ${
            this.ofModule && this.ofModule.idSettingsGUI != -1
                ? layoutInfo.parkedItemWidth
                : 0
        }px - ${layoutInfo.rightMenuWidth}px`;
        if (this.isExpand) {
