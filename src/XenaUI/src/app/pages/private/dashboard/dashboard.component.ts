  ...
  implements OnInit, OnDestroy, AfterViewInit
{
    ...
    this.commonService
      ...
      .subscribe((response: ApiResultResponse) => {
        this.appErrorHandler.executeAction(() => {
          ...
          this.store.dispatch(
            this.xnCommonActions.getModuleToPersonType(result, this.ofModule)
          );
        });
      });
  }
    ...
    this.selectedParkedItemStateSubscription =
      this.selectedParkedItemState.subscribe((selectedParkedItemState: any) => {
        this.appErrorHandler.executeAction(() => {
          ...
          if (
            isEqual(this.selectedParkedItem, selectedParkedItemState) &&
      ...
      this.selectedSearchResultState.subscribe(
        (selectedSearchResultState: SearchResultItemModel) => {
          this.appErrorHandler.executeAction(() => {
            ...
            if (
              isEqual(this.selectedSearchResult, selectedSearchResultState) &&
        ...
        (selectedGoToModuleItemState: any) => {
          this.appErrorHandler.executeAction(() => {
            ...
            if (
              ...
              !Uti.isAllPropertiesEmpty(selectedGoToModuleItemState)
            ) {
              this.store.dispatch(
                this.processDataActions.setSelectedEntity(
                  this.ofModule,
      ...
      .subscribe(() => {
        this.appErrorHandler.executeAction(() => {
          if (!this.isHiddenParkedItem) {
            ...
            this.store.dispatch(
              this.parkedItemActions.addDraftItem(newParkedItem, this.ofModule)
            );
          } else {
            this.turnEditMode();
            this.newMainTab();
      ...
      this.isHiddenParkedItemState.subscribe(
        (isHiddenParkedItemState: boolean) => {
          this.appErrorHandler.executeAction(() => {
            ...
            if (this.isHiddenParkedItem) {
              this.store.dispatch(
                this.parkedItemActions.removeAllParkedItem(this.ofModule)
              );
            }
          });
        }
      ...
      (activeModuleState: Module) => {
        this.appErrorHandler.executeAction(() => {
          ...
          if (this.ofModule && this.ofModule.moduleNameTrim !== 'Base') {
            ...
            if (
              this.ofModule.moduleNameTrim != activeModuleState.moduleNameTrim
            ) {
              this.ref.detach();
              return;
            } else {
            ...
            if (activeModuleState.idSettingsGUI != MenuModuleId.backoffice) {
              this.store.dispatch(
                this.moduleActions.setUsingModule(activeModuleState)
              );
    ...
    this.activeSubModuleStateSubscription = this.activeSubModuleState.subscribe(
      (activeSubModuleState: Module) => {
        this.appErrorHandler.executeAction(() => {
          ...
          // Change other module, then detach the old module to disable detection changes check.
          if (
            this.ofModule.moduleNameTrim != activeSubModuleState.moduleNameTrim
          ) {
            this.ref.detach();
            return;
          } else {
          ...
          switch (activeSubModuleState.idSettingsGUIParent) {
            ...
            case MenuModuleId.logistic:
              this.store.dispatch(
                this.moduleActions.setUsingModule(activeSubModuleState)
              );
  ...
  private subcribeRequestDownloadPdfState() {
    this.requestDownloadPdfStateSubscription =
      this.requestDownloadPdfState.subscribe((requestDownloadPdfState: any) => {
        this.appErrorHandler.executeAction(() => {
  ...
  private subscribeIsShowTabButtonState() {
    this.isShowTabButtonStateSubscription = this.isShowTabButtonState.subscribe(
