...
import {
    ...
    MessageModel,
    ApiResultResponse,
    AccessRightModel, GlobalSettingModel
} from 'app/models';
...
export class DashboardComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private selectedGoToModuleItem: any = null;
    ...
    private userModuleLayoutSetting: any;
    ...
    constructor(
        ...
        this.tabHeaderDataModel = store.select(state => tabSummaryReducer.getTabSummaryState(state, this.ofModule.moduleNameTrim).tabs);
        ...
        this.viewModeState = store.select(state => processDataReducer.getProcessDataState(state, this.ofModule.moduleNameTrim).viewMode);
        ...
        this.requestDownloadPdfState = store.select(state => backofficeReducer.getBackofficeState(state, this.ofModule.moduleNameTrim).requestDownloadPdf);
        this.requestGoToTrackingPageState = store.select(state => backofficeReducer.getBackofficeState(state, this.ofModule.moduleNameTrim).requestGoToTrackingPage);
        this.requestOpenReturnRefundModuleState = store.select(state => backofficeReducer.getBackofficeState(state, this.ofModule.moduleNameTrim).requestOpenReturnRefundModule);
            ...
            (response) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (allModuleSettings && allModuleSettings.length > 0) {
                        ...
                        if (moduleSettingItem && moduleSettingItem.idSettingsGlobal && moduleSettingItem.globalName) {
                            ...
                            if (moduleSettingItem) {
                                ...
                                this.store.dispatch(this.moduleSettingActions.loadModuleSettingSuccess(afterMergeModule, this.ofModule));
                            }
                        }
                    }
                    ...
                    if (isLoadModuleSetting) {
                        ...
                        this.store.dispatch(this.moduleSettingActions.loadModuleSetting(
                            this.ofModule,
                            null,
                            null,
            ...
            this.appErrorHandler.executeAction(() => {
                if (isEmpty(selectedParkedItemState) && !isEmpty(this.selectedParkedItem)) {
                    ...
                    if (!this.selectedEntity && !this.selectedSearchResult) {
                        if (this.ofModule.idSettingsGUI != ModuleList.OrderDataEntry.idSettingsGUI && this.ofModule.idSettingsGUI != ModuleList.StockCorrection.idSettingsGUI) {
                ...
                if (isEqual(this.selectedParkedItem, selectedParkedItemState) && !Uti.isAllPropertiesEmpty(selectedParkedItemState)) {
                    this.store.dispatch(this.processDataActions.setSelectedEntity(this.ofModule, selectedParkedItemState, true, this.modulePrimaryKey));
                    return;
                }
                ...
                if (!isEmpty(this.selectedParkedItem)) {
                    if (this.selectedParkedItem.isNew) {
                        this.turnEditMode();
                        this.newMainTab();
                        this.newTabConfig = this.tabService.createNewTabConfig(true, this.ofModule, this.selectedTab, null, null, this.contentDisplayMode);
                    } else {
                        if (this.selectedParkedItem &&
                            this.selectedParkedItem.id &&
                            this.selectedParkedItem.id.value) {

                            this.selectedEntity = this.selectedParkedItem;

                            this.store.dispatch(this.processDataActions.setSelectedEntity(this.ofModule, this.selectedEntity, true, this.modulePrimaryKey));
                            ...
                            this.selectedSearchResult[this.modulePrimaryKey])) {
    ...
    private subcribeSelectedSearchResultState() {
        this.selectedSearchResultStateSubscription = this.selectedSearchResultState.subscribe((selectedSearchResultState: SearchResultItemModel) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (isEqual(this.selectedSearchResult, selectedSearchResultState) && !Uti.isAllPropertiesEmpty(selectedSearchResultState)) {
                    this.store.dispatch(this.processDataActions.setSelectedEntity(this.ofModule, selectedSearchResultState, false, this.modulePrimaryKey));
                    return;
                }
                ...
                if (!isEmpty(this.selectedSearchResult)) {
                    if (this.selectedSearchResult &&
                        ...
                        this.store.dispatch(this.processDataActions.setSelectedEntity(this.ofModule, this.selectedEntity, false, this.modulePrimaryKey));
                    ...
                    } else if (this.selectedParkedItem &&
                        this.selectedParkedItem.id &&
                        this.selectedParkedItem.id.value) {
    ...
    private subscribeSelectedGoToModuleItemState() {
        this.selectedGoToModuleItemStateSubscription = this.selectedGoToModuleItemState.subscribe((selectedGoToModuleItemState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (isEmpty(selectedGoToModuleItemState) && !isEmpty(this.selectedGoToModuleItem)) {
                    this.selectedGoToModuleItem = null;
                    this.selectedEntity = null;

                    if (!this.selectedEntity && !this.selectedParkedItem && !this.selectedSearchResult) {
                        this.store.dispatch(this.tabSummaryActions.removeAllTabs(this.ofModule));
                        this.store.dispatch(this.processDataActions.setSelectedEntity(this.ofModule, null));
                    }
                    return;
                ...
                if (isEqual(this.selectedGoToModuleItem, selectedGoToModuleItemState) && !Uti.isAllPropertiesEmpty(selectedGoToModuleItemState)) {
                    this.store.dispatch(this.processDataActions.setSelectedEntity(this.ofModule, selectedGoToModuleItemState, true, this.modulePrimaryKey));
                    return;
                }

                this.selectedGoToModuleItem = cloneDeep(selectedGoToModuleItemState);

                if (!isEmpty(this.selectedGoToModuleItem)) {
                    if (this.selectedGoToModuleItem &&
                        this.selectedGoToModuleItem[this.modulePrimaryKey]) {

                        this.selectedEntity = cloneDeep(this.selectedGoToModuleItem);
    ...
    private subcribeNewMainTabState() {
        this.newMainTabSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.NEW_MAIN_TAB && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.subModulesStateSubscription = this.subModulesState.subscribe((subModulesState: Module[]) => {
            this.appErrorHandler.executeAction(() => {
                if (!this.activeModule
                    || this.router.url.indexOf(this.activeModule.moduleNameTrim) === -1) {
                    return;
                }

                if (subModulesState.length
                    && (subModulesState[0].idSettingsGUI == -1 || subModulesState[0].idSettingsGUIParent != this.activeModule.idSettingsGUI)) {
                    return;
                }
        ...
        this.activeModuleStateSubscription = this.activeModuleState.subscribe((activeModuleState: Module) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (this.ofModule && this.ofModule.moduleNameTrim !== 'Base') {
                    ...
                    if (activeModuleState.idSettingsGUI != MenuModuleId.backoffice) {
                        this.store.dispatch(this.moduleActions.setUsingModule(activeModuleState));

                        this.activeModule = activeModuleState;

                        if (activeModuleState.idSettingsGUI == MenuModuleId.orderDataEntry
                            || activeModuleState.idSettingsGUI == MenuModuleId.systemManagement
                            || activeModuleState.idSettingsGUI == MenuModuleId.dashboard
                            || activeModuleState.idSettingsGUI == MenuModuleId.mailingData) {
    ...
    private subcribeActiveSubModuleState() {
        this.activeSubModuleStateSubscription = this.activeSubModuleState.subscribe((activeSubModuleState: Module) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if ((this.ofModule.idSettingsGUI == activeSubModuleState.idSettingsGUIParent)
                ...
                switch (activeSubModuleState.idSettingsGUIParent) {
                    ...
                    case MenuModuleId.logistic:
                        ...
                        if (activeSubModuleState.idSettingsGUI != 32) {
    ...
    private subcribeTabHeaderDataModel() {
        this.tabHeaderDataModelSubscription = this.tabHeaderDataModel.subscribe((tabHeaderDataModel: TabSummaryModel[]) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeRequestSaveState() {
        this.requestSaveStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subscribeNewOtherTabState() {
        this.newOtherTabSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.NEW_OTHER_TAB && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                this.turnEditMode();
                this.newOtherTab();
                this.newTabConfig = this.tabService.createNewTabConfig(false, this.ofModule, this.selectedTab, null, null, this.contentDisplayMode);
            });
        });
    }

    private subscribeNewSimpleTabState() {
        this.newSimpleTabSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.NEW_SIMPLE_TAB && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                this.turnEditMode();
                this.newSimpleTab();
                this.newTabConfig = this.tabService.createNewTabConfig(false, this.ofModule, this.selectedTab, this.selectedSimpleTab, this.tabSetting, this.contentDisplayMode);
            });
        });
    }
    ...
    private subcribeUpdateMainTabState() {
        this.updateMainTabSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.UPDATE_MAIN_TAB && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                this.turnEditMode();
    ...
    private subcribeRequestLoadTabsState() {
        this.requestLoadTabsSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === TabSummaryActions.REQUEST_LOAD_TABS && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeRequestDownloadPdfState() {
        this.requestDownloadPdfStateSubscription = this.requestDownloadPdfState.subscribe((requestDownloadPdfState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeRequestGoToTrackingPageState() {
        this.requestGoToTrackingPageStateSubscription = this.requestGoToTrackingPageState.subscribe((requestGoToTrackingPageState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeRequestOpenReturnRefundModuleState() {
        this.requestOpenReturnRefundModuleStateSubscription = this.requestOpenReturnRefundModuleState.subscribe((requestOpenReturnRefundModuleState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private downloadPDF(selectedEntity) {
        let fileName = selectedEntity.InvoicePDF || selectedEntity.PDF || selectedEntity.invoicePDF || selectedEntity.invoicePdf || selectedEntity.pdf;
    ...
    private gotoReturnRefundPage(selectedEntity) {
        ...
        // }));
        this.store.dispatch(this.backofficeActions.storeSelectedEntity(ModuleList.Backoffice, selectedEntity));
        if (!this.ofModule) return;
        if (this.ofModule.idSettingsGUI === MenuModuleId.returnRefund || this.ofModule.idSettingsGUI === MenuModuleId.orders) {
            this.store.dispatch(this.moduleActions.requestChangeSubModule(MenuModuleId.backoffice, MenuModuleId.returnRefund));
        } else {
            this.store.dispatch(this.processDataActions.selectSearchResult(selectedEntity, this.ofModule));
        }
    ...
    private newMainTab(tabData?) {
        ...
        let newJsonSettings = JSON.parse(newModuleSetting[0].jsonSettings);
        ...
        let firstCustomTab = newJsonSettings.Content.CustomTabs[0];
        if (firstCustomTab) {
            firstCustomTab.TabID = this.uti.getDefaultMainTabId(this.ofModule);
            if (firstCustomTab.TabContent) {
                let tabContent = firstCustomTab.TabContent[this.contentDisplayMode];
                if (tabContent.Split) {
                    delete tabContent.Split;
                }
                tabContent.Page = {};
                tabContent.Page.PageId = this.uti.getEmptyGuid();
            }
        }
    ...
    private newOtherTab() {
        ...
        newJsonSettings = this.moduleSettingService.insertNewOtherTabSetting(newJsonSettings, this.selectedTab, this.contentDisplayMode);
        newModuleSetting = this.moduleSettingService.insertNewJsonSetting(newModuleSetting, newJsonSettings);

        this.store.dispatch(this.tabSummaryActions.loadTabsForNew(newTabs, (this.ofModule.idSettingsGUI === MenuModuleId.businessCosts ? true : false), this.ofModule));
        this.store.dispatch(this.moduleSettingActions.loadModuleSettingForNew(newModuleSetting, this.ofModule));
    }
    ...
    private newSimpleTab() {
        ...
        let newModuleSetting = cloneDeep(this.moduleSetting);
        let newJsonSettings = cloneDeep(this.tabSetting);
        newJsonSettings = this.moduleSettingService.insertNewSimpleTabSetting(newJsonSettings, this.selectedTab, this.selectedSimpleTab, this.contentDisplayMode);
        newModuleSetting = this.moduleSettingService.insertNewJsonSetting(newModuleSetting, newJsonSettings);
    ...
    private loadTabs(moduleInfo: Module, selectedEntity: any, modulePrimaryKey?: string) {
        if (!moduleInfo) {
            return;
        }
        ...
        let idObject = !selectedEntity
            ...
            : typeof selectedEntity[this.modulePrimaryKey] === 'object' ? selectedEntity[this.modulePrimaryKey].value : selectedEntity[this.modulePrimaryKey];
