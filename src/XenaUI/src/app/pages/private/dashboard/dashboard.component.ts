    ...
    implements OnInit, OnDestroy, AfterViewInit
{
            ...
            (response) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (allModuleSettings && allModuleSettings.length > 0) {
                        ...
                        if (
                            moduleSettingItem &&
                            moduleSettingItem.idSettingsGlobal &&
                            moduleSettingItem.globalName
                        ) {
                            moduleSettingItem = JSON.parse(
                                moduleSettingItem.jsonSettings
                            );
        ...
        this.commonService
            ...
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.store.dispatch(
                        this.xnCommonActions.getModuleToPersonType(
                            result,
                            this.ofModule
                        )
                    );
        ...
        this.selectedParkedItemStateSubscription =
            this.selectedParkedItemState.subscribe(
                (selectedParkedItemState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        if (
                            isEmpty(selectedParkedItemState) &&
                            !isEmpty(this.selectedParkedItem)
                        ) {
                            // not remove tabs in SaveAndNext case
                            if (
                                this.requestSave &&
                                (this.requestSave.savingMode ===
                                ...
                                if (
                                    this.ofModule.idSettingsGUI !=
                                        ModuleList.OrderDataEntry
                                            .idSettingsGUI &&
                                    this.ofModule.idSettingsGUI !=
                                        ModuleList.StockCorrection.idSettingsGUI
                                ) {
                                    this.store.dispatch(
                                        this.tabSummaryActions.removeAllTabs(
                                            this.ofModule
                                        )
                                    );
                                }
                        ...
                        if (
                            ...
                            !Uti.isAllPropertiesEmpty(selectedParkedItemState)
                        ) {
                            this.store.dispatch(
                                this.processDataActions.setSelectedEntity(
                                    this.ofModule,
                        ...
                        this.selectedParkedItem = cloneDeep(
                            selectedParkedItemState
                        );
                        ...
                        if (!isEmpty(this.selectedParkedItem)) {
                            if (this.selectedParkedItem.isNew) {
                                ...
                                this.newMainTab();
                                this.newTabConfig =
                                    this.tabService.createNewTabConfig(
                                        true,
                                        this.ofModule,
                                        this.selectedTab,
                                        null,
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        this.store.dispatch(
                                            this.parkedItemActions.requestSaveParkedItemList(
                                                this.ofModule
                                            )
                                        );
                                    ...
                                    if (!this.isViewMode) {
                                        this.store.dispatch(
                                            this.processDataActions.viewMode(
                                                this.ofModule
                                            )
                                        );
                                        this.store.dispatch(
                                            this.processDataActions.turnOffFormEditMode(
                                                this.ofModule
                                            )
                                        );
                                    }
                            ...
                            this.store.dispatch(
                                this.processDataActions.clearSearchResult(
                                    this.ofModule
                                )
                            );
                            this.store.dispatch(
                                this.processDataActions.clearGoToModuleItem(
                                    this.ofModule
                                )
                            );
                        }
        ...
        this.selectedSearchResultStateSubscription =
            this.selectedSearchResultState.subscribe(
                (selectedSearchResultState: SearchResultItemModel) => {
                    this.appErrorHandler.executeAction(() => {
                        if (
                            isEmpty(selectedSearchResultState) &&
                            !isEmpty(this.selectedSearchResult)
                        ) {
                        ...
                        if (
                            ...
                            !Uti.isAllPropertiesEmpty(selectedSearchResultState)
                        ) {
                            this.store.dispatch(
                                this.processDataActions.setSelectedEntity(
                                    this.ofModule,
                        ...
                        this.selectedSearchResult = cloneDeep(
                            selectedSearchResultState
                        );
        ...
        this.selectedGoToModuleItemStateSubscription =
            this.selectedGoToModuleItemState.subscribe(
                (selectedGoToModuleItemState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        if (
                            isEmpty(selectedGoToModuleItemState) &&
                            !isEmpty(this.selectedGoToModuleItem)
                        ) {
                        ...
                        if (
                            isEqual(
                                this.selectedGoToModuleItem,
                                selectedGoToModuleItemState
                            ) &&
                            !Uti.isAllPropertiesEmpty(
                                selectedGoToModuleItemState
                            )
                        ) {
                            this.store.dispatch(
                        ...
                        this.selectedGoToModuleItem = cloneDeep(
                            selectedGoToModuleItemState
                        );
            ...
            .subscribe(() => {
                this.appErrorHandler.executeAction(() => {
                    if (!this.isHiddenParkedItem) {
                        ...
                        this.store.dispatch(
                            this.parkedItemActions.addDraftItem(
                                newParkedItem,
                                this.ofModule
                            )
                        );
                ...
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (
                        ...
                        (subModulesState[0].idSettingsGUI == -1 ||
                            subModulesState[0].idSettingsGUIParent !=
            ...
            (activeModuleState: Module) => {
                this.appErrorHandler.executeAction(() => {
                    if (isEmpty(activeModuleState)) {
                        ...
                        this.store.dispatch(
                            this.moduleActions.setUsingModule(null)
                        );
                        return;
                    }
                    ...
                    if (
                        ...
                        if (
                            activeModuleState.idSettingsGUI !=
                            MenuModuleId.backoffice
                        ) {
                            this.store.dispatch(
                                this.moduleActions.setUsingModule(
                                    activeModuleState
                                )
                            );
                            ...
                            if (
                                ...
                                activeModuleState.idSettingsGUI ==
                                    MenuModuleId.systemManagement ||
                                activeModuleState.idSettingsGUI ==
                                    MenuModuleId.dashboard ||
                                activeModuleState.idSettingsGUI ==
                                    MenuModuleId.mailingData
                            ) {
                                this.loadTabs(
                                    activeModuleState,
                                    this.selectedEntity
            ...
            this.activeSubModuleState.subscribe(
                (activeSubModuleState: Module) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        if (
                            this.ofModule.moduleNameTrim !=
                            activeSubModuleState.moduleNameTrim
                        ) {
                            this.ref.detach();
                            return;
                        } else {
    ...
    private subcribeModuleSettingState() {
        this.moduleSettingStateSubscription = this.moduleSettingState.subscribe(
            (moduleSettingState: ModuleSettingModel[]) => {
                this.appErrorHandler.executeAction(() => {
                    if (
                        !isEmpty(moduleSettingState) &&
                        moduleSettingState.length
                    ) {
                        if (!isEqual(moduleSettingState, this.moduleSetting)) {
                            this.moduleSetting = cloneDeep(moduleSettingState);
                            const jsonSettingObj =
                                this.moduleSettingService.getValidJsonSetting(
                                    this.moduleSetting
                                );
                            if (jsonSettingObj) {
                                //If load from GlobalSettings no result -> get from ModuleSettings and save cache for GlobalSettings
                                this.updateCacheGlobalSetting(
                                    this.moduleSetting[0]
                                );

                                this.tabSetting = Uti.tryParseJson(
                                    jsonSettingObj.jsonSettings
                                );

                                if (
                                    this.tabSetting.hasOwnProperty(
                                        "AdditionalInfo"
                                    ) &&
                                    this.tabSetting.AdditionalInfo
                                ) {
                                    this.tabSetting.AdditionalInfo.accessRight =
                                        this.accessRightsService.getAccessRight(
                                ...
                                if (
                                    !isNil(
                                        this.tabSetting.IsHiddenParkedItem
                                    ) &&
                                    this.tabSetting.IsHiddenParkedItem == 1
                                ) {
                                    this.store.dispatch(
                                        this.parkedItemActions.requestTogglePanel(
                                            false,
                                            this.ofModule
                                        )
                                    );
                                    this.store.dispatch(
                                        this.parkedItemActions.toggleDisabledPanel(
                                            true,
                                            this.ofModule
                                        )
                                    );
    ...
    private subcribeViewModeState() {
        this.viewModeStateSubscription = this.viewModeState.subscribe(
            ...
            (isViewModeState: boolean) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (!isViewModeState) {
                        ...
                        $(this.elementRef.nativeElement).removeClass(
                            "edit-mode"
                        );
                    }
                });
    ...
    private subcribeRequestDownloadPdfState() {
        this.requestDownloadPdfStateSubscription =
            this.requestDownloadPdfState.subscribe(
                    ...
                    this.appErrorHandler.executeAction(() => {
                        if (requestDownloadPdfState) {
                            ...
                            if (requestDownloadPdfState.selectedEntity) {
                                selectedEntity =
                                    requestDownloadPdfState.selectedEntity;
                            } else {
                                selectedEntity =
                                    this.selectedParkedItem ||
                                    this.selectedSearchResult;
                            ...
                            if (isEmpty(selectedEntity)) {
                                this.toasterService.pop(
                                    "warning",
                                    "Warning",
                                    "Please choose an order to export PDF"
                                );
                                return;
                            }
    ...
    private subcribeRequestGoToTrackingPageState() {
        this.requestGoToTrackingPageStateSubscription =
            this.requestGoToTrackingPageState.subscribe(
                    ...
                    this.appErrorHandler.executeAction(() => {
                        if (requestGoToTrackingPageState) {
                            ...
                            if (requestGoToTrackingPageState.selectedEntity) {
                                selectedEntity =
                                    requestGoToTrackingPageState.selectedEntity;
                            } else {
                                selectedEntity =
                                    this.selectedParkedItem ||
                                    this.selectedSearchResult;
                            ...
                            if (isEmpty(selectedEntity)) {
                                this.toasterService.pop(
                                    "warning",
                                    "Warning",
                                    "Please choose an order to get tracking information"
                                );
                                return;
                            }
    ...
    private subcribeRequestOpenReturnRefundModuleState() {
        this.requestOpenReturnRefundModuleStateSubscription =
            this.requestOpenReturnRefundModuleState.subscribe(
                (requestOpenReturnRefundModuleState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        if (requestOpenReturnRefundModuleState) {
                            ...
                            if (
                                requestOpenReturnRefundModuleState.selectedEntity
                            ) {
                                selectedEntity =
                                    requestOpenReturnRefundModuleState.selectedEntity;
                            } else {
    ...
    private gotoReturnRefundPage(selectedEntity) {
        ...
        if (
            this.ofModule.idSettingsGUI === MenuModuleId.returnRefund ||
            this.ofModule.idSettingsGUI === MenuModuleId.orders
        ) {
            this.store.dispatch(
            ...
            this.store.dispatch(
                this.processDataActions.selectSearchResult(
                    selectedEntity,
                    this.ofModule
                )
            );
    ...
    private restoreOriginData() {
        this.store.dispatch(
            this.moduleSettingActions.loadOriginModuleSetting(this.ofModule)
        );
        this.store.dispatch(
            this.tabSummaryActions.loadOriginTabs(this.ofModule)
        );
    ...
    private newMainTab(tabData?) {
        ...
        const newMainTab = new TabSummaryModel({
            tabSummaryInfor: new TabSummaryInfoModel({
                tabID: !tabData
                    ? this.uti.getDefaultMainTabId(this.ofModule)
                    : tabData.tabSummaryInfor.tabID,
                tabName: !tabData
                    ? this.uti.getDefaultMainTabName(this.ofModule)
                    : tabData.tabSummaryInfor.tabName,
                tabType: !tabData ? "1" : tabData.tabSummaryInfor.tabType,
                lastUpdate: !tabData
                    ? new Date()
                    : tabData.tabSummaryInfor.lastUpdate,
                isMainTab: !tabData ? true : tabData.tabSummaryInfor.isMainTab,
            }),
            tabSummaryData: [],
            active: true,
            accessRight: new AccessRightModel({
                read: true,
                new: true,
                edit: true,
                delete: true,
        ...
        if (
            !newModuleSetting.length ||
            !this.moduleSettingService.isJson(newModuleSetting[0].jsonSettings)
        ) {
            return;
        }
        ...
        newJsonSettings.Content.CustomTabs =
            newJsonSettings.Content.CustomTabs.slice(0, 1);
        ...
        if (firstCustomTab) {
            ...
            if (firstCustomTab.TabContent) {
                let tabContent =
                    firstCustomTab.TabContent[this.contentDisplayMode];
                if (tabContent.Split) {
        ...
        this.store.dispatch(
            this.tabSummaryActions.loadTabsForNew(newTabs, true, this.ofModule)
        );
        this.store.dispatch(
            this.moduleSettingActions.loadModuleSettingForNew(
                newModuleSetting,
                this.ofModule
            )
        );
    ...
    private newOtherTab() {
        ...
        newJsonSettings = this.moduleSettingService.insertNewOtherTabSetting(
            newJsonSettings,
            this.selectedTab,
            this.contentDisplayMode
        );
        newModuleSetting = this.moduleSettingService.insertNewJsonSetting(
            newModuleSetting,
            newJsonSettings
        );
    ...
    private newSimpleTab() {
        ...
        this.store.dispatch(
            this.tabSummaryActions.loadTabsForNew(newTabs, false, this.ofModule)
        );
        this.store.dispatch(
            this.moduleSettingActions.loadModuleSettingForNew(
                newModuleSetting,
    ...
    private storeCurrentData() {
        ...
        this.store.dispatch(
            this.moduleSettingActions.storeOriginModuleSetting(this.ofModule)
        );
        this.store.dispatch(
            this.tabSummaryActions.storeOriginTabs(this.ofModule)
        );
    }
    ...
    private loadUndergroundTabs(moduleInfo: Module) {
        ...
        this.tabService
            .getTabSummaryInfor(param)
            .subscribe((response: ApiResultResponse) => {
                if (!Uti.isResquestSuccess(response)) {
                    return;
                }
                this.store.dispatch(
                    this.tabSummaryActions.storeUndergroundTabs(
                        response.item,
                        this.ofModule
                    )
                );
