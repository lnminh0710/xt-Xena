...
export class WidgetTemplateSettingService extends BaseService {
    ...
    constructor(injector: Injector, @Inject(forwardRef(() => DatatableService)) public datatableService: DatatableService) {
        super(injector);
    }
    ...
    public createWidgetSetting(widgetSettingModel: WidgetSettingModel): Observable<any> {
        return this.post<any>(this.serUrl.createWidgetSetting, JSON.stringify(widgetSettingModel)).map((result: any) => {
            return result.item;
        });
    }
    ...
    public updateWidgetSetting(widgetSettingModel: WidgetSettingModel): Observable<any> {
        return this.post<any>(this.serUrl.updateWidgetSetting, JSON.stringify(widgetSettingModel)).map((result: any) => {
            return result.item;
        });
    }
    ...
    public deleteWidgetSetting(widgetSettingModel: WidgetSettingModel): Observable<any> {
        return this.post<any>(this.serUrl.deleteWidgetSetting, JSON.stringify(widgetSettingModel)).map((result: any) => {
            return result.item;
        });
    }
    ...
    public getWidgetToolbar(objectParam?: string, idSettingsModule?: string, objectNr?: any, moduleType?: string): Observable<any> {
        const param = {
        ...
        return BaseService.cacheService.get('getWidgetToolbar:' + (objectNr || '-1'),
     ...
     * @param filterParam
     */
    public getWidgetDetailByRequestString(widgetDetail: WidgetDetail, filterParam: any, isGetDataForTranslate?: boolean, isGetDataForOrderBy?: boolean): Observable<WidgetDetail> {
        if (!widgetDetail.request) {
            //console.log('widgetDetail.request cannot empty', { widgetDetail: widgetDetail, filterParam: filterParam });
            return Observable.of(widgetDetail);
        }
        this.addHeader('request', encodeURIComponent(widgetDetail.request));
        let s = '';
        if (filterParam) {
            ...
            Object.keys(filterParam).forEach((key, index, list) => {
                if (key != 'item') {
                    s += '\\"' + key + '\\"' + ':' + '\\"' + (isNil(filterParam[key]) ? '' : filterParam[key]) + '\\"';
        ...
        return this.get<string>(this.serUrl.getWidgetDetailByRequestString, requestString).map((result: any) => {
    ...
    public getDetailByRequestString(request: string): Observable<any> {
        this.addHeader('request', encodeURIComponent(request));
        return this.get<any>(this.serUrl.getWidgetDetailByRequestString).map((result: any) => {
            return result.item;
        });
    }

    public updateWidgetInfo(data: any, requestUpdate: string, module: Module, updateKey: string, customData?: any, jsonTextUpdate?: string, mode?: string): Observable<any> {
        ...
        return this.post<any>(this.serUrl.updateWidgetInfo, JSON.stringify(dataUpdate)).map((result: any) => {
            return result.item;
        });
    }
