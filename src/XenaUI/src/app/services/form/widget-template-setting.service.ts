...
export class WidgetTemplateSettingService extends BaseService {
     ...
     */
    public createWidgetSetting(
        widgetSettingModel: WidgetSettingModel
    ): Observable<any> {
        return this.post<any>(
            this.serUrl.createWidgetSetting,
            JSON.stringify(widgetSettingModel)
        ).map((result: any) => {
            return result.item;
        });
    }
     ...
     * @param widgetSettingModel
     */
    public updateWidgetSetting(
        widgetSettingModel: WidgetSettingModel
    ): Observable<any> {
        return this.post<any>(
            this.serUrl.updateWidgetSetting,
            JSON.stringify(widgetSettingModel)
        ).map((result: any) => {
            return result.item;
        });
     ...
     * @param widgetSettingModel
     */
    public deleteWidgetSetting(
        widgetSettingModel: WidgetSettingModel
    ): Observable<any> {
        return this.post<any>(
            this.serUrl.deleteWidgetSetting,
            JSON.stringify(widgetSettingModel)
        ).map((result: any) => {
            return result.item;
        });
    ...
    public hasVisibleWidgets(widgetToolbar) {
        for (const wt of widgetToolbar) {
            const hiddenWidgets = wt.Widgets.filter(
                (x) => x.accessRight && !x.accessRight.read
            );
    ...
    public getWidgetDetailByRequestString(
        ...
        if (filterParam) {
            ...
            for (let item in filterParam) {
                if (
                    filterParam[item] === this._const.widgetDotNotAddThisFilter
                ) {
                    delete filterParam[item];
                }
            }
            Object.keys(filterParam).forEach((key, index, list) => {
                if (key != "item") {
                    s +=
                        ...
                        ":" +
                        '\\"' +
                        (isNil(filterParam[key]) ? "" : filterParam[key]) +
                        '\\"';
        ...
        return this.get<string>(
            this.serUrl.getWidgetDetailByRequestString,
            requestString
        )
            .map((result: any) => {
                ...
                if (widgetUtils.isTableWidgetDataType(widgetDetail)) {
                    if (
                        ...
                        widgetDetail.idRepWidgetApp === 172 // Open status
                    ) {
                        result.item = {
                            collectionData: get(result, ["item", "data", 0]),
                            columnSettings: {},
    ...
    public getDetailByRequestString(request: string): Observable<any> {
        ...
        return this.get<any>(this.serUrl.getWidgetDetailByRequestString).map(
            (result: any) => {
                return result.item;
            }
        );
    ...
    public updateWidgetInfo(
        ...
        if (dataUpdate.UsingReplaceString) {
            ...
            if (
                dataUpdate.data[0] == "{" &&
                dataUpdate.data[dataUpdate.data.length - 1] == "}"
            ) {
                dataUpdate.data = JSON.stringify(dataUpdate.data);
                dataUpdate.data = dataUpdate.data.substring(
                    2,
            ...
            // =>  ""\"[{\\\"IdPersonInterface\\\":\\\"12097\\\",\\\"Notes\\\":\\\"/dasd qweqw eqw/eqee//23^^*^*---\\\\\\\\\\\\\\\\\\\\\\\\\\\"}]\"""
                ...
                dataUpdate.data = JSON.stringify(
                    JSON.stringify(dataUpdate.data)
                );
                dataUpdate.data = dataUpdate.data.substring(
                    3,
                    dataUpdate.data.length - 3
                );
            }
            //#endregion
        }
    ...
    private removeTableNameOfPropertyName(obj: any): any {
        ...
        Object.keys(obj).forEach((prop) => {
            const newPropName = prop.substring(
                prop.indexOf("_") + 1,
                prop.length
            );
    ...
    public saveWidgetOrderBy(widgetOrderBys: any): Observable<any> {
        return this.post<any>(
            this.serUrl.saveWidgetOrderBy,
            JSON.stringify(widgetOrderBys)
        ).map((result: any) => {
            return result.item;
        });
    }
