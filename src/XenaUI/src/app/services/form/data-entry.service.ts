...
export class DataEntryService extends BaseService {
    public loadBufferOrderKeys = {
        ...
        getMainCurrencyAndPaymentType: 'ODE-ScanningStatus-getMainCurrencyAndPaymentType:',
        getMainCurrencyAndPaymentTypeForOrder: 'Order-ScanningStatus-getMainCurrencyAndPaymentType:',
    ...
    public getMainCurrencyAndPaymentType(mediacode: string, campaignNr: string, useCache?: boolean): Observable<any> {
        if (useCache) {
            const cacheKey = this.loadBufferOrderKeys.getMainCurrencyAndPaymentType + mediacode + campaignNr;

            return BaseService.cacheService.get(cacheKey, this.get<any>(this.serUrl.getMainCurrencyAndPaymentType, {
                mediacode: mediacode,
    ...
    public getMainCurrencyAndPaymentTypeForOrder(mediacode: string, campaignNr: string, useCache?: boolean): Observable<any> {
        if (useCache) {
            const cacheKey = this.loadBufferOrderKeys.getMainCurrencyAndPaymentTypeForOrder + mediacode + campaignNr;

            return BaseService.cacheService.get(cacheKey, this.get<any>(this.serUrl.getMainCurrencyAndPaymentTypeForOrder, {
                mediacode: mediacode,
                campaignNr: campaignNr
            })).map((result: any) => {
                return result.item;
    ...
    public getArticleDataByMediacodeNr(mediacode: string, useCache?: boolean): Observable<any> {
        if (useCache) {
            const cacheKey = this.loadBufferOrderKeys.getArticleData + mediacode;
            return BaseService.cacheService.get(cacheKey, this.get<any>(this.serUrl.getArticleData, {
                mediacode: mediacode
            })).map((result: any) => {
                return result.item;
            });
    ...
    public getCustomerDataByCustomerNr(customerNr: string, useCache?: boolean, mediaCode?: string): Observable<any> {
        if (!mediaCode) {
            mediaCode = null;
        }
        if (useCache) {
            ...
            return BaseService.cacheService.get(cacheKey, this.get<any>(this.serUrl.getCustomerData, {
    ...
    public getPreloadOrderData(skipIdPreload?: any, idScansContainerItems?: any, lotId?: any): Observable<any> {
        let params: any = {
        ...
        return this.get<any>(this.serUrl.getPreloadOrderData, params).map((result: any) => {
            return result.item;
        });
    }

    public saveOrderDataEntry(data: any, ignoreBarProcess?: boolean): Observable<any> {
        return this.post<any>(this.serUrl.saveOrderDataEntry, JSON.stringify(data), null, null, null, ignoreBarProcess).map((result: any) => {
            return result.item;
        });
    }

    public savePaymentForOrder(data: any, ignoreBarProcess?: boolean): Observable<any> {
        return this.post<any>(this.serUrl.savePaymentForOrder, JSON.stringify(data), null, null, null, ignoreBarProcess).map((result: any) => {
            return result.item;
        });
    }

    public sendOrderToAdministrator(data: any): Observable<any> {
        return this.post<any>(this.serUrl.sendOrderToAdministrator, JSON.stringify(data)).map((result: any) => {
            return result.item;
        });
    }
    ...
    public buildImageFullPath(data, ext?: string) {
        ...
        return encodeURIComponent(data[0].fileUrl + '\\' + data[0].fileName + '.' + (data[0].numberOfImages / data[0].numberOfImages) + (ext || '.png'));
    }
    ...
    public buildListImages(data) {
        ...
        for (let i = 1; i <= total; i++) {
            urls.push({
                index: i,
                url: encodeURIComponent(data[0].fileUrl + '\\' + data[0].fileName + '.' + i + '.png')
            });
        }
    ...
    public getOrderDataEntryByLogin(mode: string, dateFrom?: string, dateTo?: string): Observable<any> {
    ...
    public getOrderFailed(tabID: string, idScansContainerItems: string): Observable<any> {
        ...
        //get from file
        return this.get<any>(this.serUrl.getOrderFailed, { tabID: tabID, idScansContainerItems: idScansContainerItems }).map((result: any) => {
            return result.item;
        });
    }
    ...
    public saveOrderFailed(data: any): Observable<any> {
        ...
        //save file
        return this.post<any>(this.serUrl.saveOrderFailed, jsonData).map((result: any) => {
    ...
    public deleteOrderFailed(tabID: string, idScansContainerItems: string): Observable<any> {
        let data = {
            TabID: tabID,
            IdScansContainerItems: idScansContainerItems
        };
        ...
        //delete file
        return this.post<any>(this.serUrl.deleteOrderFailed, jsonData).map((result: any) => {
    ...
    public deleteAllOrderFailed(tabID: string): Observable<any> {
        ...
        //delete all files
        return this.post<any>(this.serUrl.deleteAllOrderFailed, jsonData).map((result: any) => {
            if (result.item && result.item.success) {
    ...
    public getListOrderFailed(tabID: string, pageIndex?: number, pageSize?: number): Observable<any> {
        pageIndex = pageIndex || 0;
        pageSize = pageSize || 20;
        const isGetSummaryFile = true;

        //get list files from HardDisk
        return this.get<any>(this.serUrl.getListOrderFailed, { tabID: tabID, pageIndex: pageIndex, pageSize: pageSize, isGetSummaryFile: isGetSummaryFile }).map((rs: any) => {
    ...
    public hasOrderFailedInLocalStorage(): boolean {
        const index = Object.keys(localStorage).findIndex(key => key.indexOf('ODE-Failed:') !== -1);
            ...
            if (key.indexOf('ODE-Failed:') !== -1) {
                ...
                if (content) {
                    observableBatch.push(
                        //save file
                        this.post<any>(this.serUrl.saveOrderFailed, content).map((result: any) => {
                            //delete localStorage after saving file successfully
                            if (result.item && result.item.success) {
                                localStorage.removeItem(key);
                            }
