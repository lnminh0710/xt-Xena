...
export class DataEntryService extends BaseService {
        ...
        if (useCache) {
            ...
            return BaseService.cacheService
                .get(
                    cacheKey,
                    this.get<any>(
                        this.serUrl.getMainCurrencyAndPaymentTypeForOrder,
                        {
                            mediacode: mediacode,
                            campaignNr: campaignNr,
                        }
    ...
    public getArticleDataByMediacodeNr(
        ...
        if (useCache) {
            const cacheKey =
                this.loadBufferOrderKeys.getArticleData + mediacode;
            return BaseService.cacheService
                .get(
                    cacheKey,
    ...
    public getCustomerDataByCustomerNr(
        ...
        if (useCache) {
            let cacheKey =
                this.loadBufferOrderKeys.getCustomerData + customerNr;
            if (mediaCode) {
                cacheKey += "-" + mediaCode;
            }
    ...
    public saveOrderFailed(data: any): Observable<any> {
        ...
        //save localStorage
        const key = this.getOrderFailedKey(
            data.TabID,
