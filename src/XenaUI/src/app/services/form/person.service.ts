...
export class PersonService extends BaseService {
    ...
    public getPersonById(idPerson: string): Observable<ApiResultResponse> {
        ...
        if (!idPerson) {
            ...
            return BaseService.cacheService.get(cacheKey, this.get<any>(this.serUrl.getPersonById, {
                idPerson: idPerson
            }));
        }
    ...
    public createPaymentAccount(paymentAccount: any): Observable<any> {
        return this.post<any>(this.serUrl.createPaymentAccount, JSON.stringify(paymentAccount)).map((result: any) => {
            return result.item;
        });
    }

    public createCostProvider(costProvider: any): Observable<any> {
        return this.post<any>(this.serUrl.createCostProvider, JSON.stringify(costProvider))
            .map((result: any) => {
                return result.item;
            });
    }
    ...
    public createCCPRN(cCCPRNData: any): Observable<any> {
        return this.post<any>(this.serUrl.createCCPRN, JSON.stringify(cCCPRNData)).map((result: any) => {
            return result.item;
        });
    }
    ...
    public getMandatoryField(mode: string): Observable<any> {
        ...
        if (mode === 'Customer') {
            ...
            return BaseService.cacheService.get(cacheKey, this.get<any>(this.serUrl.getMandatoryField, {
                mode: mode
            }));
        }
