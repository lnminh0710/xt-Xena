...
export class CommonService extends BaseService {
    ...
    private notAllowCachedKeys: Array<string> = ['principal', 'allMandant', 'serviceProvider', 'wareHouse', 'campaignWizardAddress',
    ...
    private getStringKeyList(identityKeys: Array<string>) {
        ...
        for (var i = 0; i < identityKeys.length; i++) {
            ...
            if (!this.isExistInObject(identityKeys[i], ComboBoxTypeConstant)
    ...
    private getRequestKeyList(stringKeys: Array<any>) {
        ...
        for (var i = 0; i < stringKeys.length; i++) {
            ...
            if (!this.comboBoxResultList || !this.comboBoxResultList[stringKey] ||
    ...
    public getListComboBox(comboBoxList: string, extraData?: string, noCache?: boolean): Observable<any> {
        ...
        if (requestKeys.length == 0 && !noCache) {
            return Observable.of(new ApiResultResponse({
            ...
            let observable = this.get<any>(this.serUrl.getComboxBoxList, obj).map((response: ApiResultResponse) => {
                let res = response.item;
                if (res) {
                    var resKeys = Object.keys(res);
                    resKeys.forEach((resKey) => {
                        // Check if this key don't allow cached.
                        const notAllowCachedKey = this.notAllowCachedKeys.find(p => Uti.toLowerCase(p) == Uti.toLowerCase(resKey));
                        // Allow cache case
    ...
    public getComboBoxDataByCondition(comboboxName: string, condition: string): Observable<any> {
        ...
        return BaseService.cacheService.get(cacheKey, this.get<any>(this.serUrl.getComboxBoxList, {
    ...
    public getModuleToPersonType() {
        return BaseService.cacheService.get(this.serUrl.getModuleToPersonType, this.get<any>(this.serUrl.getModuleToPersonType));
    }
    ...
    public updateContact(contact: any): Observable<any> {
        contact = Uti.convertDataEmptyToNull(contact);
        return this.post<any>(this.serUrl.updateContact, JSON.stringify(contact));
    }
    ...
    public donwloadReportNote(idPerson): Observable<any> {
        ...
        return this.getV2<any>(`${this.serUrl.donwloadReportNote}?fieldname=IdPerson&Value=${idPerson}`, options);
    }
    ...
    public getPublicSetting(): Observable<any> {
        ...
        return BaseService.cacheService.get(this.serUrl.getPublicSetting, this.get<any>(this.serUrl.getPublicSetting)).map((result: any) => {
    ...
    public savingSharingTreeGroups(model: any): Observable<any> {
        return this.post<any>(this.serUrl.savingSharingTreeGroups, JSON.stringify(model));
    }

    public editImagesGallery(model: any): Observable<any> {
        return this.post<any>(this.serUrl.editImagesGallery, JSON.stringify(model));
    }
