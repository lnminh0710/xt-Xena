...
export class MenuStatusService {
    ...
    public buildMenuStatusSettings(settings: WidgetMenuStatusModel, data: WidgetDetail, isSwitchedFromGridToForm: boolean) {
        if (!settings) return;
        ...
        if (settings.hasRightSettingButton && idRepWidgetApp !== RepWidgetAppIdEnum.RepositoryDetail &&
            !isSwitchedFromGridToForm && idRepWidgetType >= WidgetType.DataGrid && idRepWidgetType <= WidgetType.EditableTable &&
            !this.hideBtnSetting(idRepWidgetType))
            settings.btnSettingGrid.enable = true;
        else
            settings.btnSettingGrid.enable = false;

        //Menu Button Settings for Form
        if (settings.hasRightSettingButton &&
            ...
            settings.btnSettingForm.enable = false;

        //Menu Button Settings for Grid and Form
        if (settings.hasRightSettingButton && idRepWidgetType === WidgetType.Combination)
            settings.btnSettingGridAndForm.enable = true;
        else
            settings.btnSettingGridAndForm.enable = false;
        ...
        settings.btnToolbar.enable = settings.hasRightToolbarButton && (isSupportWidgetSetting || isShowToogleButton);
        settings.btnRefresh.enable = isSupportWidgetSetting || isShowToogleButton;
        settings.btnBackToWidget.enable = true;

        settings.btnEditForm.enable = settings.hasRightEdit;
        settings.btnEditWidgetOptions.enable = idRepWidgetType !== WidgetType.EditableRoleTreeGrid;

        settings.btnEditCountryWidget.enable = settings.hasRightEdit;
        settings.btnEditTreeViewWidget.enable = settings.hasRightEdit;

        settings.btnWidgetTranslation.enable = settings.hasRightToolbarButton_TranslateButton &&
            (
        ...
        settings.btnWidgetTranslationOptions.enable = (idRepWidgetType == WidgetType.DataGrid
            || idRepWidgetType === WidgetType.EditableGrid
            || idRepWidgetType === WidgetType.Combination);

        settings.btnGoToNextcolumnOrRow.enable = (idRepWidgetType === WidgetType.DataGrid
            || idRepWidgetType === WidgetType.EditableGrid
            || idRepWidgetType === WidgetType.TableWithFilter
            || idRepWidgetType === WidgetType.ReturnRefund);

        //btnEditTemplate
        settings.btnEditTemplate.enable = settings.hasRightToolbarButton_EditTemplateButton &&
        ...
        settings.btnTableFieldsTranslate.enable = settings.hasRightToolbarButton_FieldTranslateButton && idRepWidgetType >= WidgetType.FieldSet;

        settings.txtFilterTable.enable = (idRepWidgetType === WidgetType.DataGrid
            || idRepWidgetType === WidgetType.EditableGrid
            || idRepWidgetType === WidgetType.TableWithFilter
            || idRepWidgetType === WidgetType.EditableRoleTreeGrid);

        settings.btnSaveSettings.enable = idRepWidgetApp !== RepWidgetAppIdEnum.RepositoryDetail;

        settings.btnOpenUserRoleDialog.enable = idRepWidgetApp === RepWidgetAppIdEnum.UserList;

        settings.btnOpenTranslateForCombineWidget.enable = settings.hasRightToolbarButton_TranslateButton && idRepWidgetType == WidgetType.Combination;

        settings.btnArticleNameTranslation.enable = settings.hasRightToolbarButton_TranslateButton;
        settings.btnPrintWidget.enable = settings.hasRightToolbarButton_PrintButton && this.isSupportPrint(idRepWidgetType);
        settings.btnSupportSearch.enable = this.isSupportSearch(idRepWidgetType);
        settings.btnSupportExcelExport.enable = this.isSupportExcelExport(idRepWidgetType);
        settings.btnAddRowForEditableTableWidget.enable = settings.hasRightEdit;
        settings.btnAddNoteForm.enable = true;
        settings.btnAddPerson.enable = idRepWidgetApp === RepWidgetAppIdEnum.CustomerDoublette;
        ...
        settings.btnSaveCountryWidget.enable = settings.hasRightEdit || settings.hasRightDelete;
    ...
    public buildMenuStatusSettingsWhenSwitchedFromGridToForm(settings: WidgetMenuStatusModel, data: WidgetDetail, isSwitchedFromGridToForm: boolean) {
        if (!settings) return;
