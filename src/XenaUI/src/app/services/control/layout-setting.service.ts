...
export class LayoutSettingService {
    ...
    public translateLayoutSetting2GoldenLayoutSetting(layoutSetting: any, editMode?: boolean) {
        ...
        for (let tab of layoutSettingContent.CustomTabs) {
            let layoutObj: any = this.createLayoutMappingObj(tab, layoutSetting.ModuleID, editMode);
    ...
    public convertPageLayoutContent2GoldenLayoutContent(content: any): Array<any> {
        ...
        if (content['Split']) {
            if (content['Split']['SplitType'] === 'Horizontal') {
                const row = GoldenLayoutService.createRow('Row', content['TabName'], content['TabID'], content['ID']);
            ...
            if (content['Split']['SplitType'] === 'Vertical') {
                const column = GoldenLayoutService.createColumn('Column', content['TabName'], content['TabID']);
                for (let i = 0; i < content['Split'].Items.length; i++) {
        ...
        if (content['SimpleTabs'] && content['SimpleTabs'].length) {
            const stack = GoldenLayoutService.createStack('Stack', content['TabName'], content['TabID']);
    ...
    private createGoldenLayoutPageItem(layoutItem: any): any {
        ...
        let contentSize = this.parseContentSizeToGoldenSize(layoutItem['ContentSize']);

        if (layoutItem['Page']) {
            goldenLayoutItem = GoldenLayoutService.createPage('Page', layoutItem['TabName'], layoutItem['Page']['PageId'], contentSize, contentSize, layoutItem['ID']);
        }

        if (layoutItem['Split']) {
            if (layoutItem['Split']['SplitType'] === 'Horizontal') {
                goldenLayoutItem = GoldenLayoutService.createRow('Row', layoutItem['TabName'], layoutItem['TabID'], contentSize, contentSize);
            ...
            if (layoutItem['Split']['SplitType'] === 'Vertical') {
                goldenLayoutItem = GoldenLayoutService.createColumn('Column', layoutItem['TabName'], layoutItem['TabID'], contentSize, contentSize);
                for (let i = 0; i < layoutItem['Split'].Items.length; i++) {
                    goldenLayoutItem.content.push(this.createGoldenLayoutPageItem(layoutItem['Split'].Items[i]));
                }
            }
        ...
        if (layoutItem['SimpleTabs'] && layoutItem['SimpleTabs'].length) {
            goldenLayoutItem = GoldenLayoutService.createStack('Stack', layoutItem['TabName'], layoutItem['TabID']);
    ...
    private createXenaItem(layoutItem) {
        ...
        if (layoutItem.type == 'component') {
            xenaItem = this.createPage(layoutItem.id, layoutItem.id, layoutItem.title, this.calculateContentSize(layoutItem), layoutItem.itemId);
        }
        else if (layoutItem.type == 'stack') {
            if (layoutItem.content.length == 1) {
                let item = layoutItem.content[0];
                if (item.type == 'component')
                    xenaItem = this.createPage(item.id, item.id, item.title, this.calculateContentSize(layoutItem), item.itemId);
                else if (item.type == 'row') {
                    if (item.content.length > 1) {
                        layoutItem = item;
                    }
                    xenaItem = this.createRow(layoutItem.id, layoutItem.title, this.calculateContentSize(layoutItem));//Horizontal: left -> right
                }
                else if (item.type == 'column') {
                    if (item.content.length > 1) {
                        layoutItem = item;
                    }
                    xenaItem = this.createColumn(layoutItem.id, layoutItem.title, this.calculateContentSize(layoutItem));//Vertical: top -> bottom
                }
            }
            else {
                xenaItem = this.createStack(layoutItem.id, layoutItem.title, this.calculateContentSize(layoutItem));
            }
        }
