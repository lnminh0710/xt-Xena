...
export class PropertyPanelService extends BaseService {
    ...
    public updateWidgetTableStyle(properties, tableStyle) {
        ...
        const propertyOddRowBackground = this.getItemRecursive(properties, 'RowStyleOddBackground');
        const propertyEvenRowBackground = this.getItemRecursive(properties, 'RowStyleEvenBackground');
        const propertyBorderRow = this.getItemRecursive(properties, 'RowStyleBorder');
    ...
    public updateWidgetFormStyle(properties, formStyle, styleType, ofType) {
        ...
        const propSeparateValue = this.getItemRecursive(properties, ofType + 'Value');
        if (propSeparateValue) {
            newFormStyle[styleType]['value'] = propSeparateValue.value;
        }

        const propFormHeightStyle = this.getItemRecursive(properties, ofType + FormStyle.Height);
        const propFormHeightAutoHeightStyle = this.getItemRecursive(properties, ofType + FormStyle.AutoHeight);
        const valueHeightAutoHeightStyle = propFormHeightAutoHeightStyle && propFormHeightAutoHeightStyle.value;
        newFormStyle[styleType][CssStyleForm.Height] = valueHeightAutoHeightStyle ? 'auto' : propFormHeightStyle && propFormHeightStyle.value;


        const propFormStyleAlign = this.getItemRecursive(properties, ofType + FormStyle.JustifyContent);
        if (propFormStyleAlign && propFormStyleAlign.value) {
            newFormStyle[styleType][CssStyleForm.JustifyContent] = propFormStyleAlign.value;
        }

        const propFormStyleBackgroundColor = this.getItemRecursive(properties, ofType + FormStyle.BackgroundColor);
        if (propFormStyleBackgroundColor) {
            newFormStyle[styleType][CssStyleForm.BackgroundColor] = propFormStyleBackgroundColor.value ? propFormStyleBackgroundColor.value.toLowerCase() : '';
        }

        const propFormStyleColor = this.getItemRecursive(properties, ofType + FormStyle.Color);
        if (propFormStyleColor) {
            newFormStyle[styleType][CssStyleForm.Color] = propFormStyleColor.value ? propFormStyleColor.value.toLowerCase() : '';
        }

        const propBorderRowStyleColor = this.getItemRecursive(properties, ofType + FormStyle.Border);
        if (propBorderRowStyleColor) {
            newFormStyle[styleType][CssStyleForm.BorderRight] = propBorderRowStyleColor.value ? propBorderRowStyleColor.value : '';
        }

        const propOddBackgroundRowStyleColor = this.getItemRecursive(properties, ofType + FormStyle.OddBackground);
        if (propOddBackgroundRowStyleColor) {
            newFormStyle[styleType][CssStyleForm.OddRow] = propOddBackgroundRowStyleColor.value ? propOddBackgroundRowStyleColor.value : '';
        }

        const propEvenBackgroundRowStyleColor = this.getItemRecursive(properties, ofType + FormStyle.EvenBackground);
        if (propEvenBackgroundRowStyleColor) {
            newFormStyle[styleType][CssStyleForm.EvenRow] = propEvenBackgroundRowStyleColor.value ? propEvenBackgroundRowStyleColor.value : '';
        }

        const propFormStyleFontName = this.getItemRecursive(properties, ofType + FormStyle.FontName);
        if (propFormStyleFontName) {
            if (!propFormStyleFontName.options || !propFormStyleFontName.options.length) {
                propFormStyleFontName.options = this.defaultFontNameOptions();
            }

            if (propFormStyleFontName.value) {
                newFormStyle[styleType][CssStyleForm.FontFamily] = propFormStyleFontName.value;
            }
        }

        const propFormStyleFontSize = this.getItemRecursive(properties, ofType + FormStyle.FontSize);
        if (propFormStyleFontSize) {
            if (!propFormStyleFontSize.options || !propFormStyleFontSize.options.length) {
                propFormStyleFontSize.options = this.defaultFontSizeOptions();
            }

            if (propFormStyleFontSize.value) {
                newFormStyle[styleType][CssStyleForm.FontSize] = propFormStyleFontSize.value + 'px';
            }
        }

        const propFormStyleBold = this.getItemRecursive(properties, ofType + FormStyle.Bold);
        if (propFormStyleBold) {
            newFormStyle[styleType][CssStyleForm.FontWeight] = propFormStyleBold.value ? 'bold' : 'normal';
        }

        const propFormStyleItalic = this.getItemRecursive(properties, ofType + FormStyle.Italic);
        if (propFormStyleItalic) {
            newFormStyle[styleType][CssStyleForm.FontStyle] = propFormStyleItalic.value ? 'italic' : 'unset';
        }

        const propFormStyleUnderline = this.getItemRecursive(properties, ofType + FormStyle.Underline);
        if (propFormStyleUnderline) {
            newFormStyle[styleType][CssStyleForm.Underline] = propFormStyleUnderline.value ? 'underline' : 'unset';
        }

        const propFormStyleWidth = this.getItemRecursive(properties, ofType + FormStyle.Width);
        const propFormStyleAutoWidth = this.getItemRecursive(properties, ofType + FormStyle.AutoWidth);
        const valueLabelStyleAutoWidth = propFormStyleAutoWidth && propFormStyleAutoWidth.value;
        newFormStyle[styleType][CssStyleForm.Width] = valueLabelStyleAutoWidth ? 'auto' : propFormStyleWidth && propFormStyleWidth.value;
    ...
    public updateInlineLabelStyle(properties, inlineLabelStyle) {
        ...
        const propFormStyleLineHeight = this.getItemRecursive(properties, 'LabelStyleLineHeight');
        if (propFormStyleLineHeight) {
            if (!propFormStyleLineHeight.options || !propFormStyleLineHeight.options.length) {
    ...
    public getValueDropdownFromGlobalProperties(globalProperties: any[]) {
        let propShowDropdownWhenFocus = this.getItemRecursive(globalProperties, 'ShowDropdownWhenFocus');
    ...
    public getAutoSwitchToDetail(properties: any[]) {
        const propertiesAutoSwitchToDetail = this.getItemRecursive(properties, 'AutoSwitchToDetail');
        return propertiesAutoSwitchToDetail ? propertiesAutoSwitchToDetail.value : null;
    }

    public getValueDropdownFromProperties(globalProperties: any[]) {
        const propShowDropDownOfField = this.getItemRecursive(globalProperties, 'ShowDropDownOfField');
    ...
    public getValueBackgroundImageFromProperties(properties: any[]) {
        const propBackgroundImage = this.getItemRecursive(properties, 'WidgetBackgroundStyleImage');
    ...
    public getValueDropdownApplyOnWidgetFromProperties(globalProperties: any[]) {
        const proDropdownApplyOnWidget = this.getItemRecursive(globalProperties, 'ApplyOnWidget');
    ...
    public buildGlobalInputDateFormatFromProperties(globalProperties: any[]) {
        ...
        if ((inputDateSeparator) &&
            (inputDateDateFormat && inputDateDateFormat.value)) {
