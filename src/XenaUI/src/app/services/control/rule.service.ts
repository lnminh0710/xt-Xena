...
export class RuleService extends BaseService {
    ...
    public getComboBoxForRuleBuilder(
    ...
    public saveProjectRules(projectRules): Observable<any> {
        return this.post<any>(
            this.serUrl.saveProjectRules,
            JSON.stringify(projectRules)
        );
    }
    ...
    */
    public getTemplate(idSelectionWidget: any): Observable<any> {
        return this.get<any>(this.serUrl.getTemplate, {
            idSelectionWidget: idSelectionWidget,
        });
    }
    ...
    public saveBlackListProfile(data: any): Observable<any> {
        return this.post<any>(
            this.serUrl.saveBlackListProfile,
            JSON.stringify(data)
        );
    }
    ...
    public hasEmptyFieldRecursive(data, fieldName) {
        ...
        if (!isNil(data.rules)) {
            for (let i = 0; i < data.rules.length; i++) {
                let found = this.hasEmptyFieldRecursive(
                    data.rules[i],
                    fieldName
                );
                if (found) {
    ...
    public hasEmptySubFieldRecursive(data, parentFieldName, fieldName) {
        if (
            ...
            !parentFieldName ||
            !fieldName ||
            (data.hasOwnProperty(parentFieldName) &&
                isEmpty(data[parentFieldName]))
        ) {
            return true;
        }
        ...
        for (let i = 0; i < data[parentFieldName].length; i++) {
            if (
                !data[parentFieldName][i].hasOwnProperty(fieldName) ||
            ...
            let found = this.hasEmptySubFieldRecursive(
                data[parentFieldName][i],
                parentFieldName,
                fieldName
            );
            if (found) {
    ...
    public hasEmptyPropValue(obj) {
        ...
        for (let i = 0; i < Object.keys(obj).length; i++) {
            if (
                (isSingleType && Object.keys(obj)[i] === "groupOperators") ||
                Object.keys(obj)[i] === "groupQuantity"
            ) {
                continue;
            }
            if (
                isNil(obj[Object.keys(obj)[i]]) ||
        ...
        return isSingleType
            ? result
            : result || !groupOperatorsValue || !groupQuantityValue;
    }
