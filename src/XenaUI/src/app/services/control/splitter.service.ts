...
export class SplitterService {
    ...
    public updateSplitterState(moduleSetting:any, splittersSize: any, pageItems: Array<any>) {
        ...
        for (let i = 0; i < pageItems.length; i++) {
            this.loopRecursive(moduleContent.CustomTabs, pageItems[i].ID, splittersSize);
        }
    ...
    private loopRecursive(items: Array<any>, id: string, splittersSize: any): void {
            ...
            if (item.TabContent) {
                if (item.TabContent.ViewMode) {
                    if (item.TabContent.ViewMode.Split) {
                        this.loopRecursive(item.TabContent.ViewMode.Split.Items, id, splittersSize);
                    }

                    if (item.TabContent.ViewMode.SimpleTabs) {
                        this.loopRecursive(item.TabContent.ViewMode.SimpleTabs, id, splittersSize);
                    }
                }

                if (item.TabContent.EditMode) {
                    if (item.TabContent.EditMode.SimpleTabs) {
                        this.loopRecursive(item.TabContent.EditMode.SimpleTabs, id, splittersSize);
                    }
