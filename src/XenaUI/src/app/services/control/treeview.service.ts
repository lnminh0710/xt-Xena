...
export class TreeViewService extends BaseService {
    public makeDataForSaveTreeView(
        data: any,
        dataEdited: any,
        idArticle: any
    ): Array<any> {
        ...
        for (const newTreeViewItem of dataEdited) {
            ...
            if (
                this.compareOldAndNewTreeViewItem(
                    newTreeViewItem,
                    oldTreeViewItem
                )
            ) {
                ...
                result.push({
                    IdSharingTreeGroupsRootname:
                        newTreeViewItem.idSharingTreeGroupsRootname,
                    IdArticle: idArticle,
                    IdSharingTreeGroups: newTreeViewItem.id,
                    IdArticleGroups: newTreeViewItem.idArticleGroups
                        ? newTreeViewItem.idArticleGroups
                        : Uti.getTempId2() + "",
                    IsActive: newTreeViewItem.select ? 1 : 0,
                    IsMainGroup: newTreeViewItem.isMain ? 1 : 0,
                    GroupInPercent: this.parsePercent(newTreeViewItem.percent),
                    GroupName: newTreeViewItem.text,
                });
    ...
    public makeDataForSaveTreeViewMaster(
        addData: Array<any>,
    ...
    public makeTreeViewItemData(rawItem: any): any {
        return {
            idSharingTreeGroupsRootname: this.strValObj(
                rawItem.idSharingTreeGroupsRootname.value
            ),
    ...
    private compareOldAndNewTreeViewItem(oldItem: any, newItem: any): boolean {
        for (const pro in oldItem) {
            if (
                this.isExistsPropertiesWantToUpdate(pro) &&
                oldItem[pro] !== newItem[pro]
            ) {
                return true;
            }
        }
