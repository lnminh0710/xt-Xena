...
export class DatatableService {
  ...
  hasControlType(column, controlName?) {
    ...
    if (this.hasSetting(column)) {
      const settingContainsControlType = this.getSettingContainsControlType(
  ...
  hasDisplayField(column, fieldName?) {
    let result = false;
    if (this.hasSetting(column)) {
      const settingContainsDisplayField = this.getSettingContainsDisplayField(
        column.setting.Setting
      );
  ...
  hasCustomStyle(column) {
    if (this.hasSetting(column)) {
      const settingContainsCustomStyle = this.getSettingContainsCustomStyle(
        column.setting.Setting
      );
  ...
  getDisableRowByValue(column) {
    if (this.hasSetting(column)) {
      ...
      for (let i = 0; i < settingArray.length; i++) {
        if (!isUndefined(settingArray[i].DisableRowByValue)) {
  ...
  getSettingContainsDisplayField(settingArray) {
    for (let i = 0; i < settingArray.length; i++) {
      if (!isUndefined(settingArray[i].DisplayField)) {
        return settingArray[i];
      }
    }
  ...
  getSettingContainsCustomStyle(settingArray) {
    for (let i = 0; i < settingArray.length; i++) {
      if (!isUndefined(settingArray[i].CustomStyle)) {
        return settingArray[i];
      }
    }
  ...
  getSettingContainsValidation(settingArray) {
    for (let i = 0; i < settingArray.length; i++) {
      if (!isUndefined(settingArray[i].Validation)) {
        return settingArray[i];
      }
    }
  ...
  hasEmptyRow(rows) {
    if (isUndefined(rows) || isEmpty(rows.data())) {
      return false;
    }
  ...
  createEmptyRowData(newItem, column, config, dataLength) {
    ...
    if (column.data === BUTTON_COLUMNS.download && config.allowDownload) {
    ...
    if (column.data === CHECKBOX_COLUMNS.deleted && config.allowDelete) {
      newItem[CHECKBOX_COLUMNS.deleted] = false;
    }

    if (this.continueButton.indexOf(column.data) > -1) {
  ...
  buildWijmoDataSource(dataSource, config?, agGridConfig?) {
    ...
    let isFormatMediaSize = false;
    let treeViewMode = false;
    if (agGridConfig) {
      isFormatMediaSize = agGridConfig.isFormatMediaSize;
      treeViewMode = agGridConfig.treeViewMode;
    }
  ...
  private continueButton = [
    BUTTON_COLUMNS.InvoicePDF,
    BUTTON_COLUMNS.RunSchedule,
    BUTTON_COLUMNS.PDF,
    BUTTON_COLUMNS.Tracking,
    BUTTON_COLUMNS.Return,
    BUTTON_COLUMNS.SendLetter,
