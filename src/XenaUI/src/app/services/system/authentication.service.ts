...
export class AuthenticationService extends BaseService {
    ...
    public login(user: User): Observable<any> {
        return this.post<UserAuthentication>(this.serUrl.serviceAuthenticateUrl,
            JSON.stringify({ 
    ...
    public loginByUserId(idLogin: string): Observable<any> {
        return this.post<UserAuthentication>(this.serUrl.loginByUserIdUrl, JSON.stringify({ idLogin: idLogin }));
    }

    public forgotPassword(loginName: string): Observable<any> {
        return this.post<any>(this.serUrl.serviceForgotPasswordUrl, JSON.stringify({ LoginName: loginName }));
    }

    public resetPassword(password: string, token?: string): Observable<any> {
        return this.post<any>(this.serUrl.serviceUpdatePasswordUrl, JSON.stringify({ NewPassword: password }), {}, token);
    }

    public changePassword(password: string, newPassword: string): Observable<any> {
        return this.post<any>(this.serUrl.serviceUpdatePasswordUrl, JSON.stringify({ Password: password, NewPassword: newPassword }));
    }
    ...
    public checkToken(token: any) {
        return this.post<any>(this.serUrl.checkTokenUrl, JSON.stringify({ AccessToken: token })).map((result: any) => {
            return result.item;
        });
    }
