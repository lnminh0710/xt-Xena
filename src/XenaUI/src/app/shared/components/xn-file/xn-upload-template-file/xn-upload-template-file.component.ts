  ...
  implements OnInit, OnDestroy
{
  ...
  private buildSqlColumnDataGrid() {
    if (this.checkChangeFromColumnViewMode()) {
      const currentData = cloneDeep(this.sqlColumnsDataSourceTable.data);
      this.resetGridData();
      this.sqlColumnsDataSourceTable = new ControlGridModel({
        columns: this.createGridColumns(),
        data: currentData,
  ...
  private makeCountriesCached(originalCountries: Array<any>) {
    const tempCountries = cloneDeep(this.countryCheckListData);
    if (!tempCountries || !tempCountries.length) {
      tempCountries.length = 0;
      return;
    }
    const originaleCountriesObject = Uti.mapArrayToObjectWithSelfPropertyName(
      originalCountries,
      'IdCountrylanguage'
    );
    ...
    for (const item of tempCountries) {
      item.isActive = false;
  ...
  private getTemplateData() {
    this._commonService
      .getListComboBox(ComboBoxTypeConstant.repAppSystemColumnNameTemplate)
      .subscribe((response: ApiResultResponse) => {
        this._appErrorHandler.executeAction(() => {
          if (
            !Uti.isResquestSuccess(response) ||
  ...
  private getCountries() {
    this._campaignService.getCampaignWizardCountry().subscribe((response) => {
      this._appErrorHandler.executeAction(() => {
