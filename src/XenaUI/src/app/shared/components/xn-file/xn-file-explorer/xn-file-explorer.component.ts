  ...
  implements OnInit, OnDestroy
{
  ...
  public onRowClickHandler($event) {
    ...
    this._currentItem =
      this.dataSourceTable.data.data.find(
        (x) =>
          x.IdSalesCampaignAddOnDocTemplate === idSalesCampaignAddOnDocTemplate
      ) || {};
    ...
    setTimeout(() => {
      ...
      this._template.idValue =
        this._currentItem.IdRepAppSystemColumnNameTemplate + '';
      this.templateId = {
        templateId: this._currentItem.IdRepAppSystemColumnNameTemplate + '',
      };
  ...
  private setShowFileUploadGrid() {
    switch (this.fileUploadModuleType) {
      ...
      case FileUploadModuleType.Campaign:
        this.showFileUploadGird = true;
        this.allowUploadeExtension = this.configuration.campaignExtension;
        setTimeout(() => {
  ...
  private listTreeItemByIdSharingTreeGroupsRootname(
    ...
    this.campaignService
      .listTreeItemByIdSharingTreeGroupsRootname(_idSharingTreeGroupRootName)
      .subscribe((response: ApiResultResponse) => {
        this.appErrorHandler.executeAction(() => {
          if (!Uti.isResquestSuccess(response)) {
            return;
  ...
  public onDeleteFilesCompleted(event) {
    if (event) {
      ...
      this.callRefreshData(
        this.fileUploadModuleType === FileUploadModuleType.ToolsFileTemplate
      );
