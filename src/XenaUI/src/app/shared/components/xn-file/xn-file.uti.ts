...
export class XnFileUti {
    private static _REG_GET_FIELDS = new RegExp("select (.*?) from", "gi");
    private static _REG_REPLACE_FIELD = new RegExp("\\.\\s?(?![^\\(]*\\))");
    private static _REG_REPLACE_SQL = new RegExp("\\,\\s?(?![^\\(]*\\))");

    public static builDataSourceFromSqlText(
        sqlText: string,
        isOnlyFields?: boolean
    ): Array<any> {
        if (!sqlText || isEmpty(sqlText) || !sqlText.trim()) {
            return [];
        }
        let sqlVariables: any = "";
        if (!isOnlyFields) {
            const fieldMatch = sqlText.match(this._REG_GET_FIELDS);
            sqlVariables =
                fieldMatch && fieldMatch.length > 0 ? fieldMatch[1] : "";
            sqlVariables =
                sqlVariables && sqlVariables.length ? sqlVariables[0] : "";
        } else {
            sqlVariables = sqlText;
        }
        return this.parseVariablesStringToColumns(sqlVariables);
    }
    private static parseVariablesStringToColumns(
        sqlVariables: string
    ): Array<any> {
        if (!sqlVariables || !sqlVariables.trim()) {
            return [];
        }
        const sqlVariableArr = sqlVariables
            .replace(/\n/g, "")
            .split(this._REG_REPLACE_SQL);
        let result = [];
    ...
    private static parseSQLVariableItemToObject(sqlVariableItem: string): any {
        if (!sqlVariableItem || !sqlVariableItem.trim()) {
            return null;
        }
        ...
        let templateField = sqlVariableItem.substring(
            asIndex + 3,
            sqlVariableItem.length
        );
        ...
        return {
            ...
            DataField: templateField
                ? templateField
                : this.getRightDataField(dataField),
