                            ...
                            selectionWidgetSplitterSettings.forEach(
                                (setting) => {
                                    ...
                                    this.splitContainerComponent.updateArea(
                                        this.splitContainerComponent.areas[0]
                                            .component,
                                        1,
                                    ...
                                    this.splitContainerComponent.updateArea(
                                        this.splitContainerComponent.areas[1]
                                            .component,
                                        1,
                ...
                this.appErrorHandler.executeAction(() => {
                    if (
                        ...
                        if (
                            ...
                            this.totalLoadingSteps =
                                response.item[0][0].TotalLoop;
                            let stepLength = Math.round(
                                100 / this.totalLoadingSteps
                            );
                            ...
                            if (
                                ...
                                this.percentage =
                                    this.currentLoadingStep * stepLength;
                                elem.style.width = this.percentage + "%";
                                document.getElementById(
                                    "loading-text"
                                ).innerHTML = response.item[0][0].Info;
                            }
                        }
    ...
    private _subscribeRequestBuildFrequencyState() {
        this.requestBuildFrequencyStateSubscription = this.dispatcher
            .filter((action: CustomAction) => {
                return (
                    action.type ===
                        ProcessDataActions.REQUEST_BUILD_FREQUENCY &&
                    action.module.idSettingsGUI == this.ofModule.idSettingsGUI
                );
            })
            .subscribe(() => {
                ...
                this.frequencyServiceSubscription = this.frequencyService
                    ...
                    .subscribe((response: ApiResultResponse) => {
                        this.appErrorHandler.executeAction(() => {
                            ...
                            this.frequencyGridData = this.buildGrid(
                                response.item[0]
                            );
    ...
    private toggleCountryColumns(showCountryOnly) {
        ...
        if (showCountryOnly) {
            ...
            for (let i = 0; i < cols.length; i++) {
                if (
                    cols[i].isVisible() &&
                    cols[i].getColDef().field != "Country"
                ) {
        ...
        if (this.frequencyGrid) {
            ...
            if (!this.fitWidthColumn) {
                if (!this.originalFrequencyColumnsWidth) {
                    ...
                    for (
                        let i = 0;
                        i < this.frequencyGrid.flex.columns.length;
                        i++
                    ) {
                ...
                if (!this.originalFrequencyRowsHeight) {
                    ...
                    for (
                        let i = 0;
                        i < this.frequencyGrid.flex.rows.length;
                        i++
                    ) {
                ...
                for (
                    ...
                    this.frequencyGrid.flex.columns[i].size =
                        this.originalFrequencyColumnsWidth[i] *
                        MAPPING[value].ratio;
                ...
                for (let i = 0; i < this.frequencyGrid.flex.rows.length; i++) {
                    this.frequencyGrid.flex.rows[i].size =
                        this.originalFrequencyRowsHeight[i] *
                        MAPPING[value].ratio;
            ...
            switch (mode) {
                ...
                case FilterModeEnum.EmptyData:
                    for (
                        ...
                        for (
                            let j = 0;
                            j < this.frequencyGrid.gridData.items.length;
                            j++
                        ) {
                            let thisColData =
