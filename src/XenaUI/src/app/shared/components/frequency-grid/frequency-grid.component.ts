...
export class FrequencyGridComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    ngAfterViewInit() {
        $('split-gutter', this.elementRef.nativeElement).bind('dblclick', this.onSplitterDblClick.bind(this));
    }
                    ...
                    if (data && data.length) {
                        ...
                        if (selectionWidgetSplitterSettings && selectionWidgetSplitterSettings.length) {
                            selectionWidgetSplitterSettings.forEach(setting => {
                                ...
                                this.splitContainerComponent.updateArea(this.splitContainerComponent.areas[0].component, 1, this.splitterConfig.leftHorizontal, 20);
                                this.splitContainerComponent.updateArea(this.splitContainerComponent.areas[1].component, 1, this.splitterConfig.rightHorizontal, 20);
                            });
                        }
    ...
    private loadDataForFrequency() {
        this.rebuildFrequenciesServiceSubscription = this.frequencyService.rebuildFrequencies(this.selectedEntity[camelCase(this.widgetListenKey)])
    ...
    private getFrequencyBusyIndicator() {
        this.percentage = 0;
        this.loadingInterval = setInterval(this.processGetFrequencyBusyIndicator.bind(this), 2000);

        this.changeDetectorRef.detectChanges();
    }

    private processGetFrequencyBusyIndicator() {
        this.frequencyService.getFrequencyBusyIndicator(this.selectedEntity[camelCase(this.widgetListenKey)])
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    if (Uti.isResquestSuccess(response)
                        && response.item
                        ...
                        if ((!response.item[0][0].CurrentLoop && !response.item[0][0].TotalLoop)
                            || (!this.currentLoadingStep && response.item[0][0].CurrentLoop == response.item[0][0].TotalLoop)) {
                        ...
                        if (this.currentLoadingStep != response.item[0][0].CurrentLoop || this.totalLoadingSteps != response.item[0][0].TotalLoop) {
                            ...
                            if (this.currentLoadingStep == this.totalLoadingSteps) {
                                ...
                                document.getElementById("loading-text").innerHTML = response.item[0][0].Info;
                            }
                        }
    ...
    private stopInterval(elem) {
        ...
        document.getElementById("loading-info").className = "text-green text-center w3-animate-opacity";
    ...
    private _subcribeSelectedTabState() {
        this.selectedTabStateSubscription = this.selectedTabState.subscribe((selectedTabState: TabSummaryModel) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (this.selectedEntity && this.selectedTab && this.selectedTab.tabSummaryInfor.tabID == 'Frequencies' && !this.countryGridData) {
                    this.initCountryGridData();
                }
    ...
    private _subscribeRequestSaveWidgetState() {
        this.requestSaveWidgetStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.REQUEST_SAVE_WIDGET && action.module.idSettingsGUI == this.ofModule.idSettingsGUI && this.elementRef.nativeElement.offsetParent != null;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                this.submit();
            });
    ...
    private _subscribeRequestResetWidgetState() {
        this.requestSaveWidgetStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === WidgetDetailActions.REQUEST_RESET_WIDGET && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
    ...
    private _subscribeRequestBuildFrequencyState() {
        this.requestBuildFrequencyStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.REQUEST_BUILD_FREQUENCY && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                if (this.selectedTab && this.selectedTab.tabSummaryInfor.tabID == 'Frequencies') {
    ...
    private loadFrequencyFromSeletedCountry() {
        if (this.countryGrid && this.countryGrid.selectedNode) {
            ...
            if (!this.ignoreCache && this.cacheData[countryLanguageId]) {
                this.showLoading = false;
                this.frequencyGridData = cloneDeep(this.cacheData[countryLanguageId].gridData);
            } else {
                this.frequencyServiceSubscription = this.frequencyService.getFrequency(this.selectedEntity[camelCase(this.widgetListenKey)], countryId)
                    .subscribe((response: ApiResultResponse) => {
                        this.appErrorHandler.executeAction(() => {
                            this.showLoading = false;
        ...
        formatRawData[0].forEach(dt => {
            columns.push(
                new ControlGridColumnModel({
                    ...
                    data: dt.header,
                    readOnly: true,
                    minWidth: typeof dt.value === 'object' ? 70 : (dt.header == 'Rules' ? 70 : (dt.header == 'Total Qty' ? 70 : null)),
                    setting: typeof dt.value === 'object'
                        ? {
                            Setting: [{
                                ControlType: {
                        ...
                        : (dt.header == 'Total Qty'
                            ? {
                                Setting: [{
                                    ...
                                    DisplayField: { Align: 'Center', Width: 70 }
                                }]
                            }
                            : (dt.header == 'Rules'
                                ? {
                                    Setting: [{
                                        DisplayField: { Align: 'Left', Width: 350 }
                                    }]
                                }
                                : null
                            )
    ...
    public submit(callback?) {
        ...
        this.frequencyServiceSubscription = this.frequencyService.saveFrequency(saveData)
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    if (!Uti.isResquestSuccess(response)) {
                        return;
    ...
    private createJsonData() {
        ...
        Object.keys(countryItems).forEach(countryLanguageId => {
            ...
            saveData.Frequencies = this.buildFrequencyDataForSave(countryItems[countryLanguageId].gridData.data, countryId);
        });
    ...
    public changeZoom(value) {
        if (this.frequencyGrid) {
            $(this.frequencyGrid.flex.hostElement).css('font-size', MAPPING[value].font + 'px');
            ...
            if (!this.fitWidthColumn) {
                if (!this.originalFrequencyColumnsWidth) {
                    ...
                    for (let i = 0; i < this.frequencyGrid.flex.columns.length; i++) {
                        this.originalFrequencyColumnsWidth.push(this.frequencyGrid.flex.columns[i].renderSize);
                    }
                }
                ...
                if (!this.originalFrequencyRowsHeight) {
                    ...
                    for (let i = 0; i < this.frequencyGrid.flex.rows.length; i++) {
                        this.originalFrequencyRowsHeight.push(this.frequencyGrid.flex.rows[i].renderSize);
                    }
                }

                for (let i = 0; i < this.frequencyGrid.flex.columns.length; i++) {
                    this.frequencyGrid.flex.columns[i].size = this.originalFrequencyColumnsWidth[i] * MAPPING[value].ratio;
                    this.frequencyGrid.flex.columns[i].width = this.originalFrequencyColumnsWidth[i] * MAPPING[value].ratio;
                }

                for (let i = 0; i < this.frequencyGrid.flex.rows.length; i++) {
                    this.frequencyGrid.flex.rows[i].size = this.originalFrequencyRowsHeight[i] * MAPPING[value].ratio;
                    this.frequencyGrid.flex.rows[i].height = this.originalFrequencyRowsHeight[i] * MAPPING[value].ratio;
                }
            } else {
                for (let i = 0; i < this.frequencyGrid.flex.columns.length; i++) {
            ...
            let cellHeight = $('.wj-cells .wj-cell.readonly', this.frequencyGrid.flex.hostElement).outerHeight(true);
                ...
                case FilterModeEnum.EmptyData:
                    for (let i = 0; i < this.frequencyGrid.flex.columns.length; i++) {
                        ...
                        for (let j = 0; j < this.frequencyGrid.gridData.items.length; j++) {
                            let thisColData = this.frequencyGrid.gridData.items[j][this.frequencyGrid.flex.columns[i].binding];

                            if (typeof thisColData === 'object') {
                                if (!((isNil(thisColData['ExportQty']) || thisColData['ExportQty'] === 0)
