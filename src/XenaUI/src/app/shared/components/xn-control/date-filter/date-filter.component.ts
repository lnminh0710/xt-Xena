    ...
    implements OnInit, OnDestroy
{
    ...
    private makeOutputRequestData() {
        ...
        this.outputDataAction.emit({
            IdRepAppSystemScheduleServiceName:
                this._idRepAppSystemScheduleServiceName,
    ...
    private makeOutputRequestDataByDate() {
        ...
        const model = {
            IdRepAppSystemScheduleServiceName:
                this._idRepAppSystemScheduleServiceName,
            Mode: this.getModeForRequestService(),
            FromDate: this.uti.formatLocale(
                this.byDateDateFrom.value,
                this._const.dateFormatInDataBase
            ),
            ToDate: this.uti.formatLocale(
                this.byDateDateTo.value,
                this._const.dateFormatInDataBase
            ),
        };
    ...
    private executeData(data: any) {
        const keyObject = data.widgetDataType.listenKeyRequest(
            this.ofModule.moduleNameTrim
        );
        this._idRepAppSystemScheduleServiceName =
            keyObject && keyObject.IdRepAppSystemScheduleServiceName
                ? keyObject.IdRepAppSystemScheduleServiceName
                : "";
    ...
    private showQuantityData(data: any) {
        ...
        if (
            ...
            this.uiFilterData[0].count =
                data.contentDetail.data[0][0].TotalCount || 0;
            this.uiFilterData[1].count =
                data.contentDetail.data[0][1].TotalCount || 0;
            this.uiFilterData[2].count =
                data.contentDetail.data[0][2].TotalCount || 0;
        }
    ...
    private buildDateDetail(user: User) {
        this.uiFilterData[0].subTitle = this.uti.formatLocale(
            this.today,
            "cccc"
        );
        this.uiFilterData[1].subTitle = this.uti.formatLocale(this.today, "ww");
        this.uiFilterData[2].subTitle = this.uti.formatLocale(
            this.today,
            "MMMM"
        );
    }
    ...
    private callGetCounterForByDate(requestData) {
        this._toolsService
            .getSummayFileResultSystemSchedule(requestData)
            .subscribe((response: any) => {
                this._appErrorHandler.executeAction(() => {
                    if (
                        ...
                        return;
                    this.uiFilterData[3].count =
                        response.item.data[0][0].TotalCount || 0;
                });
            });
    }
}
