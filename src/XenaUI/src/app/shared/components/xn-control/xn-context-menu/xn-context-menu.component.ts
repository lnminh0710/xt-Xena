...
export class XnContextMenuComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.showMenuSubcribe = this.contextMenuService.show.subscribe((e) =>
      this.showMenu(e.event, e.obj)
    );
    this.setMenuDataSubcribe = this.contextMenuService.setMenuData.subscribe(
      (e) => this.setMenuData(e.obj)
    );
  }
  ...
  public hoverOnLink($event) {
    ...
    if (childHeight + link.offset().top > fullHeight) {
      child.css(
        'top',
        '-' + (childHeight - (fullHeight - link.offset().top)).toString() + 'px'
      );
    }
    child.css('opacity', '1');
  }
  ...
  public menuItemClicked(func) {
    ...
    this.store.dispatch(
      this.xnCommonActions.contextMenuClicked(true, this.ofModule)
    );
    func();
  }
  ...
  private subcribeDontWantToShowContext() {
    this.dontWantToShowContextSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === ProcessDataActions.DONT_WANT_TO_SHOW_CONTEXT_MENU
        );
      })
      .subscribe(() => {
  ...
  private showMenu(event: any, menuData: any) {
    if (this.childrenData && this.childrenData.length) {
      this.isChildren = true;
      this.menuData = this.buildTranslateKey(
        this.childrenData.filter((x) => !x.hidden)
      );
      return;
    }
    this.mouseLocation = {
  ...
  private setMenuData(menuData: any) {
    ...
    this.menuData = this.buildTranslateKey(
      cloneDeep(menuData.filter((x) => !x.hidden))
    );
    this.appendDefaultValueToTranslateResource();
  }
