...
describe("MatInput without forms", function () {
    ...
    it("should default to floating placeholders", () => {
        ...
        expect(formField.floatPlaceholder).toBe(
            "auto",
    ...
    it("should not be treated as empty if type is date", inject(
        [Platform],
        (platform: Platform) => {
            if (
                !(
                ...
                expect(el.classList.contains("mat-form-field-empty")).toBe(
                    false
                );
            }
        }
    ...
    it("should be treated as empty if type is date on Firefox and IE", inject(
        [Platform],
        (platform: Platform) => {
            if (
                platform.TRIDENT ||
    ...
    it("should not be empty when the value set before view init", async(() => {
        ...
        let placeholderEl = fixture.debugElement.query(
            By.css(".mat-form-field-placeholder")
        ).nativeElement;
    ...
    it("supports the disabled attribute as binding", async(() => {
        ...
        const underlineEl = fixture.debugElement.query(
            By.css(".mat-form-field-underline")
        ).nativeElement;
        const inputEl = fixture.debugElement.query(
            By.css("input")
        ).nativeElement;

        expect(underlineEl.classList.contains("mat-disabled")).toBe(
            false,
            `Expected underline not to start out disabled.`
        );
        ...
        expect(underlineEl.classList.contains("mat-disabled")).toBe(
            true,
            `Expected underline to look disabled after property is set.`
        );
    ...
    it("sets the aria-describedby when a hintLabel is set, in addition to a mat-hint", () => {
        ...
        let hintLabel = fixture.debugElement.query(
            By.css(".mat-hint:not(.mat-right)")
        ).nativeElement;
        let endLabel = fixture.debugElement.query(
            By.css(".mat-hint.mat-right")
        ).nativeElement;
        let input = fixture.debugElement.query(By.css("input")).nativeElement;
        let ariaValue = input.getAttribute("aria-describedby");

        expect(ariaValue).toBe(
            `${hintLabel.getAttribute("id")} ${endLabel.getAttribute("id")}`
        );
    });
    ...
    it("should be able to toggle the floating placeholder programmatically", () => {
        ...
        const placeholder = formField.nativeElement.querySelector(
            ".mat-form-field-placeholder"
        );

        expect(containerInstance.floatPlaceholder).toBe("auto");
        expect(placeholder.classList).toContain(
            "mat-form-field-empty",
            "Expected input to be considered empty."
        );

        containerInstance.floatPlaceholder = "always";
        fixture.detectChanges();

        expect(placeholder.classList).not.toContain(
            "mat-form-field-empty",
    ...
    it("should not have prefix and suffix elements when none are specified", () => {
        ...
        let prefixEl = fixture.debugElement.query(
            By.css(".mat-form-field-prefix")
        );
        let suffixEl = fixture.debugElement.query(
            By.css(".mat-form-field-suffix")
        );
...
describe("MatInput with forms", () => {
    ...
    describe("error messages", () => {
        ...
        it("should display an error message when the parent form group is submitted", async(() => {
            ...
            let groupFixture = TestBed.createComponent(
                MatInputWithFormGroupErrorMessages
            );
            ...
            inputEl = groupFixture.debugElement.query(
                By.css("input")
            ).nativeElement;
        ...
        it("should hide the errors and show the hints once the input becomes valid", async(() => {
            ...
            fixture.whenStable().then(() => {
                ...
                fixture.whenStable().then(() => {
                    ...
                    expect(
                        containerEl.querySelectorAll("mat-error").length
                    ).toBe(
                        0,
                        "Expected no error messages when the input is valid."
                    );
                    expect(
                        containerEl.querySelectorAll("mat-hint").length
                    ).toBe(
                        1,
        ...
        it("should not hide the hint if there are no error messages", async(() => {
            ...
            expect(containerEl.querySelectorAll("mat-hint").length).toBe(
                1,
                "Expected one hint to be shown on load."
            );
            ...
            fixture.whenStable().then(() => {
                expect(containerEl.querySelectorAll("mat-hint").length).toBe(
                    1,
                    "Expected one hint to still be shown."
                );
            });
        }));
        ...
        it("should set the proper role on the error messages", () => {
            ...
            expect(
                containerEl.querySelector("mat-error")!.getAttribute("role")
            ).toBe("alert");
        });
    ...
    it("should display disabled styles when using FormControl.disable()", () => {
        ...
        expect(underlineEl.classList).not.toContain(
            "mat-disabled",
            `Expected underline not to start out disabled.`
        );
        expect(inputEl.disabled).toBe(false);
...
@Component({
    template: `<mat-form-field
        ><input matInput /><mat-hint>{{ label }}</mat-hint></mat-form-field
    >`,
})
...
@Component({
    template: `<mat-form-field [hintLabel]="label"
        ><input matInput
    /></mat-form-field>`,
})
...
@Component({
    template: ` <mat-form-field hintLabel="Hello">
        <input matInput />
...
@Component({
    template: ` <mat-form-field [floatPlaceholder]="shouldFloat">
        <input matInput placeholder="Label" />
    </mat-form-field>`,
})
