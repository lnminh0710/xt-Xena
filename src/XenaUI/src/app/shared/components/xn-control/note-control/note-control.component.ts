    ...
    implements ControlValueAccessor, AfterViewInit, OnDestroy
{
    ...
    @Output() onUpadteEditingWidget: EventEmitter<any> =
    ...
    onSubscribe() {
        ...
        this.dispatcher
            .filter((action: CustomAction) => {
                return (
                    action.type === ProcessDataActions.TURN_OFF_FORM_EDIT_MODE
                );
            })
            .takeUntil(this.getUnsubscriberNotifier())
            .subscribe(() => {
            ...
            value.forEach((v) => {
                const form = this.fb.group(
                    new Note({
                        ...
                        [this.settingModel.formName]:
                            v[this.settingModel.formName],
                        IsActive: v.IsActive,
                    })
                );
    ...
    editForm(form: AbstractControl, edited: boolean, indexForm: number) {
        ...
        if (!edited) {
            oldValue = this.pristineFormArrays.find(
                (f) =>
                    f[this.settingModel.idName] ===
                    form.value[this.settingModel.idName]
            );
            form.patchValue({
                ...oldValue,
                Editing: edited,
    ...
    saveForm(
        ...
        form.patchValue({
            IsDeleted: isDeleted,
            IsActive:
                isDeleted === NoteEnum.ZERO ? NoteEnum.ONE : NoteEnum.ZERO,
        });
    ...
    getFormatTime() {
        switch (
            this.propertyPanelService.getItemRecursive(this.props, "Display")
                .value
        ) {
