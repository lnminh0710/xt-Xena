...
import { AbstractControl, ControlValueAccessor, FormArray, FormBuilder, FormControl, NG_VALUE_ACCESSOR } from '@angular/forms';
import { DomSanitizer } from '@angular/platform-browser';
import { User } from 'app/models';
import { Note, NoteActionEnum, NoteEnum, NoteFormDataAction, NoteLoading } from 'app/models/note.model';
...
import { CustomAction, ProcessDataActions, WidgetDetailActions } from 'app/state-management/store/actions';
...
export class NoteControlComponent extends BaseComponent implements ControlValueAccessor, AfterViewInit, OnDestroy {
    ...
    @Input() settingModel: Partial<{ formName: string; idName: string; bgHeaderColor: string; hideActionButtons: boolean }> = {};
    ...
    @Output() onFocusForm: EventEmitter<{ form: FormControl, id: string }> = new EventEmitter<{ form: FormControl, id: string }>();
    ...
    ngAfterViewInit() {
        this.formArrays.valueChanges.takeUntil(this.getUnsubscriberNotifier()).subscribe(() => {
            this.onTouched();
            ...
            if (this.noteFormDataAction && this.noteFormDataAction.status) {
                ...
                this.noteFormDataAction.status.hasData = this.addingForm
                    && this.addingForm.get('Notes')
                    && this.addingForm.get('Notes').value
                    && this.addingForm.get('Notes').value.trim();

                this.onUpadteEditingWidget.emit(!!this.noteFormDataAction.status.hasData)
            }
        });
    }
    ...
    onSubscribe() {
        this.dispatcher.filter((action: CustomAction) => {
            const value = action.type === WidgetDetailActions.REQUEST_SAVE
                && action.module
            ...
            return value;
        }).takeUntil(this.getUnsubscriberNotifier()).subscribe(() => {
            if (!!this.noteFormDataAction && !!this.noteFormDataAction.status && !!this.noteFormDataAction.status.hasData)
                this.saveForm(this.addingForm, NoteEnum.ZERO);
            else if (this.addingForm)
                this.removeForm(0);
        });
        ...
        this.dispatcher.filter((action: CustomAction) => {
            ...
            if (!this.noteFormDataAction || !this.noteFormDataAction.action || !this.noteFormDataAction.status) return;
    ...
    writeValue(value: Note[]) {
        ...
        if (!!value && value.length) {
            value.forEach(v => {
                const form = this.fb.group(new Note({
                    date: v.date, Notes: v.Notes, LoginName: v.LoginName, IdLogin: v.IdLogin, [this.settingModel.idName]: v[this.settingModel.idName],
                    IsDeleted: v.IsDeleted, Editing: v.Editing, Removeable: v.Removeable, Cancelable: v.Cancelable,
                    [this.settingModel.formName]: v[this.settingModel.formName], IsActive: v.IsActive
                }));
    ...
    addNote() {
        const newForm = this.fb.group(new Note(
            { date: new Date(), Notes: '', LoginName: '', IdLogin: this.currentUser.id, Editing: true, Removeable: true, Cancelable: false }
        ));
    ...
    editForm(form: AbstractControl, edited: boolean, indexForm: number) {
        let oldValue: Note;
        if (!edited) {
            oldValue = this.pristineFormArrays.find(f => f[this.settingModel.idName] === form.value[this.settingModel.idName]);
    ...
    saveForm(form: AbstractControl, isDeleted: NoteEnum, hasEmit: boolean = true) {
        // emit form
        const callback = (): void => {
            form.patchValue({ Editing: false, Cancelable: false, Removeable: false });
            let index = this.pristineFormArrays.findIndex(f => f[this.settingModel.idName] === form.value[this.settingModel.idName]);
            if (index > -1) {
        ...
        form.patchValue({ IsDeleted: isDeleted, IsActive: isDeleted === NoteEnum.ZERO ? NoteEnum.ONE : NoteEnum.ZERO });
    ...
    getFormatTime() {
        switch (this.propertyPanelService.getItemRecursive(this.props, 'Display').value) {
