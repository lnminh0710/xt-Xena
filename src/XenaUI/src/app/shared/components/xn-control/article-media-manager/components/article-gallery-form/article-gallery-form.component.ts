...
export class ArticleGalleryFormComponent implements OnDestroy {
    ...
    private createFormGroup(image: GalleryImage) {
        ...
        this.articleMediaForm = new FormGroup({
            articleGroupsMedia: new FormGroup({
                idSharingTreeMedia: new FormControl(articleMedia.idSharingTreeMedia.value),
                idArticleGroupsMedia: new FormControl(articleMedia.idArticleGroupsMedia.value)
            }),
    ...
    onSubmit({ value, valid }: { value: any, valid: boolean }) {
        if (valid) {
            this.articleServiceSubscription = this.articleService.updateArticleMedia(value).subscribe(rs => {
                this.appErrorHandler.executeAction(() => {
                    this.activeImage.title = value.sharingTreeMedia.mediaTitle;
                    this.activeImage.description = value.sharingTreeMedia.mediaDescription;
