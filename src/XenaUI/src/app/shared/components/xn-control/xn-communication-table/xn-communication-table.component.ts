...
export class XnCommunicationTableComponent implements OnInit, OnDestroy {
    ...
    private buildDatatable(data?: any) {
        if (
            !this.dataSourceTable ||
            !this.dataSourceTable.columns ||
            !this.dataSourceTable.columns.length
        )
            return;
        this.dataSourceTable = {
            data: data,
    ...
    private emitOutputData(isDirty?: boolean) {
        ...
        if (this.isWidgetInfo)
            this.outputData.emit(
                this.buildOutputDataForDataEntryWidget(isDirty)
            );
        else this.outputData.emit(this.mapToSaveData());
    }
    ...
    private getAddedData(): Array<any> {
        ...
        const addedItem = updatedItems.itemsAdded.filter(
            (x) =>
                x.DT_RowId.indexOf("newrow") > -1 &&
                !x.IsDeleted &&
                !x[ColHeaderKey.Delete]
        );
        const editedItem = updatedItems.itemsEdited.filter(
            (x) =>
                (!x.DT_RowId || x.DT_RowId.indexOf("newrow") < 0) &&
                !x.IsDeleted &&
                !x[ColHeaderKey.Delete]
        );
        const deletedItem = updatedItems.itemsRemoved.filter(
            (x) => !x.DT_RowId || x.DT_RowId.indexOf("newrow") < 0
        );
        ...
        if (!isNil(eventData)) {
            if (this.xnAgGridComponent) {
                if (!eventData.showCommandButtons) {
                    ...
                    this.xnAgGridComponent.isMarkedAsDelete =
