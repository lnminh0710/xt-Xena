        ...
        AfterViewInit
{
    ...
    @Output() public onUpdateValue: EventEmitter<ControlBase<any>> =
        new EventEmitter();
    ...
    public initRichCombo() {
        this.multiselectControl.formatItem.addHandler((s, event: any) => {
            ...
            event.item.addEventListener("click", (e) => {
                e.preventDefault();
                let itemKey =
                    e.currentTarget.querySelector("div[data-key]").dataset.key;
                let item = this.multiselectControl.itemsSource.find(
    ...
    private initPriorityList() {
        this.priorities = [];
        const priorityList = this.multiselectControl.itemsSource.map(
            (o) => o["priority"]
        );
