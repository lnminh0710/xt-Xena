...
export class AgeFilterComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @Input() set comboData(data: any) {
        ...
        if (!this.listComboBoxRaw || !this.listComboBoxRaw['Types'])
            return;
    ...
    set data(item: any) {
        ...
        if (item) {
            this.ageForm = new FormGroup({
                ...
                fromValue: new FormControl(item['fromValue'], [Validators.required]),
                toValue: new FormControl(item['toValue'], [Validators.required]),
                groupType: new FormControl(item['groupType'], [Validators.required]),
                groupOperators: new FormControl(item['groupOperators'], [Validators.required]),
                groupQuantity: new FormControl(item['groupQuantity'], [Validators.required])
            });
        }
        else {
            this.ageForm = new FormGroup({
    ...
    public setMinMaxValueForControls() {
        if (this.ageForm.value['range'] == 'dates') {
            if (this.fromDateControl) {
                this.fromDateControl.max = !this.ageForm.value['toValue'] ? this.today : this.ageForm.value['toValue'];
            }
            if (this.toDateControl) {
                this.toDateControl.min = !this.ageForm.value['fromValue'] ? null : this.ageForm.value['fromValue'];
                this.toDateControl.max = this.today;
            }
        } else {
            if (this.fromNumberControl) {
                this.fromNumberMax = !this.ageForm.value['toValue'] ? null : this.ageForm.value['toValue'];
                this.fromNumberMin = 0;
            }
            if (this.toNumberControl) {
                this.toNumberMin = !this.ageForm.value['fromValue'] ? 0 : this.ageForm.value['fromValue'];
            }
        }
    }
    ...
    public updateFieldValidatorOnValueChange() {
        if (this.ageForm.value['groupType'] == 'Single') {
            ...
            this.ageForm.controls['groupOperators'].setValidators(Validators.required);
            this.ageForm.controls['groupQuantity'].setValidators(Validators.required);

            if (this.ageForm['submitted']) {
                this.ageForm.controls['groupOperators'].setErrors({ 'required': true });
                this.ageForm.controls['groupQuantity'].setErrors({ 'required': true });
            }
        }
    }
}
