...
export class DynamicFilterComponent extends DynamicFilterBase implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(private _eref: ElementRef, private commonService: CommonService) {
        super();
    }
    ...
    public onUpdateValue(control: ControlBase<any>) {
        ...
        if (control.controlType === 'dropdown' && !control.isHidden) {
            ...
            if (targetControl && targetControl.children && targetControl.children.length) {
                // Set all hidden
