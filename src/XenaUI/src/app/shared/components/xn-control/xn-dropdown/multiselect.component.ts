    ...
    OnDestroy
{
        ...
        if (this.selectedItems && this.selectedItems.length) {
          ...
          try {
            if (this.itemFormatter) {
              defaultText =
                '' +
                this.itemFormatter(this.selectedIndex, this.selectedItems[0]);
            }
          } catch {}
          return defaultText;
      ...
      if (this.settings.singleSelection) {
        try {
          if (this.selectedValue != value) {
            ...
            //if (this.selectedItems && this.selectedItems.length) {
            //    this.onSelect.emit(this.selectedItems[0]);
            //}
          }
  ...
  /*
    private setValueAndFocusTextInput(value) {
        // use this code to avoid data binding (improve perfermance)
  ...
  private appendDropdownToBody() {
    ...
    //this.renderer.setElementStyle(this.dropdownListElem.nativeElement, 'top', domRect.top + domRect.height + 'px');
    //this.renderer.setElementStyle(this.dropdownListElem.nativeElement, 'left', domRect.left + 'px');
    //this.renderer.setElementStyle(this.dropdownListElem.nativeElement, 'width', domRect.width + 'px');
  ...
  filterGroupedList() {
    ...
    this.groupedData = this.groupedData.filter((obj) => {
      ...
      return arr.some((cat) => {
        return (
          cat.itemName.toLowerCase().indexOf(this.filter.toLowerCase()) > -1
        );
      });
    });
          ...
          this.data.filter((el: any) => {
            if (
              el[this.settings.searchBy[t].toString()]
                .toString()
                .toLowerCase()
                .indexOf(evt.target.value.toString().toLowerCase()) >= 0
            ) {
  ...
  private replaceBetweenCharacter(value: string, index, replacement) {
    return value.substr(0, index) + replacement + value.substr(index);
  }
  ...
  onSearchChange($event, value: string) {
    ...
    if (value) {
      ...
      if (isBackspaceOrDelete) {
        if (selectionStart == selectionEnd) {
          value =
            value.substr(0, selectionStart - 1) + value.substr(selectionEnd);
        } else {
    ...
    if (searchList && searchList.length) {
      ...
      if (
        searchList[0] &&
        currentPosVal === searchList[0][this.settings.labelKey].toString()
      ) {
      ...
      /*
            if (!this.isEditable) {
                if (this.selectedItems && this.selectedItems.length) {
                    this.renderCustomText(this.selectedItems[0][this.settings.labelKey]);
                    // this.textInput.nativeElement.value = this.selectedItems[0][this.settings.labelKey];
                }
                else {
                ...
                this.renderCustomText(currentPosVal);
                // this.textInput.nativeElement.value = currentPosVal;
                this.selectedItems = [];
                if (isBackspace) {
                    if (selectionStart == selectionEnd) {
  ...
  //        .resolveComponentFactory(ItemListComponent)
  //        .create(this.injector);
  //    const itemListComponent: ItemListComponent = componentRef.instance;
  //    itemListComponent.data = this.data;
  //    itemListComponent.isActive = this.isActive;
  //    itemListComponent.isFilterSelectAll = this.isFilterSelectAll;
  //    itemListComponent.isInfiniteFilterSelectAll = this.isInfiniteFilterSelectAll;
  //    itemListComponent.isSelectAll = this.isSelectAll;
  //    itemListComponent.selectedItems = this.selectedItems;
  //    itemListComponent.settings = this.settings;
  ...
  //    domElem.style.top = domRect.top + domRect.height + 'px';
  //    domElem.style.left = domRect.left + 'px';
  //    domElem.style.width = domRect.width + 'px';
