...
export class VirtualScrollComponent implements OnInit, OnChanges, OnDestroy {
    ...
    public get bufferAmount(): number {
        return Math.max(
            this._bufferAmount,
            this.enableUnequalChildrenSizes ? 5 : 0
        );
    }
    ...
    public set parentScroll(value: Element | Window) {
        ...
        if (scrollElement !== this.element.nativeElement) {
            ...
            scrollElement.style["overflow-y"] = this.horizontal
                ? "visible"
                : "auto";
            scrollElement.style["overflow-x"] = this.horizontal
                ? "auto"
                : "visible";
        }
    }
    ...
    @Output()
    public vsChange: EventEmitter<ChangeEvent> =
    ...
    public scrollToIndex(
        ...
        let retryIfNeeded = () => {
            ...
            let desiredStartIndex: any = Math.min(
                Math.max(index, 0),
    ...
    protected scrollToIndex_internal(
        ...
        let dimensions: any = this.calculateDimensions();
        let scroll: any =
            this.calculatePadding(index, dimensions, false) +
            offset +
            additionalOffset;
        if (!alignToBeginning) {
            scroll -=
                dimensions.wrapGroupsPerPage * dimensions[this._childScrollDim];
        }
    ...
    protected checkScrollElementResized(): void {
        ...
        if (!this.previousScrollBoundingRect) {
            ...
            let widthChange: any = Math.abs(
                boundingRect.width - this.previousScrollBoundingRect.width
            );
            let heightChange: any = Math.abs(
                boundingRect.height - this.previousScrollBoundingRect.height
            );
            sizeChanged =
                widthChange > this.resizeBypassRefreshTheshold ||
                heightChange > this.resizeBypassRefreshTheshold;
        }
    ...
    protected refresh_internal(
        ...
        this.zone.runOutsideAngular(() => {
            requestAnimationFrame(() => {
                ...
                let startChanged: any =
                    itemsArrayModified ||
                    viewport.startIndex !== this.previousViewPort.startIndex;
                let endChanged: any =
                    itemsArrayModified ||
                    viewport.endIndex !== this.previousViewPort.endIndex;
                let scrollLengthChanged: any =
                    viewport.scrollLength !==
                    this.previousViewPort.scrollLength;
                let paddingChanged: any =
                ...
                if (startChanged || endChanged) {
                    ...
                    if (
                        maxRunTimes > 0 &&
                        (scrollLengthChanged || paddingChanged)
                    ) {
                        this.refresh_internal(
                            false,
                            refreshCompletedCallback,
        ...
        this.zone.runOutsideAngular(() => {
            if (this.parentScroll instanceof Window) {
                ...
                if (this._checkResizeInterval > 0) {
                    this.checkScrollElementResizedTimer = <any>(
                        setInterval(() => {
    ...
    protected getScrollPosition(): number {
        ...
        return (
            windowScrollValue || this.getScrollElement()[this._scrollType] || 0
        );
    }
    ...
    protected resetWrapGroupDimensions(): void {
        ...
        for (
            let wrapGroupIndex: any = 0;
            wrapGroupIndex <
            oldWrapGroupDimensions.maxChildSizePerWrapGroup.length;
            ++wrapGroupIndex
        ) {
    ...
    protected calculateDimensions(): IDimensions {
        ...
        if (!this.enableUnequalChildrenSizes) {
            ...
            defaultChildWidth =
                this.childWidth || this.minMeasuredChildWidth || viewWidth;
            defaultChildHeight =
                this.childHeight || this.minMeasuredChildHeight || viewHeight;
            let itemsPerRow: any = Math.max(
                Math.ceil(viewWidth / defaultChildWidth),
                1
            );
            let itemsPerCol: any = Math.max(
                Math.ceil(viewHeight / defaultChildHeight),
                1
            );
            ...
            let arrayStartIndex: any =
                this.previousViewPort.startIndexWithBuffer || 0;
            let wrapGroupIndex: any = Math.ceil(
            ...
            for (let i = 0; i < content.children.length; ++i) {
                ...
                if (arrayStartIndex % itemsPerWrapGroup === 0) {
                    ...
                    if (this.horizontal) {
                        let maxVisibleWidthForWrapGroup: any = Math.min(
                            maxWidthForWrapGroup,
                            Math.max(viewWidth - sumOfVisibleMaxWidths, 0)
                        );
                        if (scrollOffset > 0) {
                            let scrollOffsetToRemove: any = Math.min(
                                scrollOffset,
                        ...
                        if (
                            ...
                            viewWidth >= sumOfVisibleMaxWidths
                        ) {
                            ++wrapGroupsPerPage;
                        }
                    } else {
                        let maxVisibleHeightForWrapGroup: any = Math.min(
                            maxHeightForWrapGroup,
                            Math.max(viewHeight - sumOfVisibleMaxHeights, 0)
                        );
                        if (scrollOffset > 0) {
                            let scrollOffsetToRemove: any = Math.min(
                                scrollOffset,
                        ...
                        if (
                            ...
                            viewHeight >= sumOfVisibleMaxHeights
                        ) {
                            ++wrapGroupsPerPage;
                        }
                    }
            ...
            let averageChildWidth: any =
                this.wrapGroupDimensions.sumOfKnownWrapGroupChildWidths /
                this.wrapGroupDimensions.numberOfKnownWrapGroupChildSizes;
            let averageChildHeight: any =
                this.wrapGroupDimensions.sumOfKnownWrapGroupChildHeights /
                this.wrapGroupDimensions.numberOfKnownWrapGroupChildSizes;
            defaultChildWidth =
                this.childWidth || averageChildWidth || viewWidth;
            defaultChildHeight =
                this.childHeight || averageChildHeight || viewHeight;

            if (this.horizontal) {
                if (viewWidth > sumOfVisibleMaxWidths) {
                    wrapGroupsPerPage += Math.ceil(
                        (viewWidth - sumOfVisibleMaxWidths) / defaultChildWidth
                    );
                }
            } else {
        ...
        let defaultScrollLengthPerWrapGroup: any = this.horizontal
            ? defaultChildWidth
            : defaultChildHeight;
        if (this.enableUnequalChildrenSizes) {
    ...
    protected calculatePadding(
        ...
        dimensions: any,
        allowUnequalChildrenSizes_Experimental: boolean
    ): number {
        if (dimensions.itemCount === 0) {
            return 0;
        }

        let defaultScrollLengthPerWrapGroup: number =
            dimensions[this._childScrollDim];
        let startingWrapGroupIndex: number =
            Math.ceil(
                arrayStartIndexWithBuffer / dimensions.itemsPerWrapGroup
            ) || 0;
        ...
        for (let i = 0; i < startingWrapGroupIndex; ++i) {
            let childSize: WrapGroupDimension =
                this.wrapGroupDimensions.maxChildSizePerWrapGroup[i] &&
                this.wrapGroupDimensions.maxChildSizePerWrapGroup[i][
                    this._childScrollDim
                ];
    ...
    protected calculatePageInfo(
        ...
        let startingArrayIndex_fractional: any =
            Math.min(
        ...
        let arrayStartIndex: any = Math.min(
            Math.floor(startingArrayIndex_fractional),
            maxStart
        );
        arrayStartIndex -= arrayStartIndex % dimensions.itemsPerWrapGroup; // round down to start of wrapGroup

        let arrayEndIndex: any =
            Math.ceil(startingArrayIndex_fractional) +
            dimensions.itemsPerPage -
            1;
        arrayEndIndex +=
            dimensions.itemsPerWrapGroup -
            ((arrayEndIndex + 1) % dimensions.itemsPerWrapGroup); // round up to end of wrapGroup
        ...
        let startIndexWithBuffer: any = Math.min(
            Math.max(arrayStartIndex - bufferSize, 0),
            dimensions.itemCount - 1
        );
        let endIndexWithBuffer: any = Math.min(
    ...
    protected calculateViewport(): IViewport {
        ...
        let newPadding: any = this.calculatePadding(
