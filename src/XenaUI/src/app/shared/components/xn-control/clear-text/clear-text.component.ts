...
export class ClearTextComponent implements OnInit {
  ...
  private bindEventToControl(currentLoopTimes: number) {
    ...
    setTimeout(() => {
      if (this.controlName && this.eRef) {
        ...
        this.control = $(
          'input[type=text][name=' + this.controlName + ']',
          $(this.eRef.nativeElement).parent()
        );
        if (this.control) {
  ...
  public clearText(event) {
    if (
      this.control &&
      this.control['length'] &&
      !isNil($(this.control).val()) &&
      $(this.control).val()['length'] > 0
    ) {
      if (this.inputControl) {
        if (this.inputControl.setValue) {
          ...
          if (this.controlType === 'number') this.inputControl.value = 0;
          else this.inputControl.value = '';
        }
      }
      $(this.control).val('');
      $(this.control).focus();
