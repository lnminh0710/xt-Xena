...
export class XnMessageModalComponent implements OnInit, OnDestroy {
    ...
    @ViewChildren("buttonfooter") buttonfooters: QueryList<any>;
    ...
    constructor(
        ...
        this.xnModalStateModalData = this.store.select(
            (state) => state.modalState.modalData
        );
        this.xnModalStateModalShow = this.store.select(
            (state) => state.modalState.modalShowMessage
        );
        this.xnModalStateModalClose = this.store.select(
            (state) => state.modalState.modalCloseMessage
        );
        this.xnModalHasTranslatePopup = this.store.select(
            (state) => state.modalState.hasTranslatePopup
        );
    }
    ...
    private subscibeData() {
        this.xnModalStateModalDataSubcription =
            this.xnModalStateModalData.subscribe((data: MessageModalModel) => {
        ...
        this.xnModalStateModalShowSubcription =
            this.xnModalStateModalShow.subscribe((data: any) => {
                this.appErrorHandler.executeAction(() => {
                    if (!data) {
                        return;
                    }
                    this.showModal();
                    this.store.dispatch(
                        this.modalActions.modalShowMessage(false)
                    );
                });
            });

        this.xnModalStateModalCloseSubcription =
            this.xnModalStateModalClose.subscribe((data: any) => {
                this.appErrorHandler.executeAction(() => {
                    if (!data) {
                        return;
                    }
                    this.hideModal();
                    this.store.dispatch(
                        this.modalActions.modalCloseMessage(false)
                    );
                });
            });

        this.xnModalHasTranslatePopupSubcription =
            this.xnModalHasTranslatePopup.subscribe((data: any) => {
                this.appErrorHandler.executeAction(() => {
    ...
    private makeMessageOption(option: message.MessageModalModel) {
        ...
        this.showCloseButton = isNil(option.showCloseButton)
    ...
    private makeMessageFooter(option: message.MessageModalModel) {
        if (
            !option ||
            !option.footer ||
            !option.footer.buttonList ||
            option.footer.buttonList.length <= 0
        )
            return null;
        for (const item of option.footer.buttonList) {
            item.buttonType =
                item.buttonType || MessageModal.ButtonType.primary;
            item.callBackFunc =
                item.callBackFunc ||
                (() => {
    ...
    private focusOnPrimaryButton() {
        ...
        let button = this.buttonfooters.find((item, i) => {
            return (
                item._elementRef &&
                item._elementRef.nativeElement &&
                item._elementRef.nativeElement.className &&
                item._elementRef.nativeElement.className.includes(
                    "mat-force-focus"
                )
            );
        });
        if (!button)
            button = this.buttonfooters.find((item, i) => {
                return (
                    item._elementRef &&
                    item._elementRef.nativeElement &&
                    item._elementRef.nativeElement.className &&
                    item._elementRef.nativeElement.className.indexOf(
                        "mat-btn-blue"
                    ) !== -1
                );
            });
    ...
    public onKeyRight($event: any, index: number) {
        index++;
        index =
            index >= this.buttonfooters.length
                ? this.buttonfooters.length - 1
                : index;
    ...
    private appendDefaultValueToTranslateResource() {
        ...
        defaultLanguage[
            "Message_Modal__btn_" + this.messageOption.header.text
        ] = this.messageOption.header.text;
    }
    ...
    private setIndexForMessageModal() {
        ...
        setTimeout(() => {
            ...
            modalBackdrop.style.setProperty(
                "z-index",
