...
export class XnTreeViewLeftComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  public onBlur(treeNode: any, $event) {
    $event.preventDefault();
    if (!treeNode.text) treeNode.text = 'New Category';
    this.unRename.emit();
  }
  ...
  private findParentRawByChildId(child: any): any {
    return this.rawData.find(
      (x) => x.id === (!child.parentId ? child.id : child.parentId)
    );
  }
  ...
  private setSelectParentDeep(child: any, parent: any) {
    if (this.hasParent || !parent.children || !parent.children.length) {
      return;
    }
  ...
  public setPercentTextViewMode(percentText?: any): string {
    return percentText && parseFloat(percentText) > 0
      ? ' (' + percentText + '%)'
      : '';
  }
