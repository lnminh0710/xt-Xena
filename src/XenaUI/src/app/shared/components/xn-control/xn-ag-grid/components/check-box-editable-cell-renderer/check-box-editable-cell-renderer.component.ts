  ...
  implements ICellEditorAngularComp, AfterViewInit
{
  ...
  ngAfterViewInit() {
    if (this.params) {
      const hasDisabledBy = this.datatableService.hasDisplayField(
        this.params.colDef.refData,
        'DisabledBy'
      );
      if (hasDisabledBy) {
        const disabledByField = this.datatableService.getDisplayFieldValue(
          this.params.colDef.refData,
          'DisabledBy'
        );
        if (disabledByField) {
      ...
      if (
        this.componentParent.disableCheckBoxWhenInActive &&
