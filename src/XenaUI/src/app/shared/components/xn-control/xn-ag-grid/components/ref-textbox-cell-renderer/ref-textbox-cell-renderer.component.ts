...
export class RefTextboxCellRenderer
            ...
            this.commonService
                ...
                .subscribe((response: ApiResultResponse) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        this.buildDropdown(
                            response.item[comboboxType.name],
    ...
    private buildDropdown(data: any[], field) {
        if (
            ...
            document.getElementById(
                "wj-dropdown-panel-" + this.params.column.colDef.field
            )
        ) {
            return;
        }
        ...
        setTimeout(() => {
            let thisElmData = this.elementRef.nativeElement
                ? this.elementRef.nativeElement.getBoundingClientRect()
                : { width: 0, top: 0, left: 0 };

            let div = document.createElement("div");
            div.className =
                "wj-dropdown-panel wj-content wj-control wj-listbox";
            ...
            for (let i = 0; i < data.length; i++) {
                innerHTML +=
                    `<div class="wj-listbox-item wj-listbox-item-` +
                    field +
                    `" tabindex="-1">` +
                    data[i]["textValue"] +
                    `</div>`;
