...
export class ButtonAndCheckboxHeaderCellRenderer extends BaseAgGridCellComponent<boolean> implements IHeaderAngularComp, OnDestroy {
    ...
    public getCustomParam() {
        this._onDataChangeStateSubscription = this.componentParent.onDataChange.subscribe((data) => {
            ...
            for (let item of data.data) {
                if (!Uti.getValueFromObjectJsonString(item[this.colDef.field], 'Value')) {
                    this.value = false;
                    return;
                }
            }
        ...
        this._cellValueChangedSubscription = this.componentParent.buttonAndCheckboxValueChanged.subscribe((data) => {
            if (!data || !data.colDef || data.colDef.field != this.colDef.field) {
                return;
            }
            let hasFalseValue: boolean = false;
            this.params.api.forEachNode(node => {
                if (node && node.data && !Uti.getValueFromObjectJsonString(node.data[this.colDef.field], 'Value')) {
                    hasFalseValue = true;
                }
            });
        ...
        this.params.api.forEachNode(node => {
            ...
            if (data) {
                if (data && data[this.colDef.field]) {
                    data[this.colDef.field] = this.datatableService.updateValueForButtonAndCheckboxColumn(data[this.colDef.field], status);
