...
export class TemplateButtonCellRenderer extends BaseAgGridCellComponent<string> implements ICellRendererAngularComp {
    ...
    constructor(private store: Store<AppState>, private backofficeActions: BackofficeActions) {
        super();
    }
    ...
    public btnClick(eventData?: any) {
        switch (this.mode) {
            case 'InvoicePDF':
                this.store.dispatch(this.backofficeActions.requestDownloadPdf(this.componentParent.currentModule, this.params.data));
                break;
            case 'Tracking':
                this.store.dispatch(this.backofficeActions.requestGoToTrackingPage(this.componentParent.currentModule, this.params.data));
                break;
            case 'Return':
                this.store.dispatch(this.backofficeActions.requestOpenReturnRefundModule(this.componentParent.currentModule, this.params.data));
                break;
            case 'Mediacode':
            ...
            case 'Delete':
                if (this.componentParent.redRowOnDelete) {
                    this.componentParent.setDeleteCheckboxStatus(this.params.data, !this.params.data[ColHeaderKey.Delete], (item) => {
            ...
            case 'RunSchedule':
                ...
                break;
            case 'KillProcess':
              this.componentParent.killProcessClick.emit({data : this.params.data, setting : get(this.colDef , ['refData','setting','Setting',0,'StoreProcedure'])});
              break;
            case 'ShrinkFile':
              this.componentParent.shrinkFileClick.emit({data : this.params.data, setting : get(this.colDef , ['refData','setting','Setting',0,'StoreProcedure'])});
              break;
            case 'FilterExtended':
            ...
            case 'rowColCheckAll':
                switch (eventData) {
                    case 'horizontal':
                        let allTrue = this.params.data.RMDelete && this.params.data.RMEdit && this.params.data.RMExport && this.params.data.RMNew && this.params.data.RMRead;
                        if (allTrue) {
    ...
    private updateRowColCheckAllRecursive(children, parentData) {
        for (let i = 0; i < children.length; i++) {
            children[i].RMDelete = children[i].RMDelete != -1 && parentData.RMDelete != -1 ? parentData.RMDelete : children[i].RMDelete;
            children[i].RMEdit = children[i].RMEdit != -1 && parentData.RMEdit != -1 ? parentData.RMEdit : children[i].RMEdit;
            children[i].RMExport = children[i].RMExport != -1 && parentData.RMExport != -1 ? parentData.RMExport : children[i].RMExport;
            children[i].RMNew = children[i].RMNew != -1 && parentData.RMNew != -1 ? parentData.RMNew : children[i].RMNew;
            children[i].RMRead = children[i].RMRead != -1 && parentData.RMRead != -1 ? parentData.RMRead : children[i].RMRead;
