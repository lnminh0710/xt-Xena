  ...
  implements OnInit, OnDestroy
{
  ...
  ngOnInit() {
    ...
    this.moduleParkedItemAccessRight = this.accessRightService.getAccessRight(
  ...
  selectParkedItem(parkedItem) {
    ...
    this.store.dispatch(
      this.moduleActions.moveSelectedParkedItemToTop(this.ofModule, parkedItem)
    );
  }
  ...
  selectSubModule(subModule) {
    ...
    if (this.activeModule.idSettingsGUI != MenuModuleId.administration) {
      let newRoute =
        this.activeModule.moduleNameTrim + '/' + subModule.moduleNameTrim;
  ...
  public downloadScanningTool() {
    if (this.scanningTool && this.scanningTool.downloadUrl) {
      ...
      //let serverUrl = '/api/FileManager/GetFile?name=';
      //a.href = serverUrl + this.scanningTool.downloadUrl;
