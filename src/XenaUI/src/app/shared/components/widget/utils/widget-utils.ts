...
export class WidgetUtils {
          ...
          if (listenKeyRequest) {
            ...
            Object.keys(listenKeyRequest).forEach((key, index) => {
              if (
                ...
                objTemp[key.toLowerCase()] === listenKeyRequest[key]
              ) {
                count += 1;
              }
            });
    ...
    if (relatedWidgetDetail.idRepWidgetType === WidgetType.Combination) {
      for (let i = 0; i < relatedWidgetDetail.contentDetail.data.length; i++) {
        for (
          ...
          if (
            Object.keys(relatedWidgetDetail.contentDetail.data[i][j]).findIndex(
              (key) => {
            ...
            collectionData =
              relatedWidgetDetail.contentDetail.data[i][j].collectionData;
          }
        }
      }
    } else {
  ...
  public contextMenuInEditMode(
    ...
    contextMenuData[7].hidden =
      !(conditions && conditions.isShowUploadTemplate) || !accessRight.edit; // Upload Template
    contextMenuData[8].hidden =
      !(conditions && conditions.isShowDeleteTemplate) || !accessRight.edit; // Delete Template
    ...
    contextMenuData[10].hidden =
      !accessRight.ToolbarButton || !accessRight.ToolbarButton__TranslateButton; // Translate
    ...
    contextMenuData[12].hidden =
      !accessRight.ToolbarButton || !accessRight.ToolbarButton__PrintButton; // Print
  ...
  public contextMenuInViewMode(
    ...
    let newEntityMenuContextTitle =
      (currentModule.idSettingsGUI == ModuleList.BusinessCosts.idSettingsGUI
        ? 'Add '
    ...
    if (
      (currentModule &&
        this.excludeContextMenuModule.indexOf(currentModule.idSettingsGUI) !==
          -1) ||
      !toolbarSetting ||
  ...
  public mapProperty(
    ...
    switch (widgetType) {
      ...
      case WidgetType.SynchronizeElasticsSearch: {
        ...
        totalControl.push(
          Uti.adjustPropertyCommon(synchronizeElasticsSearch, property)
        );
        break;
      }
    ...
    widgetSettingDataArray.forEach((widgetSettingData) => {
      ...
      if (idRepWidgetApp && widgetType) {
        ...
        const widgetDetailPage: WidgetDetailPage = new WidgetDetailPage({
          ...
          columnsLayoutSettings: widgetConfig.columnsLayoutSettings || {
            settings: '',
          },
