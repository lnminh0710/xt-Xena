  ...
  implements OnInit, OnDestroy
{
  ...
  public widgetType: typeof WidgetType = WidgetType;
          ...
          this.appErrorHandler.executeAction(() => {
            ...
            if (this.layoutPageInfo && this.layoutPageInfo.length) {
              if (this.communicationWidgetInfo) {
                this.updateTitle(
                  this.communicationWidgetInfo.sameTypeWidgetInfos
                );
                this.updateTitle(
                  this.communicationWidgetInfo.relatingWidgetInfos
                );
              }
            }
          });
      ...
      widgetInfos.forEach((widgetInfo: IWidgetInfo) => {
        for (let i = 0; i < this.layoutPageInfo.length; i++) {
          if (
            ...
            let widgetBox = this.layoutPageInfo[i].widgetboxesTitle.find(
              (p) => p.id == widgetInfo.id
            );
            if (widgetBox) {
