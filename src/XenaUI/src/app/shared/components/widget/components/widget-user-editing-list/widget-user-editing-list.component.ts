...
export class WidgetUserEditingListComponent
    ...
    implements OnInit, OnDestroy
{
    ...
    @Input() set data(data: Array<SignalRNotifyModel>) {
        this.executeData(data);
    }
    @Input() set parent(data: any) {
        this.executeParent(data);
    }
    ...
    public toggleUserList() {
        ...
        this.commandIcon = this.isShowUserList
            ? "fa-caret-up"
            : "fa-caret-down";
    ...
    private calcMaxHeightForEachItem() {
        let remainHeight =
            this.parentHeight -
            this.userHeaderHeight * this.userList.length -
            this.spacing * (this.userList.length + 1);
        this.fieldBodyStyle = {
