...
export function MixinWidgetTable<T extends Constructor<TableServiceInjector>>(
    ...
    implements WidgetDetailInfo, WidgetMenu, WidgetAction
  {
    ...
    public showSelectionFilter = false;
    public showTotalRow: any = {};
    ...
    public allowGridTranslation = false;
    ...
    abstract get tabHeaderTableFilterInfo(): any;
    // abstract get widgetWijmoGridComponent(): QueryList<WijmoGridComponent>;
    ...
    abstract get savLetterTemplateComponent(): SavLetterTemplateComponent;
    ...
    private updateTableWidget() {
      if (this.agGridComponent) {
        ...
        const itemsRemoved =
          lengthRemoved.length > 0 && lengthRemoved.some((v) => v.IsDeleted);
        const itemsAdded =
          lengthAdded.length > 0 && lengthAdded.some((v) => v.IsDeleted);
    ...
    public updateTableStatusAfterCompletedSaving() {
      this.manageEditableTableStatusButtonsAfterSaving();
    ...
    protected onTableEditStart(eventData) {
      if (
        this.dataInfo.idRepWidgetApp == RepWidgetAppIdEnum.PrinterAndConfirm
      ) {
        return;
      }
    ...
    protected onRowMarkedAsDeleted(eventData) {
      if (!isNil(eventData)) {
        ...
        if (eventData.showCommandButtons) {
          ...
          this.checkToShowCommandButtons(eventData.showCommandButtons === false)
        ) {
          this.isOnEditingTable = false;
          this.controlMenuStatusToolButtons(false);
        }
      ...
      if (eventData && eventData.length) {
        for (const itemData of eventData) {
          ...
          if (itemData && itemData.isNew) {
            ...
            isNil(
              this.dataSourceTable.data.find((item) => item && item.isNew)
            ) &&
            this.widgetMenuStatusInfo
          ) {
            this.widgetMenuStatusInfo.manageAddRowTableButtonStatus(false);
          ...
          if (groupData) {
            ...
            if (keys.length) {
              ...
              if (this.groupNumber > this.groupTotal) {
                this.groupNumber = this.groupTotal;
              }

              this.moveToDoubletteGroup(this.groupNumber);
            }
          }
        }
      }
          ...
          if (
            ...
            if (objTemp[columnFilterKey.toLowerCase()]) {
              const rs: Array<string> = expectedValueArray.filter(
                (p) =>
                  p.toLowerCase() ===
                  (objTemp[columnFilterKey.toLowerCase()] + '').toLowerCase()
              );
              if (rs.length === 0) {
                isValidRow = false;
              }
    ...
    private _updateRowDisplayMode(instance: any) {
      ...
      const proSwitchToDetail = this.propertyPanelService.getItemRecursive(
        this.propertiesInfo,
        'AutoSwitchToDetail'
      );
      if (
      ...
      if (valueMultipleDisplayRowMode) {
        ...
        instance.changeToSingleRowMode(valueSwitchToDetail);
      }
    }
    ...
    protected resetFileExplorer(): void {
      if (this.xnFileExplorerComponentCtrl)
        this.xnFileExplorerComponentCtrl.refreshData(null);
      this.isDeletedFiles = false;
      this.isOnEditFileExplorer = false;
    }
    ...
    public onUploadFileClick($event?: any) {
      ...
      if (this.savLetterTemplateComponent)
        this.savLetterTemplateComponent.showDialog = true;
    }

    public onClickDeleteFiles() {
      if (this.xnFileExplorerComponentCtrl)
        this.xnFileExplorerComponentCtrl.onClickDeleteFiles();
    }
      ...
      if (isCustomerModule && isCustomerDoubletteWidget) {
        ...
        if (rawData) {
          ...
          const isDragFromCustomerModule =
            data.refData && data.refData.idSettingsGUI == MenuModuleId.customer;
        ...
        if (
          ...
          if (items.length > 1) {
            const notMatchingItem = items.find(
              (p) => p['NotMatching'] == true || p['NotMatching'] == 1
            );
            if (notMatchingItem) {
              this.modalService.warningMessage([
                {
        ...
        cols.forEach((col) => {
          Object.keys(data).forEach((key) => {
            if (col.data.toLowerCase() == key.toLowerCase()) {
              obj[col.data] =
                isObject(data[key]) && isEmpty(data[key]) ? '' : data[key];
            }
          });
        });
