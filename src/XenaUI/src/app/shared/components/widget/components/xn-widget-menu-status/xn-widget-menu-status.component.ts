  ...
  implements OnInit, OnDestroy, OnChanges, AfterViewInit
{
  ...
  public noteFormButtonStatus: NoteFormButtonStatus = new NoteFormButtonStatus(
    true,
    false
  );
  ...
  private initPropDisplayMode(): boolean {
    ...
    if (isWidgetNote) {
      ...
      if (
        ...
        if (
          propDisplayMode &&
          (!propDisplayMode.options || !propDisplayMode.options.length)
        ) {
  ...
  private initPropDisplayFields(): boolean {
    ...
    if (
      (isNotTableWidget ||
        isReadonlyTable ||
        this.data.idRepWidgetApp === RepWidgetAppIdEnum.OrderListSummary) &&
      propDisplayFields &&
      hasFilterData &&
      ...
      this.fieldFilters.forEach((item) => {
        if (!item.isTableField) {
          const indexFilteredItem = propDisplayFields.options.findIndex(
    ...
    if (
      ...
      this.fieldFilters.forEach((item) => {
        if (item.isTableField || !isWidgetCombination) {
          const indexFilteredItem = propDisplayColumns.options.findIndex(
            (_item) => _item.key === item.fieldName
          );
          const newItem = {
            key: item.fieldName,
    ...
    const hasNoDropDownOptions =
      propDropDown && (!propDropDown.options || !propDropDown.options.length);
    ...
    if (
      ...
      if (widgetDetail.contentDetail) {
        if (widgetDetail.contentDetail.columnSettings) {
          ...
          widgetDetailData = {
            ...widgetDetail,
            contentDetail: this.datatableService.formatDataTableFromRawData(
              widgetDetail.contentDetail.data
            ),
          };
        }
  ...
  private buildFieldFilterByDataSet(content: any): FieldFilter {
    ...
    if (content.Setting && content.Setting.length) {
      ...
      dataType =
        setting.ControlType &&
        (setting.ControlType.Type.toLowerCase() == ControlType.ComboBox ||
          setting.ControlType.Type.toLowerCase() == ControlType.DateTime)
      ...
      if (
        ...
        ((_setting.Validation && _setting.Validation.IsRequired) ||
          (_setting.ControlType &&
            _setting.ControlType.Type.toLowerCase() == ControlType.ComboBox))
      ) {
        dataType =
          _setting.ControlType &&
          (_setting.ControlType.Type.toLowerCase() == ControlType.ComboBox ||
            _setting.ControlType.Type.toLowerCase() == ControlType.DateTime)
  ...
  private initOwnerForMenuWidgetStatus() {
    ...
    if (this.editFormDropdown && !this.editFormDropdown.owner) {
      ...
      this.editFormDropdown.addEventListener(
        ...
        'mouseleave',
        (event) =>
          this.manageEditFormDropdown(event, true, EditWidgetTypeEnum.Form),
        null
      );
    }
    ...
    if (this.editTableDropdown && !this.editTableDropdown.owner) {
      ...
      this.editTableDropdown.addEventListener(
        ...
        'mouseleave',
        (event) =>
          this.manageEditFormDropdown(event, true, EditWidgetTypeEnum.Table),
        null
      );
    }
    ...
    if (this.editCountryDropdown && !this.editCountryDropdown.owner) {
      ...
      this.editCountryDropdown.addEventListener(
        ...
        'mouseleave',
        (event) =>
          this.manageEditFormDropdown(event, true, EditWidgetTypeEnum.Country),
        null
      );
    }
    ...
    if (this.editTreeviewDropdown && !this.editTreeviewDropdown.owner) {
      ...
      this.editTreeviewDropdown.addEventListener(
        ...
        'mouseleave',
        (event) =>
          this.manageEditFormDropdown(event, true, EditWidgetTypeEnum.TreeView),
        null
      );
    }
      ...
      this.fullscreenDropdowns.forEach((item) => {
        ...
        arrayDropdowns.forEach((mode) => {
          if (
            ...
            item.hostElement.className.indexOf(mode + '_Mode') !== -1
          ) {
            item.owner = $(
            ...
            item.addEventListener(
              item.hostElement,
              'mouseleave',
              (event) => this.manageEditFormDropdown(event, true, null),
              null
            );
            eleFound = true;
          }
