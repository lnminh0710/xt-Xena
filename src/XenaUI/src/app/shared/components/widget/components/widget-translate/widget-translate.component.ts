    ...
    implements OnInit, OnDestroy
{
    ...
    private updateScroll() {
        const currentItem = this.translateData.find(
            (x) => x.id === this.editingId
        );
        if (!currentItem || currentItem.id === undefined) return;
        const currentElement = $(
            this.classContainerClass + " #" + currentItem.id
        );
        const scrollContainer = document.querySelector(
            this.classContainerClass
        );
    ...
    public onTableEditSuccess($event: any) {
        ...
        if (this.gridComponent) {
            this.editedData = this.mergeUniqueRecords(this.editedData, [
                $event,
            ]);
        }
    }
                    ...
                    this.appErrorHandler.executeAction(() => {
                        if (
                            ...
                            if (hasTranslatedTextModeWidgetOnly) {
                                ...
                                this.currentTranslateMode =
                                    TranslateModeEnum.WidgetOnly;
        ...
        const filteredOldData = (oldData as Array<any>).filter((item) => {
            return isNil(
                (newData as Array<any>).find(
                    (i) =>
                        i.IdRepLanguage === item.IdRepLanguage &&
                        i.Mode === item.Mode
                )
            );
        });
