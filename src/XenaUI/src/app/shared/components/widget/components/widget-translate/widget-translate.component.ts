...
export class WidgetTranslateComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  @Input() set widgetData(data: any) {
    ...
    if (
      ...
      this.tableId =
        data.widgetDataType.listenKeyRequest[data.widgetDataType.filterKey];
    }

    this.isSupportedTranslateValue =
      data &&
      (data.idRepWidgetType === WidgetType.FieldSet ||
  ...
  resized() {
    if (this.gridComponent) this.gridComponent.refresh();
  }
  ...
  private saveCallBack(saveOnly?: boolean, thenCloseCb?: any) {
    ...
    this.toasterService.pop(
      'success',
      'Success',
      'Translation text saved successfully'
    );
    if (!saveOnly) {
  ...
  prepareDataFroSaving(): any {
    ...
    if (isCurrentModeAll) {
      editData = this.mergeUniqueRecords(
        editData,
    ...
    (editData as Array<any>).forEach((item) => {
      ...
      if (
        !(
          isDeleted &&
          (isNil(item.IdTranslateLabelText) || item.IdTranslateLabelText <= 0)
        )
      ) {
        const isModeAll = item.Mode === TranslateModeEnum.All;
        if (
          isModeAll === isCurrentModeAll ||
          ...
          result.push({
            ...
            IsDeleted:
              isDeleted ||
              (!isModeAll && isCurrentModeAll && item.IdTranslateLabelText > 0)
                ? '1'
                : null,
  ...
  private translateTextClicked(
    textId: any,
    text: any,
    translateDataType?: TranslateDataTypeEnum,
    item?: any
  ) {
    setTimeout(() => {
      if (
        ...
        translateDataType === this.translateDataType
      )
        return;
      if (this.itemDirty) {
      ...
      if (item) this.fieldName = item.text;
    ...
    for (let i = 0; i < this.translateData.length; i++) {
      if (this.translateData[i].id === this.editingId) {
        ...
        if (i === this.translateData.length - 1) {
          this.translateTextClicked(
            this.translateData[0].id,
            this.translateData[0].text,
            this.translateDataType,
            this.translateData[0]
          );
          return;
        }
        // move next item
        this.translateTextClicked(
          this.translateData[i + 1].id,
          this.translateData[i + 1].text,
          this.translateDataType,
          this.translateData[i + 1]
        );
        return;
      }
    }
  ...
  private processNextItemForDataType() {
    const filteredData = this.translateData.filter(
      (item) =>
        item.dataType == 'nvarchar' && !isNil(item.value) && item.value != ''
    );
  ...
  private findNextValidItemForDataType(index: number): any {
    ...
    const filteredItem = this.translateData.find(
      (item, _index) =>
        _index > index &&
    ...
    if (filteredItem) return filteredItem;
      ...
      return this.translateData.find(
        (item) =>
          item.dataType == 'nvarchar' && !isNil(item.value) && item.value != ''
      );
  }
  ...
  private updateScroll() {
    ...
    if (
      currentElement.offset().top - this.currentItemHeight * 2 <=
      scrollContainerOffset.offset().top
    ) {
    ...
    if (
      currentElement.offset().top + this.currentItemHeight * 2 >=
      scrollContainerOffset.offset().top + scrollContainerOffset.height()
    ) {
  ...
  private removeEditingData() {
    ...
    if (
      !Uti.isExistItemInArray(
        this.tempDataEditing,
        this.editingId,
        'textId',
        currentEditingData
      )
  ...
  changeTranslateDataType(value) {
    ...
    const index = this.translateData.findIndex(
    ...
    if (filteredItem) {
      if (value === TranslateDataTypeEnum.Label) {
        ...
        if (
          filteredItem.dataType == 'nvarchar' &&
          !isNil(filteredItem.value) &&
  ...
  private mergeUniqueRecords(oldData: any, newData: any): any {
    const filteredOldData = (oldData as Array<any>).filter((item) => {
      return isNil(
        (newData as Array<any>).find(
          (i) => i.IdRepLanguage === item.IdRepLanguage && i.Mode === item.Mode
        )
      );
    });
