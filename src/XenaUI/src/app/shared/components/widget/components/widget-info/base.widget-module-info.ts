...
export abstract class BaseWidgetModuleInfo extends BaseWidgetModuleInfoMixin {
    ...
    switch (this.data.idRepWidgetType) {
      ...
      case WidgetType.NoteForm:
        if (
          ...
          isEdited =
            this['noteFormDataAction'].action !== NoteActionEnum.VIEW_MODE;
        }
        break;
    }
    return isEdited;
  ...
  get widgetEditedStatus(): boolean {
    return (
      this.isWidgetDataEdited || this.checkCurrentWidgetHasChildrenInEditMode()
    );
  }
  ...
  public printWidget(): void {
    const factory =
      this.componentFactoryResolver.resolveComponentFactory(PaperworkComponent);
  ...
  onClickOutside(event: any) {
    ...
    if (
      ...
      if (
        ...
        this.store.dispatch(
          this.propertyPanelActions.requestClearProperties(this.currentModule)
        );
      }
    } else {
  ...
  private isChildOfWidgetModuleInfo(target: any): boolean {
    ...
    if (
      ...
      (target.parentElement &&
        target.parentElement.className.indexOf('xn-widget-module-info') > -1)
    ) {
      return true;
    }
  ...
  protected afterCheckDirtyWhenClickOutSide() {
    ...
    if (this.eventClick && this.eventClick['value'] === true) {
      if (!this.isClickInsideSpecialCase(this.eventClick.target)) {
        ...
        if (!this.isRenderWidgetInfoTranslation) {
          ...
          this.store.dispatch(
            this.widgetDetailActions.activeWidget(null, this.currentModule)
          );
        }
      } else {
        this.reattach();
    ...
    // Inside
      ...
      setTimeout(() => {
        this.store.dispatch(
          this.widgetDetailActions.activeWidget(this.data, this.currentModule)
        );
      });
    }
    ...
    Object.keys(this.data.contentDetail.columnSettings).forEach((key) => {
      this.fieldFilters.push(
        new FieldFilter({
          ...
          fieldName:
            this.data.contentDetail.columnSettings[key].OriginalColumnName,
          selected: false,
          isHidden: false,
          isEditable: false,
        })
