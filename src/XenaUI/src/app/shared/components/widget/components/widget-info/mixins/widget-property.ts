...
>(base: T) {
    abstract class AbstracPropertytBase
        extends base
        implements WidgetDetailInfo
    {
        ...
        protected updateConnectedWidgetStatusProperty(data: WidgetDetail) {
            ...
            // Connect to other widget
                ...
                if (widgetNameProp && data.widgetDataType.parentWidgetIds) {
                    widgetNameProp.value =
                        data.widgetDataType.parentWidgetIds[0];
                }
            } else {
                if (withParkedItemProp) {
                    withParkedItemProp.value = false;
        ...
        protected updateSeries() {
            ...
            if (chartType && chartType.dirty) {
                ...
                switch (chartType.value) {
                    ...
                    case ChartTypeNgx.PieGrid:
                        ...
                        comboMultiYSeries.value.forEach(
                            (x) => (x.$checked = false)
                        );
            ...
            switch (chartType.value) {
                ...
                case ChartTypeNgx.ComboChart:
                    xValue = comboSingleXSerie.value
                        ? [comboSingleXSerie.value]
                        : [];
                    yValue = comboSingleYSerie.value
                        ? [comboSingleYSerie.value]
                        : [];
                    let zValue = comboMultiXSerie.value
                        ? [comboMultiXSerie.value]
                        : [];
        ...
        protected updateOptionsBarChart() {
            ...
            if (propShowGridLines) {
                this.optionChart.barChart.showGirdLines =
                    propShowGridLines.value;
            }
            if (propGradients) {
            ...
            if (propShowXAxisLabel) {
                this.optionChart.barChart.showXAxisLabel =
                    propShowXAxisLabel.value;
            }
            if (propXAxisLabel) {
            ...
            if (propShowYAxisLabel) {
                this.optionChart.barChart.showYAxisLabel =
                    propShowYAxisLabel.value;
            }
            if (propYAxisLabel) {
        ...
        protected updateOptionsPieChart() {
            ...
            if (propShowDataLabel) {
                this.optionChart.pieChart.showDataLabel =
                    propShowDataLabel.value;
            }
            if (propDisableTooltip) {
                this.optionChart.pieChart.disableTooltip =
                    propDisableTooltip.value;
            }
            if (propExplodeSlices) {
                this.optionChart.pieChart.explodeSlices =
                    propExplodeSlices.value;
            }
            if (propLegendTitle) {
            ...
            if (labelDisplayProp) {
                ...
                if (fieldsEntity && fieldsEntity.length) {
                    fieldsEntity.forEach((item: StyleFormatFieldEntity) => {
                        ...
                        fieldStyle =
                            this.propertyPanelService.updateWidgetFormStyle(
                                item.stylePoperties,
                                fieldStyle,
                                "labelStyle",
                                "DataLabelStyle"
                            );
                        this.fieldFormatStyle[item.originalColumnName] =
                            fieldStyle;
                    });
                }
            }
