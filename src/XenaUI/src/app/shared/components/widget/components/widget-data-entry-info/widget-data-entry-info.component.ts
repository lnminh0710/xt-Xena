  ...
  private getTranslateText() {
    if (this.currentComponent) {
      ...
      this.globalSettingService
        .getTranslateText(model.widgetMainID, model.widgetCloneID, model.fields)
    ...
    this.globalPropertiesStateSubscription =
      this.globalPropertiesState.subscribe((globalProperties: any) => {
        this.appErrorHandler.executeAction(() => {
          ...
          const globalBorder = this.propertyPanelService.getItemRecursive(
            globalProperties,
            'Border'
          );
          const globalBorderColor = this.propertyPanelService.getItemRecursive(
            globalProperties,
            'BorderColor'
          );
      ...
      if (this.currentComponent) {
        switch (this.dataEntryWidgetType) {
          case 125:
            ...
            this.currentComponent.formDisplayType = isFormDisplayWithGroup
