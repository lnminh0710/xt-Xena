...
import { TranslateModeEnum, TranslateDataTypeEnum, ComboBoxTypeConstant, Configuration } from 'app/app.constants';
import { UUID } from 'angular2-uuid';
import { GlobalSettingService, AppErrorHandler, ModalService, WidgetTemplateSettingService, CommonService, ToolsService, DatatableService } from 'app/services';
...
export class WidgetExportBlockedComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('blockedCustomerResult') public blockedCustomerResultGrid: XnAgGridComponent;
    ...
    private registerEventFromParentInstance() {
        ...
        this.exportExcelSubscription = this.parentInstance.onExportExcel.subscribe(data => {
            this.exportExcel();
        });

        this.refreshSubscription = this.parentInstance.onResetWidget.subscribe(data => {
            this.loadData();
            // this.searchBlockedCustomer();
        });
    }
    ...
    private getBlockedCustomerResult(customerStatus, businessStatus) {
        this.isLoading = true;
        this.toolsService.exportCustomerAndBusinessStatus(customerStatus, businessStatus).finally(() => {
            this.isLoading = false;
        }).subscribe(result => {
            if (result) {
                result = this.datatableService.formatDataTableFromRawData(result.data);
                this.blockedCodeResultGridData = this.datatableService.buildDataSource(result);
    ...
    public dragEnd(items) {
        ...
        this.parentInstance.data.extensionData = Object.assign(this.parentInstance.data.extensionData, setting);
        ...
        this.commonService.getListComboBox(ComboBoxTypeConstant.mailingReturnReason)
            ...
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    let items: Array<any> = response.item[ComboBoxTypeConstant.mailingReturnReason];
                    items.forEach(item => {
                        dataRow.push({
                    ...
                    this.blockedCodeGridData = Object.assign({}, this.blockedCodeGridData);
                });
            });
    ...
    public onSelectedAllChecked() {
        if (this.parentInstance && this.parentInstance.widgetMenuStatusComponent) {
            ...
            this.onSelectedAllCheckedTimeout = setTimeout(() => {
                this.parentInstance.widgetMenuStatusComponent.forceToggleToolButtons(true);
            }, 300);
        }
        if (this.parentInstance) {
            this.parentInstance.reattach();
        ...
        if (this.parentInstance && this.parentInstance.data) {
            ...
            if (settings) {
                if (settings[this.blockedCustomerId]) {
                    this.blockedCustomerColumnsLayoutSettings = settings[this.blockedCustomerId].columnsLayoutSettings;
                }
                if (settings[this.blockedCustomerResultId]) {
                    this.blockedCustomerResultColumnsLayoutSettings = settings[this.blockedCustomerResultId].columnsLayoutSettings;
                }
