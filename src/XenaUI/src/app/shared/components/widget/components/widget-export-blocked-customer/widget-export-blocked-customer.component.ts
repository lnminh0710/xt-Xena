    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    private registerEventFromParentInstance() {
        this.searchSubscription = this.parentInstance.onSearch.subscribe(
            (data) => {
                this.searchBlockedCustomer();
            }
        );

        this.exportExcelSubscription =
            this.parentInstance.onExportExcel.subscribe((data) => {
                this.exportExcel();
            });
                ...
                nodeItems.forEach((item) => {
                    if (item.selectAll) {
                        if (item.statusType == "IdRepPersonStatus") {
                            ...
                            item.statusType == "IdRepPersonBusinessStatus"
                        ) {
