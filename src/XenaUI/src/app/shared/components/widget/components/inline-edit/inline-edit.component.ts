...
export class InlineEditComponent implements ControlValueAccessor, Validator, OnDestroy {
    ...
    @Output() public onDoubleClickEditAction: EventEmitter<any> = new EventEmitter();
    ...
    @Input() set editing(_editing: boolean) {
        ...
        if (_editing) {
            ...
            if (this.controlWidth < controlWidth)
    ...
    @Input() set globalProperties(globalProperties: any[]) {
        ...
        if (this.control && this.control.key == 'B00CashProviderContract_IdRepPaymentsMethods') {
            this.control.options = this.control.options.filter(v => v.key == PaymentMethod.CHECK || v.key == PaymentMethod.CREDIT_CARD);
        }
    }
    @Input() set widgetProperties(widgetProperties: any[]) {
        this.widgetPropertiesLocal = widgetProperties;
        this.propertiesAutoSwitchToDetail = this.propertyPanelService.getAutoSwitchToDetail(widgetProperties);
        this.updateProperties();
        this.applyOnWidget = this.propertyPanelService.getValueDropdownApplyOnWidgetFromProperties(this.widgetPropertiesLocal);
    ...
    writeValue(value: any) {
        ...
        //}
        if (this.isSAVLetter && this.control.key === 'B00SharingTreeMedia_MediaSize') {
    ...
    edit(value, nativeEle) {
        ...
        if (this.control.readOnly || this.isEditingLayout)
            return;
    ...
    focus() {
        setTimeout(_ => {
            ...
            switch (this.control.controlType) {
                case 'checkbox':
                    this._renderer.invokeElementMethod((this.inlineEditControl as MatCheckbox)._inputElement.nativeElement, 'focus', []);
                    break;
                // case 'numberbox':
                case 'date':
                case 'textboxMask':
                    this._renderer.invokeElementMethod(this.inlineEditControl.hostElement, 'focus', []);
                    break;
                case 'dropdown':
    ...
    public updateProperties() {
        setTimeout(() => {
            const propertyDropdownForm = this.propertyPanelService.getValueDropdownFromProperties(this.widgetPropertiesLocal);
            if (propertyDropdownForm) {
                propertyDropdownForm.forEach(v => {
                    if (this.control && (this.control.controlType == 'dropdown' || this.control.controlType == 'date')) {
                        ...
                        if (isMatched && !v.selected) {
                            ...
                            this.isShowDropdownWhenFocusCombobox = this.dontShowCalendarWhenFocus;
                        }
                    }
                });
            }
    ...
    public formatDate(date) {
        ...
        return this.uti.formatLocale(date, formatDate ? formatDate : this.globalDateFormat);
    }
    ...
    public setThreeDotsForValue() {
        ...
        setTimeout(() => {
            ...
            while (label.outerHeight() >= div.height()) {
                if (this.counterSetText > 100 ||
                    _text === label.text()) break;
                label.text((index, text) => {
                    this.counterSetText++;
                    _text = text;
                    return text.replace(/\W*\s(\S)*$/, '...');
