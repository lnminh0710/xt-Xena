    ...
    implements ControlValueAccessor, Validator, OnDestroy
{
    ...
    @Input() set editing(_editing: boolean) {
        ...
        if (_editing) {
            const containerHeight = $(this.element.nativeElement)
                .parent()
                .height();
    ...
    @Input() set globalProperties(globalProperties: any[]) {
        ...
        if (
            ...
            this.control.options = this.control.options.filter(
                (v) =>
                    v.key == PaymentMethod.CHECK ||
                    v.key == PaymentMethod.CREDIT_CARD
            );
        }
    }
            ...
            switch (this.control.controlType) {
                case "checkbox":
                    this._renderer.invokeElementMethod(
                        (this.inlineEditControl as MatCheckbox)._inputElement
                            .nativeElement,
                        "focus",
                        []
                    );
            ...
            if (propertyDropdownForm) {
                propertyDropdownForm.forEach((v) => {
                    if (
                        ...
                        const isMatched =
                            controlKey === v.value || label == v.value;
    ...
    private onChangeValue(event) {
        if (this.control.controlType === "dropdown") {
            ...
            this.setValueWithoutUpdateValue =
                this.inlineEditControl.rawValue || "";
        } else {
    ...
    public getDateFormatForDOB() {
        const formatDate = (this.control as DateControl).format;
        return (
            (formatDate ? formatDate : this.globalDateFormat) || "MM/dd/yyyy"
        );
    }
