...
export class FilterMenuComponent implements OnInit, OnDestroy, OnChanges {
  ...
  @Input() showColumnDesign: boolean = false;
  ...
  //     for (let filterMode of this.filterModes) {
  //         defaultLanguage['Filter_Menu__' + filterMode.value] = filterMode.value;
  //     }
  // }
  ...
  private initData() {
    if (
      ...
      this.filterModes.find(
        (item) =>
          item.selected && item.mode === FilterModeEnum.ShowAllWithoutFilter
      )
    ) {
  ...
  public designTempateClicked() {
    this.designColumnsAction.emit();
  }
  ...
  public handleMouseOverSubmenu($event) {
    setTimeout(() => {
      ...
      if (this._eref.nativeElement.attributes['name']) {
        const parentOfTopParentMenu = $(this._eref.nativeElement).closest(
          'ul.widget-toolbar-combination-menu'
        );
        const menuItemHeight = 30;
        if (parentOfTopParentMenu.length) {
          topParentMenuWidth += parentOfTopParentMenu.width();
          if (
            this._eref.nativeElement.attributes['name'].name.indexOf(
              'second'
            ) >= 0
          ) {
