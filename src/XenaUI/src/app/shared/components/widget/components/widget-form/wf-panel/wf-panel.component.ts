...
import {ModalService, PropertyPanelService, WidgetFieldService} from 'app/services';
...
export class WfPanelComponent extends BaseComponent implements OnInit, OnDestroy, OnChanges {
    ...
    public ngOnChanges(changes: SimpleChanges) {
        if (!changes['panel'] &&
            !changes['isSettingDialog']) return;
        const hasChangesPanel = Uti.hasChanges(changes['panel']);
        if (hasChangesPanel) {
            this.updateRenderData();
        }
        const hasChangesIsSettingDialog = Uti.hasChanges(changes['isSettingDialog']);
        if (hasChangesIsSettingDialog && !changes['isSettingDialog'].currentValue) {
            this.appendResizeIconToWrapper();
        }
    }
    ...
    public onDeleteLineBreakHandler($event) {
        this.onDeleteLineBreakAction.emit({parent: this.panel, lineBreak: $event});
    }
    ...
    public changePropertiesForTitleStyle(controlConfig: any) {
        ...
        const propPanelTitleAlign = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleAlign');
        const propPanelTitleText = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleText');
        const propPanelColor = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleColor');
        const propPanelBackgroundColor = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleBackgroundColor');
        const propPanelFontName = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleFontName');
        const propPanelFontSize = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleFontSize') || 0;
        const propPanelBold = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleBold');
        const propPanelItalic = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleItalic');
        const propPanelUnderline = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleUnderline');
        const propPanelTitleShow = this.propertyPanelService.getItemRecursive(controlConfig, 'TitleStyleShow');
        const valueTitleAlign = propPanelTitleAlign && propPanelTitleAlign.value;
        this.panel.title = propPanelTitleText && propPanelTitleText.value ? propPanelTitleText.value : this.panel.title;
        ...
        this.configTitleStyle = {
            ...
            visibility: propPanelTitleShow && propPanelTitleShow.value ? '' : 'hidden',
            'background-color': propPanelBackgroundColor && propPanelBackgroundColor.value ? propPanelBackgroundColor.value : 'white',
            'font-family': propPanelFontName && propPanelFontName.value,
            'font-size': propPanelFontSize && propPanelFontSize.value + 'px',
            'font-weight': propPanelBold && propPanelBold.value ? 'bold' : 'normal',
            'font-style': propPanelItalic && propPanelItalic.value ? 'italic' : 'unset',
            'text-decoration': propPanelUnderline && propPanelUnderline.value ? 'underline' : 'unset',
        };
    }
    ...
    public changePropertiesForPanelStyle(controlConfig: any) {
        ...
        const propShowPanelBorder = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderShowBorder');
        const propPanelBorderColor = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderColor');
        const propPanelBorderStyle = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderStyle');
        const propPanelBorderWeight = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderWeight');
        const valueWeight = propPanelBorderWeight && propPanelBorderWeight.value;
        const valueBorderStyle = propPanelBorderStyle && propPanelBorderStyle.value;
        const valueBorderColor = propPanelBorderColor && propPanelBorderColor.value;
        const propPanelShowShadow = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleShadowShowShadow');
        const propPanelShadowColor = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleShadowColor');
        const propPanelBackground = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBackgroundBackgroundColor');
        const propPanelHeight = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleHeight');
        const propPanelAutoHeight = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleAutoHeight');
        const propPanelPaddingRight = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleRight');
        const propPanelPaddingLeft = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleLeft');
        const valuePanelBackgroundColor = propPanelBackground && propPanelBackground.value;
        const valuePanelShadowColor = propPanelShadowColor && propPanelShadowColor.value;
        const valuePanelAutoHeight = propPanelAutoHeight && propPanelAutoHeight.value;
        this.panel.height = valuePanelAutoHeight ? 'auto' : propPanelHeight && `${propPanelHeight.value}px`;
        if (this.panel && !this.panel.isRow) {
            this.configPanelStyle['border'] = propShowPanelBorder && propShowPanelBorder.value ? `${valueWeight}px ${valueBorderStyle} ${valueBorderColor}` : '';
        }
        this.configPanelStyle['padding-right'] = propPanelPaddingRight && propPanelPaddingRight.value ? `${propPanelPaddingRight.value}px` : '';
        this.configPanelStyle['padding-left'] = propPanelPaddingLeft && propPanelPaddingLeft.value ? `${propPanelPaddingLeft.value}px` : '';
        this.configPanelStyle['box-shadow'] = propPanelShowShadow && propPanelShowShadow.value ? `5px 5px 5px 0px ${valuePanelShadowColor}` : '';
        this.configPanelStyle['background-color'] = valuePanelBackgroundColor ? valuePanelBackgroundColor : 'white';
    }
    ...
    public changePropertiesForUnderlineStyle(controlConfig: any) {
        ...
        const propShowUnderline = this.propertyPanelService.getItemRecursive(controlConfig, 'UnderLineStyleUnderLineShow');
        const propUnderlineColor = this.propertyPanelService.getItemRecursive(controlConfig, 'UnderLineStyleUnderLineColor');
        const propUnderlineWeight = this.propertyPanelService.getItemRecursive(controlConfig, 'UnderLineStyleUnderLineWeight');
        const propUnderlineShadowShow = this.propertyPanelService.getItemRecursive(controlConfig, 'UnderLineStyleShadowShow');
        const propUnderlineShadowColor = this.propertyPanelService.getItemRecursive(controlConfig, 'UnderLineStyleShadowColor');
        const valueUnderlineColor = propUnderlineColor && propUnderlineColor.value;
        const valueUnderlineWeight = propUnderlineWeight && propUnderlineWeight.value;
        const valueUnderlineShadowShow = propUnderlineShadowShow && propUnderlineShadowShow.value;
        const valueShowUnderline = propShowUnderline && propShowUnderline.value;
        const valueUnderlineShadowColor = propUnderlineShadowColor && propUnderlineShadowColor.value;
        this.configUnderlineStyle = {
            'border-bottom': valueShowUnderline ? `${valueUnderlineWeight}px solid ${valueUnderlineColor}` : '',
            'box-shadow': valueShowUnderline && valueUnderlineShadowShow ? `5px 5px 5px 0px ${valueUnderlineShadowColor}` : '',
        }
    }
    ...
    public changePropertiesForBehaviour(controlConfig: any) {
        ...
        const behaviourIsRow = this.propertyPanelService.getItemRecursive(controlConfig, 'BehaviourIsRow');
        const propPanelBorderStyle = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderStyle');
        const propPanelBorderWeight = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderWeight');
        const propPanelBorderColor = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderColor');
        const propShowPanelBorder = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleBorderShowBorder');
    ...
    public onMousedownOnResize(panel: any) {
        if (this.isSettingDialog ||
            panel.height === 'auto' || this.isDuplicatedDialogForm) return;
    ...
    public onResizeStart(event: ResizeEvent, panel: any) {
        if (this.isSettingDialog || panel.height === 'auto' || this.isDuplicatedDialogForm) return;
    ...
    public setThreeDotsForValue() {
        if (!this.wfFieldComponents ||
            !this.wfFieldComponents.length) return;
    ...
    public updatePanelChildren() {
        this.panel.children = [...this.panelChildren, ...this.panel.children.filter(x => !!x && x.isHidden)];
    }

    public updateRenderData() {
        this.panelChildren = [...this.panel.children.filter(x => !!x && !x.isHidden)];
        this._control = this.panelChildren.filter(x => x.layoutType === TypeForm.Control);
    }
    ...
    private setStyleForPanel(propPanelFontSize: any) {
        // setTimeout(() => {
            if (propPanelFontSize && !this.panel.isRow) {
                ...
                this.configPanelStyle['padding-top'] = (propPanelFontSize > 5 ? (propPanelFontSize - 5) : 0) + 'px';
            } else {
    ...
    private initDragulaEvents() {
        ...
        this.onOverSubscription = this.dragulaService.out.subscribe(this.onOut.bind(this));
    }

    private switchToAutoHeight(controlConfig: any) {
        const behaviourIsRowOld = this.propertyPanelService.getItemRecursive(this.firstTimeConfig, 'BehaviourIsRow');
        const propPanelAutoHeightOld = this.propertyPanelService.getItemRecursive(this.firstTimeConfig, 'PanelStyleAutoHeight');
        const behaviourIsRowNew = this.propertyPanelService.getItemRecursive(controlConfig, 'BehaviourIsRow');
        const propPanelAutoHeightNew = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleAutoHeight');
        if (!behaviourIsRowOld || !propPanelAutoHeightOld) return;
        if (propPanelAutoHeightOld.value !== propPanelAutoHeightNew.value) return;

        if (behaviourIsRowOld.value === behaviourIsRowNew.value) {
            const propPanelHeight = this.propertyPanelService.getItemRecursive(controlConfig, 'PanelStyleHeight');
            propPanelAutoHeightNew.value = propPanelAutoHeightOld.value;
            this.panel.height = propPanelAutoHeightOld.value ? 'auto' : propPanelHeight && `${propPanelHeight.value}px`;
    ...
    private appendResizeIconToWrapper() {
        setTimeout(() => {
            const elWrapperPanelId = $('*[elWrapperPanelId="' + this.panel.key + '"]', this.elementRef.nativeElement);
            const elPanelId = $('*[elPanelId="' + this.panel.key + '"]', this.elementRef.nativeElement);
            if (!elWrapperPanelId || !elWrapperPanelId.length || !elWrapperPanelId || !elWrapperPanelId.length) return;
    ...
    private onOver(args: any) {
        ...
        this.isHiddenWatermark = (bagTarget.innerText.indexOf('Drag fields here') !== -1);
    ...
    private getControlHeightAfterResize(elementHeight: number) {
        return elementHeight < (this.panel.isRow ? this.minPanelRowHeight : this.minPanelHeight) ? (this.panel.isRow ? this.minPanelRowHeight : this.minPanelHeight)
            : elementHeight;
    }
}
