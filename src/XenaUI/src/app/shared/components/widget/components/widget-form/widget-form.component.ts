  ...
  implements OnInit, OnChanges, OnDestroy, AfterViewInit
{
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (hasChanges && this.dataSource) {
      ...
      if (
        (!this.isFormChanged && !this.editFieldMode && !this.editFormMode) ||
        this.isForceReset
      ) {
        this.processData();
              ...
              if (
                ...
                options.push({
                  ...
                  sharingAddressHiddenFields:
                    comboOptions[i][this.SharingAddressHiddenFields],
                  validationZip2MaskFormat:
                    comboOptions[i][this.ValidationZip2MaskFormatField],
                  validationZip2RegEx:
                    comboOptions[i][this.ValidationZip2RegExField],
                  validationZipMaskFormat:
                    comboOptions[i][this.ValidationZipMaskFormatField],
                  validationZipRegEx:
                    comboOptions[i][this.ValidationZipRegExField],
                });
              } else {
                options.push({
                  key: comboOptions[i].idValue,
  ...
  private updateMaskForZipControl(
    ...
    if (zipTxtControl) {
      ...
      if (validationZipMaskFormatField) {
        validationZipMaskFormatField = validationZipMaskFormatField.replace(
          /§/g,
          'A'
        );
  ...
  private changeStatusCheckBox(checkboxControl: CheckboxControl) {
    if (checkboxControl.value && checkboxControl.groupName) {
      ...
      const checkBoxGroup = controls.filter(
        (p) => p.key !== checkboxControl.key && p.controlType === 'checkbox'
      );
  ...
  private signalRGetObjectId() {
    ...
    const keyValue: string = this.dataSource.widgetDataType.listenKeyRequest(
  ...
  private updateWhenOpenConfigDialog() {
    for (const wfColumnComponent of (<any>this.wfColumnComponents)._results) {
      if (wfColumnComponent.updateWhenOpenConfigDialog(this.currentItemControl))
        return;
    }
  }
  ...
  public addPanel(isRow?: boolean) {
    ...
    if (
      ...
      (!this.currentPanel && !this.currentControl && !this.currentLineBreak)
    ) {
  ...
  public onColumnResizeEndHandler(data: any) {
    ...
    for (const column of this.controlColumnList) {
      ...
      if (isFound) {
        ...
        newNextColumnWidth =
          column.width + remainWidth * (newWidth > data.column.width ? -1 : 1);
    ...
    $('body').append(
      $(
        `
          ...
          (this.checkedItems.length <= 1 ? '1' : this.checkedItems.length) +
          ` item` +
  ...
  private addControlForColumn(
    ...
    this.currentColumn.children.push({
      ...
      children: [],
      config:
        layoutType === 'panel' ? DefaultWidgetItemConfiguration.Panel : '[]',
    });
    this.reSetOrderNumber(this.currentColumn.children);
    this.setDirtyForChangeColumnLayout();
    this.resetCurrentItemSelection();
