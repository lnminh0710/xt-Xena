...
export function MixinWidgetCommunication<
    T extends Constructor<CommunicationServiceInjector>
>(base: T) {
    abstract class AbstractWidgetCommunicationBase
        extends base
        implements WidgetDetailInfo
    {
        ...
        public onDragOverWidget(
            ...
            switch (dragDropCommunicationData.mode) {
                case DragMode.Default:
                    isValid = this.widgetUtils.isValidWidgetToConnectOfChild(
                        dragDropCommunicationData.srcWidgetDetail,
                        data
                    );
                    if (!isValid) {
                        isValid = this.widgetUtils.isValidTableWidgetToConnect(
                            dragDropCommunicationData.srcWidgetDetail,
                            data
                        );
                    }
                ...
                case DragMode.Translate:
                    isValid = this.isValidTranslateWidgetToConnect(
                        dragDropCommunicationData.srcWidgetDetail,
                        data
                    );
                    break;
         ...
         */
        private isValidTranslateWidgetToConnect(
            srcWidgetDetail: WidgetDetail,
            data: WidgetDetail
        ) {
            let isValid = false;
            if (
                (srcWidgetDetail.idRepWidgetType == WidgetType.FieldSet ||
                    srcWidgetDetail.idRepWidgetType ==
                        WidgetType.FieldSetReadonly ||
        ...
        private isValidTranslateGridWidget(srcWidgetDetail: WidgetDetail) {
            if (
                srcWidgetDetail.idRepWidgetType == WidgetType.DataGrid ||
                ...
                if (
                    !srcWidgetDetail["gridSelectedRow"] ||
                    !srcWidgetDetail["gridSelectedRow"].length
                ) {
                    this.modalService.warningText(
                        "Modal_Message__Select_At_Least_One_Row"
                    );
                    return false;
                }
                return true;
        ...
        public onConnectWidget(dropResultData: any, data: WidgetDetail) {
            ...
            if (dragDropCommunicationData.mode == DragMode.Default) {
                ...
                if (isValid) {
                    let isMain = !!(
                        data.widgetDataType &&
                        data.widgetDataType.isPrimaryKeyForMain
                    );
                    ...
                    if (!isMain) {
                        ...
                        srcWidgetDetail.widgetDataType.parentWidgetIds.push(
                            data.id
                        );
                    }
                    ...
                    if (isValid) {
                        srcWidgetDetail.syncWidgetIds =
                            srcWidgetDetail.syncWidgetIds || [];
                        data.syncWidgetIds = data.syncWidgetIds || [];
                        const iRet = srcWidgetDetail.syncWidgetIds.filter(
                            (p) => p == data.id
                        );
                        if (!iRet.length) {
                        ...
                        isValid =
                            this.widgetUtils.isValidChartWidgetToConnectTable(
                                dragDropCommunicationData.srcWidgetDetail,
                                data
                            );
                        if (isValid) {
         ...
         * @param event
         */
        public removeLinkWidget(callBack1?: Function, callBack2?: Function) {
            this.modalService.confirmMessageHtmlContent(
                new MessageModel({
                    ...
                    message: [
                        { key: "<p>" },
                        {
                            key: "Modal_Message__Remove_Connection_From_This_Widget",
                        },
                        { key: "</p>" },
                    ],
