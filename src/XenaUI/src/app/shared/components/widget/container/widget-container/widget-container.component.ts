  ...
  implements OnInit, OnChanges, OnDestroy
{
  ...
  private tabHeaderTableFilterStateSubscription: Subscription;
  ...
  private tabHeaderTableFilterState: Observable<any>;
  ...
  public tabHeaderTableFilter: any;
  ...
  private subscribeState() {
    ...
    this.subscribeTabHeaderTableFilterState();
    ...
    this.subscribeWidgetActionStorageState();
    if (!this.onPopup) {
      this.watchingPopupEffectedData();
    }
  }
            ...
            for (let i = 0; i < widgetBoxesDirty.length; i++) {
              ...
              for (let j = 0; j < widgetModuleComponents.length; j++) {
                if (
                  ...
                  widgetModuleComponents[j].propertiesForSaving.properties =
                    cloneDeep(widgetModuleComponents[j].properties);
                }
              }
            }
  ...
  private resizeChartWidget() {
    for (let i = 0; i < this.widgetBoxes.length; i++) {
      ...
      for (let j = 0; j < widgetModuleComponents.length; j++) {
        if (
          widgetModuleComponents[j].data.idRepWidgetType === WidgetType.Chart
        ) {
        ...
        this.appErrorHandler.executeAction(() => {
                ...
                if (widgetModuleComponent.data.id != rowData.widgetDetail.id) {
                  if (
                    ...
                    if (keyArr && keyArr.length) {
                      keyArr.forEach((key) => {
                        const iRet = primayIdValueCells.find(
                          (p) => p.key == key
                        );
                        if (iRet) {
                          count++;
          ...
          primayIdValueCells.forEach((primayIdValueCell) => {
            ...
            const widgetTargetRender: IWidgetTargetRender = {
              ...
              syncWidgetIds:
                !foundWidgetDetails ||
                (foundWidgetDetails && !foundWidgetDetails.length)
                  ? null
   ...
   */
  private subscribeTabHeaderTableFilterState() {
    ...
    this.tabHeaderTableFilterStateSubscription =
      this.tabHeaderTableFilterState.subscribe((tabHeaderTableFilter: any) => {
              ...
              if (
                ...
                if (
                  ...
                  widgetDetailUpdates.push(
                    new WidgetDetail(widgetModuleComponent.data)
                  );
                }
              }
        ...
        this.appErrorHandler.executeAction(() => {
          const widgetModuleComponent = this.widgetModuleComponents.find(
            (p) =>
              p &&
              p.data &&
              p.data.idRepWidgetApp == RepWidgetAppIdEnum.CustomerDoublette
          );
          if (widgetModuleComponent) {
  ...
  updateWidgetContent(
    ...
    for (let i = 0; i < widgets.length; i++) {
      ...
      if (redrawWidgetBoxes) {
        ...
        if (properties) {
          let propTitleText = this.propertyPanelService.getItemRecursive(
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            if (
              count &&
              count === widget.widgetDetail.widgetDataType.listenKeyCount
            ) {
              isValidWidget = true;
            }
          }
  ...
  private rebuildWidgetProperty() {
    for (let item of this.widgetBoxes) {
      if (item.payload.idRepWidgetApp != 149) {
        continue;
      }
      this.detectPropertyGetAsynData(item.properties.properties);
    }
  }
      ...
      .subscribe((response: any) => {
        this.appErrorHandler.executeAction(() => {
          ...
          for (let prop in response.item) {
            let propertyItem = this.propertyPanelService.getItemRecursive(
              property,
  ...
  transferDataSuccess($event: any) {
    ...
    const wgDetail = this.widgetUtils.mapWidgetDetailFromWidgetTemplateSetting(
      widgetTemplateSettingModel
    );
  ...
  private dispatchActionSetWidgetboxesInfo(widgetDetail?: WidgetDetail) {
    ...
    this.store.dispatch(
      this.xnCommonActions.setWidgetboxesInfo(layoutPageInfo, this.ofModule)
    );
  }
  ...
  private updateDefaultWidgetTitleFromProperty(
    ...
    if (properties) {
      const propTitleText: WidgetPropertyModel =
        this.propertyPanelService.getItemRecursive(properties, 'TitleText');
      if (propTitleText) {
        if (
  ...
  saveWidgetPage(
    ...
    widgets.forEach((widget: WidgetDetailPage) => {
      ...
      if (rs.length) {
        ...
        widgetSettingModel = new WidgetSettingModel({
          ...
          widgetName:
            typeof rs[0].data.title == 'string' ? rs[0].data.title : '',
    ...
    observableBatch.push(
      this.pageSettingService.savePageSetting(this.createPageSettingModel(data))
    );
  ...
  onRowTableClick($event: any) {
    ...
    this.store.dispatch(
      this.widgetDetailActions.loadWidgetTypeDetail(rowData, this.ofModule)
    );
  ...
  onCancelEditingWidget(widgetDetail: WidgetDetail) {
    // Use LightWidgetDetail to avoid unnescessary param that causing huge memmory in store.
    const cancelEditingWidgetDettail: LightWidgetDetail = new LightWidgetDetail(
      widgetDetail
    );
    const editingWidget: EditingWidget = {
    ...
    this.store.dispatch(
      this.widgetDetailActions.cancelWidgetEditing(editingWidget, this.ofModule)
    );
  }
  ...
  onSaveSuccessWidget(widgetDetail: WidgetDetail, widgetEditDialog) {
    ...
    if (widgetDetail.idRepWidgetType === WidgetType.FieldSet) {
      updateInfo =
        this.widgetUtils.getKeyValueListFromFieldSetWidget(widgetDetail);
        ...
        if (this.widgetEditDialogs && this.widgetEditDialogs.length)
          this.widgetEditDialogs.forEach((widgetEditDialog) => {
            if (
              ...
              if (
                widgetEditDialog.widgetData.data.idRepWidgetType ==
                  WidgetType.DataGrid ||
                widgetEditDialog.widgetData.data.idRepWidgetType ==
  ...
  public setMaximizeWidget(isMaximized) {
    if (isMaximized) {
      ...
      $curentBox.removeClass('split-area-hide').addClass('split-area-maximize');
  ...
  //#region SignalR
  //send message when Widget customization is saved successfully
    ...
    this.syncStateSubscription = Observable.fromEvent<StorageEvent>(
      ...
      .subscribe((evt) => {
        ...
        if (
          !newValue ||
          !newValue.data ||
          (newValue.browserTabId && newValue.browserTabId != this.browserTabId)
        )
          return;
        switch (evt.key) {
          case LocalStorageWidgetRowDataFromPopup:
            this.handleRelatingWidget(
              newValue.data,
              this.getLinkWidgetIdsByWidgetId(newValue.data.widgetDetail)
            );
            break;
        }
      });
  ...
  private handleRelatingWidget(data: any, relatingWidgetIds: Array<string>) {
    ...
    this.widgetModuleComponents.forEach((widgetModuleComponent) => {
      ...
      this.store.dispatch(
        this.widgetDetailActions.loadWidgetTypeDetail(data, this.ofModule)
      );
      i++;
    });
  }

  public reloadLinkWidgetsHandler(data: WidgetDetail) {
    ...
    this.reloadWidgetDetails(reloadWidget);
  }
}
