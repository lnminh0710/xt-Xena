...
export abstract class BaseWidgetContainer extends BaseComponent {
  ...
  protected widgetModuleComponents: QueryList<WidgetModuleComponent>;

  @Input() pageId: string;
  @Input() tabID: string;

  private actionStorageStateSubscription: Subscription;
  ...
  public reloadSharingWidgetDetails(
    ...
    const requestWidgetDetails = uniqBy(
      widgetDetails,
      (widgetDetail: WidgetDetail) => {
        const filterParam = widgetDetail.widgetDataType.listenKeyRequest(
          this.ofModule.moduleNameTrim
        );
        return (
          ...
          (widgetDetail: WidgetDetail) => {
            if (widgetDetail) {
              ...
              widgetDetails.forEach((data: WidgetDetail) => {
                const filterParamTarget = data.widgetDataType.listenKeyRequest(
                  this.ofModule.moduleNameTrim
                );
                if (
    ...
    widgetDetails.forEach((wgDetail: WidgetDetail) => {
      ...
      if (this.widgetModuleComponents && this.widgetModuleComponents.length) {
        ...
        if (widgetModuleComponent && widgetModuleComponent.templateId) {
          ...
          if (
            !widgetModuleComponent.widgetMenuStatusComponent.isEditTemplateMode
          ) {
  ...
  protected getExternalParam(widgetDetail: WidgetDetail) {
    ...
    if (widgetBox && widgetBox.properties) {
      ...
      if (properties) {
        const externalParam = this.propertyPanelService.getItemRecursive(
          properties,
          'ExternalParam'
        );
          ...
          if (
            ...
            layoutPageInfoModel.widgetboxesTitle.forEach((box) => {
              ...
              if (targetWidgetDetail) {
                const status = this.widgetUtils.isValidWidgetToConnect(
                  widgetDetail,
                  targetWidgetDetail
                );
  ...
  protected resetWidgetTableOnEntityChanged() {
    if (this.widgetModuleComponents && this.widgetModuleComponents.length) {
      this.widgetModuleComponents.forEach((widgetModuleComponent) => {
        ...
        if (
          this.widgetUtils.isTableWidgetDataType(widgetModuleComponent.data) &&
  ...
  public subscribeWidgetActionStorageState() {
    ...
    this.actionStorageStateSubscription = Observable.fromEvent<StorageEvent>(
      window,
      'storage'
    )
      .filter((evt) => {
