...
export class NgGrid implements OnInit, DoCheck, OnDestroy {
    ...
    if (this.isResizing) {
      ...
      if (canResize) {
        if (calcSize.x != itemSize.x || calcSize.y != itemSize.y) {
          ...
          if (
            ['up', 'down', 'left', 'right', 'up-left'].indexOf(this.cascade) >=
            0
          ) {
  ...
  private _calculateGridSize(width: number, height: number): NgGridItemSize {
    ...
    if (
      !this._isWithinBoundsX({ col: 1, row: 1 }, { x: sizex, y: sizey }, null)
    )
      sizex = this._maxCols;
  ...
  private _calculateGridPosition(
    ...
    if (!this._isWithinBoundsX({ col: col, row: row }, { x: 1, y: 1 }, null))
      col = this._maxCols;
    ...
    while (this._hasGridCollision(pos, dims) && retries-- > 0) {
      ...
      switch (directionMode) {
        case 'top':
          ...
          if (
            !this._isWithinBoundsY(itemCollisionPos, itemCollisionDims) &&
            fixSizeYIfInvalid
          ) {
  ...
  public _isWithinBounds(
    ...
    return (
      this._isWithinBoundsX(pos, dims, item) && this._isWithinBoundsY(pos, dims)
    );
  }
