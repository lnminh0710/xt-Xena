...
export class NgGridItem implements OnInit, OnDestroy {
  ...
  public caculatePositionFromPercentage(
    ...
    let maxHeight =
      designHeight + designHeight * this._ngGrid.design_size_diff_percentage;
      ...
      default:
        ...
        // View Mode
          ...
          if (this.col == 1) {
            ...
            let leftValue =
              'calc(' + posPercentage.leftPercentageViewMode * 100 + '% + 5px)';
          ...
          if (this.row == 1) {
            ...
            let topValue =
              'calc(' + posPercentage.topPercentageViewMode * 100 + '% + 5px)';
  ...
  public updateTopLeftPercentage() {
    if (this._ngGrid.getCurrentMaxWidth()) {
      this._leftPercentage =
        Math.round(
          (this._elemLeft / this._ngGrid.getCurrentMaxWidth()) * 100000
        ) / 100000;
    }
    if (this._ngGrid.getCurrentMaxHeight()) {
      this._topPercentage =
        Math.round(
          (this._elemTop / this._ngGrid.getCurrentMaxHeight()) * 100000
        ) / 100000;
    }
  ...
  private updateSizePercentageForViewMode() {
    ...
    try {
      ...
      if (this.col + this.sizex >= this._ngGrid.getContainerColumns() - 2) {
        let left =
          this.col == 1 ? 5 : Math.round(maxWidth * this._leftPercentage);
        // Check and add padding to fit page if missing px.
        widthMargin =
          widthMargin +
          Math.max(
            maxWidth - (left + (widthMargin - this._ngGrid.marginLeft)),
            0
          );
      }
      ...
      if (this.row + this.sizey >= this._ngGrid.getContainerRow() - 2) {
        let top =
          this.row == 1 ? 5 : Math.round(maxHeight * this._topPercentage);
        // Check and add padding to fit page if missing px.
        heightMargin =
          heightMargin +
          Math.max(
            maxHeight - (top + (heightMargin - this._ngGrid.marginTop)),
            0
          );
      }
  ...
  public updateWidthHeightPercentage() {
    if (this._ngGrid.getCurrentMaxWidth()) {
      this._widthPercentage =
        Math.round(
          (this._elemWidth / this._ngGrid.getCurrentMaxWidth()) * 100000
        ) / 100000;
    ...
    if (this._ngGrid.getCurrentMaxHeight()) {
      this._heightPercentage =
        Math.round(
          (this._elemHeight / this._ngGrid.getCurrentMaxHeight()) * 100000
        ) / 100000;
