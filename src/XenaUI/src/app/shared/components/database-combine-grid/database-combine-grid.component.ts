  ...
  implements OnInit, OnDestroy, AfterViewInit, OnChanges
{
    ...
    this.databaseServiceSubscription = this.databaseService
      ...
      .subscribe((response: ApiResultResponse) => {
        this.appErrorHandler.executeAction(() => {
          ...
          let tableData = this.datatableService.formatDataTableFromRawData(
            response.item
          );
          ...
          this.databaseServiceSubscription = this.databaseService
            ...
            .subscribe((response: ApiResultResponse) => {
              this.appErrorHandler.executeAction(() => {
                ...
                countryTableData =
                  this.datatableService.buildDataSource(countryTableData);
  ...
  private updateCache() {
    ...
    if (this.cacheData[currentDatabaseItem['IdSelectionDatabaseName']]) {
      this.cacheData[currentDatabaseItem['IdSelectionDatabaseName']].isDirty =
        true;
      this.cacheData[currentDatabaseItem['IdSelectionDatabaseName']].isDeleted =
        ...
        databaseItems[key].gridData.data.forEach((country) => {
          let saveItem: any = {
            ...
            Priority: this.databaseGrid
              .getCurrentNodeItems()
              .find((x) => x.IdSelectionDatabaseName == key).Priority,
            QtyToNeeded:
          ...
          if (
            ...
            typeof country['IdSelectionProjectDatabase'] === 'object'
          ) {
  ...
  public onCountryGridEditEnded(editingItem) {
    if (this.cacheData && editingItem) {
      ...
      let currentItemIndex = selectingDatabaseItem.gridData.data.findIndex(
        (dt) =>
          dt.IdSelectionProjectCountry == editingItem.IdSelectionProjectCountry
      );
