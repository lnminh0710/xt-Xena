...
export class ImageZoomerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    this.lockedCursorShadeState = store.select(
      (state) =>
        dataEntryReducer.getDataEntryState(state, this.tabID).lockedCursorShade
    );
    ...
    this.showSmallImageState = store.select(
      (state) =>
        dataEntryReducer.getDataEntryState(state, this.tabID).showSmallImage
    );
  }
  ...
  private subscribeLockedCursorShadeState() {
    this.lockedCursorShadeStateSubscription =
      this.lockedCursorShadeState.subscribe((lockedCursorShadeState: any) => {
  ...
  private subscribeScanningStatusDataState() {
    this.scanningStatusDataStateSubscription =
      this.scanningStatusDataState.subscribe((scanningStatusDataState: any) => {
        this.appErrorHandler.executeAction(() => {
          if (scanningStatusDataState) {
            ...
            this.imageUrl = this.dataEntryService.buildImageFullPath(
              scanningStatusDataState
            );
            this.imageUrls = this.dataEntryService.buildListImages(
              scanningStatusDataState
            );
    ...
    this.requestDownloadSubscription = this.showSmallImageState.subscribe(
      (showSmallImageState: any) => {
        this.appErrorHandler.executeAction(() => {
          if (
            showSmallImageState &&
            this.smallImageCollapsed != showSmallImageState.isCollapsed
          ) {
            this.toggleSmallImage();
          }
        });
  ...
  private moveImage($tracker, $maginner, $cursorshade, e, moveType?) {
    ...
    const fiz = this,
      ...
      pagex = moveType
        ? specs.lastpagex +
            (specs.lastpagex === trackercoords.left ? 2 : 1) * newLeft >
      ...
      $bigimage.on('loadevt', (event, _e) => {
        if (_e.type === 'error') {
          ...
          if (window.console && console.error) {
            console.error(
              'Cannot find Featured Image Zoomer larger image: ' + src
            );
          } else {
        ...
        $magnifier.on('mousemove', (event) => {
          if (_DRAGGGING_STARTED == 1) {
            ...
            specs.lastpagex =
              $cursorshade.offset().left + $cursorshade.width() / 2;
            specs.lastpagey =
              $cursorshade.offset().top + $cursorshade.height() / 2;
          }
        });
