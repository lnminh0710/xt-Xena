  ...
  private initPerfectScroll() {
    if (this.selectedParkedItemLocal && this.parkedItem) {
      let selected =
        ...
        this.selectedParkedItemLocal.id.value == this.parkedItem.id.value;
        ...
        (widgetDataUpdatedState: WidgetDataUpdated) => {
          this.appErrorHandler.executeAction(() => {
            if (
              ...
              this.parkedItem = this.parkedItemService.mergeUpdateInfoData(
                this.parkedItem,
    ...
    this.editingFormDataSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          ...
          action.module.idSettingsGUI == this.activeModule.idSettingsGUI
        );
      })
  ...
  private mapFormDataToParkedItem(formData, parkedItemFields: any[]) {
    for (const field in formData) {
      ...
      if (typeof formData[field] === 'object') {
        ...
        let found = parkedItemFields.find(
          (x) =>
            x.fieldName === field || this.lowerFirstChar(x.fieldName) == field
        );
        if (found) {
      ...
      for (let i = 0; i < this.config.length; i++) {
        if (this.config[i].fieldName.toLowerCase() !== 'idperson') {
          this.parkedItemFields.push({
            ...
            tooltipPlacement: this.parkedItemService.buildTooltipPlacement(
