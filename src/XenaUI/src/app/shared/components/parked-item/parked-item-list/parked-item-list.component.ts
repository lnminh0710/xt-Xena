import { Component, Input, OnInit, OnDestroy, Renderer, ChangeDetectionStrategy, ChangeDetectorRef, ElementRef, ViewChild } from '@angular/core';
...
import { ParkedItemService, ModalService, AppErrorHandler, GlobalSettingService, AccessRightsService, ParkedItemProcess } from 'app/services';
...
import {
    PageSize, MenuModuleId,
    GlobalSettingConstant, MessageModal, ModuleType, Configuration, TabButtonActionConst
...
export class ParkedItemListComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    public options: any = {
        ...
        moves: function (el: any, container: any, handle: any): any {
            if ($(handle).closest('div.parkedItemBody').length || $(handle).closest('div.parkedItemFooter').length)
                return false;

            if (el.tagName.toLowerCase() === 'tr' && (isUndefined($(el).data('allow-drag')) || $(el).data('allow-drag') === false)) {
                return false;
            }
    ...
    constructor(
        ...
        this.idSettingsModuleState = store.select(state => parkedItemReducer.getParkedItemState(state, this.ofModule.moduleNameTrim).idSettingsModule);
        ...
        this.moduleStatesState = store.select(state => state.mainModule.moduleStates);
        this.requestTogglePanelState = store.select(state => parkedItemReducer.getParkedItemState(state, this.ofModule.moduleNameTrim).requestTogglePanel);
        this.toggleDisabledPanelState = store.select(state => parkedItemReducer.getParkedItemState(state, this.ofModule.moduleNameTrim).toggleDisabledPanel);
        ...
        this.selectedGoToModuleItemState = store.select(state => processDataReducer.getProcessDataState(state, this.ofModule.moduleNameTrim).selectedGoToModuleItem);

        //accessRight For ParkedItem
        this.accessRight = this.accessRightsService.GetAccessRightsForParkedItem(this.ofModule);
    ...
    onRouteChanged() {
        ...
        this.store.dispatch(this.processDataActions.loadParkedItemsCompleted(this.ofModule));
    }
    ...
    private subcribeNewOrEditModeState() {
        this.newOrEditSubscription = this.dispatcher.filter((action: CustomAction) => {
            return (action.type === ProcessDataActions.NEW_MAIN_TAB ||
    ...
    private subcribeViewModeState() {
        this.newOrEditSubscription = this.dispatcher.filter((action: CustomAction) => {
            return (action.type === ProcessDataActions.VIEW_MODE) &&
                action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
    ...
    private forceCollapseSidebar() {
        if (!Configuration.PublicSettings.enableOrderFailed && this.usingModule.idSettingsGUI == MenuModuleId.orderDataEntry) {
            this.showList = false;
            this.isPanelDisabled = true;
            this.store.dispatch(this.parkedItemActions.setCollapseState(true, this.ofModule));
            this.store.dispatch(this.layoutInfoActions.setParkedItemWidth(this.pageSize.ParkedItemHideSize.toString(), this.ofModule));
    ...
    private checkSidebarCollapse() {
        ...
        const $body = $('body');
        if (this.showList && (this.accessRight.read || this.usingModule.idSettingsGUI == MenuModuleId.orderDataEntry) && $body.hasClass('sidebar-collapse')) {
            $body.removeClass('sidebar-collapse');
            setTimeout(() => {
                if ($('.parked-item-panel').hasClass('parked-item-panel-disabled'))
                    $body.addClass('sidebar-collapse');
            }, 100);
        } else if ((!this.showList || (!this.accessRight.read && this.usingModule.idSettingsGUI != 7)) && !$body.hasClass('sidebar-collapse')) {
            $body.addClass('sidebar-collapse');
        }
    }
    ...
    private subcribeParkedItemsState() {
        this.parkedItemsStateSubscription = this.parkedItemsState.skip(1).subscribe((parkedItemsStateData: any) => {
            this.appErrorHandler.executeAction(() => {
                ...
                this.store.dispatch(this.moduleActions.addWorkingModule(this.ofModule, this.subModules, parkedItemsStateData, this.fieldConfig));
                this.store.dispatch(this.moduleActions.resetSelectingWorkingModuleParkedItem(this.ofModule));

                this.parkedItems = parkedItemsStateData;
                this.processForParkedItems();

                setTimeout(() => {
                    if (this.usingModule.idSettingsGUI == ModuleList.OrderDataEntry.idSettingsGUI) {
                        if (!this.parkedItems.length && this.showList && this.toggleButton) {
                            this.toggle();
                        }
    ...
    private subcribeIdSettingsModuleState() {
        this.idSettingsModuleStateSubscription = this.idSettingsModuleState.subscribe((idSettingsModuleState: string) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeSelectedTabState() {
        this.selectedTabStateSubscription = this.selectedTabState.subscribe((selectedTabState: TabSummaryModel) => {
            this.appErrorHandler.executeAction(() => {
                // Firefox hack
                if (!this.showList) {
                    $('#vertical-text', this.elmRef.nativeElement).removeClass('margin-top-10px');
                    setTimeout(() => {
                        $('#vertical-text', this.elmRef.nativeElement).addClass('margin-top-10px');
                    }, 2000);
                }
            });
        });
    ...
    private subcribeOkToChangeParkedItemState() {
        this.okToChangeParkedItemSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.OK_TO_CHANGE_PARKED_ITEM && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeEditingWidgetsState() {
        this.editingWidgetsStateSubscription = this.editingWidgetsState.subscribe((editingWidgets: Array<EditingWidget>) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (!this.editingWidgets.length && this.willChangeParkedItem) {

                    this.store.dispatch(this.parkedItemActions.selectParkedItem(this.willChangeParkedItem, this.ofModule));
                    ...
                    this.store.dispatch(this.moduleActions.moveSelectedParkedItemToTop(this.ofModule, this.willChangeParkedItem));
    ...
    private subscribeToggleDisabledPanelState() {
        this.toggleDisabledPanelStateSubscription = this.toggleDisabledPanelState.subscribe((toggleDisabledPanelState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (toggleDisabledPanelState) {
                    this.isPanelDisabled = toggleDisabledPanelState.isDisabled;
    ...
    private subcribeModuleStatesState() {
        this.moduleStatesStateSubscription = this.moduleStatesState.subscribe((moduleStatesState: Array<any>) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeRequestSaveParkedItemListState() {
        this.requestSaveParkedItemListSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ParkedItemActions.REQUEST_SAVE_PARKED_ITEM_LIST && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                setTimeout(() => {
                    this.saveParkedItem();
    ...
    private subscribeTogglePanelState() {
        this.requestTogglePanelStateSubscription = this.requestTogglePanelState.subscribe((requestTogglePanelState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (requestTogglePanelState && this.showList !== requestTogglePanelState.isShow) {
    ...
    private subcribeSelectedSearchResultState() {
        this.selectedSearchResultSubscription = this.selectedSearchResultState.subscribe((selectedSearchResultState: SearchResultItemModel) => {
            this.appErrorHandler.executeAction(() => {
                this.selectedSearchResult = selectedSearchResultState;

                if (selectedSearchResultState) {
                    this.parkedItemService.getParkedItemById(this.ofModule, selectedSearchResultState[this.modulePrimaryKey]).subscribe(result => {
                        this.appErrorHandler.executeAction(() => {
                            if (result && result.collectionParkedtems && result.collectionParkedtems[0]) {
                                this.selectedEntity = result.collectionParkedtems[0];
                                this.ref.markForCheck();
                            }
                        });
                    });
    ...
    private subscribeSelectedGoToModuleItemState() {
        this.selectedGoToModuleItemStateSubscription = this.selectedGoToModuleItemState.subscribe((selectedGoToModuleItemState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeRequestAddToParkedItemsState() {
        this.requestAddToParkedItemsSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.REQUEST_ADD_TO_PARKED_ITEMS && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
    ...
    private subcribeRequestRemoveFromParkedItemsState() {
        this.requestRemoveFromParkedItemsSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.REQUEST_REMOVE_FROM_PARKED_ITEMS && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe((data: CustomAction) => {
            this.appErrorHandler.executeAction(() => {
                if (!isEmpty(data.payload)) {
    ...
    private selectParkedItemFromModuleState() {
        if (this.ofModule && this.moduleStates.length) {
            const moduleState = this.moduleStates.find(ms => ms.currentModule.idSettingsGUI === this.ofModule.idSettingsGUI);
            if (moduleState && moduleState.selectedParkedItem && moduleState.selectedParkedItem.id) {
                if (!this.selectedParkedItem || (this.selectedParkedItem && this.selectedParkedItem.id && this.selectedParkedItem.id !== moduleState.selectedParkedItem.id)) {

                    this.store.dispatch(this.parkedItemActions.selectParkedItem(moduleState.selectedParkedItem, this.ofModule));
                    this.store.dispatch(this.moduleActions.moveSelectedParkedItemToTop(this.ofModule, moduleState.selectedParkedItem));

                    setTimeout(() => {
                        if (moduleState.selectedTab) {
                            this.store.dispatch(this.tabSummaryActions.requestSelectTab(moduleState.selectedTab.tabSummaryInfor.tabID, this.ofModule));
                        }

                        if (moduleState.selectedAiTab) {
                            this.store.dispatch(this.additionalInformationActions.requestSelectAiTab(moduleState.selectedAiTab.TabID, this.ofModule));
                        }

                        this.store.dispatch(this.moduleActions.clearModuleState(moduleState.currentModule));
                    }, 500);
                }
            }
        }
    ...
    private subscribeSelectedODETabState() {
        this.selectedODETabStateSubscription = this.selectedODETabState.subscribe((selectedODETabState: any) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (this.selectedODETab && Configuration.PublicSettings.enableOrderFailed) {
                    this.dataEntryService.syncOrderFailedFromLocalStorageToFiles()
                        .finally(() => {
                        }).subscribe(response => {
                            this.appErrorHandler.executeAction(() => {
                                this.getParkedItems(this.usingModule, this.selectedODETab);
                            });
                        });
                }
            });
    ...
    private subscribeRequestReloadListState() {
        this.requestReloadListStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ParkedItemActions.REQUEST_RELOAD_LIST && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                    ...
                    .subscribe((results: ParkedItemMenuModel[]) => {
                        this.appErrorHandler.executeAction(() => {
                            if (!$.isEmptyObject(results)) {
                                ...
                                this.fieldConfig = this.parkedItemService.buildFieldConfig(results);
                            } else {
                                this.menuConfig = [];
                                this.fieldConfig = [];
                            }

                            this.store.dispatch(this.parkedItemActions.storeFieldConfig(this.fieldConfig, activeModule));
    ...
    getParkedItems(activeModule, ODETab?: any) {
        this.store.dispatch(this.parkedItemActions.loadParkedItems(activeModule, ODETab));
    }
    ...
    saveParkedItem(isRemoved?: boolean, parkedItem?: ParkedItemModel) {
        ...
        const parkedItemObj = {
            ...
            Description: null,
            JsonSettings: this.parkedItemService.buildParkedItemId(this.parkedItems),
            IsActive: 'true',
            IdSettingsModule: this.idSettingsModule || null
        }

        this.parkedItemServiceSubscription = this.parkedItemService.saveParkedItemByModule(parkedItemObj)
            .finally(() => {
                ...
                this.appErrorHandler.executeAction(() => {
                    if (result) {
                        ...
                        if (isRemoved && parkedItem) {
                            this.toasterService.pop('success', 'Success', 'Parked item removed successfully');
                            this.store.dispatch(this.parkedItemActions.removeParkedItem(parkedItem, this.ofModule));
                        } else {
                            this.toasterService.pop('success', 'Success', 'Parked item list saved successfully');
                        }
    ...
    private reloadODETab() {
        ...
        this.store.dispatch(this.dataEntryActions.dataEntryCallReload(true, this.selectedODETab.TabID));
        this.store.dispatch(this.tabSummaryActions.requestSelectSimpleTab(0, this.ofModule));//Active the first SimpleTab
    }
    ...
    closeParkedItem(parkedItem) {
        ...
        if (idx > -1) {
            ...
            if (!this.selectedODETab) {
                this.store.dispatch(this.moduleActions.removeParkedItemOfWorkingModule(this.ofModule, parkedItem));
            }
            else if (Configuration.PublicSettings.enableOrderFailed) {
                //Reload ODE
                this.reloadODETab();

                this.dataEntryService.deleteOrderFailed(this.selectedODETab.TabID, parkedItem.idScansContainerItems.value).subscribe((response: any) => {
                    this.appErrorHandler.executeAction(() => {
    ...
    removeParkedItemById(id: any) {
        const currentParkedItem = this.parkedItems.find(x => x.id && x.id.value == id);
        if (isEmpty(currentParkedItem)) { return; }
    ...
    selectParkedItem(parkedItem) {
        if (!isEqual(this.selectedParkedItem, parkedItem)) {
            if (parkedItem.isNew) {
                ...
                this.store.dispatch(this.processDataActions.requestChangeParkedItem(this.ofModule));
            }
        }
    }
    ...
    toggle() {
        ...
        if (this.isPanelDisabled ||
            (!this.accessRight.read && this.usingModule.idSettingsGUI !== MenuModuleId.orderDataEntry)) {
            return;
        }
        ...
        if (this.showList) {
            setTimeout(() => {
                $('#vertical-text', this.elmRef.nativeElement).addClass('margin-top-9px');
            }, 200);
            setTimeout(() => {
                $('#vertical-text', this.elmRef.nativeElement).addClass('margin-top-10px');
            }, 300);
        } else {
            $('#vertical-text', this.elmRef.nativeElement).removeClass('margin-top-9px');
            $('#vertical-text', this.elmRef.nativeElement).removeClass('margin-top-10px');
        }
        // Firefox hack
        ...
        this.store.dispatch(this.parkedItemActions.setCollapseState(!this.showList, this.ofModule));

        if (!this.showList) {
            this.store.dispatch(this.layoutInfoActions.setParkedItemWidth(this.pageSize.ParkedItemHideSize.toString(), this.ofModule));
        } else {
            this.store.dispatch(this.layoutInfoActions.setParkedItemWidth(this.pageSize.ParkedItemShowSize.toString(), this.ofModule));
        }
        if (!this.isAutoClickToggle)
    ...
    transferDataSuccess(event: any) {
        if (!event || !event.dragData || !event.dragData[this.modulePrimaryKey]) {
            return;
        }
    ...
    onDrop(event: DragEvent) {
        this.renderer.setElementClass(event.currentTarget, 'drag-border', false);
    ...
    onDragOver(event: DragEvent) {
        ...
        const elem: Element = this.domHandler.findSingle(document, '.xn-grid-drag');
        if (elem) {
            if (this.selectedSearchResult
                && !this.parkedItemService.checkItemExist(this.selectedSearchResult, this.modulePrimaryKey, this.parkedItems)) {
    ...
    onDragLeave(event: DragEvent) {
        ...
        const node = this.domHandler.findParent(event['toElement'], 'parked-item');
        if (node && node.length) {
            this.ref.markForCheck();
            return;
        }
    ...
    addItemToParkedItems(data: any) {
        if (this.parkedItemService.checkItemExist(data.data, this.modulePrimaryKey, this.parkedItems)) {
            if (data.adddition && data.adddition.thenSelect) {
                this.selectParkedItem(Uti.makeParkedItemObjectFromStraightObject(data.data, data.adddition.idName));
                return;
            }
        ...
        this.parkedItemServiceSubscription = this.parkedItemService.getParkedItemById(this.ofModule, data.data[this.modulePrimaryKey]).subscribe(result => {
            this.appErrorHandler.executeAction(() => {
                if (result && result.collectionParkedtems && result.collectionParkedtems[0]) {
                    ...
                    this.store.dispatch(this.moduleActions.moveSelectedParkedItemToTop(this.ofModule, newParkedItem));
                    if (document.querySelector('.parked-item-content') != null)
                        document.querySelector('.parked-item-content').scrollTop = 0;
                    this.saveParkedItem();
                    this.processForParkedItems();
                    this.ref.markForCheck();
                }
    ...
    removeItemFromParkedItems(item) {
        if (!this.parkedItemService.checkItemExist(item, this.modulePrimaryKey, this.parkedItems)) {
            return;
        }
    ...
    public deleteAllParkedItem($event: any) {
        ...
        this.modalService.confirmMessageHtmlContent(new MessageModel({
            ...
            message: [{ key: '<p>' }, { key: 'Modal_Message__Do_You_Want_To_Delete_All_Packed_Items' },
            { key: '</p>' }],
            buttonType1: MessageModal.ButtonType.danger,
            callBack1: () => { this.deleteParkedItems(); }
        }));
            ...
            .subscribe((result: any) => {
                this.appErrorHandler.executeAction(() => {
                    if (result) {
                        ...
                        this.store.dispatch(this.moduleActions.removeAllParkedItemsOfWorkingModule(this.ofModule));
                        this.store.dispatch(this.parkedItemActions.unselectParkedItem(this.ofModule));
                        this.store.dispatch(this.tabSummaryActions.removeAllTabs(this.ofModule));

                        if (this.selectedODETab && Configuration.PublicSettings.enableOrderFailed) {
                            //delete all order failed
                            this.dataEntryService.deleteAllOrderFailed(this.selectedODETab.TabID).subscribe();
                            this.reloadODETab();
                        }
                    }
                });
    ...
    private getExpandCollapse() {
        this.getGlobalSettingSubscription = this.globalSettingService.getAllGlobalSettings(this.ofModule.idSettingsGUI).subscribe(
    ...
    private getAllGlobalSettingSuccess(data: GlobalSettingModel[]) {
        ...
        this.store.dispatch(this.parkedItemActions.setCollapseState(!showList, this.ofModule));
        ...
        if (!this.showList || this.isPanelDisabled ||
            (!this.accessRight.read && this.usingModule.idSettingsGUI != MenuModuleId.orderDataEntry)) {
            this.store.dispatch(this.layoutInfoActions.setParkedItemWidth(this.pageSize.ParkedItemHideSize.toString(), this.ofModule));
        } else {
            this.store.dispatch(this.layoutInfoActions.setParkedItemWidth(this.pageSize.ParkedItemShowSize.toString(), this.ofModule));
    ...
    private reloadAndSaveExpandConfig() {
        ...
        this.getGlobalSettingSubscription = this.globalSettingService.getAllGlobalSettings(this.ofModule.idSettingsGUI).subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private saveExpandConfig(data: GlobalSettingModel[]) {
        ...
        this.currentGlobalSettingModel.jsonSettings = JSON.stringify({ IsExpand: this.showList });
    ...
    private setParkedItemNewTime() {
        ...
        if (applicationSettings && applicationSettings.children && applicationSettings.children.length) {
            const parkedItemNewTimeItem = applicationSettings.children.find((item, i) => {
                return item.id == 'ParkedItemNewTime';
            });

            //in hours
            if (parkedItemNewTimeItem) {
                const isCallProcessForParkedItems: boolean = this.parkedItemNewTime != parkedItemNewTimeItem.value;
    ...
    private processForParkedItems() {
        ...
        if (!this.parkedItems || !this.parkedItems.length || !this.parkedItemNewTime) return;
        ...
        if (serverDateNow) {
            ...
            serverDateNow.setTime(serverDateNow.getTime() - (newTimeSettings * 60 * 60 * 1000));
