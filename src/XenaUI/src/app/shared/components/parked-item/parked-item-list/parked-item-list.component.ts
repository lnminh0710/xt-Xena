  ...
  implements OnInit, OnDestroy
{
  ...
  private checkSidebarCollapse() {
    ...
    if (
      ...
      (!this.showList ||
        (!this.accessRight.read && this.usingModule.idSettingsGUI != 7)) &&
      !$body.hasClass('sidebar-collapse')
    ) {
      $body.addClass('sidebar-collapse');
    }
  ...
  private subcribeIdSettingsModuleState() {
    this.idSettingsModuleStateSubscription =
      this.idSettingsModuleState.subscribe((idSettingsModuleState: string) => {
        this.appErrorHandler.executeAction(() => {
  ...
  private subscribeRequestSaveParkedItemListState() {
    this.requestSaveParkedItemListSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === ParkedItemActions.REQUEST_SAVE_PARKED_ITEM_LIST &&
          action.module.idSettingsGUI == this.ofModule.idSettingsGUI
        );
      })
      .subscribe(() => {
          ...
          this.appErrorHandler.executeAction(() => {
            ...
            if (selectedSearchResultState) {
              this.parkedItemService
                .getParkedItemById(
                  this.ofModule,
                  selectedSearchResultState[this.modulePrimaryKey]
                )
                .subscribe((result) => {
                  this.appErrorHandler.executeAction(() => {
                    if (
  ...
  private subcribeRequestAddToParkedItemsState() {
    this.requestAddToParkedItemsSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === ProcessDataActions.REQUEST_ADD_TO_PARKED_ITEMS &&
          action.module.idSettingsGUI == this.ofModule.idSettingsGUI
        );
      })
  ...
  private subcribeRequestRemoveFromParkedItemsState() {
    this.requestRemoveFromParkedItemsSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === ProcessDataActions.REQUEST_REMOVE_FROM_PARKED_ITEMS &&
          action.module.idSettingsGUI == this.ofModule.idSettingsGUI
        );
      })
      .subscribe((data: CustomAction) => {
  ...
  private selectParkedItemFromModuleState() {
    if (this.ofModule && this.moduleStates.length) {
      const moduleState = this.moduleStates.find(
        (ms) => ms.currentModule.idSettingsGUI === this.ofModule.idSettingsGUI
      );
      if (
        moduleState &&
        ...
        if (
          ...
          (this.selectedParkedItem &&
            ...
            this.selectedParkedItem.id !== moduleState.selectedParkedItem.id)
        ) {
          this.store.dispatch(
            this.parkedItemActions.selectParkedItem(
              moduleState.selectedParkedItem,
          ...
          setTimeout(() => {
            if (moduleState.selectedTab) {
              this.store.dispatch(
                this.tabSummaryActions.requestSelectTab(
                  moduleState.selectedTab.tabSummaryInfor.tabID,
                  this.ofModule
                )
              );
            }
            ...
            this.store.dispatch(
              this.moduleActions.clearModuleState(moduleState.currentModule)
            );
          }, 500);
        }
      }
  ...
  private reloadODETab() {
    ...
    this.store.dispatch(
      this.dataEntryActions.dataEntryCallReload(true, this.selectedODETab.TabID)
    );
    this.store.dispatch(
  ...
  selectParkedItem(parkedItem) {
    if (!isEqual(this.selectedParkedItem, parkedItem)) {
      if (parkedItem.isNew) {
        ...
        this.store.dispatch(
          this.processDataActions.requestChangeParkedItem(this.ofModule)
        );
      }
    }
  }
  ...
  toggle() {
    ...
    this.store.dispatch(
      this.parkedItemActions.setCollapseState(!this.showList, this.ofModule)
    );
  ...
  private setParkedItemNewTime() {
    ...
    if (
      ...
      if (parkedItemNewTimeItem) {
        ...
        if (!parkedItemNewTimeItem.value) this.clearProcessForParkedItems();
