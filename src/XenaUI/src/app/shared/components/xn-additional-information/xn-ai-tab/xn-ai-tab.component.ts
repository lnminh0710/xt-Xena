...
export class XnAdditionalInformationTabComponent
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  constructor(
    ...
    this.requestSelectAiTabState = this.store.select(
      (state) =>
  ...
  private appendDefaultValueToTranslateResource() {
    for (let tab of this.tabs) {
      defaultLanguage['XnAiTab__' + tab.TabName] = tab.TabName;
    }
  }
  ...
  public selectTab(tab: any) {
    ...
    this.store.dispatch(
      this.additionalInformationActions.selectAdditionalInformationTab(
        tab,
        this.ofModule
      )
    );
  ...
  private buildTabsFromGlobalSetting(
    ...
    this.tabListSettings = data.find(
      (x) => x.globalName === this.globalSettingAIName
    );
  ...
  private getModule() {
    this.globalSettingAIName = uti.String.Format(
      '{0}_{1}',
  ...
  private subscribeRequestSelectAiTabState() {
    this.requestSelectAiTabStateSubscription =
      this.requestSelectAiTabState.subscribe((requestSelectAiTabState: any) => {
        this.appErrorHandler.executeAction(() => {
          if (requestSelectAiTabState) {
            const selectedTab = this.tabs.find(
              (t) => t.TabID === requestSelectAiTabState.aiTabId
            );
            if (selectedTab) {
              setTimeout(() => {
  ...
  private subscribeRequestResize() {
    this.requestResizeSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === AdditionalInformationActions.REQUEST_RESIZE &&
  ...
  public adjustScrollingArea() {
    ...
    if (
      $('.ai-tab-summary-wrapper', this._eref.nativeElement).outerWidth() +
        $('.ai-tab-summary-list', this._eref.nativeElement).position().left *
          -1 >
      this.widthOfList()
    ) {
    ...
    if (this.getLeftPos() < 0) {
      ...
      $('.ai-tab-summary-list', this._eref.nativeElement).animate(
  ...
  private maxScrollLeftItems() {
    return (
      $('.ai-tab-summary-list li', this._eref.nativeElement).length -
  ...
  private numberOfHiddenItems() {
    if (parseInt($('.ai-tab-summary-list').get(0).style.left) < 0) {
      return (
        (parseInt($('.ai-tab-summary-list').get(0).style.left) * -1) /
        this.widthOfItem()
      );
    }

    return (
      parseInt($('.ai-tab-summary-list').get(0).style.left) / this.widthOfItem()
    );
  }
  ...
  public scrollerRightClick(event) {
    ...
    setTimeout(() => {
      if (this.getLeftPos() < 0) {
        if (
          $('.ai-tab-summary-wrapper', this._eref.nativeElement).outerWidth() +
            $('.ai-tab-summary-list', this._eref.nativeElement).position()
              .left *
              -1 >
          this.widthOfList()
        ) {
  ...
  public onWindowResize(event) {
    setTimeout(() => {
      if (
        $('.ai-scroller-left', this._eref.nativeElement).is(':visible') &&
