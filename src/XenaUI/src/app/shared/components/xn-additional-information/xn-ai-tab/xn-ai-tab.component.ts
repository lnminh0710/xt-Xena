    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    private subscribeRequestSelectAiTabState() {
        this.requestSelectAiTabStateSubscription =
            this.requestSelectAiTabState.subscribe(
                (requestSelectAiTabState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        if (requestSelectAiTabState) {
                            const selectedTab = this.tabs.find(
                                (t) =>
                                    t.TabID === requestSelectAiTabState.aiTabId
                            );
                            if (selectedTab) {
                                setTimeout(() => {
    ...
    private subscribeRequestResize() {
        this.requestResizeSubscription = this.dispatcher
            .filter((action: CustomAction) => {
                return (
                    action.type ===
                        AdditionalInformationActions.REQUEST_RESIZE &&
    ...
    private getLeftPos() {
        return $(".ai-tab-summary-list", this._eref.nativeElement).position()
            .left;
    }

    private widthOfList() {
        let itemsWidth = 0;
        $(".ai-tab-summary-list li", this._eref.nativeElement).each(
            function () {
    ...
    private numberOfHiddenItems() {
        ...
        return (
            parseInt($(".ai-tab-summary-list").get(0).style.left) /
            this.widthOfItem()
        );
    }
    ...
    public onWindowResize(event) {
        setTimeout(() => {
            if (
                $(".ai-scroller-left", this._eref.nativeElement).is(
                    ":visible"
                ) &&
