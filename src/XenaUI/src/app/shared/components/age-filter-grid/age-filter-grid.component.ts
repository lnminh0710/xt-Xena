    ...
    implements OnInit, OnDestroy, AfterViewInit, OnChanges
{
        ...
        if (data && data.item && data.item[0]) {
            ...
            array.forEach((item) => {
                ...
                let dynamicRulesType: DynamicRulesType = new DynamicRulesType({
                    ...
                    value: item.Value,
                    config: item.Config
                        ? Uti.tryParseJson(item.Config, null)
                        : null,
    ...
    private loadTemplateFromSettingSuccess(data: Array<any>) {
        if (data && data.length) {
            let setting = data.find(
                (p) =>
                    p.globalName == this.globalSettingConstant.ageFilterTemplate
            );
            if (setting) {
    ...
    private processNewRule(isForAllCountry: boolean) {
        ...
        if (
            this.filterGridData &&
            this.filterGrid.getCurrentNodeItems().length
        ) {
