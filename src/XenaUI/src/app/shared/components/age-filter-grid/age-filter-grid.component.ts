...
export class AgeFilterGridComponent
  ...
  implements OnInit, OnDestroy, AfterViewInit, OnChanges
{
        ...
        this.config = {
          fields: {
            age: {
              ...
              dynamicRulesType: this.dynamicRulesType,
            },
          },
        };
  ...
  private loadTemplateFromSettingSuccess(data: Array<any>) {
    if (data && data.length) {
      let setting = data.find(
        (p) => p.globalName == this.globalSettingConstant.ageFilterTemplate
      );
      if (setting) {
  ...
  private getComboBoxForRuleBuilder() {
    this.getComboBoxForRuleBuilderSubscription = this.ruleService
      .getComboBoxForRuleBuilder(QueryObject.OrdersRules)
  ...
  private loadFilterFromSeletedCountry(forceLoadFromService?: boolean) {
    if (this.countryGrid && this.countryGrid.getSelectedNode()) {
      ...
      if (!forceLoadFromService && this.cacheData[countryLanguageId]) {
        ...
        this.filterGridData = {
          data: this.cacheData[countryLanguageId].gridData.data.filter(
            (dt) => !dt.willDelete
          ),
          columns: this.initFilterGridSetting(),
        };
              ...
              if (!response || !response.item || !response.item.length) {
                ...
                if (this.profileData && this.profileData.length) {
                  this.profileData = this.profileData.filter(
                    (p) => p[ID_STRING.COUNTRY_LANGUAGE_ID] != countryLanguageId
                  );
                }
  ...
  private processApplyProfile(rules, country, selectingCountry) {
    ...
    let currentData =
      this.cacheData[country[ID_STRING.COUNTRY_LANGUAGE_ID]].gridData.data;
  ...
  private setCacheDataOfAllCountry(filterRawData: any) {
    ...
    if (this.cacheDataForAllCountry && this.cacheDataForAllCountry.length) {
      filterRawData = filterRawData.concat(
        cloneDeep(this.cacheDataForAllCountry)
      );
    }
      ...
      if (ruleSet && ruleSet.rules) {
        ...
        if (rule) {
          ...
          filterData.push({
            ...
            FromValue: filterRule.range
              ? filterRule.range.toLowerCase() === 'dates'
                ? this.convertToDay(
                    this.createFromToValue(filterRule.toValue),
                    filterRule.range
                  )
                : this.convertToDay(
                    this.createFromToValue(filterRule.fromValue),
                    filterRule.range
                  )
              : '',
            ToValue: filterRule.range
              ? filterRule.range.toLowerCase() === 'dates'
                ? this.convertToDay(
                    this.createFromToValue(filterRule.fromValue),
                    filterRule.range
            ...
            Filter:
              (filterRule.groupOperators || '') +
              (filterRule.groupQuantity || ''),
            Priority:
  ...
  private buildRangeText(filterRule) {
    ...
    if (filterRule && filterRule.range) {
      if (filterRule.range.toLowerCase() === 'dates') {
        rs =
          this.createFromToValue(filterRule.fromValue, true) +
          '->' +
          this.createFromToValue(filterRule.toValue, true);
      } else {
    ...
    if (typeof value === 'string') {
      return formatDate
        ? this.datePipe.transform(
            this.formatUtcDate(value),
            this.globalDateFormat
          )
        : this.formatUtcDate(value);
    } else if (typeof value === 'object') {
      return formatDate
        ? this.datePipe.transform(value, this.globalDateFormat)
        : value;
    }
  ...
  public newRule(isForAllCountry?: boolean) {
    if (isForAllCountry !== undefined) this.isForAllCountry = isForAllCountry;
  ...
  private onDeleteRule(deletedItem, isForAllCountry?: boolean) {
    if (this.filterGrid) {
      let willDeletedItem = this.filterGrid
        .getCurrentNodeItems()
        .find(
          (dt) =>
      ...
      if (isForAllCountry) {
        Object.keys(this.cacheData).forEach((countryId) => {
          ...
          if (willDeletedItemIndex !== -1) {
            if (
              selectingCountryItem.gridData.data[willDeletedItemIndex][
                ID_STRING.ID_SELECTION_PROJECT_RULES
              ]
            ) {
              selectingCountryItem.gridData.data[willDeletedItemIndex][
        ...
        let willDeletedItemIndex = selectingCountryItem.gridData.data.findIndex(
          (dt) =>
            dt['DT_RowId'] == deletedItem['DT_RowId'] ||
            dt[ID_STRING.ID_SELECTION_PROJECT_RULES] ==
              deletedItem[ID_STRING.ID_SELECTION_PROJECT_RULES] ||
  ...
  private isRuleValid(query, selectedFilterRow) {
    ...
    if (selectedFilterRow) {
      filterGridData = this.filterGrid
        .getCurrentNodeItems()
        .filter((x) => x['DT_RowId'] != selectedFilterRow['DT_RowId']);
    } else {
    ...
    let fromValue =
      firstRuleValue.range.toLowerCase() === 'dates'
        ? this.convertToDay(
            this.createFromToValue(firstRuleValue.toValue),
            firstRuleValue.range
          )
        : this.convertToDay(
            this.createFromToValue(firstRuleValue.fromValue),
            firstRuleValue.range
          );
    let toValue =
      firstRuleValue.range.toLowerCase() === 'dates'
    ...
    for (let i = 0; i < fromToRange.length; i++) {
      ...
      if (
        fromValue == fromToRange[i][0] ||
        toValue == fromToRange[i][1] ||
        fromValue == fromToRange[i][1] ||
        toValue == fromToRange[i][0] ||
  ...
  private processEditRule() {
    ...
    if (this.cacheData) {
      let selectingCountryItem =
        this.cacheData[
          this.countryGrid.getSelectedNode().data[ID_STRING.COUNTRY_LANGUAGE_ID]
        ];
      selectingCountryItem.isDirty = true;
  ...
  private createNewRuleItem() {
    return {
      ...
      IdSelectionProjectCountry:
        this.countryGrid.getSelectedNode().data[ID_STRING.COUNTRY_ID],
      IdCountrylanguage:
        this.countryGrid.getSelectedNode().data[ID_STRING.COUNTRY_LANGUAGE_ID],
  ...
  public deleteDataToUsed() {
    this.modalService.confirmDeleteMessageHtmlContent({
      ...
      callBack1: () => {
        this.ruleService.deleteDataToUsed().subscribe(
          (response) => {
            if (response) {
