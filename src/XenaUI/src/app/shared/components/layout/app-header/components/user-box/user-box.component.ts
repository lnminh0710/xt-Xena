import {
    ...
    OnDestroy, ViewChild, ViewChildren,
    ChangeDetectorRef, QueryList, EventEmitter
} from '@angular/core';
import {
    UserService,
...
import { MessageModal, UploadFileMode, Configuration, ComboBoxTypeConstant } from 'app/app.constants';
...
export class UserBoxComponent extends UnsavedModuleCanDeactivate implements OnInit, OnDestroy {
    ...
    @ViewChildren(XnImageLoaderDirective) xnImageLoaders: QueryList<XnImageLoaderDirective>;
    ...
    constructor(
        ...
        this.userServ.currentUser.subscribe((user: User) => {
            this.appErrorHandler.executeAction(() => {
                ...
                this.profileImageUrl = Uti.getFileUrl(this.currentUser.loginPicture, UploadFileMode.Profile) + '&w=300';
    ...
    public logout = (): void => {
        if (this.dirtyModules.length) {
            ...
            this.modalService.confirmMessageHtmlContent(new MessageModel({
                ...
                message: [{key: 'Modal_Message__Do_You_Want_To_Logout_System'}],
        ...
        this.activeModuleStateSubscription = this.activeModuleState.subscribe((activeModuleState: Module) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (this.activeModule && this.activeModule.moduleNameTrim) {
                    ...
                    this.editingWidgetsState = this.store.select(state => widgetContentReducer.getWidgetContentDetailState(state, this.activeModule.moduleNameTrim).editingWidgets);
                    this.subscribeWidgetDetailState();

                    if (this.formDirtyStateSubscription) {
                        this.formDirtyStateSubscription.unsubscribe();
                    }
                    this.formDirtyState = this.store.select(state => processDataReducer.getProcessDataState(state, this.activeModule.moduleNameTrim).formDirty);
    ...
    private getRoles() {
        this.userProfileService.listUserRoleByUserId()
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    let defaultRoles = this.roles.filter(r => isBoolean(r.IsDefault) && r.IsDefault && r.IdLoginRoles != -1);
    ...
    public roleChanged() {
        if (this.isRoleFocused && this.selectedRole) {
            this.modalService.confirmMessageHtmlContent(new MessageModel({
                ...
                messageType: MessageModal.MessageType.error,
                message: [{key: '<p>'}, {key: 'Modal_Message__The_Page_Will_Reload_To_Apply_New_Role'},
                    {key: '</p>'}],
                buttonType1: MessageModal.ButtonType.danger,
                callBack1: () => {
                    ...
                    if (role)
                        this.uti.storeDefaultRole(role.IdLoginRoles);

                    const isSetDefaultRole = "1";
                    this.userProfileService.saveRolesForUser(this.buildRolesData(), isSetDefaultRole)
                        .subscribe((response: ApiResultResponse) => {
                            this.appErrorHandler.executeAction(() => {
