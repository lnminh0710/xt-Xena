    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    constructor(
        ...
        this.activeModuleState = store.select(
            mainModuleReducer.getActiveModule
        );
            ...
            (mainModulesState: Module[]) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (
                        ...
                        let moduleName: any = Uti.getModuleNamesFromUrl(
                            this.router.url
                        );
                        ...
                        if (module) {
                            this.store.dispatch(
                                this.moduleActions.activeModule(module)
                            );
                        }
                ...
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (this.continueToSelectSubModule) {
                        requestedSubModule = this.subModules.find(
                            (m) =>
                                m.moduleNameTrim ==
                                this.continueToSelectSubModule
                        );
    ...
    private subscribeActiveSubModuleModel() {
        this.activeSubModuleModelSubscription =
            this.activeSubModuleModel.subscribe(
                ...
                (requestCreateNewModuleItemState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        if (requestCreateNewModuleItemState) {
                            this.onClickNewModule(
                                requestCreateNewModuleItemState
                            );
                        }
                    });
                }
    ...
    private subscribeLayoutSettingModeState() {
        this.layoutSettingModeStateSubscription =
            this.layoutSettingModeState.subscribe(
                (enableLayoutSetting: boolean) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        if (enableLayoutSetting) {
                            ...
                            this.store.dispatch(
                                this.layoutSettingActions.requestTogglePanel(
                                    true,
                                    this.ofModule
                                )
                            );
    ...
    public onSelectedSubModule(selectedSubModule: Module): void {
        let okToChangeSubModule =
            ...
            (!this.activeSubModule ||
                selectedSubModule.idSettingsGUI !==
                    this.activeSubModule.idSettingsGUI);
    ...
    private subscribeOkToChangeSubModuleState() {
        this.okToChangeSubModuleSubscription = this.dispatcher
            .filter((action: CustomAction) => {
                return (
                    action.type ===
                        ProcessDataActions.OK_TO_CHANGE_SUB_MODULE &&
                    action.module.idSettingsGUI == this.ofModule.idSettingsGUI
                );
            })
            .subscribe(() => {
                ...
                (requestChangeModuleState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        if (requestChangeModuleState) {
                            this.onSelectedModule(
                                requestChangeModuleState.requestedModule
                            );
                            this.store.dispatch(
        ...
        this.requestChangeSubModuleStateSubscription =
            this.requestChangeSubModuleState.subscribe(
                (requestChangeSubModuleState: any) => {
                    this.appErrorHandler.executeAction(() => {
                        this.requestChangeSubModule =
                            requestChangeSubModuleState;
                        ...
                        if (requestChangeSubModuleState) {
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    const logisticModules =
                                        this.subModules.find(
                                            (m) =>
                                                m.idSettingsGUI ==
                                                MenuModuleId.logistic
                                        );
    ...
    private okToChangeOrCreateNewModule() {
        if (this.willChangeModule) {
            ...
            this.store.dispatch(
                this.layoutSettingActions.updateEditModeStatus(
                    false,
                    this.ofModule
                )
            );
            ...
            this.router.navigate([
                Uti.getPrivateUrlWithModuleName(
                    this.willChangeModule.moduleNameTrim
                ),
            ]);
    ...
    private okToChangeSubModule() {
        if (this.willChangeSubModule) {
            ...
            if (
                ...
                this.router.navigate([
                    Uti.getPrivateUrlWithModuleName(newRoute),
                ]);
            }
    ...
    public onSearchingModule(selectedModule: Module) {
        this.store.dispatch(
            this.moduleActions.searchKeywordModule(selectedModule)
        );
    }
    ...
    private getCheckedModulesSuccess(data: GlobalSettingModel[]) {
        ...
        for (let i = 0; i < this.checkedModuleIds.length; i++) {
            ...
            if (
                moduleInfo &&
                moduleInfo.accessRight &&
                moduleInfo.accessRight.read
            ) {
                this.store.dispatch(
                    this.moduleActions.addWorkingModule(
                        moduleInfo,
                ...
                if (
                    i == 0 &&
                    this.router &&
                    uti.Uti.isRootUrl(this.router.url)
                ) {
