import { Component, OnInit, OnDestroy, ChangeDetectorRef, ViewChild, ElementRef, ChangeDetectionStrategy, AfterViewInit } from '@angular/core';
...
import {
    ...
    GlobalSearchActions, BackofficeActions
...
export class AppHeaderComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        this.selectedAiTabState = store.select(state => additionalInformationReducer.getAdditionalInformationState(state, this.ofModule.moduleNameTrim).additionalInfromationTabModel);
        ...
        this.widgetTemplateModeState = store.select(state => widgetTemplateReducer.getWidgetTemplateState(state, this.ofModule.moduleNameTrim).enableWidgetTemplate);
        this.layoutSettingModeState = store.select(state => layoutSettingReducer.getLayoutSettingState(state, this.ofModule.moduleNameTrim).enableLayoutSetting);
        ...
        this.isParkedItemCollapsedState = store.select(state => parkedItemReducer.getParkedItemState(state, this.ofModule.moduleNameTrim).isCollapsed);
        ...
        location.onPopState(() => {
            setTimeout(() => {
                ...
                if (currentModule && currentModule.idSettingsGUI != -1) {
                    ...
                    } else if (currentModule.idSettingsGUI !== MenuModuleId.logistic &&
                        (!this.activeSubModule || currentModule.idSettingsGUI !== this.activeSubModule.idSettingsGUI)) {
                        this.onSelectedModule(currentModule);
                    }
    ...
    private registerEventControlSidebar() {
        ...
        setTimeout(() => {
            ...
            if (!$controlSidebar || !$controlSidebar.length) {
                console.log('app-header.component -> registerEventControlSidebar: can not find control-sidebar')
    ...
    private subscribeMainModulesState() {
        this.mainModulesStateSubscription = this.mainModulesState.subscribe((mainModulesState: Module[]) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (this.mainModules.length && this.router && !Uti.isRootUrl(this.router.url)) {
                    let moduleName: any = Uti.getModuleNamesFromUrl(this.router.url);

                    let module: Module = this.mainModules.find(m => m.moduleNameTrim == moduleName[0]);
        ...
        this.subModulesStateSubscription = this.subModulesState.subscribe((subModulesState: Module[]) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (this.continueToSelectSubModule) {
                    requestedSubModule = this.subModules.find(m => m.moduleNameTrim == this.continueToSelectSubModule);
                    if (!requestedSubModule) {
                        ...
                        if (logisticModules && logisticModules.children) {
                            requestedSubModule = logisticModules.children.find(m => m.moduleNameTrim == this.continueToSelectSubModule);
                        }
                    }
                } else if (this.requestChangeSubModule) {
                    requestedSubModule = this.subModules.find(m => m.idSettingsGUI == this.requestChangeSubModule.requestedSubModuleId);
                }
    ...
    private subcribeSelectedAiTabState() {
        this.selectedAiTabStateSubscription = this.selectedAiTabState.subscribe((selectedAiTabState: AdditionalInfromationTabModel) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeRequestCreateNewModuleItemState() {
        this.requestCreateNewModuleItemStateSubscription = this.requestCreateNewModuleItemState.subscribe((requestCreateNewModuleItemState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeWidgetTemplateModeState() {
        this.widgetTemplateModeStateSubscription = this.widgetTemplateModeState.subscribe((widgetTemplateModeState: boolean) => {
            this.appErrorHandler.executeAction(() => {
                if (!isNil(widgetTemplateModeState)) {
                    this.isViewMode = !widgetTemplateModeState;

                    this.store.dispatch(this.widgetTemplateActions.toggleWidgetTemplateSettingPanel(!this.isViewMode, this.ofModule));
                }
    ...
    private subscribeLayoutSettingModeState() {
        this.layoutSettingModeStateSubscription = this.layoutSettingModeState.subscribe((enableLayoutSetting: boolean) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeRequestGoToModuleState() {
        this.requestGoToModuleStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.REQUEST_GO_TO_MODULE && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
            ...
            this.appErrorHandler.executeAction(() => {
                ...
                if (data) {
                    ...
                    if (typeof primaryKeyValue === 'string' && primaryKeyValue.indexOf(',') !== -1) {
                        const module: Module = ModuleList[data.ModuleName];
                        module.searchKeyword = primaryKeyValue;
                        this.onSearchingModule(module);
                    } else {
                        this.parkedItemService.getParkedItemById(ModuleList[data.ModuleName], primaryKeyValue).subscribe(result => {
                            this.appErrorHandler.executeAction(() => {
                                let module: Module;
                                ...
                                if (result && result.collectionParkedtems && result.collectionParkedtems[0]) {
                                    ...
                                    if (data.IdSettingsGUI != this.ofModule.idSettingsGUI) {
                                        this.store.dispatch(this.processDataActions.toggleDashboardLoading(true, module));
                                        if (module.idSettingsGUIParent) {
                                            let mainModule: Module = this.mainModules.find(md => md.idSettingsGUI === module.idSettingsGUIParent);

                                            if (module.idSettingsGUIParent == MenuModuleId.logistic) {
                                                mainModule = this.mainModules.find(md => md.idSettingsGUI == MenuModuleId.backoffice);
                                            }

                                            if (module.idSettingsGUIParent !== this.ofModule.idSettingsGUI) {
                                                if (mainModule) {
                                            ...
                                            setTimeout(() => {
                                                this.store.dispatch(this.moduleActions.requestChangeSubModule(mainModule.idSettingsGUI, module.idSettingsGUI));
                                            }, 200);
                                        } else {
                                            this.store.dispatch(this.moduleActions.requestChangeModule(module));
                                        }

                                        setTimeout(() => {
                                            this.store.dispatch(this.processDataActions.selectGoToModuleItem(entity, ModuleList[data.ModuleName]));
                                    ...
                                    const dataRowReturnRefund = value && value.payload && value.payload.data;
                                    if (data.IdSettingsGUI != this.ofModule.idSettingsGUI) {
                                        this.store.dispatch(this.processDataActions.toggleDashboardLoading(true, module));
                                        if (module.idSettingsGUIParent) {
                                            let mainModule: Module = this.mainModules.find(md => md.idSettingsGUI === module.idSettingsGUIParent);
    ...
    public onSelectedSubModule(selectedSubModule: Module): void {
        let okToChangeSubModule = selectedSubModule
    ...
    private subscribeOkToCreateNewFromModuleDropdownState() {
        this.okToCreateNewFromModuleDropdownSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.OK_TO_CREATE_NEW_FROM_MODULE_DROPDOWN && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeOkToChangeModuleState() {
        this.okToChangeModuleSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.OK_TO_CHANGE_MODULE && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                if (!isEmpty(this.selectedEntity)) {
                    this.store.dispatch(this.propertyPanelActions.requestClearProperties(this.ofModule));
    ...
    private subscribeOkToChangeSubModuleState() {
        this.okToChangeSubModuleSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.OK_TO_CHANGE_SUB_MODULE && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                if (!isEmpty(this.selectedEntity)) {
                    this.store.dispatch(this.propertyPanelActions.requestClearProperties(this.ofModule));
    ...
    private subscribeRequestChangeModuleState() {
        this.requestChangeModuleStateSubscription = this.requestChangeModuleState.subscribe((requestChangeModuleState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeRequestChangeSubModuleState() {
        this.requestChangeSubModuleStateSubscription = this.requestChangeSubModuleState.subscribe((requestChangeSubModuleState: any) => {
            this.appErrorHandler.executeAction(() => {
                this.requestChangeSubModule = requestChangeSubModuleState;

                if (requestChangeSubModuleState) {
                    let requestedModule = this.mainModules.find(m => m.idSettingsGUI == requestChangeSubModuleState.requestedModuleId);
                    if (requestedModule && this.activeModule.idSettingsGUI == requestedModule.idSettingsGUI) {
                        let requestedSubModule = this.subModules.find(m => m.idSettingsGUI == requestChangeSubModuleState.requestedSubModuleId);

                        if (!requestedSubModule && (requestedModule.idSettingsGUI == MenuModuleId.backoffice)) {
                            const logisticModules = this.subModules.find(m => m.idSettingsGUI == MenuModuleId.logistic);
                            if (logisticModules && logisticModules.children) {
                                requestedSubModule = logisticModules.children.find(m => m.idSettingsGUI == requestChangeSubModuleState.requestedSubModuleId);
                            }
                        }
    ...
    private okToChangeOrCreateNewModule() {
        if (this.willChangeModule) {
            ...
            if (this.selectedEntity && this.selectedEntity.hasOwnProperty('selectedParkedItem')) {
                this.store.dispatch(this.moduleActions.storeModuleStates(this.activeModule, this.selectedEntity['selectedParkedItem'], this.selectedTab, this.selectedAiTab));
            }
    ...
    private okToChangeSubModule() {
        if (this.willChangeSubModule) {
            ...
            if (this.activeModule.idSettingsGUI != MenuModuleId.administration) {
                let newRoute = this.activeModule.moduleNameTrim + '/' + this.willChangeSubModule.moduleNameTrim;
    ...
    public onClickNewModule(selectedModule: Module) {
        ...
        if (this.activeModule && this.activeModule.idSettingsGUI == MenuModuleId.backoffice
            && (!this.activeSubModule
                || this.activeSubModule.idSettingsGUI != selectedModule.idSettingsGUIParent)) {
            let subModule: Module = this.moduleService.getModuleRecursive(this.subModules, selectedModule.idSettingsGUIParent);
    ...
    private closeCurrentSubModule() {
        if (this.activeSubModule.idSettingsGUIParent === MenuModuleId.backoffice
    ...
    search($event) {
        if (this.searchText) {
            this.store.dispatch(this.moduleActions.searchKeywordModule(new Module({
    ...
    private getCurrentCheckedModules(data: GlobalSettingModel[]): any {
        let currentGlobalSettingModel = data.find(x => x.globalName === this.getSettingName());
