...
export class BreadcrumbMasterComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    @Input() set tabID(tabID: string) {
        ...
        if (this.tabIDLocal) {
            this.selectedCampaignNumberState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabIDLocal).selectedCampaignNumberData);
    ...
    constructor(
        ...
        this.selectedTabState = this.store.select(state => tabSummaryReducer.getTabSummaryState(state, this.ofModule.moduleNameTrim).selectedTab);
        this.moduleDisplayNameKeyState = this.store.select(state => moduleSettingReducer.getModuleSettingState(state, this.ofModule.moduleNameTrim).moduleDisplayNameKey);
        this.moduleDisplayNameFormatState = this.store.select(state => moduleSettingReducer.getModuleSettingState(state, this.ofModule.moduleNameTrim).moduleDisplayNameFormat);
    ...
    private subscribe(): void {
        this.selectedEntityStateSubcription = this.selectedEntityState.subscribe((selectedEntityState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (selectedEntityState) {
                    this.selectedEntity = selectedEntityState;
                    this.reupdateBreadcrumbItem(this.moduleDisplayNameKey, this.moduleDisplayNameFormat);
                } else {
        ...
        this.isViewModeStateSubscription = this.isViewModeState.subscribe((selectedEditModeState: any) => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.selectedTabStateSubcription = this.selectedTabState.subscribe((selectedTabState: TabSummaryModel) => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.moduleDisplayNameFormatStateSubcription = this.moduleDisplayNameFormatState.subscribe((moduleDisplayNameFormat: string) => {
            this.appErrorHandler.executeAction(() => {
                this.moduleDisplayNameFormat = moduleDisplayNameFormat;
            });
        });

        this.moduleDisplayNameKeyStateSubcription = this.moduleDisplayNameKeyState.subscribe((moduleDisplayNameKey: string) => {
            this.appErrorHandler.executeAction(() => {
                this.moduleDisplayNameKey = moduleDisplayNameKey;
                this.reupdateBreadcrumbItem(moduleDisplayNameKey, this.moduleDisplayNameFormat);
            });
        });
    }
    ...
    private reupdateBreadcrumbItem(moduleDisplayNameKey: string, moduleDisplayNameFormat: string): void {
        if (this.selectedEntity && moduleDisplayNameKey) {
            ...
            for (var i = 0; i < properties.length; i++) {
                if (properties[i]) propertyArray.push(this.selectedEntity[properties[i]]);
            }
            ...
            if (propertyArray.length > 0) {
                ...
                if (this.breadcrumbItem == null) {
                    this.breadcrumbItem = new BreadcrumbModel({
                        Name: moduleDisplayNameFormat ? String.Format.apply(this, propertyArray) : propertyArray.join(' '),
                        IsFirst: true
                    });
                } else {
                    this.breadcrumbItem.Name = moduleDisplayNameFormat ? String.Format.apply(this, propertyArray) : propertyArray.join(' ');
                }
                this.ref.markForCheck();
            }
        }
