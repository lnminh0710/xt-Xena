...
import { TabSummaryModel, GlobalSettingModel } from "app/models";
...
export class EditLayoutSettingComponent
    ...
    implements OnInit, OnDestroy
{
            ...
            .subscribe((data: any) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (globalSettingItem && globalSettingItem.jsonSettings) {
                        let moduleSetting = JSON.parse(
                            globalSettingItem.jsonSettings
                        );
                        if (moduleSetting.item) {
                            const jsonSettings = JSON.parse(
                                moduleSetting.item[0].jsonSettings
                            );
                            this.layoutSettingService.translateLayoutSetting2GoldenLayoutSetting(
                                jsonSettings,
                                editMode
                            );
    ...
    private loadGoldenLayout(tabID: any) {
        let init = () => {
            this.layoutObj =
                this.layoutSettingService.createLayoutWithLayoutContent(tabID);
    ...
    private initGoldenLayout() {
        ...
        this.initGoldenLayoutTimeout = setTimeout(() => {
            if (this.layoutObj && this.layoutObj.goldenLayout) {
                let cachedConfigContent =
                    this.layoutObj.goldenLayout.cachedConfigContent;
                let configContent =
                    cachedConfigContent && cachedConfigContent.length
                        ? cachedConfigContent
                        : this.layoutObj.goldenContent;
    ...
    public save($event) {
        ...
        let layoutSettingContent =
            this.layoutSettingService.convertGoldenLayoutContent2PageLayoutContent();
        ...
        this.globalSettingServiceSubscription = this.globalSettingService
            ...
            .subscribe((data: any) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    let customTabs =
                        moduleSetting.item[0].jsonSettings.Content.CustomTabs;
                    for (
                        let i = 0, length = customTabs.length;
                        i < length;
                        i++
                    ) {
                        ...
                        if (
                            tab &&
                            tab.TabID &&
                            tab.TabID == this.layoutObj.tabID
                        ) {
                            layoutSettingContent.TabID = this.layoutObj.tabID;
                            layoutSettingContent.TabName =
                                this.layoutObj.tabName;
                    ...
                    const updateJsonSettingsString = JSON.stringify(
                        moduleSetting.item[0].jsonSettings
                    );
                    if (originalJsonSettingsString == updateJsonSettingsString)
                        return;
                    ...
                    moduleSetting.item[0].jsonSettings =
                        updateJsonSettingsString;
                    globalSettingItem.jsonSettings =
                        JSON.stringify(moduleSetting);
                    globalSettingItem.idSettingsGUI =
                    ...
                    this.globalSettingServiceSubscription =
                        this.globalSettingService
                            .saveGlobalSetting(globalSettingItem)
    ...
    public restore($event) {
        ...
        if (this.layoutObj && this.layoutObj.goldenLayout) {
            ...
            const goldenContent =
                this.layoutSettingService.convertPageLayoutContent2GoldenLayoutContent(
                    xenaContent
                );
    ...
    public undo($event) {
        ...
        if (this.layoutObj && this.layoutObj.goldenLayout) {
            const configContent =
                this.layoutObj.goldenLayout.popCacheConfigContent();
            if (configContent)
                this.layoutObj.goldenLayout.initLayout(configContent);
        }
    }
