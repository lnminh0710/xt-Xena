...
import { AppErrorHandler, DatatableService, ModalService, UserModuleWorkspaceService } from 'app/services';
import { XnAgGridComponent } from 'app/shared/components/xn-control/xn-ag-grid/pages/ag-grid-container/xn-ag-grid.component';
import {BaseComponent, ModuleList} from 'app/pages/private/base';
import { Router } from '@angular/router';
import { MessageModel, Module } from 'app/models';
import {Configuration, MessageModal, ModuleName, ModuleIdSettingGuid} from 'app/app.constants';
...
export class DialogModuleLayoutSettingsComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    @ViewChild(DialogSaveWorkspaceTemplateComponent) set dialogSaveWorkspaceTemplateInstance(dialogSaveWorkspaceTemplateInstance: DialogSaveWorkspaceTemplateComponent) {
        this.dialogSaveWorkspaceTemplate = dialogSaveWorkspaceTemplateInstance;
    }

    private pDialogModuleLayoutSetting: any;
    @ViewChild('pDialogModuleLayoutSetting') set pDialogModuleLayoutSettingInstance(pDialogModuleLayoutSettingInstance: Dialog) {
    ...
    private getModuleList() {
        ...
        if (!this.isSelectionProject) {
            ...
            for (let i = 0; i < objectKeys.length; i++) {
                if (objectKeys[i] !== ModuleName.BackOffice
    ...
    private maximize() {
        ...
        if (this.pDialogModuleLayoutSetting) {
            this.preDialogW = this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.width;
            this.preDialogH = this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.height;
            this.preDialogLeft = this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.left;
            this.preDialogTop = this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.top;

            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.width = $(document).width() + 'px';
            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.height = $(document).height() + 'px';
            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.top = '0px';
            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.left = '0px';
        }
    }
    ...
    private restore() {
        ...
        if (this.pDialogModuleLayoutSetting) {
            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.width = this.preDialogW;
            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.height = this.preDialogH;
            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.top = this.preDialogTop;
            this.pDialogModuleLayoutSetting.containerViewChild.nativeElement.style.left = this.preDialogLeft;
        }
        // setTimeout(() => {
    ...
    public onModuleComboChanged() {
        const idSettingsGui = this.moduleCombo && this.moduleCombo.selectedItem && this.moduleCombo.selectedItem.idSettingsGUI;
    ...
    private getWorkspaceTemplate(idSettingsGui?: string) {
        ...
        this.userModuleWorkspaceService.getAllWorkspaceTemplates(resultIdSettingsGui).subscribe(response => {
            this.appErrorHandler.executeAction(() => {
                if (!Uti.isResquestSuccess(response)) {
                    return;
                }

                const dataSource = this.datatableService.buildEditableDataSource(response.item.data);

                const sortData: any[] = [];

                const systemTemplate = dataSource.data.find(item => item.IsSystem === true);
                if (systemTemplate) {
                    sortData.push(systemTemplate);
                }

                const defaultTemplate = dataSource.data.find(item => item.IsUserDefault === true);
                if (defaultTemplate) {
                    sortData.push(defaultTemplate);
                }

                for (let i = 0; i < dataSource.data.length; i++) {
                    if (systemTemplate && dataSource.data[i].IdWorkspaceTemplate == systemTemplate.IdWorkspaceTemplate) {
                        continue;
                    }

                    if (defaultTemplate && dataSource.data[i].IdWorkspaceTemplate == defaultTemplate.IdWorkspaceTemplate) {
                        continue;
                    }
    ...
    public applyTemplate() {
        const idSettingsGUI = this.moduleCombo && this.moduleCombo.selectedItem.idSettingsGUI;
        this.modalService.confirmMessageHtmlContent(new MessageModel({
            ...
            message: [{key: '<p>'}, {key: 'Modal_Message__Apply_Workspace_Template_Will_Reload_Page_Do_You_Want_To_Change'},
                {key: '</p>'}],
            buttonType1: MessageModal.ButtonType.warning,
            callBack1: () => {
                const data = {
                    ObjectNr: idSettingsGUI,
                    IdWorkspaceTemplate: this.workspaceGrid.getSelectedNode().data['IdWorkspaceTemplate']
                };
                if (idSettingsGUI === ModuleIdSettingGuid.AllModules) {
                    this.userModuleWorkspaceService.applyWorkspaceTemplateAll(data).subscribe(response => {
                        this.appErrorHandler.executeAction(() => {
                            if (!Uti.isResquestSuccess(response)) {
                                return;
                            }
                    ...
                    this.userModuleWorkspaceService.applyWorkspaceTemplate(data).subscribe(response => {
                        this.appErrorHandler.executeAction(() => {
                            if (!Uti.isResquestSuccess(response)) {
                                return;
                            }
    ...
    public updateTemplate() {
        const idSettingsGUI = this.moduleCombo && this.moduleCombo.selectedItem.idSettingsGUI;
        this.userModuleWorkspaceService.getWorkspaceTemplate(this.workspaceGrid.getSelectedNode().data.IdWorkspaceTemplate, idSettingsGUI).subscribe(response => {
            this.appErrorHandler.executeAction(() => {
                if (!Uti.isResquestSuccess(response)) {
                    return;
                }
    ...
    public onDeleteWorkspace(e) {
        ...
        if (e) {
            this.modalService.confirmMessageHtmlContent(new MessageModel({
                ...
                callBack1: () => {
                    this.userModuleWorkspaceService.deleteWorkspaceTemplate({
                        IdWorkspaceTemplate: e.IdWorkspaceTemplate,
                        IsDeleted: 1
                    }).subscribe(response => {
                        this.toasterService.pop('success', 'Success', `Workspace is deleted successfully`);
                        this.getWorkspaceTemplate(idSettingsGUI);
                    })
                }
            }));
    ...
    public saveDefaultTemplate() {
        ...
        this.userModuleWorkspaceService.saveDefaultWorkspaceTemplate(data)
            .subscribe((response) => {
                this.toasterService.pop('success', 'Success', `Default workspace is saved successfully`);
                this.getWorkspaceTemplate(idSettingsGUI);
            });
    }
