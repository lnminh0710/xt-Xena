  ...
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    this.updateStatusSubscription = BaseService.toggleSlimLoadingBar$.subscribe(
      (state) => {
  ...
  public collapsed(): void {
    ...
    this.store.dispatch(
      this.widgetTemplateActions.updateEditModeStatus(false, this.ofModule)
    );
    ...
    this.widgetTemplateServiceSubscription = this.widgetTemplateService
      ...
      .subscribe((response: ApiResultResponse) => {
        ...
        if (response.item.length && response.item[0].jsonSettings) {
          ...
          this.widgetToolbar =
            this.widgetTemplateService.hasVisibleWidgets(widgetToolbar);
