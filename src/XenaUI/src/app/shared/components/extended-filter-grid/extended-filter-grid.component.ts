...
export class ExtendedFilterGridComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
    ...
    @Input() set defaultMode(value: boolean) {
        ...
        if (!this._defaultMode) {
            ...
            this.config = {
                fields: {
                    extended: { name: 'Extended Filter', type: 'extended-filter', template: '' }
                }
            }
        }
    }
    ...
    private loadRulesByCountry(countryId, countryLanguageId) {
        return this.ruleService.getProjectRules(RuleEnum.ExtendedRules, this.selectedEntity[camelCase(this.widgetListenKey)], countryId)
    ...
    private loadTemplateRule() {
        this.globalSettingService.getAllGlobalSettings(-1).finally(() => {
            this.config = {
                fields: {
                    extended: { name: 'Extended Filter', type: 'extended-filter', template: this.template }
                }
            }
    ...
    private loadTemplateFromSettingSuccess(data: Array<any>) {
        if (data && data.length) {
            let setting = data.find(p => p.globalName == this.globalSettingConstant.extendedFilterTemplate);
            if (setting) {
    ...
    private getComboBoxForRuleBuilder() {
        this.getComboBoxForRuleBuilderSubscription = this.ruleService.getComboBoxForRuleBuilder(QueryObject.ExtendRules)
    ...
    private loadFilterFromSeletedCountry(forceLoadFromService?: boolean) {
        if (this.countryGrid && this.countryGrid.selectedNode) {
            ...
            // load data from service when data is empty
                ...
                this.ruleServiceSubscription = this.ruleService.getProjectRules(RuleEnum.ExtendedRules, this.selectedEntity[camelCase(this.widgetListenKey)], countryId)
                    .subscribe((response: ApiResultResponse) => {
                        this.appErrorHandler.executeAction(() => {
    ...
    public deleteRule(isForAllCountry?: boolean) {
        this.modalService.confirmMessageHtmlContent(new MessageModel({
            ...
            message: [{key: '<p>'}, {key: 'Modal_Message__Do_You_Want_To_Delete_Selected_Rule'},
                {key: '</p>'}],
    ...
    private onDeleteRule(deletedItem, isForAllCountry?: boolean) {
        if (this.filterGridData) {
            let willDeletedItemIndex = this.filterGrid.getCurrentNodeItems().findIndex(dt => dt['DT_RowId'] == deletedItem['DT_RowId'] || dt[ID_STRING.ID_SELECTION_PROJECT_RULES] == deletedItem[ID_STRING.ID_SELECTION_PROJECT_RULES]);
    ...
    private createNewRuleItem() {
        return {
            DT_RowId: GuidHelper.generateGUID(),
            IdSelectionProjectCountry: this.countryGrid.selectedNode.data[ID_STRING.COUNTRY_ID],
            IdCountrylanguage: this.countryGrid.selectedNode.data[ID_STRING.COUNTRY_LANGUAGE_ID],
    ...
    private processNewRule(isForAllCountry: boolean) {
        ...
        if (isForAllCountry) {
            ...
            if (!this.cacheData[this.countryGrid.selectedNode.data[ID_STRING.COUNTRY_LANGUAGE_ID]]) {
                this.cacheData[this.countryGrid.selectedNode.data[ID_STRING.COUNTRY_LANGUAGE_ID]] = {
                    gridData: {
                        data: [newData[0]],
                        columns: this.initFilterGridSetting()
                    },
                ...
                this.cacheData[this.countryGrid.selectedNode.data[ID_STRING.COUNTRY_LANGUAGE_ID]].isDirty = true;
                this.cacheData[this.countryGrid.selectedNode.data[ID_STRING.COUNTRY_LANGUAGE_ID]].gridData.data.push(newData[0]);
            }
        }
