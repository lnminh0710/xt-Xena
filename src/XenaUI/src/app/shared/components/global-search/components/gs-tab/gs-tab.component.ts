...
export class GlobalSearchTabComponent implements OnInit, OnDestroy {
    ...
    private searchItemClassNameSmall: string =
        this.searchItemClassName + "--sm";
    ...
    private searchTextClassNameSmall: string =
        this.searchTextClassName + "--sm";
    ...
    private subscribeModuleToPersonTypeState() {
        this.moduleToPersonTypeStateSubcription =
            this.moduleToPersonTypeState.subscribe(
    ...
    private setAllowDrag(currentTab) {
        ...
        if (
            ...
            currentTab.module.idSettingsGUI ===
    ...
    private getAllModulesSuccess(data: ApiResultResponse) {
        ...
        if (
            ...
            this.globalSearchModuleModels.length
        ) {
        ...
        if (currentModule) {
            this.currentTab.module = new Module(
                Object.assign({}, currentModule)
            );
        }
        ...
        for (let i = 0; i < this.globalSearchModuleModels.length; i++) {
            ...
            for (let s of [
                ...
                for (let j = 0; j < item.children.length; j++) {
                    item.children[j].gridId =
                        GridId[s + "ModuleGlobalSearch"][j];
                }
            }
            ...
            if (adminModule.length > 0) {
                ...
                this.getSearchSummary(
                    ...
                    this.appErrorHandler.executeAction(() => {
                        ...
                        adminModule[0].isSearchEmpty =
                            adminModule[0].searchResult === 0;
                        this.onSearchResultCompleted.emit();
                        this.changeDetectorRef();
                    });
                });
            ...
            .map((indexSearchSummaries) => {
                ...
                for (const indexSearchSummary of indexSearchSummaries.item) {
                    ...
                    for (let child of rs[0].children) {
                        const currentChildResult =
                            indexSearchSummaries.item.find(
            ...
            for (let menuItem of menuContext) {
                if (menuItem !== "string") {
                    if (menuItem.key === "AddToParkedItems")
                        menuItem.cssClasses = !hasNotParkedItem
                            ? ["xn-disabled"]
                            : [""];
                ...
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (
                        ...
                        this.globalSettingItem = new GlobalSettingModel({
                            ...
                            globalType:
                                this._globalSettingConstant
                                    .gsHistoryTabSearching,
                            idSettingsGUI: "-1",
                            isActive: true,
                            objectNr: "-1",
                        });
