import { Component, OnInit, OnDestroy, Output, EventEmitter, ViewChild, Input, ChangeDetectorRef, ChangeDetectionStrategy } from '@angular/core';
import {
    GlobalSearchConstant, MenuModuleId,
    Configuration, LocalStorageKey,
    GridId, GlobalSettingConstant
...
export class GlobalSearchTabComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.moduleToPersonTypeState = this.store.select(state => commonReducer.getCommonState(state, this.ofModule ? this.ofModule.moduleNameTrim : '').moduleToPersonType);
        this.that = this;
    }
    ...
    public ngOnInit() {
        ...
        if (this.ofModule) {
            ...
            this.parkedItemsState = this.store.select(state => parkedItemReducer.getParkedItemState(state, this.ofModule.moduleNameTrim).parkedItems);
            this.xnContextMenuState = this.store.select(state => commonReducer.getCommonState(state, this.ofModule.moduleNameTrim).contextMenuData);
    ...
    private subscribeModuleToPersonTypeState() {
        this.moduleToPersonTypeStateSubcription = this.moduleToPersonTypeState.subscribe((moduleToPersonTypeState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private setAllowDrag(currentTab) {
        ...
        if (childModule && this.activeModule.idSettingsGUI === childModule.idSettingsGUIParent) {
            ...
            || (this.activeSubModule && currentTab.module.idSettingsGUI === this.activeSubModule.idSettingsGUI)) {
            return true;
        }
    ...
    private addToParkedItem(data) {
        if (data) {
            if (Uti.isSearchUrl())
                this.storeMenuContextAction({ action: 'AddToParkedItem', data: data });
            else {
                let module = this.ofModule;
                if (module.idSettingsGUIParent && module.idSettingsGUIParent == ModuleList.Administration.idSettingsGUI) {
                    module = ModuleList.Administration;
                }

                this.store.dispatch(this.processDataActions.requestAddToParkedItems(data, module));
            }
        }
    }
    ...
    private addToDoublet(data) {
        if (data) {
            if (Uti.isSearchUrl())
                this.storeMenuContextAction({ action: 'AddToDoublet', data: data });
            else
                this.store.dispatch(this.processDataActions.requestAddToDoublet(data, this.ofModule));
        }
    }
    ...
    private removeFromParkedItem(data) {
        if (data) {
            if (Uti.isSearchUrl())
                this.storeMenuContextAction({ action: 'RemoveFromParkedItem', data: data });
            else
                this.store.dispatch(this.processDataActions.requestRemoveFromParkedItems(data, this.ofModule));
        }
    }

    private getAllModule() {
        this.globalServ.getAllSearchModules()
            .subscribe(
    ...
    private getAllModulesSuccess(data: ApiResultResponse) {
        ...
        const currentModule = this.globalServ.getCurrentModule(this.globalSearchModuleModels, this.globalSearchModuleModelsAdminChildren, this.currentTab);
    ...
    private makeSubDataForMainModule() {
        ...
        for (let i = 0; i < this.globalSearchModuleModels.length; i++) {
            ...
            if ((//item.idSettingsGUI != MenuModuleId.selectionCampaign &&
            ...
            for (let s of ['SelectionCampaign', 'SelectionBroker', 'SelectionCollect']) {
    ...
    private makeClientDataForAllModules() {
        if (this.globalSearchModuleModels.length <= 0) { return; }
        // Make for master
        ...
        if (adminChildrenModules.length <= 0) { return; }
        // Make for detail
    ...
    private addGridIdForSubAdminModule() {
        for (let i = 0; i < this.globalSearchModuleModelsAdminChildren.length; i++) {
            this.globalSearchModuleModelsAdminChildren[i].gridId = GridId.SubAdminModuleGlobalSearch[i];
        }
    }

    private getAdminChildrenModule(): GlobalSearchModuleModel[] {
        const moduleAdministration = this.globalServ.getModuleByName(this.globalSearchModuleModels, this.globalSearchConsts.searchAdministration);
        if (!moduleAdministration || !moduleAdministration.children || moduleAdministration.children.length <= 0) { return []; }
    ...
    public search(value: string) {
        ...
        if (!this.currentTab.textSearch) {
            this.globalServ.setSearchResultForModule(
                this.globalSearchModuleModels, null, this.searchItemBgClassName + '  ' + this.searchItemClassName);
            this.globalServ.setSearchResultForModule(
                this.globalSearchModuleModelsAdminChildren, null, this.searchItemBgClassName + '  ' + this.searchItemClassNameSmall);
    ...
    private searchOtherGroup() {
        ...
        this.searchOtherGroupTimeout = setTimeout(() => {
            const indexSearchKeys = this.globalSearchModuleModels.map(p => p.searchIndexKey).filter(p => p).join(',');
            this.getSearchSummary(indexSearchKeys, this.currentTab.textSearch, this.globalSearchModuleModels).subscribe(rs => {
                this.appErrorHandler.executeAction(() => {
                    this.onSearchResultCompleted.emit();
                    this.changeDetectorRef();
                });
    ...
    public globalSearchItemClicked($event: any) {
        ...
        if (//$event.idSettingsGUI == MenuModuleId.selectionCampaign ||
    ...
    private makeClientDataForChildrenModules(parentModule: any) {
        const globalSearchModuleModelChildren = parentModule ? parentModule.children : {};
    ...
    private searchForAdminstration() {
        ...
        this.searchForAdminstrationTimeout = setTimeout(() => {
            ...
            if (adminModule.length > 0) {
                ...
                this.getSearchSummary(indexSearchKeys, this.currentTab.textSearch, this.globalSearchModuleModelsAdminChildren).subscribe(rs => {
                    this.appErrorHandler.executeAction(() => {
    ...
    private getSearchSummary(idexSearchKeys: string, textSearch: string, searchModuleModels: Array<GlobalSearchModuleModel>) {
        return this.globalServ.getSearchSummary(textSearch, idexSearchKeys, this.isWithStar).map(indexSearchSummaries => {
            let count = 0;
            for (const indexSearchSummary of indexSearchSummaries.item) {
                count += indexSearchSummary.count;
                const rs = searchModuleModels.filter(p => p.searchIndexKey === indexSearchSummary.key);
                ...
                // Used for Selection modules
                if ((//rs[0].idSettingsGUI != MenuModuleId.selectionCampaign &&
                    rs[0].idSettingsGUI != MenuModuleId.selectionBroker &&
                    rs[0].idSettingsGUI != MenuModuleId.selectionCollect)
                    || !rs[0].children || !rs[0].children.length) {
                    continue;
                }
                // Set search result for children Item
                for (let child of rs[0].children) {
                    const currentChildResult = indexSearchSummaries.item.find(x => x.key === child.searchIndexKey);
    ...
    public searchEachItem(keyWord: string, model: GlobalSearchModuleModel) {
        this.globalServ.getSearchSummary(keyWord, model.searchIndexKey, this.isWithStar)
    ...
    private getSearchSummarySuccess(data: Array<any>, model: GlobalSearchModuleModel) {
        ...
        model.controlClassName = this.searchItemBgClassNameSearched + '  ' +
    ...
    public onTableRowClicked(data) {
        ...
        if (this.activeModule.idSettingsGUI == MenuModuleId.administration && data.idRepPersonType) {
        ...
        const currentModule = this.globalServ.getCurrentModule(
            this.globalSearchModuleModels, this.globalSearchModuleModelsAdminChildren, this.currentTab);
        ...
        if (status) {
            ...
            setTimeout(() => {
                if (this.activeModule.idSettingsGUI == MenuModuleId.administration && this.currentTab.moduleID) {
        ...
        this.store.dispatch(this.globalSearchActions.rowDoubleClick({
            data,
            selectedModule
        }));
    }
    ...
    private createGridMenuContextData(data: any) {
        ...
        if (this.accessRight.read) {
            result.push({
                ...
                action: (event) => { this.addToParkedItem(data); },
                cssClasses: [''],
            ...
            result.push({
                ...
                action: (event) => { this.removeFromParkedItem(data); },
                cssClasses: [''],
        ...
        if (this.currentTab.searchIndex.toLowerCase() === 'customer') {
            result.push({
                name: 'Add to doublet',
                action: (event) => { this.addToDoublet(data); },
                cssClasses: [''],
    ...
    private getGlobalAndSaveData(addNewItem?: any) {
        this._globalSettingService.getAllGlobalSettings('-1').subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
                let globalSettingName = String.Format('{0}_{1}',
