...
export class XnTabHeaderSubFormComponent
    ...
    implements OnInit, OnDestroy
{
    ...
    constructor(
        ...
        this.formEditTextDataSubTabState = store.select(
            (state) =>
        ...
        this.formEditDataActiveSubTabState = store.select(
            (state) =>
                tabSummaryReducer.getTabSummaryState(
                    state,
                    this.ofModule.moduleNameTrim
    ...
    private appendDefaultValueToTranslateResource() {
        defaultLanguage["XnTabHeaderSubForm__" + this.textData.main.title] =
            this.textData.main.title;
        defaultLanguage["XnTabHeaderSubForm__" + this.textData.detail.title] =
            this.textData.detail.title;
    }

    private subScribeFormEditTextDataSubTab() {
        this.formEditTextDataSubTabStateSubscription =
            this.formEditTextDataSubTabState.subscribe(
                (formEditTextDataSubTab: any) => {
                    this.appErrorHandler.executeAction(() => {
                        if (!formEditTextDataSubTab) return;
                        this.textData.main.title = formEditTextDataSubTab.main;
                        this.textData.detail.title =
                            formEditTextDataSubTab.detail;
                        this.textData.main.toolbar = this.setting
                            ? this.setting.Content.CustomTabs[0].Toolbar
                            : null;
                        this.textData.detail.toolbar = this.setting
                            ? this.setting.Content.CustomTabs[1].Toolbar
                            : null;
                    });
                }
            );
    ...
    private subScribeEditFormTabActive() {
        this.formEditDataActiveSubTabStateSubscription =
            this.formEditDataActiveSubTabState.subscribe(
                (formEditActiveSubTab: any) => {
                    this.appErrorHandler.executeAction(() => {
                        if (!formEditActiveSubTab) return;

                        this.isMainActive = formEditActiveSubTab.isMainActive;

                        if (this.isMainActive) {
                            this.store.dispatch(
                                this.tabSummaryActions.selectSubTab(
                                    this.textData.main,
                                    this.ofModule
                                )
                            );
    ...
    public onClickMainStep(tab: any) {
        if (this.currentAction === "NEW_MAIN_TAB") return;
        this.store.dispatch(
            this.tabSummaryActions.setFormEditSubTab1Click(this.ofModule)
        );
        this.store.dispatch(
            this.tabSummaryActions.selectSubTab(tab, this.ofModule)
        );
    ...
    public onClickDetailStep(tab: any) {
        if (this.currentAction === "NEW_MAIN_TAB") return;
        this.store.dispatch(
            this.tabSummaryActions.setFormEditSubTab2Click(this.ofModule)
        );
        this.store.dispatch(
            this.tabSummaryActions.selectSubTab(tab, this.ofModule)
        );
