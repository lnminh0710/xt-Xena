  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  private subcribeIsTabCollapsedState() {
    this.isTabCollapsedStateSubscription = this.isTabCollapsedState.subscribe(
  ...
  private subscribeRequestSelectTabState() {
    this.requestSelectTabStateSubscription =
      this.requestSelectTabState.subscribe((requestSelectTabState: any) => {
        this.appErrorHandler.executeAction(() => {
            ...
            if (
              ...
              if (otherTab && otherTab.visible && !otherTab.disabled) {
                ...
                if (this.setting.Content) {
                  let tabSetting = this.setting.Content.CustomTabs.find(
                    (t) =>
                      t.TabID == this.selectedTabHeader.tabSummaryInfor.tabID
                  );
                  this.store.dispatch(
                    this.processDataActions.requestChangeTab(
                      tabSetting,
                ...
                this.store.dispatch(
                  this.tabSummaryActions.tabChangedFailed(this.ofModule)
                );
              }
            }
          }
  ...
  private processChangeTab() {
    ...
    this.store.dispatch(
      this.tabSummaryActions.selectTab(this.willChangeTab.tab, this.ofModule)
    );
    this.store.dispatch(
    ...
    for (const tab of otherTabsHeader) {
      for (const tabId of invisibleTabIdList) {
        if (tab.tabSummaryInfor.tabID == tabId) {
          ...
          if (tab.active) {
            this.store.dispatch(
              this.tabSummaryActions.selectTab(mainTabHeader, this.ofModule)
            );
          }
        }
      }
  ...
  private processDisabledTab() {
    setTimeout(() => {
      ...
      if (
        ((this.selectedParkedItem && this.selectedParkedItem.isNew != true) ||
  ...
  private numberOfHiddenItems() {
    ...
    return (
      parseInt($('.tab-summary-list').get(0).style.left) / this.widthOfItem()
    );
  }
  ...
  private adjustScrollingArea() {
    ...
    if (
      ...
      this.store.dispatch(
        this.tabButtonActions.tabHeaderHasScroller('right', true, this.ofModule)
      );
    } else {
  ...
  public scrollerRightClick(event) {
    ...
    setTimeout(() => {
      if (this.getLeftPos() < 0) {
        if (
          $('.tab-summary-wrapper', this.elmRef.nativeElement).outerWidth() +
            $('.tab-summary-list', this.elmRef.nativeElement).position().left *
              -1 >
          this.widthOfList()
        ) {
          $('.scroller-right', this.elmRef.nativeElement).addClass(
  ...
  private onTabIconClick(otherTabHeader, sumData) {
    ...
    this.store.dispatch(
      this.tabSummaryActions.uncheckTabHeaderTableFilterList(this.ofModule)
    );
  }
  ...
  private isConnectedToMainItem(srcWidgetDetail: WidgetDetail) {
    ...
    do {
      ...
      if (srcWidgetDetail.widgetDataType.listenKey.main) {
        isConnected = true;
      }
      break;
    } while (true);
          ...
          if (relatingWidget) {
            if (relatingWidget.mode == 'hover') {
              ...
              if (this.linkableWidget) {
                this.isConnectedToMain = this.isConnectedToMainItem(
                  relatingWidget.scrWidgetDetail
                );
              }
            } else {
  ...
  //     for (let otherTabHeader of this.otherTabsHeader) {
  //         defaultLanguage['Simple_Tab__' + otherTabHeader.tabSummaryInfor.tabName] = otherTabHeader.tabSummaryInfor.tabName;
  //     }
  //     defaultLanguage['Simple_Tab__' + this.mainTabHeader.tabSummaryInfor.tabName] = this.mainTabHeader.tabSummaryInfor.tabName;
  // }
}
