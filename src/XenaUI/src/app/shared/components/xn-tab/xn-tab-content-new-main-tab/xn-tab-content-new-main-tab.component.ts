...
export class XnTabContentNewMainTabComponent
        ...
        this.activeSubModuleModelSubscription =
            this.activeSubModuleModel.subscribe((activeSubModule: Module) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (isEmpty(activeSubModule)) {
                        return;
                    }
                    this.administrtionConfig = {
                        principal:
                            activeSubModule.idSettingsGUI ===
                            MenuModuleId.mandant,
                    };
                });
            });

        this.moduleStateSubscription = this.moduleModel.subscribe(
            (moduleState: ModuleState) => {
                this.appErrorHandler.executeAction(() => {
                    if (
                        moduleState.activeModule &&
                        !isEmpty(moduleState.activeModule)
                    ) {
                        this.searchIndexKey =
                            moduleState.activeModule.searchIndexKey;
                    }
                    if (
                        moduleState.activeSubModule &&
                        !isEmpty(moduleState.activeSubModule)
                    ) {
                        this.searchIndexKey =
                            moduleState.activeSubModule.searchIndexKey;
                    }
                });
            }
        );
