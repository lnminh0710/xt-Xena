        ...
        this.appErrorHandler.executeAction(() => {
          this.tabContainerStyle = {
            ...
            height: `calc(100vh - ${layoutInfo.headerHeight}px - ${
              ...
              layoutInfo.makeSpaceForTabButton.isActive ? ' - 25px' : ''
            } )`,
          };
        });
      }
        ...
        this.appErrorHandler.executeAction(() => {
          ...
          setTimeout(() => {
            if (
              ...
              if (this.contextMenuData[0])
                this.contextMenuData[0].title =
                  'New ' + this.activeSubModule.moduleName;

              if (this.contextMenuData[1])
                this.contextMenuData[1].title =
                  'Edit ' + this.activeSubModule.moduleName;
            }
          }, 1000);
        });
      }
  ...
  private dispatchData(data, isMainForm) {
    ...
    if (data) {
      ...
      if (typeof data.isValid === 'boolean') {
        this.store.dispatch(
          this.processDataActions.formValid(data.isValid, this.ofModule)
        );
      }

      if (typeof data.isDirty === 'boolean') {
        this.store.dispatch(
          this.processDataActions.formDirty(data.isDirty, this.ofModule)
        );
      }
      ...
      if (!isNil(data.submitResult)) {
        if (
          ...
          isMainForm
            ? this.store.dispatch(
                this.processDataActions.saveMainTabResult(data, this.ofModule)
              )
            : this.store.dispatch(
                this.processDataActions.saveOtherTabResult(data, this.ofModule)
              );
        }
      }
    }
  ...
  private initContextMenu() {
    this.contextMenuData = [
      ...
      },
      {
        ...
        callback: (event) => {
          this.store.dispatch(
            this.tabButtonActions.requestAddAsNewAndEdit(this.ofModule)
          );
        },
        subject: new Subject(),
        disabled: false,
