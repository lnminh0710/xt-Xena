...
export class XnTabComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.formValidState = store.select(state => processDataReducer.getProcessDataState(state, this.ofModule.moduleNameTrim).formValid);
    ...
    private subscribeActiveSubModuleState() {
        this.activeSubModuleStateSubscription = this.activeSubModuleState.subscribe((activeSubModuleState: Module) => {
            this.appErrorHandler.executeAction(() => {
                ...
                setTimeout(() => {
                    if (!isEmpty(this.activeSubModule)
                        && this.activeSubModule.idSettingsGUIParent === MenuModuleId.administration
                        && this.contextMenuData.length) {
                        if (this.contextMenuData[0])
    ...
    private dispatchData(data, isMainForm) {
        this.store.dispatch(this.processDataActions.formValue(data, this.ofModule));
        if (data) {
            ...
            if (!isNil(data.submitResult)) {
                if (this.formEditMode && !isUndefined(data.savingMode) && data.savingMode === RequestSavingMode.SaveOnly) {
                    isMainForm
    ...
    private initContextMenu() {
        this.contextMenuData = [
            ...
            },
            {
                ...
                iconName: 'fa-plus  green-color',
                callback: (event) => {
                    this.store.dispatch(this.tabButtonActions.requestAddAsNewAndEdit(this.ofModule));
                },
                subject: new Subject(),
                disabled: false,
                children: [],
