import {
    Component, OnInit, OnDestroy, AfterViewInit, trigger, transition, style, animate,
...
import {
    ...
    BaseService,
    DataEntryProcess,
    AccessRightsService, EventEmitterService, LayoutSettingService, ParkedItemProcess
} from 'app/services';
...
import {
    ...
    CustomAction,
    LayoutInfoActions, LayoutSettingActions
...
export class XnTabButtonComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    public odeButtonsAccessRight: { save: boolean, skip: boolean, download: boolean, sendToAdmin: boolean, print: boolean ,delete:boolean} = {
        save: false,
        skip: false,
        download: false,
        sendToAdmin: false,
    ...
    @ViewChild('btnOrderDataEntrySaveCtrl') btnOrderDataEntrySaveCtrl: ElementRef;//MatButton;
    ...
    @ViewChild(SelectionExportDataDialogComponent) set selectionExportDataDialogInstance(selectionExportDataDialogInstance: SelectionExportDataDialogComponent) {
    ...
    onKeyUp(event) {
        ...
        if (e.keyCode == 34) {
            if (this.btnSaveAllWidgetCtrl && !this.btnSaveAllWidgetCtrl['disabled']) {
                ...
                this.btnOrderDataEntrySaveCtrl && this.btnOrderDataEntrySaveCtrl.nativeElement) {
    ...
    constructor(
        ...
        this.showTabButtonState = store.select(state => tabSummaryReducer.getTabSummaryState(state, this.ofModule.moduleNameTrim).showTabButton);
        ...
        this.activeWidgetState = this.store.select(state => widgetContentReducer.getWidgetContentDetailState(state, this.ofModule.moduleNameTrim).activeWidget);
        ...
        this.formOutputDataState = store.select(state => processDataReducer.getProcessDataState(state, this.ofModule.moduleNameTrim).formOutputData);
    ...
    ngOnInit() {
        ...
        this.eventEmitterService.invokeFirstComponentFunction.subscribe((message) => {
    ...
    private listenGlobalpropertiesChange() {
        this.globalPropertiesStateSubscription = this.globalPropertiesState.subscribe((globalproperties: any) => {
            this.appErrorHandler.executeAction(() => {
                if (globalproperties) {
                    const propertyUnsavedDialog = this.propertyPanelService.getItemRecursive(globalproperties, 'UnsavedDialog');
        ...
        this.selectedODETabStateSubscription = this.selectedODETabState.subscribe((selectedODETabState: any) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (this.selectedODETab) {
                    ...
                    this.scanningStatusState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.selectedODETab.TabID).scanningStatusData);
    ...
    private subcribeSaveOtherTabResultState() {
        this.saveOtherTabResultStateSubscription = this.saveOtherTabResultState.subscribe((saveOtherTabResultState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeRequestChangeParkedItemState() {
        this.requestChangeParkedItemStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestRemoveTabState() {
        this.requestRemoveTabStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestCreateNewFromModuleDropdownState() {
        this.requestCreateNewFromModuleDropdownStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestCreateNewMainTabState() {
        this.requestCreateNewMainTabStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestCancelState() {
        this.requestCancelStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            ...
            this.appErrorHandler.executeAction(() => {
                if (requestCancelState.fromMediaCode) {
                    ...
                    this.store.dispatch(this.warehouseMovementActions.setSubForm(WarehouseMovementFormEnum.None, this.ofModule));
    ...
    private subcribeSetCommandDisableState() {
        this.setCommandDisableStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === TabButtonActions.SET_COMMAND_DISABLE && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).map((action: CustomAction) => {
    ...
    private subcribeRequestEditState() {
        this.requestEditStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestNewState() {
        this.requestNewStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestAddAsNewAndEditState() {
        this.requestNewStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === TabButtonActions.REQUEST_ADD_AS_NEW_AND_EDIT && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subcribeDblClickTabHeaderState() {
        this.dblClickTabHeaderStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestChangeModuleState() {
        this.requestChangeModuleStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestChangeSubModuleState() {
        this.requestChangeSubModuleStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subscribeCheckDirtyPropertiesWidget() {
        this.dispatcher.filter((action: CustomAction) => {
            return action.type === PropertyPanelActions.CHECK_DIRTY_PROPERTIES_OF_WIDGET;
    ...
    private subscribeRequestClearPropertiesSuccessState() {
        this.requestClearPropertiesSuccessStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subscribeRequestGoToFirstStepState() {
        this.requestGoToFirstStepStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subscribeRequestGoToSecondStepState() {
        this.requestGoToSecondStepStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subscribeRequestChangeBusinessCostRowState() {
        this.requestChangeBusinessCostRowStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeRequestChangeSearchResultState() {
        this.requestChangeSearchResultStateSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    private subcribeIsOrderDataEntrySaveDisabledState() {
        if (this.isOrderDataEntrySaveDisabledStateSubscription) this.isOrderDataEntrySaveDisabledStateSubscription.unsubscribe();

        this.isOrderDataEntrySaveDisabledStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === DataEntryActions.DATA_ENTRY_DISABLED_SAVE_BUTTON && this.selectedODETab && action.area == this.selectedODETab.TabID;
        }).map((action: CustomAction) => {
            return action.payload;
        }).subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeSaveOrderDataEntryResultState() {
        ...
        this.saveOrderDataEntryResultStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === DataEntryActions.DATA_ENTRY_SAVE_RESULT && this.selectedODETab && action.area == this.selectedODETab.TabID;
        }).map((action: CustomAction) => {
            return action.payload;
        }).subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (!isNil(data.returnID)) {
                    if (data.returnID && (typeof data.returnID == 'string' || typeof data.returnID == 'number')) {
                        switch (this.currentAction) {
    ...
    private subscribeRowsDataState() {
        this.rowsDataStateSubscription = this.rowsDataState.subscribe((rowsDataState: any[]) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeRequestSaveOnlyWithoutControllingTabState() {
        this.requestSaveOnlyWithoutControllingTabStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === TabButtonActions.REQUEST_SAVE_ONLY_WITHOUT_CONTROLLING_TAB && this.ofModule.idSettingsGUI == action.module.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                if (this.btnOrderDataEntrySaveCtrl && this.btnOrderDataEntrySaveCtrl.nativeElement)
                    this.btnOrderDataEntrySaveCtrl.nativeElement.click();
            });
        });
    }
    ...
    private subscribeActiveWidgetState() {
        this.activeWidgetStateSubscription = this.activeWidgetState.subscribe((activeWidgetState: LightWidgetDetail) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeFormOutputDataState() {
        this.formOutputDataStateSubscription = this.formOutputDataState.subscribe((formOutputData: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeShowTabButtonState() {
        this.showTabButtonStateSubscription = this.showTabButtonState.subscribe((showTabButtonState: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeContentDisplayModeState() {
        this.contentDisplayModeStateSubscription = this.contentDisplayModeState.subscribe((contentDisplayModeState: string) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private setButtonStateWhenFormOutputDataChanged() {
        if (this.subForm === WarehouseMovementFormEnum.SortingGood) {
            const confirmAllEnable = this.formOutputData && this.formOutputData.payload ? this.formOutputData.payload.isConfirmAllDirty : false;
    ...
    private subscribeConfirmAllSortingGoodsResult() {
        this.requestRemoveTabStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === WarehouseMovementActions.REQUEST_CONFIRM_ALL_SUCCESS || action.type === WarehouseMovementActions.REQUEST_SAVE_AND_CONFIRM_SUCCESS && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe((action: CustomAction) => {
            this.appErrorHandler.executeAction(() => {
                switch (this.currentAction) {
                    case TabButtonActionConst.WAREHOUSE_MOVEMENT_CONFIRM_ALL_SORTING_GOODS:
    ...
    public skipOrderDataEntry() {
        this.store.dispatch(this.dataEntryAction.dataEntryScanningStatusCallSkip(true, this.selectedODETab.TabID));
    }

    public reloadOrderDataEntry() {
        this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.RELOAD_ORDER_DATA_ENTRY, this.ofModule));

        if (this.isDirty()) {
            this.showDirtyWarningMessage();
        } else {
            this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.FIRST_LOAD, this.ofModule));
            this.store.dispatch(this.dataEntryAction.dataEntryCallReload(true, this.selectedODETab.TabID));
        }
    }

    public downloadOrderDataEntryScanningImage() {
        this.store.dispatch(this.dataEntryAction.requestDownloadScanningImage(this.selectedODETab.TabID));
    }
        ...
        setTimeout(() => {
            ...
            switch (this.currentAction) {
                ...
                case TabButtonActionConst.EDIT_SIMPLE_TAB:
                    this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.SAVE_AND_CLOSE_SIMPLE_TAB, this.ofModule));
                    break;
    ...
    public saveAndNext_Widget() {
        ...
        switch (this.selectedTab.tabSummaryInfor.tabID) {
            ...
            case 'Database':
                ...
                break;
            case 'Logic':
                this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.SAVE_AND_NEXT_STEP_LOGIC, this.ofModule));
                break;
            case 'Frequencies':
        ...
        this.store.dispatch(this.processDataActions.requestSaveWidget(this.ofModule, this.selectedTab.tabSummaryInfor.tabID));
    }
    ...
    public saveOnlyWithoutControllingTab() {
        setTimeout(() => {
            ...
            if (this.ofModule.idSettingsGUI == MenuModuleId.orderDataEntry) {
                switch (this.currentAction) {
                    case TabButtonActionConst.CHANGE_TAB:
                        this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.SAVE_ORDER_DATA_ENTRY_AND_CHANGE_TAB, this.ofModule));
                        break;

                    case TabButtonActionConst.REMOVE_TAB:
                        this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.SAVE_ORDER_DATA_ENTRY_AND_REMOVE_TAB, this.ofModule));
                        break;

                    case TabButtonActionConst.RELOAD_ORDER_DATA_ENTRY:
                        this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.SAVE_ORDER_DATA_ENTRY_AND_RELOAD, this.ofModule));
                        break;

                    default:
                        if (!this.isOrderDataEntrySaveDisabled || !this.isOrderDataEntrySaveDisabled.status || (this.selectedODETab && this.selectedODETab.TabID == 'Scanning' && !this.idScansContainerItems && this.isOrderDataEntrySaveDisabled.orderType != '2')) {
                            isValid = false;
                            this.toasterService.pop('warning', 'Validation Failed', 'Validation failed, please check again!');
                        }
                        else {
                            this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.SAVE_ORDER_DATA_ENTRY, this.ofModule));
                        }
                        break;
                }
            }
            ...
            if (isValid) {
                ...
                this.store.dispatch(this.processDataActions.requestSave(this.ofModule, RequestSavingMode.SaveOnly, tabId));
            }
        ...
        if (tabSetting) {
            for (let editingWidget of this.editingWidgets) {
                ...
                if (editingWidget.tabId == tabSetting.CurrentTabID) {
                    if (tabSetting.TabContent[this.contentDisplayMode].Split && tabSetting.TabContent[this.contentDisplayMode].Split.Items) {
                        let splitItem = tabSetting.TabContent[this.contentDisplayMode].Split.Items.find(si => si.Page && si.Page.PageId != pageId);
                        if (splitItem) {
                            return true;
                        }
                    }
                    else if (tabSetting.TabContent[this.contentDisplayMode].Page && tabSetting.TabContent[this.contentDisplayMode].Page.PageId != pageId) {
                        return true;
                    }
                }
            }//for
    ...
    private showDirtyWarningMessage() {
        ...
        if (this.ofModule.idSettingsGUI == MenuModuleId.orderDataEntry) {
            ...
            modalOptions.yesButtonDisabled = !this.isOrderDataEntrySaveDisabled || !this.isOrderDataEntrySaveDisabled.status || (this.selectedODETab && this.selectedODETab.TabID == OrderDataEntryTabEnum.Scanning && !this.idScansContainerItems && this.isOrderDataEntrySaveDisabled.orderType != '2');
    ...
    private onModalSaveAndExit() {
        switch (this.currentAction) {
            ...
            case TabButtonActionConst.BEFORE_CLOSE:
                if (this.ofModule.idSettingsGUI == MenuModuleId.orderDataEntry) {
                    if (this.isOrderDataEntrySaveDisabled && this.isOrderDataEntrySaveDisabled.status) {
    ...
    private processClose() {
        ...
        this.store.dispatch(this.processDataActions.clearGoToModuleItem(this.ofModule));
    ...
    private processEditMainTabSuccess() {
        const editedItem = this.parkedItems.find(i => i.id.value == this.editMainTabResultId);
    ...
    private subscribeRequestExportSelectionDataFromContextMenuState() {
        this.requestExportSelectionDataFromContextMenuStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ProcessDataActions.REQUEST_EXPORT_SELECTION_DATA_FROM_CONTEXT_MENU && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe((action: CustomAction) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeShowSendToAdminCompleteState() {
        this.showSendToAdminCompleteSubscription = this.dispatcher.filter((action: CustomAction) => {
    ...
    public confirmAllSortingGood(): void {
        this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.WAREHOUSE_MOVEMENT_CONFIRM_ALL_SORTING_GOODS, this.ofModule));
        this.store.dispatch(this.warehouseMovementActions.requestConfirmAll(this.ofModule));
    }

    public saveAndConfirmSortingGood(): void {
        this.store.dispatch(this.tabButtonActions.setCurrentAction(TabButtonActionConst.WAREHOUSE_MOVEMENT_CONFIRM_ALL_SORTING_GOODS, this.ofModule));
        this.store.dispatch(this.warehouseMovementActions.requestSaveAndConfirm(this.ofModule));
    }

    public editSortingGood(): void {
        this.store.dispatch(this.warehouseMovementActions.setSubForm(WarehouseMovementFormEnum.SortingGood, this.ofModule));
    ...
    public refreshWidgets() {
        this.store.dispatch(this.propertyPanelActions.clearPropertiesWhenRefreshBtn(this.ofModule));
    }
    ...
    public getTabButtonAccessRight() {
        ...
        if (this.ofModule.idSettingsGUI === ModuleList.OrderDataEntry.idSettingsGUI) {
            ...
            this.odeButtonsAccessRight = {
                save: true,
                download: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Download' }).read,
                skip: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Skip' }).read,
                sendToAdmin: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'SendToAdmin' }).read,
                delete :  this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Delete' }).read,
                print: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Print' }).read,
            };
        }

        tabId = this.selectedTab && this.selectedTab.tabSummaryInfor ? this.selectedTab.tabSummaryInfor.tabID : '';
        if (this.ofModule.idSettingsGUI == ModuleList.Orders.idSettingsGUI) {
            this.ordersButtonsAccessRight = {
                pdf: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Pdf' }).read,
                tracking: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Tracking' }).read,
                returnRefund: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'ReturnRefund' }).read,
            };
        }

        if (this.ofModule.idSettingsGUI == ModuleList.ReturnRefund.idSettingsGUI) {
            this.returnRefundButtonsAccessRight = {
                confirm: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Confirm' }).read,
                newInvoice: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'NewInvoice' }).read,
            };
        }

        if (this.ofModule.idSettingsGUI == ModuleList.SystemManagement.idSettingsGUI) {
            this.sysManageWidgetTabButtonsAccessRight = {
                clone: this.accessRightService.getAccessRight(AccessRightTypeEnum.TabButton, { idSettingsGUIParent: this.ofModule.idSettingsGUIParent, idSettingsGUI: this.ofModule.idSettingsGUI, tabID: tabId, buttonName: 'Clone' }).read,
            };
        }
    }
    ...
    public isEditBtnActive(): boolean {
        ...
        if (this.activeWidget && this.rowsData.length) {
            let selectedRowOfActiveWidget = this.rowsData.find(r => r.widgetDetail.id === this.activeWidget.id);
            if (!selectedRowOfActiveWidget
                || !selectedRowOfActiveWidget.widgetDetail
                || !selectedRowOfActiveWidget.widgetDetail['idRepWidgetApp']) {
                return false;
            }
            return (this.selectedTab.tabSummaryInfor['editWidgetAppId'] == selectedRowOfActiveWidget.widgetDetail['idRepWidgetApp']);
        }
    ...
    public syncMediacode() {
        this.modalService.confirmMessageHtmlContent(new MessageModel({
            ...
            message: [{ key: '<p>' }, { key: 'Modal_Message__Are_You_Sure_You_Want_To_Sync_Mediacode' },
            { key: '</p>' }],
            buttonType1: MessageModal.ButtonType.info,
            callBack1: () => {
                this.store.dispatch(this.processDataActions.requestSyncMediacode(this.ofModule));
            },
            callBack2: () => { },
            callBackCloseButton: () => { }
        }));
        ...
        setTimeout(() => {
            ...
            if (buttonContainerElm.length) {
                ...
                if (lastTabElm.length) {
                    let lastTabElmRightPos = lastTabElm.offset().left + lastTabElm.width();
                    let buttonContainerElmWidth = this.hitTabHeader ? buttonContainerElm.height() + 60 : buttonContainerElm.width();

                    if (window.innerWidth - lastTabElmRightPos <= buttonContainerElmWidth) {
                        this.hitTabHeader = true;
                        this.store.dispatch(this.layoutInfoActions.toggleMakeSpaceForTabButton(this.hitTabHeader, this.ofModule));
                        return;
                    }
                }
    ...
    public deleteOrderDataEntry(){
      this.modalService.confirmDeleteMessageHtmlContent(new MessageModel({
        headerText: 'Delete',
        messageType: MessageModal.MessageType.information,
        message: [{ key: '<p>' }, { key: 'Modal_Message__Are_You_Sure_You_Want_To_Delete' },
        { key: '</p>' }],
        buttonType1: MessageModal.ButtonType.info,
        callBack1: () => {
            this.store.dispatch(this.dataEntryAction.dataEntryScanningStatusCallDelete(true, this.selectedODETab.TabID));
        },
    }));
