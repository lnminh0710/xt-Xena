...
export class XnTabContentSimpleTabsComponent
    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    @Input() set data(data: any) {
        if (data["SimpleTabs"]) {
            ...
            if (this.simpleTabs.length) {
                ...
                const curActiveTab = this.simpleTabs.find(
                    (tab) => tab.Active == true
                );
                if (!curActiveTab) {
    ...
    ngAfterViewInit() {
        ...
        if (this.isSelectionProject) this.initXnTabPageViewChanges();
    }
    ...
    private initXnTabPageViewChanges() {
        ...
        this.xnTabPageViewSubscription =
            this.xnTabPageViewCtrls.changes.subscribe(() => {
                if (this.xnTabPageViewCtrls.length == this.simpleTabs.length) {
                    this.prcessForXnTabPageViewChanges();
                }
            });
    ...
    private processForActiveSimpleTab(tab?: SimpleTabModel) {
        ...
        const activeSimpleTab =
            tab || this.simpleTabs.find((item) => item.Active);
        this.xnTabPageViewCtrls.forEach((tabCtrl) => {
            tabCtrl.isActivated =
                activeSimpleTab.TabID == tabCtrl.tabPageView["TabID"];
        });
    }
                ...
                this.appErrorHandler.executeAction(() => {
                    if (layoutInfo) {
                        this.contentStyle = {
                            ...
                            height: `calc(100vh - ${layoutInfo.headerHeight}px - ${layoutInfo.tabHeaderHeight}px - ${layoutInfo.smallHeaderLineHeight}px - ${layoutInfo.dashboardPaddingTop}px - ${layoutInfo.simpleTabHeight}px)`,
                        };
                    }
                });
            }
    ...
    private subcribeRequestSelectSimpleTabState() {
        this.requestSelectSimpleTabStateSubscription = this.dispatcher
            .filter((action: CustomAction) => {
                return (
                    action.type ===
                        TabSummaryActions.REQUEST_SELECT_SIMPLE_TAB &&
                ...
                this.appErrorHandler.executeAction(() => {
                    if (!isNil(simpleTabID)) {
                        ...
                        if (typeof simpleTabID === "string") {
                            //get tab by ID
                            simpleTab = this.simpleTabs.find(
                                (tab) => tab.TabID == simpleTabID
                            );
    ...
    public selectTab(tab: SimpleTabModel) {
        ...
        this.store.dispatch(
            this.tabSummaryActions.selectSimpleTab(tab, this.ofModule)
        );
    ...
    private widthOfList() {
        let itemsWidth = 0;
        $(".simple-tab-summary-list li", this._eref.nativeElement).each(
            function () {
    ...
    public adjustScrollingArea() {
        if (
            !$(".simple-tab-summary-wrapper", this._eref.nativeElement).length
        ) {
            return;
        }
        ...
        if (this.getLeftPos() < 0) {
            ...
            $(".simple-tab-summary-list", this._eref.nativeElement).animate(
    ...
    private widthOfMainContainer() {
        return $(
            "xn-tab-content-simple-tabs div.simple-tab-header-container"
        ).width();
    }

    private maxScrollLeftItems() {
        return (
            $(".simple-tab-summary-list li", this._eref.nativeElement).length -
    ...
    private numberOfHiddenItems() {
        if (parseInt($(".simple-tab-summary-list").get(0).style.left) < 0) {
            return (
                (parseInt($(".simple-tab-summary-list").get(0).style.left) *
                    -1) /
                this.widthOfItem()
            );
        }
    ...
    public onWindowResize(event) {
        setTimeout(() => {
            if (
                $(".simple-scroller-left", this._eref.nativeElement).is(
                    ":visible"
                ) &&
