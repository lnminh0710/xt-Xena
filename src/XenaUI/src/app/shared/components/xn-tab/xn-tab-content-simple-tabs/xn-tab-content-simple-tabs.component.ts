  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  private subcribeRequestSelectSimpleTabState() {
    this.requestSelectSimpleTabStateSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === TabSummaryActions.REQUEST_SELECT_SIMPLE_TAB &&
  ...
  private getLeftPos() {
    return $('.simple-tab-summary-list', this._eref.nativeElement).position()
      .left;
  }
  ...
  public adjustScrollingArea() {
    ...
    if (
      $('.simple-tab-summary-wrapper', this._eref.nativeElement).outerWidth() <
      this.widthOfList()
    ) {
  ...
  private numberOfHiddenItems() {
    if (parseInt($('.simple-tab-summary-list').get(0).style.left) < 0) {
      return (
        (parseInt($('.simple-tab-summary-list').get(0).style.left) * -1) /
        this.widthOfItem()
      );
    }
      ...
      if (this.getLeftPos() < 0) {
        if (
          $(
            ...
            $('.simple-tab-summary-list', this._eref.nativeElement).position()
              .left *
              -1 >
          this.widthOfList()
        ) {
  ...
  public onWindowResize(event) {
    setTimeout(() => {
      if (
        $('.simple-scroller-left', this._eref.nativeElement).is(':visible') &&
