...
export class XnTabHeaderMenuComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    set data(data: any) {
        if (!this.tabHeaderTableFilterList.length) {
            ...
            this.store.dispatch(this.tabSummaryActions.storeTabHeaderTableFilterList(menuItems, this.ofModule));
        }
    }
    ...
    constructor(
        ...
        this.tabHeaderTableFilterListState = store.select(state => tabSummaryReducer.getTabSummaryState(state, this.ofModule.moduleNameTrim).tabHeaderTableFilterList);
    }
    ...
    private subscribeTabHeaderTableFilterListState() {
        this.tabHeaderTableFilterListSubscription = this.tabHeaderTableFilterListState.subscribe((tabHeaderTableFilterListState: Array<any>) => {
            this.appErrorHandler.executeAction(() => {
                if (!isEmpty(tabHeaderTableFilterListState) && !isEqual(this.tabHeaderTableFilterList, tabHeaderTableFilterListState)) {
                    this.tabHeaderTableFilterList = cloneDeep(tabHeaderTableFilterListState);
                }
            });
        });
    }
    ...
    apply() {
        const menuItems = this.tabService.getActiveMenuItems(this.tabHeaderTableFilterList);
        ...
        this.store.dispatch(this.tabSummaryActions.storeTabHeaderTableFilterList(this.tabHeaderTableFilterList, this.ofModule));
        this.toggled(false);
