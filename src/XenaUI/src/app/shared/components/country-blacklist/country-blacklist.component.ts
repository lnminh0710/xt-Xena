  ...
  implements OnInit, OnDestroy, AfterViewInit, OnChanges
{
        ...
        results.forEach((response) => {
          ...
          if (response.item && response.item.length && response.item[0][0]) {
            ...
            this.cacheData[response.countryId] =
              rulesFromService[ID_STRING.ID_SELECTION_PROJECT_RULES];
  ...
  private processApplyProfile(rules, selectingCountry) {
    this.countryBlackListItem[selectingCountry[ID_STRING.COUNTRY_LANGUAGE_ID]] =
    ...
    rules.forEach((rule) => {
      const rs = this.blackListGrid
        ...
        .find(
          (p) =>
            p[ID_STRING.ID_COLUMN_NAME] == rule.value[ID_STRING.ID_COLUMN_NAME]
        );
      if (rs) {
        rs.noExport = true;
        this.blackListGrid.updateRowData([rs]);
  ...
  onCheckboxExportChanged(item) {
    ...
    if (this.isForAllCountry) {
      this.countryGrid.getGridData().forEach((country) => {
        if (item.noExport) {
          if (
            !this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]]
          ) {
            this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]] =
      ...
      if (this.countryGrid.selectedNode) {
        ...
        if (item.noExport) {
          if (
            !this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]]
          ) {
            this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]] =
              [item[ID_STRING.ID_COLUMN_NAME]];
          }
  ...
  public onCountryGridEditEnded($event) {
    if (this.countryBlackListItem) {
      if (
        isNil(this.countryBlackListItem[$event[ID_STRING.COUNTRY_LANGUAGE_ID]])
      ) {
