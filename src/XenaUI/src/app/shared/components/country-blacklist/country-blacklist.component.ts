...
export class CountryBlacklistComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
    ...
    private initCountryGridData(callBack?: any) {
        this.countrySelectionServiceSubscription = this.countrySelectionService.getSelectionProjectCountry(this.selectedEntity[camelCase(this.widgetListenKey)], RuleEnum.Blacklist)
    ...
    private loadRulesByCountry(countryId, countryLanguageId) {
        return this.ruleService.getProjectRules(RuleEnum.Blacklist, this.selectedEntity[camelCase(this.widgetListenKey)], countryId)
    ...
    private preloadData() {
        ...
        Observable.forkJoin(observableBatch).finally(() => {
            ...
            results.forEach(response => {
                ...
                if (response.item && response.item.length && response.item[0][0]) {
                    let rulesFromService = response.item[0][0];
                    this.cacheData[response.countryId] = rulesFromService[ID_STRING.ID_SELECTION_PROJECT_RULES];
                    let rules = Uti.tryParseJson(response.item[0][0].JsonCaseRules).rules;
                    rules.forEach(rule => {
                        ...
                        this.updateBlackListItemForCountry(response.countryLanguageId, rule.value[ID_STRING.ID_COLUMN_NAME], true);
                    });
    ...
    private initBlackListGridData(callBack?: any) {
        this.blacklistServiceSubscription = this.ruleService.getBlackListRules()
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    if (!Uti.isResquestSuccess(response) || !response.item.length || isNil(response.item[1])) {
                        return;
    ...
    private loadBlackListFromSeletedCountry() {
        if (this.countryGrid && this.countryGrid.selectedNode) {
            ...
            if (this.profileData && this.profileData.length) {
                ...
                if (rules) {
                    if (this.isForAllCountry) {
                        ...
                        this.processApplyProfile(rules[0].rules, selectingCountry);
                    }
                }
            ...
            if (this.countryBlackListItem[countryLanguageId]) {
                let activeBlackListItems = this.countryBlackListItem[countryLanguageId];
                activeBlackListItems.forEach(id => {
                    const rs = this.blackListGrid.getCurrentNodeItems().find(p => p[ID_STRING.ID_COLUMN_NAME] == id);
                    if (rs) {
                        rs.noExport = true;
                        this.blackListGrid.updateRowData([rs]);
                    }
                ...
                this.ruleServiceSubscription = this.ruleService.getProjectRules(RuleEnum.Blacklist, this.selectedEntity[camelCase(this.widgetListenKey)], countryId)
                    .subscribe((response: ApiResultResponse) => {
                        this.appErrorHandler.executeAction(() => {
                            if (!response) {
                                return;
                            ...
                            if (response.item && response.item.length && response.item[0][0]) {
                                let rulesFromService = response.item[0][0];
                                this.cacheData[countryId] = rulesFromService[ID_STRING.ID_SELECTION_PROJECT_RULES];
                                let rules = Uti.tryParseJson(response.item[0][0].JsonCaseRules).rules;
                                rules.forEach(rule => {
                                    const rs = this.blackListGrid.getCurrentNodeItems().find(p => p[ID_STRING.ID_COLUMN_NAME] == rule.value[ID_STRING.ID_COLUMN_NAME]);
                                    if (rs) {
    ...
    private processApplyProfile(rules, selectingCountry) {
        this.countryBlackListItem[selectingCountry[ID_STRING.COUNTRY_LANGUAGE_ID]] = rules.map(rule => rule.value.IdRepPersonStatus);
     ...
     * @param value
     */
    private updateBlackListItemForCountry(countryLanguageId, blackListId, checked) {
        if (this.countryBlackListItem[countryLanguageId]) {
            const rs = this.countryBlackListItem[countryLanguageId].find(p => p == blackListId);
            if (!rs) {
                this.countryBlackListItem[countryLanguageId].push(blackListId);
            } else if (!checked) {
                this.countryBlackListItem[countryLanguageId] = this.countryBlackListItem[countryLanguageId].filter(x => x != blackListId);
            }
        }
        else {
    ...
    onCheckboxExportChanged(item) {
        ...
        if (this.isForAllCountry) {
            this.countryGrid.getGridData().forEach(country => {
                if (item.noExport) {
                    if (!this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]]) {
                        this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]] = [item[ID_STRING.ID_COLUMN_NAME]];
                    }

                    if (this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]].indexOf(item[ID_STRING.ID_COLUMN_NAME]) === -1) {
                        this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]].push(item[ID_STRING.ID_COLUMN_NAME])
                    }
                } else {
                    let idx = this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]].indexOf(item[ID_STRING.ID_COLUMN_NAME]);
                    if (idx !== -1) {
                        this.countryBlackListItem[country[ID_STRING.COUNTRY_LANGUAGE_ID]].splice(idx, 1);
                    }
                }
            })
        } else {
            if (this.countryGrid.selectedNode) {
                ...
                if (item.noExport) {
                    if (!this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]]) {
                        this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]] = [item[ID_STRING.ID_COLUMN_NAME]];
                    }

                    if (this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]].indexOf(item[ID_STRING.ID_COLUMN_NAME]) === -1) {
                        this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]].push(item[ID_STRING.ID_COLUMN_NAME])
                    }
                } else {
                    let idx = this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]].indexOf(item[ID_STRING.ID_COLUMN_NAME]);
                    if (idx !== -1) {
                        this.countryBlackListItem[dataItem[ID_STRING.COUNTRY_LANGUAGE_ID]].splice(idx, 1);
                    }
                }
            }
        }
    ...
    private checkAllCountriesBeforeSubmit() {
        if (this.isForAllCountry) {
            let selectingCountry = this.countryGrid.getSelectedNode().data;
            let mainData = this.countryBlackListItem[selectingCountry[ID_STRING.COUNTRY_LANGUAGE_ID]];
            Object.keys(this.countryBlackListItem).forEach(idCountryLanguage => {
    ...
    public createJsonDataForProfile(): Observable<any> {
        return this.ruleService.getProjectRulesForTemplate(RuleEnum.Blacklist, this.selectedEntity[camelCase(this.widgetListenKey)])
            .map((response: ApiResultResponse) => {
                let saveData = {
                    IdSelectionWidget: RuleEnum.Blacklist,
                    IdSelectionProject: this.selectedEntity[camelCase(this.widgetListenKey)],
                        ...
                        Rules: [
                            {
                                ...
                                rules: this.buildRuleForSave(this.countryBlackListItem[this.currentSelectedGridItem('countryGrid')[ID_STRING.COUNTRY_LANGUAGE_ID]])
                            }
                        ]
                    });
                }
        ...
        if (this.countryBlackListItem) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (this.countryBlackListItem[keys[i]].length || mediaCode) {
                    ...
                    let idSelectionProjectRules = this.buildIdSelectionProjectRules(countryId);
                    if (idSelectionProjectRules) {
                        result.push({
                            ...
                            Rules: [
                                {
                                    IsDeleted: 1,
                                    IdSelectionProjectRules: idSelectionProjectRules
                                }
                            ]
                        });
                    }
    ...
    private buildRulesForHiddenCountry(allRules, countryId) {
        ...
        for (let i = 0; i < allRules.length; i++) {
            if (allRules[i][ID_STRING.COUNTRY_ID] == countryId) {
                result.push({
                    IsActive: 1,
                    IdSelectionProjectRules: allRules[i][ID_STRING.ID_SELECTION_PROJECT_RULES],
    ...
    public onCountryGridEditEnded($event) {
        if (this.countryBlackListItem) {
            if (isNil(this.countryBlackListItem[$event[ID_STRING.COUNTRY_LANGUAGE_ID]])) {
                this.countryBlackListItem[$event[ID_STRING.COUNTRY_LANGUAGE_ID]] = [];
            }
    ...
    private validateBlacklistGrid() {
        setTimeout(() => {
            ...
            if (readOnly !== this.isBlacklistGridReadOnly || disabled !== this.isBlacklistGridDisabled) {
