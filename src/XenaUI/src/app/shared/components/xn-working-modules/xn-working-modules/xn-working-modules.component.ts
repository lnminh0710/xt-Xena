...
export class XnWorkingModulesComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        super(router);
        this.workingModulesState = store.select(state => state.mainModule.workingModules);
    ...
    ngOnDestroy() {
        $(this.elementRef.nativeElement).off('mouseover', 'a.module-caret-container', this.onModuleNameMouseover.bind(this));
        $(this.elementRef.nativeElement).off('mouseleave', 'a.module-caret-container', this.onModuleNameMouseleave.bind(this));
        this.wmContainer.nativeElement.removeEventListener('dragstart', this.wmContainerDragStart.bind(this), false);
        document.removeEventListener('dragover', this.documentDragOver.bind(this), false);
        document.removeEventListener('drop', this.documentDrop.bind(this), false);
        document.removeEventListener('dragleave', this.documentDragLeave.bind(this), false);
    ...
    ngAfterViewInit() {
        $(this.elementRef.nativeElement).on('mouseover', 'a.module-caret-container', this.onModuleNameMouseover.bind(this));
        $(this.elementRef.nativeElement).on('mouseleave', 'a.module-caret-container', this.onModuleNameMouseleave.bind(this));
        this.wmContainer.nativeElement.addEventListener('dragstart', this.wmContainerDragStart.bind(this), false);
        document.addEventListener('dragover', this.documentDragOver.bind(this), false);
        document.addEventListener('drop', this.documentDrop.bind(this), false);
        document.addEventListener('dragleave', this.documentDragLeave.bind(this), false);
    ...
    private setPositionFromSetting() {
        this.globalSettingService.getAllGlobalSettings().subscribe(
    ...
    private getAllGlobalSettingSuccess(data: GlobalSettingModel[]) {
        ...
        if (this.currentGlobalSettingModel && this.currentGlobalSettingModel.idSettingsGlobal) {
            const position = JSON.parse(this.currentGlobalSettingModel.jsonSettings).position;
            let screenSize = Uti.getTrueScreenSize();
            let top = position.top > screenSize.height ? screenSize.height : position.top < 0 ? 0 : position.top;
            let left = position.left > screenSize.width ? screenSize.width : position.left < 0 ? 0 : position.left;
    ...
    private savePositionToSetting(data: GlobalSettingModel[]) {
        ...
        this.currentGlobalSettingModel.jsonSettings = JSON.stringify({ position: this.getCurrentPosition() });
    ...
    private getCurrentPosition(): any {
        return $(this.elementRef.nativeElement).find('.working-modules-container').position();
    }
    ...
    private wmContainerDragStart(event) {
        this.ngZone.runOutsideAngular(() => {
            ...
            this.wmContainerOffset.left = parseInt(style.getPropertyValue("left"), 10) - event.clientX;
            this.wmContainerOffset.top = parseInt(style.getPropertyValue("top"), 10) - event.clientY;
            ...
            this.store.dispatch(this.moduleActions.toggleIsWorkingModulesDragging(true));
    ...
    private documentDragOver(event) {
        this.ngZone.runOutsideAngular(() => {
            ...
            let wmContainer = this.wmContainer.nativeElement;
            wmContainer.style.left = (event.clientX + this.wmContainerOffset.left) + 'px';
    ...
    private documentDrop(event) {
        this.ngZone.runOutsideAngular(() => {
            ...
            this.store.dispatch(this.moduleActions.toggleIsWorkingModulesDragging(false));
            let wmContainer = this.wmContainer.nativeElement,
            ...
            const nearLeft = mouseX >= 0 && mouseX < (1 / 4 * screenWidth),
                ...
                nearTop = mouseX >= (1 / 4 * screenWidth) && mouseX < (3 / 4 * screenWidth) && mouseY >= 0 && mouseY < (1 / 2 * screenHeight),
                nearBottom = mouseX >= (1 / 4 * screenWidth) && mouseX < (3 / 4 * screenWidth) && mouseY >= (1 / 2 * screenHeight) && mouseY <= screenHeight;
            if (nearLeft) {
                wmContainer.style.left = 0 + 'px';
                wmContainer.style.top = (mouseY > (screenHeight - 37) ? (screenHeight - 37) : mouseY) + 'px';
                ...
                wmContainer.style.left = (mouseX > (screenWidth - 37) ? (screenWidth - 37) : mouseX) + 'px';
    ...
    private setModuleNamePosition() {
        let $moduleName = $('.module-caret-container', this.elementRef.nativeElement);
        if ($moduleName.length) {
            for (let i = 0; i < $moduleName.length; i++) {
                ...
                switch (this.stickPosition) {
                    case 'top':
                        $($moduleName[i]).css({
                            'top': $moduleContainer.offset().top + $moduleContainer.outerHeight() + 'px',
                    ...
                    case 'left':
                        $($moduleName[i]).css({
                            ...
                            'left': $moduleContainer.offset().left + $moduleContainer.outerWidth() + 'px',
                            'right': 'auto',
                            'bottom': 'auto'
                        });
                        break;
    ...
    private subscribeWorkingModulesState() {
        this.workingModulesStateSubscription = this.workingModulesState.subscribe((workingModulesState: Array<any>) => {
            this.appErrorHandler.executeAction(() => {
    ...
    okToChangeModule() {
        if (this.willChangeWorkingModule && this.formDirty == false) {
            ...
            this.store.dispatch(this.moduleActions.storeModuleStates(this.ofModule, this.selectedParkedItem, this.selectedTab, this.selectedAiTab));

            this.store.dispatch(this.moduleActions.activeModule(this.willChangeWorkingModule));
    ...
    public selectWorkingModule(workingModule: Module) {
        ...
        if (workingModule.idSettingsGUIParent) {
            let mainModule: Module = this.mainModules.find(md => md.idSettingsGUI === workingModule.idSettingsGUIParent);
            ...
            setTimeout(() => {
                this.store.dispatch(this.moduleActions.requestChangeSubModule(mainModule.idSettingsGUI, workingModule.idSettingsGUI));
            }, 200);
        }
        else {
            //this.willChangeWorkingModule = workingModule;
            this.store.dispatch(this.moduleActions.requestChangeModule(workingModule));
        }
    ...
    private setUlPosition() {
        ...
        if ($ul.length && $blink.length) {
            ...
            if (($ul.innerHeight() + $blink.offset().top) > document.body.clientHeight) {
    ...
    public parkedItemsPanelClosed(willMoveToTopParkedItem: ParkedItemModel) {
        if (willMoveToTopParkedItem) {
            this.store.dispatch(this.moduleActions.moveSelectedParkedItemToTop(this.ofModule, willMoveToTopParkedItem));
        }
    }
}
