    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    private documentDragOver(event) {
        this.ngZone.runOutsideAngular(() => {
            ...
            wmContainer.style.top =
                event.clientY + this.wmContainerOffset.top + "px";
    ...
    private documentDrop(event) {
        this.ngZone.runOutsideAngular(() => {
            ...
            let wmContainer = this.wmContainer.nativeElement,
                mouseX =
                    $(wmContainer).offset().left + this.wmContainerOffset.left,
                mouseY =
                    $(wmContainer).offset().top + this.wmContainerOffset.top,
            ...
            mouseX =
                mouseX > screenWidth ? screenWidth : mouseX < 0 ? 0 : mouseX;
            mouseY =
                mouseY > screenHeight ? screenHeight : mouseY < 0 ? 0 : mouseY;
            ...
            const nearLeft = mouseX >= 0 && mouseX < (1 / 4) * screenWidth,
                nearRight =
                    mouseX >= (3 / 4) * screenWidth && mouseX <= screenWidth,
                nearTop =
            ...
            if (nearLeft) {
                ...
                wmContainer.style.top =
                    (mouseY > screenHeight - 37 ? screenHeight - 37 : mouseY) +
                    "px";
                ...
                wmContainer.style.left =
                    (mouseX > screenWidth - 37 ? screenWidth - 37 : mouseX) +
                    "px";
    ...
    private setModuleNamePosition() {
        ...
        if ($moduleName.length) {
            for (let i = 0; i < $moduleName.length; i++) {
                let $moduleContainer = $($moduleName[i]).closest(
                    ".module-container"
                );
    ...
    private subscribeWorkingModulesState() {
        this.workingModulesStateSubscription =
            this.workingModulesState.subscribe(
    ...
    public selectWorkingModule(workingModule: Module) {
        ...
        if (workingModule.idSettingsGUIParent) {
            ...
            if (
                workingModule.idSettingsGUIParent !==
