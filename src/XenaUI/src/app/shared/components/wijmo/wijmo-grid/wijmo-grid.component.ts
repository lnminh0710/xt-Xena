  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  public setDatasouceForGrid(dataSource, config) {
    ...
    let processData = () => {
      ...
      if (
        this.isColumnsChanged(this.gridColumns, this.wijmoDataSource.columns)
      ) {
  ...
  public hasReadOnlyCells(dataItem) {
    ...
    if (this.readOnlyCells && this.readOnlyCells.length) {
      for (let i = 0; i < this.readOnlyCells.length; i++) {
        if (
          dataItem[this.readOnlyCells[i].name] == this.readOnlyCells[i].value
        ) {
          return true;
        }
      }
  ...
  private _pastingCellHandler(p: any, e: any) {
    const isNotTheSelectingCell =
      !p.selection || p.selection.col !== e.col || p.selection.row !== e.row;
    ...
    if (this.showContextMenu) {
      this.flex.addEventListener(this.flex.hostElement, 'contextmenu', (e) => {
        ...
        if (selection.row !== ht.row || selection.col !== ht.col) {
          this.flex.select(
            new wjcGrid.CellRange(ht.row, ht.col, ht.row, ht.col)
          );
        } else {
            ...
            if (!ht.range.equals(rng)) {
              if (ht.cellType == wjcGrid.CellType.RowHeader) {
                ...
                if (
                  $(
                    'i#info-icon-' +
                      this.flex.rows[rng.row]['dataItem'].DT_RowId,
                    this.flex.hostElement
                  ).is(':visible')
                ) {
      ...
      if (prop && item) {
            ...
            .on('click', (e) => {
              ...
              setTimeout(() => {
                if (this.cellEditDialog) {
                  this.cellEditDialog.cellData.value = this.flex.getCellData(
                    r,
                    c,
                    true
                  );
        ...
        if (this.hightlightKeywords && this.hightlightKeywords.trim() !== '*') {
          ...
          if (/ or | and | [&] |[&]| [|] |[|]/i.test(this.hightlightKeywords)) {
            this.hightlightKeywords = this.hightlightKeywords.replace(
              /OR|AND|&|\|/gi,
              function (matched) {
                return '|';
              }
        ...
        if (
          ...
          if (
            ...
            !Uti.isEmptyObject(item[this.CONTROL_COLUMNS.Priority]) &&
        ...
        if (
          ...
          Uti.toLowerCase(column.data) ===
            Uti.toLowerCase(this.CHECKBOX_COLUMNS.unMergeCheckbox) &&
    ...
    this.flex.hostElement.addEventListener(
      ...
      function (e) {
        ...
        if (ht.cellType === wjcGrid.CellType.Cell) {
          flex.select(
            new wjcGrid.CellRange(ht.row, 0, ht.row, flex.columns.length - 1)
          );
  ...
  private _turnOnStarResizeMode() {
    for (let i = 0; i < this.flex.columns.length; i++) {
      if (
        this.flex.columns[i]['binding'] === this.CONTROL_COLUMNS.deleted ||
        ...
        this.gridColumns.find(
          (col) =>
            !isNil(col.width) && col.data == this.flex.columns[i]['binding']
        )
      ) {
        continue;
      } else {
  ...
  private canBeStarSize(id) {
    return isNil(
      this.gridColumns.find(
        (item) => item.data === id && (item.autoSize || !isNil(item.width))
      )
    );
  }
        ...
        if (flexCol) {
          ...
          if (
            ...
            ((selectedRow.dataItem && !selectedRow.dataItem[gridCol.data]) ||
              (typeof selectedRow.dataItem[gridCol.data] === 'object' &&
  ...
  private _hasRowMarkedAsDeleted() {
    return (
      this.gridData.items.filter((item) => item.deleted === true).length > 0
    );
  }
        ...
        for (const sprop in secondRow) {
          if (fprop === sprop) {
            if (
              ...
              if (
                firstRow[fprop]['key'] !== secondRow[sprop]['key'] ||
                firstRow[fprop]['value'] !== secondRow[sprop]['value']
              ) {
                return false;
              }
            } else {
  ...
  private _toggleNoDataMessage() {
    if (
      ...
      (!this.gridData.items.length ||
        (!this.gridData.items.length &&
          (this.gridData.itemsRemoved.length || this.deletedItemList.length)))
    ) {
  ...
  private getColumnIndexByName(name: string): number {
    for (let i = 0; i < this.flex.columns.length; i++) {
      if (
        Uti.toLowerCase(this.flex.columns[i].binding) == Uti.toLowerCase(name)
      ) {
        return i;
      }
    }
    ...
    if (row.renderSize > 0) {
      ...
      for (let c = 0; c < panel.columns.length; c++) {
        ...
        if (col.renderSize > 0) {
          ...
          let style =
              'width:' + col.renderSize + 'px;text-align:' + col.getAlignment(),
    ...
    for (let i = 0; i < priorityData.length; i++) {
      ...
      if (!notRedAnymore) {
        ...
        if (totalQuantity >= requiredNumber) {
          let diff =
            requiredNumber - (totalQuantity - priorityData[i]['ExportQty']);
      ...
      if (!priority) {
        ...
        if (maxValue < maxLength) {
          ...
          do {
            validPriority -= 1;
            rs = data.find(
              (p) => p[this.CONTROL_COLUMNS.Priority] == validPriority
            );
          } while (rs);
