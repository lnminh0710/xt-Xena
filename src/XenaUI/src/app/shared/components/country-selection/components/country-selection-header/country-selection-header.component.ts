...
export class SelCountrySelectionHeaderComponent implements OnInit, OnDestroy {
  ...
  public filterCountry($event: any) {
    ...
    tempCountryData = tempCountryData.filter((x) => {
      return (
        x.textValue.toLowerCase().indexOf(this.textSearch.toLowerCase()) > -1
      );
    });
  ...
  public createNewGroup() {
    ...
    if (newGroupCountries.length) {
      ...
      this.countrySelectionServiceSubscription = this.countrySelectionService
        .saveCountryGroups({ groupName: this.newGroupName }, newGroupCountries)
  ...
  public saveSelectCountry() {
    ...
    if (selectedCountries.length) {
      ...
      for (let country of selectedCountries) {
        if (
          this.originalSelectedGroupData.checkedCountries.indexOf(country) ===
          -1
        ) {
      ...
      this.countrySelectionServiceSubscription = this.countrySelectionService
        .saveCountryGroups(this.originalSelectedGroupData, updateGroupCountries)
        .subscribe((response: ApiResultResponse) => {
          this.appErrorHandler.executeAction(() => {
            if (!Uti.isResquestSuccess(response) || !response.item) {
              return;
  ...
  private getGroupList() {
    ...
    this.countrySelectionServiceSubscription = this.countrySelectionService
      .getCountryGroupsName(this.currentEntity[camelCase(this.widgetListenKey)])
      .subscribe((response: ApiResultResponse) => {
        this.appErrorHandler.executeAction(() => {
  ...
  private setDataForGridCountrySelected() {
    switch (this.documentMode) {
      ...
      case MenuModuleId.selectionBroker: {
        const selectedCountries = this.countrySelectedList.filter((x) => {
          return x.isActive;
        });
  ...
  private reCalculateSize() {
    ...
    if (!this.isLeftCollapse && !this.isRightCollapse) {
      ...
      middle.css(
        'flex-basis',
        'calc(' +
          (100 - (this.currentWidth.leftSize + this.currentWidth.rightSize)) +
          '% - ' +
          this.currentWidth.splitSize +
          'px)'
      );
        ...
        .subscribe((response: ApiResultResponse) => {
          this.appErrorHandler.executeAction(() => {
            ...
            checkedIdValueList = union(
              checkedIdValueList,
              response.item[0].map((currentValue, index, arr) => {
