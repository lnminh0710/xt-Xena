...
import { CountrySelectionService, AppErrorHandler } from "app/services";
...
export class SelCountrySelectionHeaderComponent implements OnInit, OnDestroy {
    ...
    @Input() set mode(data: any) {
        ...
        this.selectedCountryClass =
            data === MenuModuleId.selectionBroker ? 20 : 30;
        this.countryCheckListClass =
            data === MenuModuleId.selectionBroker ? 60 : 70;
    ...
    @ViewChild("selCountryCheckListComponent")
    selCountryCheckListComponent: SelCountryCheckListComponent;
    ...
    public filterCountry($event: any) {
        ...
        tempCountryData = tempCountryData.filter((x) => {
            return (
                x.textValue
                    .toLowerCase()
    ...
    public createNewGroup() {
        ...
        if (newGroupCountries.length) {
            newGroupCountries = newGroupCountries.map(
                (currentValue, index, arr) => {
                    return {
                        IsActive: 1,
                        IdRepCountryLangauge: currentValue,
                        IdSelectionProject:
                            this.currentEntity[camelCase(this.widgetListenKey)],
                        QtyToNeeded: 1,
                    };
                }
            );

            this.countrySelectionServiceSubscription =
                    ...
                    .subscribe((response: ApiResultResponse) => {
                        this.appErrorHandler.executeAction(() => {
                            ...
                            this.toasterService.pop(
                                "success",
                                "Success",
                                "Group name is saved successfully"
                            );
                            this.requiredMessage = "required";
                        });
                    });
    ...
    private updateCountryTempData(processingData: any) {
        ...
        for (let item of processingData) {
            const currentItem = this.countrySelectedList.find(
                (x) => x.idValue === item.idValue
            );
            ...
            const currentTempItem = this.tempCountryCheckListData.find(
                (x) => x.idValue === item.idValue
            );
        ...
        if (addMoreItem.length) {
            this.countrySelectedList =
                this.countrySelectedList.concat(addMoreItem);
        }
    }
    ...
    public groupListCheckedChange() {
        var selectedGroup = this.groupList.filter((x) => {
            return x.isActive;
        });
        if (!selectedGroup || !selectedGroup.length) {
            // uncheck all country
            this.countryData.forEach((item, index) => {
                this.selCountryCheckListComponent.setActiveForItem(
                    item.idValue,
                    false
                );
            });
    ...
    public saveSelectCountry() {
        ...
        if (selectedCountries.length) {
            ...
            for (let country of selectedCountries) {
                if (
                    this.originalSelectedGroupData.checkedCountries.indexOf(
                        country
                    ) === -1
                ) {
            ...
            for (let country of this.originalSelectedGroupData
                .checkedCountries) {
            ...
            this.countrySelectionServiceSubscription =
                this.countrySelectionService
                    .saveCountryGroups(
                        this.originalSelectedGroupData,
                        updateGroupCountries
                    )
                    .subscribe((response: ApiResultResponse) => {
                        this.appErrorHandler.executeAction(() => {
                            if (
    ...
    public collapseLeft() {
        ...
        this.collapsePanelLeftClass = this.isLeftCollapse
            ? "collapse-left-hide"
            : "panel-show";
    }
    ...
    public collapseRight() {
        ...
        this.reCalculateSize();
        this.collapsePanelRightClass = this.isRightCollapse
            ? "collapse-right-hide"
            : "panel-show";
    }
        ...
        this.countrySelectionServiceSubscription = this.countrySelectionService
            ...
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.groupList = response.item[0].map(
                        (currentValue, index, arr) => {
                            return {
                                idValue:
                                    currentValue.IdRepCountryLangaugeGroupsName,
    ...
    private setDataForGridCountrySelected() {
        switch (this.documentMode) {
            ...
            case MenuModuleId.selectionBroker: {
                const selectedCountries = this.countrySelectedList.filter(
                    (x) => {
                        return x.isActive;
                    }
                );
    ...
    private reCalculateSize() {
        ...
        const spacingRight =
            this.documentMode == MenuModuleId.selectionBroker
                ? this.showPanelWidth
                : 0;
        ...
        // Have Left or Right
        if (
            (this.isLeftCollapse && !this.isRightCollapse) ||
            (!this.isLeftCollapse && this.isRightCollapse)
        ) {
            ...
            if (!this.isLeftCollapse && this.isRightCollapse) {
                left.css(
                    "flex-basis",
                    "calc(" +
                        this.currentWidth.leftSize +
                        "% - " +
                        this.currentWidth.splitSize +
                        "px)"
                );
                middle.css(
                    "flex-basis",
                    "calc(" +
                        (100 - this.currentWidth.leftSize) +
                        "% - " +
                        (this.currentWidth.splitSize + spacingRight) +
                        "px)"
                );
                right.css("flex-basis", "0%");
                return;
            ...
            middle.css(
                "flex-basis",
                "calc(" +
                    (100 - this.currentWidth.rightSize) +
                    "% - " +
                    this.currentWidth.splitSize +
                    "px)"
            );
    ...
    private setCurrentWidth() {
        ...
        if (splitSize <= 10) this.currentWidth.splitSize = splitSize;
        if (!this.isLeftCollapse)
        ...
        if (this.isLeftCollapse && this.isRightCollapse) {
            ...
            middle.css(
                "flex-basis",
                "calc(100% - " + this.showPanelWidth + "px)"
            );
            right.css("flex-basis", "0%");
            return;
        }
        ...
        if (this.isRightCollapse) {
            ...
            middle.css(
                ...
                "calc(" +
                    (100 - this.currentWidth.leftSize) +
                    "% - " +
                    (this.currentWidth.splitSize + this.showPanelWidth) +
                    "px)"
            );
        }
    }
    ...
    private setCountryCheckListClass() {
        ...
        if (
            ...
            this.countryCheckListClass =
                this.documentMode === MenuModuleId.selectionBroker ? 80 : 70;
            return;
        }
        if (!this.isLeftCollapse && !this.isRightCollapse) {
    ...
    private buildForCountryChecked(groupSelectedItems: any) {
        ...
        for (let group of groupSelectedItems) {
            this.countrySelectionServiceSubscription =
                this.countrySelectionService
                    .getCountryGroupsList(group.idValue)
                    .subscribe((response: ApiResultResponse) => {
                        this.appErrorHandler.executeAction(() => {
                            if (
                                !Uti.isResquestSuccess(response) ||
                            ...
                            if (groupSelectedItems.length === 1) {
                                this.originalSelectedGroupData.groupId =
                                    group.idValue;
                                this.originalSelectedGroupData.groupName =
                                    group.textValue;
                                this.originalSelectedGroupData.checkedCountries =
                                    checkedIdValueList;
                            } else {
