import { Component, Output, Input, EventEmitter, OnInit, OnDestroy, AfterViewInit, ViewChild, ChangeDetectorRef, ElementRef } from '@angular/core';
import {
    ApiResultResponse,
} from 'app/models';
...
export class SelCountrySelectionCombineComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('countrySelectionHeader') countrySelectionHeader: SelCountrySelectionHeaderComponent;
    ...
    public onFooterItemEdited($event: any) {
        const currentItem = this.cachingEditedData.find(x => x.Country == $event.Country);
        if (currentItem && currentItem.Country) {
            Uti.removeItemInArray(this.cachingEditedData, currentItem, 'Country');
        }
        ...
        this.countrySelectionServiceSubscription = this.countrySelectionService.getSelectionProjectCountry(this.selectedEntity[camelCase(this.widgetListenKey)])
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    response.item[1] = uniqBy(response.item[1], 'IdCountrylanguage');
                    response.item[1] = response.item[1].filter(item => {
                        return typeof item.IdCountrylanguage !== 'object';
                    });

                    this.countryCheckListData = response.item[1].map((currentValue, index, arr) => {
                        let culture = currentValue.Culture.split('-');
                        return {
                            idValue: currentValue.IdCountrylanguage,
                            idSelectionProjectCountry: isObject(currentValue.IdSelectionProjectCountry) ? '' : currentValue.IdSelectionProjectCountry,
                            qtyToNeeded: isObject(currentValue.QtyToNeeded) ? '' : currentValue.QtyToNeeded,
                            Language: isObject(currentValue.Language) ? '' : currentValue.Language,
                            MediaCode: isObject(currentValue.MediaCode) ? '' : currentValue.MediaCode,
                            QtyFoundet: isObject(currentValue.QtyFoundet) ? '' : currentValue.QtyFoundet,
                            QtyUsed: isObject(currentValue.QtyUsed) ? '' : currentValue.QtyUsed,
                            isoCode: culture[1],
                            culture: currentValue.Culture,
                            textValue: currentValue.Country + '-' + culture[0],
                            isMain: true,
                            isActive: typeof currentValue.IsActive === 'object' ? false : currentValue.IsActive
                        }
                    });

                    this.cachingTableFirstData = cloneDeep(this.countryCheckListData);
                    this.selectedCountryData = this.buildGirdData(this.countryCheckListData);
    ...
    public submit(callback?: any): any {
        const savingData = [...this.makeInsertData(), ...this.makeDeleteData(), ...this.makeUpdateData()];
        if (!savingData || !savingData.length) {
            this.setOutputData(false);
            return;
        }
        this.countrySelectionServiceSubscription = this.countrySelectionService.saveProjectCountry(savingData)
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.store.dispatch(this.processDataActions.selectionCountriesAreUpdated(this.ofModule));
    ...
    private makeInsertData(): Array<any> {
        ...
        for (let item of this.selectedCountryData.data) {
            if (!item.isActiveDisableRow || item.idSelectionProjectCountry) continue;
            result.push({
                'IsActive': 1,
    ...
    private makeUpdateData(): Array<any> {
        ...
        for (let item of this.selectedCountryData.data) {
            if (!item.isActiveDisableRow || !item.idSelectionProjectCountry) continue;
            let currentItem = this.cachingTableFirstData.find(x => x.idSelectionProjectCountry === item.idSelectionProjectCountry);
            if (((isObject(currentItem.qtyToNeeded) || !currentItem.qtyToNeeded) ? '' : currentItem.qtyToNeeded) === (((isObject(item.ReqAddress) || !item.ReqAddress) ? '' : item.ReqAddress))) continue;
            result.push({
                'IsActive': 1,
                'IdRepCountryLangauge': item.IdRepCountryLangauge,
                'IdSelectionProject': this.selectedEntity['idSelectionProject'],
    ...
    private makeDeleteData(): Array<any> {
        ...
        for (let item of this.cachingTableFirstData) {
            ...
            let currentItem = this.selectedCountryData.data.find(x => x.idSelectionProjectCountry === item.idSelectionProjectCountry);
            if (currentItem && (currentItem.isActiveDisableRow || !currentItem.idSelectionProjectCountry)) continue;
            result.push({
                'IsDeleted': '1',
                'IdSelectionProjectCountry': item.idSelectionProjectCountry
            });
    ...
    private buildGirdData(rawData: any) {
        ...
        if (this.mode === MenuModuleId.selectionBroker) {
            selectData = selectData.filter(x => { return x.isActive; });
        }
    ...
    private setEditedData(data: any): any {
        if (!this.cachingEditedData || !this.cachingEditedData.length) return data;
