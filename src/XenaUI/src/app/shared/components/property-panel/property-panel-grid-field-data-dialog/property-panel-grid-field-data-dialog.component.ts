...
export class PropertyPanelGridFieldDataDialogComponent
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    constructor(
        ...
        this.propertiesParentDataState = this.store.select(
            (state) =>
                propertyPanelReducer.getPropertyPanelState(
                    state,
                    this.usingModule.moduleNameTrim
    ...
    private initProperty() {
        ...
        let styleUnderlineProperty: WidgetPropertyModel =
    ...
    public open(item: WidgetPropertyModel) {
        ...
        if (
            this.widgetDetail.contentDetail &&
            this.widgetDetail.contentDetail.data
        ) {
            const data: Array<RawFieldEntity> =
                this.widgetDetail.contentDetail.data[1];
            this.displayFields = Uti.styleFormatFieldsetData(data);
    ...
    private updateDisplayField(selectedField: StyleFormatFieldEntity) {
        if (
            !selectedField.stylePoperties ||
            (selectedField.stylePoperties &&
                !selectedField.stylePoperties.length)
        ) {
            if (this.formatedFields && this.formatedFields.length) {
                const rs = this.formatedFields.filter(
                    (p) =>
                        p.originalColumnName == selectedField.originalColumnName
                );
                if (rs.length) {
    ...
    public showPopover(item: StyleFormatFieldEntity, pop) {
        ...
        if (
            this.selectedField.stylePoperties &&
            this.selectedField.stylePoperties.length
        ) {
            this.selectedField.stylePoperties.forEach(
                (style: WidgetPropertyModel) => {
                    let prop: WidgetPropertyModel =
