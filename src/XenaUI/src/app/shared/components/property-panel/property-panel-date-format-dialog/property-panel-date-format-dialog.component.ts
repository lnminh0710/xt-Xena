...
export class PropertyPanelDateFormatDialogComponent {
  ...
  constructor(private changeDetectorRef: ChangeDetectorRef, private uti: Uti) {}
  ...
  public open(item) {
    ...
    if (item.value) {
      ...
      if (found) {
        ...
        this.dateSample = this.uti.formatLocale(
          new Date(),
  ...
  public checkToDisabledApplyBtn() {
    this.disabledApplyBtn =
      (!this.customDateFormat || !this.customDateFormat.length) &&
      !this.listItem.find((x) => x.active);
  }
}
