...
export class PropertyPanelOrderFieldDialogComponent
  implements OnInit, OnDestroy
{
    ...
    this.widgetTemplateSettingServiceSubscription =
      this.widgetTemplateSettingService
        .getWidgetDetailByRequestString(
          ...
          this.propertiesParentData.widgetDataType.listenKeyRequest,
          false,
          true
        )
        .subscribe((result1: any) => {
          this.appErrorHandler.executeAction(() => {
            this.widgetTemplateSettingServiceSubscription =
              this.widgetTemplateSettingService
                .getWidgetOrderBy(
                  this.propertiesParentData.idRepWidgetApp,
                  this.propertiesParentData.id
                )
  ...
  private makeOrderByData(data: any) {
    ...
    this.displayFieldsForAll = dataForAll.sort((a, b) => {
      return Uti.sortBy(a, b, 'OrderBy');
    });
    this.displayFieldsForOnly = dataForOnly.sort((a, b) => {
      return Uti.sortBy(a, b, 'OrderBy');
    });
  }
  ...
  public apply() {
    ...
    this.widgetTemplateSettingServiceSubscription =
      this.widgetTemplateSettingService
        .saveWidgetOrderBy(this.makeSaveData())
        .subscribe((result: any) => {
          this.appErrorHandler.executeAction(() => {
            this.saveCallback();
            this.store.dispatch(
              this.widgetDetailActions.requestReload(this.usingModule)
            );
          });
        });
  }
  ...
  private saveCallback() {
    this.toasterService.pop(
      'success',
      'Success',
      'Order by field saved successfully'
    );
    this.onApply.emit([
      ...this.displayFieldsForAll,
      ...this.displayFieldsForOnly,
    ]);
    this.showDialog = false;
        ...
        this.propertiesParentDataState.subscribe(
          (propertiesParentDataState: any) => {
            this.appErrorHandler.executeAction(() => {
              ...
              this.propertiesParentData = cloneDeep(
  ...
  private makeSaveData() {
      ...
      for (let i = 0; i < this.displayFieldsForAll.length; i++) {
        saveData.push({
          IdSysWidgetFieldsOrderBy:
            this.displayFieldsForAll[i].IdSysWidgetFieldsOrderBy.toString(),
    ...
    if (this.isDirtyOnly) {
      for (let i = 0; i < this.displayFieldsForOnly.length; i++) {
        saveData.push({
          IdSysWidgetFieldsOrderBy:
            this.displayFieldsForOnly[i].IdSysWidgetFieldsOrderBy.toString(),
          OrderBy: (i + 1).toString(),
        });
      }
    }
