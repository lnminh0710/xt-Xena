...
export class GradientComponent implements OnInit, OnChanges, AfterViewInit {
    ...
    @Output() onApplyBackgroundGradient: EventEmitter<GradientModal> = new EventEmitter<GradientModal>();
    @Output() onChangeBackgroundGradient: EventEmitter<GradientModal> = new EventEmitter<GradientModal>();
    ...
    ngOnInit() {
        ...
        if (this.valueGradient && this.valueGradient.options && this.valueGradient.options.length > 0) {
            this.valueGradient.options.forEach(v => {
                this.graPick.addHandler(Math.round(v.position.replace('%,', '')), this.rgb2hex(v.value));
            })
        } else {
            this.graPick.addHandler(10, '#1CB5E0');
            this.graPick.addHandler(90, '#4F1C96');
        }
        this.backgroundImage = this.valueGradient && this.valueGradient.value ? this.valueGradient.value : this.graPick.getSafeValue();
        this.selectedTypeGradient = this.valueGradient && this.valueGradient.typeGradient ? this.valueGradient.typeGradient : null;
        this.selectedDirectionGradient = this.valueGradient && this.valueGradient.directionGradient ? this.valueGradient.directionGradient : null;
        ...
        this.onApplyBackgroundGradient.emit({background: this.backgroundImage});
    ...
    private disabledCloseButton() {
        ...
        if (oneItem.length === 2) {
            for (let i = 0; i < this.document.querySelectorAll('.grp-handler-close').length; i++) {
                this.document.querySelectorAll('.grp-handler-close')[i]['style'].color = '#77777763';
                this.document.querySelectorAll('.grp-handler-close')[i]['style'].cursor = 'default';
            }
        } else {
            for (let i = 0; i < this.document.querySelectorAll('.grp-handler-close').length; i++) {
                this.document.querySelectorAll('.grp-handler-close')[i]['style'].color = '';
                this.document.querySelectorAll('.grp-handler-close')[i]['style'].cursor = '';
            }
        }
    }
    ...
    handlerChangeColorGradient() {
        ...
        this.onChangeBackgroundGradient.emit({
            background: this.backgroundImage,
            type: this.typeGradient, direction: this.directionGradient
        });
    }
