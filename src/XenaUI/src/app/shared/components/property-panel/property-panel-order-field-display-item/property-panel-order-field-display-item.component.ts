  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  private onCloned(args: any) {
    ...
    this.hasMultiple =
      ...
      (selectedItems.length == 1 && !$(original).hasClass('selected-item'));
    ...
    if (this.hasMultiple) {
      ...
      if (dragIds.length) {
        const fieldDragItems: Array<any> = this.fields.filter(
          (p) => dragIds.find((d) => d == p.IdSysWidgetFieldsOrderBy) != null
        );
  ...
  private itemClicked(event: any) {
    for (const item of this.fields) {
      item.isActive =
        event.IdSysWidgetFieldsOrderBy === item.IdSysWidgetFieldsOrderBy;
    }
