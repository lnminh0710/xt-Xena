  ...
  implements OnInit, OnDestroy, AfterViewInit
{
        ...
        if (item.OriginalColumnName) {
          switch (item.OriginalColumnName) {
            ...
            case 'B01BusinessCosts_Amount':
              result['amount'] =
                item.Value != null ? parseInt(item.Value, null) : null;
              break;
  ...
  private updateFormMainValue() {
    ...
    this.formChangeSubscription = this.businessCostForm.valueChanges.subscribe(
      (data) => {
        this.appErrorHandler.executeAction(() => {
      ...
      if (this.listComboBox && this.listComboBox.BusinessCostCompany) {
        ...
        if (this.multiSelectCompany)
          this.multiSelectCompany.selectedItem =
            this.listComboBox.BusinessCostCompany.find(
              (item) =>
                item.idValue == this.businessCostForm.controls['company'].value
            );
      }
      ...
      CampaignCosts: [
        {
          ...
          DoneDate:
            ...
            this.businessCostForm.controls['prePaymentDate'].value != null
              ? this.datePipe.transform(
                  this.businessCostForm.controls['prePaymentDate'].value,
                  this.consts.dateFormatInDataBase
                )
              : null,
  ...
  private onChangePrePaymentDate(event) {
    ...
    if (event.getTime() - new Date(invoiceDateControll.value).getTime() < 0) {
      setTimeout(() => {
        this.businessCostForm.controls['prePaymentDate'].setValue(
          this.datePipe.transform(invoiceDateControll.value, 'MM/dd/yyyy')
        );
      });
    }
  }
