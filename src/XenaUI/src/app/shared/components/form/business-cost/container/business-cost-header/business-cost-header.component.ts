import {
    ...
    ViewChild, OnInit, OnDestroy, AfterViewInit, ElementRef
} from '@angular/core';
import { ModuleList } from 'app/pages/private/base';
import { FormGroup, Validators, FormBuilder } from '@angular/forms';
import { CommonService, CampaignService, AppErrorHandler, PropertyPanelService } from 'app/services';
import { ComboBoxTypeConstant, ApiMethodResultId, Configuration } from 'app/app.constants';
...
export class BusinessCostHeaderFormComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('xnFileExplorerComponent') xnFileExplorerComponent: XnFileExplorerComponent;
    ...
    @Input() set idSalesCampaignWizard(data: any) {
        if (!isNil(data)) {
            ...
            this.campaingServiceSubscription = this.campaingService.getCampaignCosts(data)
                .subscribe((response: ApiResultResponse) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        this.data = this.mapDataFromServerToEasierStructure(response.item.data);
                        if (this.data && this.data.idBusinessCosts)
                            this.idBusinessCosts = parseInt(this.data.idBusinessCosts, null);
                        this.initFormData();
                ...
                if (item.OriginalColumnName) {
                    switch (item.OriginalColumnName) {
                        ...
                        case 'B01BusinessCosts_Amount':
                            result['amount'] = item.Value != null ? parseInt(item.Value, null) : null;
                            break;
                        ...
                        case 'B01BusinessCosts_VatAmount1':
                            result['vatAmount1'] = item.Value != null ? parseInt(item.Value, null) : null;
                            break;
                        case 'B01BusinessCosts_VatAmount2':
                            result['vatAmount2'] = item.Value != null ? parseInt(item.Value, null) : null;
                            break;
        ...
        this.comServiceSubscription = this.comService.getListComboBox(
            ...
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    setTimeout(() => {
                        if (this.businessCostForm.controls['company'].value && this.listComboBox.BusinessCostCompany) {
                            if (this.multiSelectCompany)
                                this.multiSelectCompany.selectedItem = this.listComboBox.BusinessCostCompany.find((item) => item.idValue == this.businessCostForm.controls['company'].value);
                        } else {
                            if (this.multiSelectCompany)
    ...
    private updateFormMainValue() {
        ...
        this.formChangeSubscription = this.businessCostForm.valueChanges
            .subscribe((data) => {
                this.appErrorHandler.executeAction(() => {
                    if (!this.businessCostForm.pristine && this.isRenderCompleted) {
    ...
    private initFormData() {
        ...
        if (this.businessCostForm && this.businessCostForm.controls) {
            this.businessCostForm.controls['idRepBusinessCosts'].setValue(_data.idRepBusinessCosts || '');
            this.businessCostForm.controls['principal'].setValue(_data.idPerson || '');
            this.businessCostForm.controls['invoice'].setValue(_data.invoiceNr || '');
            this.businessCostForm.controls['date'].setValue(_data.invoiceDate != null && !isEmpty(_data.invoiceDate) ? parse(_data.invoiceDate, 'dd.MM.yyyy', new Date()) : '');
            this.businessCostForm.controls['currency'].setValue(_data.idRepCurrencyCode || '');
            this.businessCostForm.controls['controlAmount'].setValue(isNil(_data.amount) ? '' : _data.amount);
            this.businessCostForm.controls['prePayment'].setValue(_data.doneDate ? true : false);
            this.businessCostForm.controls['prePaymentDate'].setValue(_data.doneDate != null && !isEmpty(_data.doneDate) ? parse(_data.doneDate, 'dd.MM.yyyy', new Date()) : '');
            this.businessCostForm.controls['vat1'].setValue(_data.idRepVat1 || '');
            this.businessCostForm.controls['vat1Amount'].setValue(_data.vatAmount1 || '');
            this.businessCostForm.controls['vat2'].setValue(_data.idRepVat2 || '');
            this.businessCostForm.controls['vat2Amount'].setValue(_data.vatAmount2 || '');
            this.businessCostForm.controls['notes'].setValue(_data.notes || '');
            this.businessCostForm.controls['company'].setValue(_data.company || '');
        } else {
            this.businessCostForm = this.formBuilder.group({
                idRepBusinessCosts: [_data.idRepBusinessCosts || '', Validators.required],
                principal: [_data.idPerson || '', Validators.required],
                invoice: [_data.invoiceNr, Validators.required],
                date: [_data.invoiceDate != null && !isEmpty(_data.invoiceDate) ?
                    parse(_data.invoiceDate, 'dd.MM.yyyy', new Date()) :
                    '', Validators.required],
                currency: [_data.idRepCurrencyCode || '', Validators.required],
                controlAmount: [isNil(_data.amount) ? '' : _data.amount, Validators.required],
                prePayment: _data.doneDate ? true : false,
                prePaymentDate: _data.doneDate != null && !isEmpty(_data.doneDate) ?
            ...
            Uti.setEnableForMultipleFormControl(this.businessCostForm,
                ['vat1', 'vat2', 'vat1Amount', 'vat2Amount'], false);
        }
        ...
        setTimeout(() => {
            if (!this.businessCostForm.controls['prePayment'].value)
                ...
                this.businessCostForm.controls['prePaymentDate'].enable();
            if (this.listComboBox && this.listComboBox.BusinessCostCompany) {
                ...
                if (this.multiSelectCompany)
                    this.multiSelectCompany.selectedItem =
                        this.listComboBox.BusinessCostCompany.find((item) => item.idValue == this.businessCostForm.controls['company'].value);
            }
    ...
    private getCurrencyText() {
        const currentCurrency = this.listComboBox.currency.find(x => { return x.idValue == this.businessCostForm.value['currency']; });
        if (!currentCurrency || !currentCurrency.idValue) return '';
        return currentCurrency.textValue;
    }

    private createCampaignCost() {
        this.campaingServiceSubscription = this.campaingService.saveCampaignCosts(this.prepareSubmitCreateData())
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    if (!response || response.statusCode != ApiMethodResultId.Success || !response.item) {
                        return;
                    }
                    this.saveData.emit({
                        submitResult: true,
            ...
            'CampaignCosts': [
                {
                    ...
                    'InvoiceDate': model.date != null ? this.datePipe.transform(model.date, this.consts.dateFormatInDataBase) : null,
                    'Amount': model.controlAmount,
                    'Notes': model.notes,
                    'DoneDate': this.businessCostForm.controls['prePaymentDate'] && this.businessCostForm.controls['prePaymentDate'].value != null ?
                        this.datePipe.transform(this.businessCostForm.controls['prePaymentDate'].value, this.consts.dateFormatInDataBase) : null,
                    'IdRepVat1': model.vat1 || null,
                    'VatAmount1': this.vat1AmountNumber.nativeElement.value || null,
                    'IdRepVat2': model.vat2 || null,
                    'VatAmount2': this.vat2AmountNumber.nativeElement.value || null,
                    'IsActive': true,
                    'IdBusinessCosts': this.data ? this.data.idBusinessCosts : null,
                    'IdPersonToCompany': isNil(model.company) || isEmpty(model.company) ? null : model.company,
                }
            ]
        };
    }
    ...
    private getDataForVATDDL(idPerson: any) {
        ...
        const filter = this.listComboBox.principal.find((item) => item.idValue === idPerson);
        if (!filter) {
            this.allowForFormValueChange();
            return;
        }
        this.idRepIsoCountryCode = filter.idRepIsoCountryCode;
        this.comServiceSubscription = this.comService.getComboBoxDataByFilter(-1, filter.idRepIsoCountryCode, 'vat')
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
    ...
    private setDatasourceForVat(filter: any, data: any) {
        ...
        if (!data || !data['vat_' + filter.idRepIsoCountryCode]) {
            ...
            Uti.setEnableForMultipleFormControl(this.businessCostForm,
                ['vat1', 'vat2', 'vat1Amount', 'vat2Amount'], false);
            Uti.setValueForMultipleFormControl(this.businessCostForm,
                ['vat1', 'vat2', 'vat1Amount', 'vat2Amount'], null);
            this.vat1AmountNumber.nativeElement.value = null;
            this.vat2AmountNumber.nativeElement.value = null;
            this.businessCostForm.updateValueAndValidity();
            this.allowForFormValueChange();
        } else {
            this.listComboBox['vat'] = data['vat_' + filter.idRepIsoCountryCode];
            setTimeout(() => {
                Uti.setEnableForMultipleFormControl(this.businessCostForm,
                    ['vat1', 'vat2', 'vat1Amount', 'vat2Amount'], true);
                this.businessCostForm.updateValueAndValidity();
                if (this.data) {
                    this.businessCostForm.controls['vat1'].setValue(this.data['idRepVat1']);
                    this.businessCostForm.controls['vat2'].setValue(this.data['idRepVat2']);
                    this.businessCostForm.controls['vat1Amount'].setValue(this.data['vatAmount1']);
                    this.businessCostForm.controls['vat2Amount'].setValue(this.data['vatAmount2']);
                    this.vat1AmountNumber.nativeElement.value = this.data['vatAmount1'];
                    this.vat2AmountNumber.nativeElement.value = this.data['vatAmount2'];
                }
                this.allowForFormValueChange();
            }, 500);
        }
    ...
    public onChangeControlAmount(event: any) {
        if (!isInteger(event)) {
            this.businessCostForm.controls['controlAmount'].setValue(parseInt(event, null));
            this.businessCostForm.controls['controlAmount'].updateValueAndValidity();
        }
    }

    public onChangeVat1Amount(event: any) {
        if (!isInteger(event)) {
            this.businessCostForm.controls['vat1Amount'].setValue(parseInt(event, null));
            this.businessCostForm.controls['vat1Amount'].updateValueAndValidity();
        }
    }

    public onChangeVat2Amount(event: any) {
        if (!isInteger(event)) {
            this.businessCostForm.controls['vat2Amount'].setValue(parseInt(event, null));
            this.businessCostForm.controls['vat2Amount'].updateValueAndValidity();
        }
    }

    public onChangePrePayment($event) {
        this.businessCostForm.controls['prePaymentDate'].setValue($event.checked ? new Date() : null);
        const prePaymentDateControl = this.businessCostForm.controls['prePaymentDate'];
        // if (event) {
    ...
    private onChangeInvoiceDate(event) {
        ...
        const prePaymentDateControll = this.businessCostForm.controls['prePaymentDate'];
        const prePaymentControll = this.businessCostForm.controls['prePayment'];
        if (prePaymentControll.value &&
            event.getTime() - (new Date(prePaymentDateControll.value)).getTime() > 0)
            prePaymentDateControll.setValue(this.datePipe.transform(event, 'MM/dd/yyyy'));
    }
    ...
    private onChangePrePaymentDate(event) {
        ...
        if (event.getTime() - (new Date(invoiceDateControll.value)).getTime() < 0) {
            setTimeout(() => {
                this.businessCostForm.controls['prePaymentDate'].setValue(this.datePipe.transform(invoiceDateControll.value, 'MM/dd/yyyy'));
            });
        }
    }
    ...
    public selectedIndexChanged(event) {
        if (this.multiSelectCompany && this.multiSelectCompany.selectedItem) {
            ...
            this.businessCostForm.controls['company'].setValue(idValue.toString());
    ...
    itemFormatter(index, item) {
        let newContent = '<div class="col-md-12  col-lg-12 xn-wj-ddl-item"><div class="col-sm-5 no-padding-left" >{personType}</div><div class="col-sm-1 hidden">-----</div><div class="col-sm-5 no-padding-right border-left" >{company}&nbsp;</div></div>';
            ...
            this.appErrorHandler.executeAction(() => {
                if (globalProperties && globalProperties.length) {
                    ...
                    if (propShowDropdownWhenFocus) {
                        this.dontShowCalendarWhenFocus = propShowDropdownWhenFocus.value;
                    }
                }
            });
        });
