  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  private timeoutFocusWhenChangeTab: any;
  ...
  public focusControlByControlName(controlName: string) {
    ...
    for (const control of this.controlList) {
      if (searchByMultipleName) {
        if (
          controlName.indexOf(',' + control.attr('name') + ',') !== -1 ||
          controlName.indexOf(',' + control.attr('formcontrolname') + ',') !==
            -1
        ) {
          findControl = control;
          break;
  ...
  private activeTabAndFocusFirstElement() {
    ...
    if (this.controlFocus && this.controlFocus.nativeElement) {
      ...
      if (thisForm) $thisPane = $(thisForm).parents('.tab-pane.active').last();
    }
  ...
  private selectTab($curentTab: any) {
    ...
    if ($currentNav.length) {
      if (tabId.indexOf('orderDataEntryPayment') !== -1)
        ...
        this.store.dispatch(
          this.tabSummaryActions.requestSelectSimpleTab(tabId, this.ofModule)
        );
    }
  }
