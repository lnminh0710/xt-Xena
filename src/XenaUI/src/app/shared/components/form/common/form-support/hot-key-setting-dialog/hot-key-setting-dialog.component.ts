...
export class HotKeySettingDialogComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.hotKeySettingForm = new FormGroup({
            hotKey: new FormControl(this.hotKey, [this.isValidHotkey(this.hotKeySetting, this.controlKey)])
        });
    }
        ...
        return (input: FormControl) => {
            ...
            Object.keys(hotKeySetting).forEach(key => {
                if (hotKeySetting[key] && input.value) {
                    if (hotKeySetting[key].toString().toLowerCase() == input.value.toString().toLowerCase() && controlKey != key) {
                        isValid = false;
                    }
                }
            });
     ...
     */
    subscribeHotKeySettingState() {
        if (this.hotKeySettingStateSubscription) this.hotKeySettingStateSubscription.unsubscribe();

        this.hotKeySettingStateSubscription = this.hotKeySettingState.subscribe(hotKeySetting => {
