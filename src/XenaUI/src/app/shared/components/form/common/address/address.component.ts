...
export class XnFormFgAddressComponent implements OnInit, OnDestroy {
    ...
    @Input() set initInformation(information: any) {
        ...
        if (information.data) this.data = information.data;

        if (information.updateData) this.updateData = information.updateData;

        this.mandatoryCheckData = information.mandatoryData || {};
        this.regularExpressionData = information.regularExpressionData || {};

        if (information.parentFG) this.parentFormGroup = information.parentFG;
    ...
    private loadDefaultData() {
        ...
        if (
            ...
            this.listComboBox.language &&
            this.listComboBox.pOBox
        ) {
            this.initData();
            return;
        }
    ...
    private initData() {
        ...
        else this.parentFormGroup.addControl("address", this.addressForm);
    ...
    public changeCountryCode() {
        if (
            ...
            this.onCountryChangedAction.emit(
                this.idRepIsoCountryCodeCombobox.selectedItem
            );

            const selectedItem = this.idRepIsoCountryCodeCombobox.selectedItem;
            let validationZip2MaskFormat =
                selectedItem.validationZip2MaskFormat;
    ...
    private updateFormValue(data) {
        if (this.isVerticalLayout) {
            ...
            if (this.addressForm.controls["streetNr"] && data["streetNr"])
                this.addressForm.controls["streetNr"].setValue(
                    data["streetNr"].value
                );

            if (
                this.addressForm.controls["streetAddition1"] &&
                data["streetAddition1"]
            )
                this.addressForm.controls["streetAddition1"].setValue(
                    data["streetAddition1"].value
                );

            if (
                this.addressForm.controls["streetAddition2"] &&
                data["streetAddition2"]
            )
                this.addressForm.controls["streetAddition2"].setValue(
                    data["streetAddition2"].value
                );
            ...
            if (this.addressForm.controls["poboxLabel"] && data["poboxLabel"])
                this.addressForm.controls["poboxLabel"].setValue(
                    data["poboxLabel"].value
                );

            if (this.addressForm.controls["idRepPoBox"] && data["idRepPoBox"])
                this.addressForm.controls["idRepPoBox"].setValue(
                    data["idRepPoBox"].value
                );

            if (this.addressForm.controls["place"] && data["place"])
                this.addressForm.controls["place"].setValue(
                    data["place"].value
                );
            ...
            if (data.idRepIsoCountryCode) {
                ...
                if (this.listComboBox && this.listComboBox.countryCode) {
                    const selectedItem = this.listComboBox.countryCode.find(
                        (p) => p.idValue == idRepIsoCountryCode
                    );
                    if (selectedItem) {
                        let validationZip2MaskFormat =
                            selectedItem.validationZip2MaskFormat;
                        let validationZip2RegEx =
                            selectedItem.validationZip2RegEx;
                        let validationZipMaskFormat =
                            selectedItem.validationZipMaskFormat;
                        let validationZipRegEx =
                            selectedItem.validationZipRegEx;
                        this.updateZipMask({
                            validationZipMaskFormat,
                            validationZipRegEx,
                            validationZip2MaskFormat,
    ...
    public updateZipMask(zipMaskPattern: ZipMaskPattern) {
        if (zipMaskPattern && zipMaskPattern.validationZipMaskFormat) {
            zipMaskPattern.validationZipMaskFormat =
                zipMaskPattern.validationZipMaskFormat.replace(/§/g, "A");
        }
        if (zipMaskPattern && zipMaskPattern.validationZip2MaskFormat) {
            zipMaskPattern.validationZip2MaskFormat =
                zipMaskPattern.validationZip2MaskFormat.replace(/§/g, "A");
        }
