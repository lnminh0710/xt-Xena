  ...
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.hotKeyState = this.store.select(
      (state) =>
        commonReducer.getCommonState(state, this.ofModule.moduleNameTrim).hotKey
    );
  }
  public ngOnInit() {
    if (this.hotKeyValue) {
            ...
            if (
              ...
              if (this.componentControl) {
                if (this.componentControl instanceof MatCheckbox) {
                  const currentStatus = (this.componentControl as MatCheckbox)
                    .checked;
  ...
  private getErrorMessage() {
    if (
      ...
      return this.errorMessage || this.control['regularExpressionMessage'];
    }
    ...
    for (const propertyName in this.control.errors) {
      if (
        ...
        return (
          label + ' ' + ValidationService.getValidatorErrorMessage(propertyName)
        );
      }
    }
    return '';
