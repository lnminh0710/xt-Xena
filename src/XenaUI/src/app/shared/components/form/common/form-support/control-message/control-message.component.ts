...
import { ValidationService, DomHandler, AppErrorHandler } from "app/services";
import { Subscription } from "rxjs/Subscription";
import isEmpty from "lodash-es/isEmpty";
import { TriggerClickInsideCboDirective } from "app/shared/directives/xn-trigger-click-inside-cbo/xn-trigger-click-inside-cbo.directive";
import { Store } from "@ngrx/store";
import { AppState } from "app/state-management/store";
import { Observable } from "rxjs/Observable";
import { HotKey } from "app/models";
    ...
    implements OnInit, OnDestroy
{
    ...
    @Input() set disabled(data: any) {
        this.classDisabled = data ? "xn-disabled" : "";
        this.disabledControl = data;
    }
    ...
    @Output() onClickComponentControl: EventEmitter<any> =
    ...
    public ngOnInit() {
        ...
        if (!this.control || !this.control.root) {
            //console.log('ControlMessage is not a control');
            return;
        }
    ...
    public clearText(event) {
        if (this.control) {
            ...
            if (
                ...
                typeof this.componentControl.focus === "function"
            )
    ...
    private subscribeFormChange() {
        this.formValueChangeErrorSubscription = this.control.root.valueChanges
            .debounceTime(300)
            .subscribe(() => {
        ...
        this.controlStatusChangeSubscription =
            this.control.statusChanges.subscribe(() => {
                this.appErrorHandler.executeAction(() => {
                    this.reUpdateStyleForControl();
                });
            });
        this.controlValueChangescription = this.control.valueChanges
            .debounceTime(100)
            .subscribe(() => {
                this.appErrorHandler.executeAction(() => {
                    this.setRequiredBorder();
    ...
    private subscribeHotkeyState() {
        if (this.hotKeyStateSubscription)
            this.hotKeyStateSubscription.unsubscribe();
        ...
        this.hotKeyStateSubscription = this.hotKeyState.subscribe(
            (hotkey: HotKey) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    const isEditHotKeyInDialog = this.domHandler.findSingle(
                        document,
                        ".prime-dialog-article-hot-key"
                    );
                    if (!isEditHotKeyInDialog) {
                        if (
                            hotkey.keyCombineCode &&
                            this.hotKeyValue &&
                            this.hotKeyValue.value &&
                            hotkey.keyCombineCode.toLowerCase() ==
                                String(this.hotKeyValue.value).toLowerCase()
                        ) {
                            if (this.componentControl) {
                                if (
                                    this.componentControl instanceof MatCheckbox
                                ) {
                                    const currentStatus = (
                                        this.componentControl as MatCheckbox
                                    ).checked;
                                    this.onClickComponentControl.emit(
                                        currentStatus
                                    );
                                } else {
                                    this.componentControl.focus();
                                    if (
                                        typeof this.componentControl.click ===
                                        "function"
                                    ) {
    ...
    private bindEventToControl(currentLoopTimes: number) {
        ...
        setTimeout(() => {
            if (this.control && this.eRef) {
                const wijmoControl = $(
                    "wj-combo-box",
                    $(this.eRef.nativeElement).parent()
                );
    ...
    private getErrorMessage() {
        if (
            (this.control.hasOwnProperty("valueIncorrect") &&
                this.control["valueIncorrect"]) ||
            (this.control.errors && this.control.errors["expressionError"])
        ) {
            return (
                this.errorMessage || this.control["regularExpressionMessage"]
            );
        }
        ...
        for (const propertyName in this.control.errors) {
            if (
                ...
                (propertyName != "required" && propertyName != "email") ||
                (this.control.root as any).submitted
            ) {
                let label = "";
                if (this.control["label"]) {
                    label = this.control["label"] + " is ";
                }
                return (
                    label +
                    " " +
                    ValidationService.getValidatorErrorMessage(propertyName)
                );
            }
        }
        return "";
    ...
    private getCssClass() {
        ...
        let cssClass = "";
        if (
            (!(this.control.root as any).submitted || !this.hasValidation()) &&
            !this.control["valueIncorrect"]
        )
            return cssClass;
        cssClass =
            this.control.valid && !this.control["valueIncorrect"]
                ? ""
                : "invalid-group-addon  ";
        cssClass +=
            this.control.pristine && this.control.value === ""
                ? "pristine-group-addon"
                : "";
        return cssClass;
    }
    ...
    public clickEditHotkey($event) {
        if (
            !this.isShowHotkeyDialog &&
            this.isHotKeyActive &&
            this.deferHotKeyDisplay
        ) {
            $event.preventDefault();
            $event.stopPropagation();
        }
