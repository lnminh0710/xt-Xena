...
export class ControlMessageComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    private subscribeFormChange() {
        this.formValueChangeErrorSubscription = this.control.root.valueChanges.debounceTime(300).subscribe(() => {
        ...
        this.controlValueChangescription = this.control.valueChanges.debounceTime(100).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                this.setRequiredBorder();
            });
        });
    ...
    private subscribeHotkeyState() {
        if (this.hotKeyStateSubscription) this.hotKeyStateSubscription.unsubscribe();
        ...
        this.hotKeyStateSubscription = this.hotKeyState.subscribe((hotkey: HotKey) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (!isEditHotKeyInDialog) {
                    if (hotkey.keyCombineCode && this.hotKeyValue && this.hotKeyValue.value && hotkey.keyCombineCode.toLowerCase() == String(this.hotKeyValue.value).toLowerCase()) {
    ...
    private getErrorMessage() {
        if ((this.control.hasOwnProperty('valueIncorrect') && this.control['valueIncorrect']) ||
        ...
        for (const propertyName in this.control.errors) {
            if ((this.control.errors.hasOwnProperty(propertyName) && !this.control.pristine) ||
                ...
                return label + ' ' + ValidationService.getValidatorErrorMessage(propertyName);
            }
        }
        return '';
    }
    ...
    public clickEditHotkey($event) {
        if (!this.isShowHotkeyDialog && this.isHotKeyActive && this.deferHotKeyDisplay) {
            $event.preventDefault();
            $event.stopPropagation();
        }
    }    
