...
export class PrinterFormDialogComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    public print() {
        if (!this.printer || !this.jspmWSStatus() || !this.fileData || !this.fileData.length) {
            return;
        }
    ...
    private printFile() {
        if (this.printingItemIndex >= this.fileData.length) {
            this._toasterService.pop('success', 'Success', 'Files are printed.');
            this.showLoading = false;
            this.printedText = (this.printingItemIndex + 1) + '/' + (this.fileData.length);
            this.showConfirmDialog();
            return;            
        }
        this.printedText = ((this.printingItemIndex ===  this.fileData.length) ? this.fileData.length : (this.printingItemIndex + 1)) + '/' + (this.fileData.length);
        const cpj = new JSPM.ClientPrintJob();
        cpj.clientPrinter = new JSPM.InstalledPrinter(this.printer);
        const myFile = new JSPM.PrintFilePDF(location.origin + '/api/FileManager/GetFile?name='
                                        + this.fileData[this.printingItemIndex]['MediaName']
                                        + '&subFolder=' + this.fileData[this.printingItemIndex]['MediaRelativePath'],
                                    JSPM.FileSourceType.URL, this.fileData[this.printingItemIndex]['MediaName'], 1);
        cpj.files.push(myFile);
        cpj.sendToClient().then((data: any) => {
            if (data !== 'OK') {
                this.errorFile.push(this.fileData[this.printingItemIndex]['MediaName']);
            }
    ...
    private showConfirmDialog() {
        if (this.errorFile.length) {
            let message: any = [{key: '<p>'}, {key: 'Modal_Message__Error_File_1'}, {key: '</p>'}];
            message.push({key: '<ul>'});
    ...
    private jspmWSStatus() {
        if (JSPM.JSPrintManager.websocket_status === JSPM.WSStatus.Open) {
            ...
            this._modalService.confirmMessageHtmlContent(new MessageModel ({
                ...
                message: [{key: '<p>'}, {key: 'Modal_Message__Download_JSPrintManager_1'}, {key: '</p>'},
                        {key: '<p>'}, {key: 'Modal_Message__Download_JSPrintManager_2'}, {key: '</p>'}],
                callBack1: () => {
                    this.isDownloadFileWarningPopupOpening = false;
                    window.open(Configuration.PublicSettings.jsPrintManagerDownloadLink, '_blank');
                },
                callBack2: () => {
                    this.isDownloadFileWarningPopupOpening = false;
                }
            ...
            this._toasterService.pop('error', 'Disabled', 'JSPM has blacklisted this website!');
            return false;
        }
    }
}
