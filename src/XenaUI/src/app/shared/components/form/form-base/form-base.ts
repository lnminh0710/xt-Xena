...
export abstract class FormBase extends BaseComponent {
  ...
  public regularExpressionData: any;
  ...
  protected makeValidationForFormControl() {
    ...
    groupChanged.forEach((groupItem) => {
      ...
      if (groupFields) {
        ...
        for (let groupField of groupFields) {
          if (
            (typeof this.crossFormGroupData[groupField.fieldName] ===
              'object' &&
              !isEmpty(this.crossFormGroupData[groupField.fieldName])) ||
            (this.crossFormGroupData[groupField.fieldName] &&
              typeof this.crossFormGroupData[groupField.fieldName] ===
                'string' &&
              !!this.crossFormGroupData[groupField.fieldName].trim()) ||
            (typeof this.crossFormGroupData[groupField.fieldName] ===
              'boolean' &&
              this.crossFormGroupData[groupField.fieldName])
          ) {
  ...
  protected processFormChanged(func1?: Function) {
  ...
  protected getFieldNameChanged() {
    ...
    for (let item in this.crossFormGroupData) {
      if (
        ...
        this.crossFormGroupData[item] !== this.crossFormGroupDataOld[item]
      ) {
        changedFields.push(item);
      }
    }
  ...
  private removeValidationForControl(formControls: any, controlName: string) {
    for (let item in formControls) {
      if (item == controlName) {
        if (!this.crossFormGroupData[item]) this.mandatoryColor[item] = '';
