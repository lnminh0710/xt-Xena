  ...
  implements OnInit, OnDestroy
{
  ...
  private subscribeTabData() {
    ...
    this.selectedParkedItemStateSubscription =
      this.selectedParkedItemState.subscribe((selectedParkedItem: any) => {
        this.appErrorHandler.executeAction(() => {
  ...
  private subscribeEntityIdData() {
    this.orderDataMediaCodeStateSubscription =
      this.orderDataMediaCodeState.subscribe((orderDataMediaCode: string) => {
        this.appErrorHandler.executeAction(() => {
          this.browserInfo.Mediacode = orderDataMediaCode || null;
        });
      });
    this.selectedCampaignNumberStateSubcription =
      this.selectedCampaignNumberState.subscribe((campaignNumber: string) => {
        this.appErrorHandler.executeAction(() => {
          this.browserInfo.CampaignNumber = campaignNumber || null;
        });
      });
    this.dataEntryCustomerDataStateSubcription =
      this.dataEntryCustomerDataState.subscribe((customerData: number) => {
        this.appErrorHandler.executeAction(() => {
      ...
      (scanningStatusState: any) => {
        this.appErrorHandler.executeAction(() => {
          if (scanningStatusState.length) {
            ...
            this.fileAttachedUrl = this.dataEntryService.buildImageFullPath(
              scanningStatusState,
              'pdf'
            );
            this.getImageToBase64(
              this.dataEntryService.buildImageFullPath(scanningStatusState)
            );
