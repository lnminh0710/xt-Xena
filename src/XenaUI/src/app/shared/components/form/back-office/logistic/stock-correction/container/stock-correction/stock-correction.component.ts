...
import { AppErrorHandler, CommonService, StockCorrectionService, PropertyPanelService, ModalService } from 'app/services';
...
export class StockCorrectionComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private getInitDropdownlistData() {
        this.comService.getListComboBox('wareHouse,warehouseCorrectionReason')
    ...
    private setOutputData(data) {
        this.outputData.emit({
            isValid: !!(this.wijmoGridComponent &&
                (this.stockCorrectionDataSource.data.length && !this.wijmoGridComponent.hasError())),
    ...
    private initForm() {
        ...
        this.stockCorrectionFormValueChangesSubscription = this.stockCorrectionForm.valueChanges.subscribe((value) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private saveData() {
        ...
        if (!this.stockCorrectionDataSource || !this.stockCorrectionDataSource.data.length)
        ...
        const processFailed = () => {
            ...
            this.toasterService.pop('error', 'Failed', 'Stock Correction Data cannot be saved');
            this.setOutputData({ submitResult: false });
        };

        this.stockService.saveStockCorrection(this.prepareSaveData())
            .subscribe(response => {
                this.appErrorHandler.executeAction(() => {
                    if (response && response.eventType == FormSaveEvenType.Successfully && response.returnID) {
                        this.toasterService.pop('success', 'Success', 'Stock Correction Data is saved successfully');
                        this.setOutputData({
                            submitResult: true,
    ...
    private checkEnableAddButton() {
        ...
        if (this.reasonCtrl && !isNil(this.reasonCtrl.selectedItem) // hasReason
            && this.warehouseCtrl && !isNil(this.warehouseCtrl.selectedItem) // hasWarehouse
    ...
    public add() {
        ...
        const existingItem = this.stockCorrectionDataSource.data.find((item) =>
    ...
    public searchArticleNumberKeyTabDown(event) {
        if (!event.shiftKey && event.keyCode == 9 && this.priceCtl.nativeElement.disabled) {
    ...
    public warehouseChanged($event: any): void {
        ...
        if (!selectedItem || !selectedItem.idRepIsoCountryCode || !selectedItem.textValue) return;

        let idRepIsoCountryCode = selectedItem.idRepIsoCountryCode;
        this.comService.getComboBoxDataByCondition('vat', idRepIsoCountryCode).subscribe(
            (response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {

                    this.listComboBox.vat = response.item && response.item.vat ? response.item.vat : [];
        ...
        this.comService.getComboBoxDataByCondition('currencyByCountry', idRepIsoCountryCode).subscribe(
            (response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    this.listComboBox.currency = response.item && response.item.currencyByCountry ? response.item.currencyByCountry : [];
    ...
    public reasonChanged($event: any): void {
        ...
        if (selectedItem && this.previousReasonItem &&
            (selectedItem.idValue != this.previousReasonItem.idValue || selectedItem.status != this.previousReasonItem.isWithPrice)) {
    ...
    private createGridColumns() {
        ...
        result.push(this.makeColumn('ReasonId', 'ReasonId', false, '35', null, 'Reason', false));
        result.push(this.makeColumn('Reason', 'Reason', true, '35', null, 'Reason', false));
        ...
        result.push(this.makeColumn('Price', 'Price', true, '', 'numeric', 'Price', false, false));
    ...
    private createNewGridItem() {
        ...
        let item = {
            ...
            QtyWithColor: parseInt((this.qtyMode === QTY_MODE_NEGATIVE ? '-' : '') + model.qty),
            Notes: model.notes,
        ...
        if (this.currencyCtl && this.currencyCtl.selectedItem) {
            item.IdRepCurrencyCode = JSON.stringify([{ "key": this.currencyCtl.selectedItem.idRepCurrencyCode, "value": this.currencyCtl.selectedItem.textValue }]);
        }

        if (this.vatCtl && this.vatCtl.selectedItem) {
            item.IdRepVat = JSON.stringify([{ "key": this.vatCtl.selectedItem.idValue, "value": this.vatCtl.selectedItem.textValue }]);
        }
    ...
    public onTableEditSuccess(dataItem) {
        ...
        const filterDataItem = this.stockCorrectionDataSource.data.find((item) => item.DT_RowId == dataItem.DT_RowId);
        if (filterDataItem)
