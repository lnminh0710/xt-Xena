import { Component, Input, Output, EventEmitter, OnInit, OnDestroy, AfterViewInit, ElementRef, ViewChild, Renderer } from "@angular/core";
import { Router } from '@angular/router';
import { ModuleSearchDialogComponent } from 'app/shared/components/form';
import {
    ControlGridModel, ControlGridColumnModel,
    ArticleOrder, EsSearchResult, FormOutputModel,
    ReturnRefundInvoiceNumberModel, SimpleTabModel,
    MessageModel, Module
} from 'app/models';
...
import {
    AppErrorHandler, ModalService, SearchService,
    ArticleService, DomHandler
} from 'app/services';
...
export class ArticleSearchComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    public that: any;
    ...
    constructor(private _eref: ElementRef,
        ...
        this.articleOrdersState = store.select(state => returnRefundReducer.getReturnRefundState(state, this.ofModule.moduleNameTrim).keepArticleOrders);
        this.invoiceNumberDataState = store.select(state => returnRefundReducer.getReturnRefundState(state, this.ofModule.moduleNameTrim).invoiceNumberData);
    ...
    private subscribeInvoiceNumberDataState() {
        this.invoiceNumberDataStateSubscription = this.invoiceNumberDataState.subscribe((data: ReturnRefundInvoiceNumberModel) => {
            this.appErrorHandler.executeAction(() => {
                if (data) {
    ...
    private subscribeInvoiceNewDataState() {
        this.invoiceNewDataStateSubscription = this.invoiceNewDataState.subscribe((data: FormOutputModel) => {
            this.appErrorHandler.executeAction(() => {
                if (data && JSON.stringify(data) != JSON.stringify(this.invoiceNewData)) {
    ...
    private subscribeSelectedSimpleTabState() {
        this.selectedSimpleTabStateSubscription = this.selectedSimpleTabState.subscribe((selectedSimpleTabState: SimpleTabModel) => {
            this.appErrorHandler.executeAction(() => {
                ...
                setTimeout(() => {
                    let parent = this.domHandler.findParent(this._eref.nativeElement, "#" + selectedSimpleTabState.TabID);
    ...
    private subscribeArticleOrdersState() {
        this.articleOrdersStateSubscription = this.articleOrdersState.subscribe((articleOrders: Array<ArticleOrder>) => {
            this.appErrorHandler.executeAction(() => {
                if (!this.dataSource || !articleOrders || !articleOrders.length)
                    return;
                ...
                for (const item of articleOrders) {
                    if (!item) return;
                    if (Uti.existItemInArray(data, {'ArticleNr': item.articleNumber}, 'ArticleNr')) continue;
                    ...
                    data.push({
                        ...
                        TotalPrice: toSafeInteger(item.price) * toSafeInteger(item.quantity),
                        Keep: true
                    });
                }
                ...
                data = data.filter(p => {
                    if (!p.Keep) {
                        ...
                        let rs = articleOrders.filter(o => o && o.articleNumber == p.ArticleNr);
                        return (rs.length > 0);
                    }
                });
    ...
    addArticle() {
        if (!this.articleNumber) {
            this.modalService.warningMessage([{key: 'Modal_Message__Article_Search_Input_Article_Number'}]);
            return;
        }
        if (!this.quantity) {
            this.modalService.warningMessage([{key: 'Modal_Message__Article_Search_Input_Quantity'}]);
            return;
        }
        ...
        if (!this.allArticlesChecked) {
            if (this.articleCampaignSearch
                ...
                const rs = this.articleCampaignSearch.dataSource.data.filter(p => p.ArticleNr == this.articleNumber);
                if (rs.length) {
            ...
            if (this.idCountrylanguage) {
                this.searchAllArticleCampaign(this.articleNumber, this.idCountrylanguage);
            }
        }
    }
    ...
    private updateInvoiceNewDataState() {
        let totalAmount = 0;
        if (this.invoiceNewData && this.invoiceNewData.formValue
    ...
    deleteClickHandler($event) {
        this.modalService.confirmMessageHtmlContent(new MessageModel({
            ...
            message: [{key: 'Modal_Message__Are_You_Sure_To_Delete_This_Item'}],
    ...
    private searchAllArticleCampaign(articleNumber, idCountrylanguage) {
        this.articleService.searchArticleByNr('', articleNumber, idCountrylanguage).subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                ...
                this.addArticleFromArticleCampaign(response.item.data[1][0], this.quantity);
            });
        });
    }
