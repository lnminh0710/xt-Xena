...
export class EmailSettingComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    @Input() set emailSettingData(data: any) {
        ...
        this.idRepSalesOrderStatus = Uti.getValueFromArrayByKey(data.settingData, 'IdRepSalesOrderStatus');
    }
    ...
    public saveAsNewTemplateClick() {
        if (!this.emailSettingForm || !this.emailSettingForm.value || !this.emailSettingForm.value['emailBody']) {
            this.modalService.warningMessage([{ key: 'Modal_Message__Please_Input_The_Email_Content' }]);
            return;
        }
        this.showDialog = true;
        this.emailTemplateSubject = this.currentTemplate ? this.currentTemplate.name : '';
    ...
    private checkExistEmailSubjectName(emailSubjectName: string, id: any): boolean {
        const currentTemplate = this.contentTemplateData.find(x => x.name === emailSubjectName && x.id != id);
    ...
    public onContentChanged($event: any) {
        ...
        let doesTagNotContainsAnyTag = $event.text.match(/<<(([^<>]+)|)<<(([^<>]+)|)>>(([^<>]+)|)>>/g);
    ...
    public callBackAfterSaveEmailTemplate(isNew, responseTemplateId) {
        ...
        if (!isNew) {
            for (let item of this.contentTemplateData) {
                if (item.id === this.currentTemplateId()) {
                    item.name = isNew ? this.currentTemplate.name : this.emailTemplateSubject;
        ...
        this.toasterService.pop('success', 'Success', 'Email Template is saved successfully');
    }
    ...
    private subscribeFormValueChange() {
        if (this.emailSettingFormValuesChangeSubscription) this.emailSettingFormValuesChangeSubscription.unsubscribe();

        this.emailSettingFormValuesChangeSubscription = this.emailSettingForm.valueChanges
    ...
    private setValueForFlagChangingVariable(hasChangeEmailBody?: boolean,
    ...
    private okDeleteRows() {
        this.deleteEmailTemplate.emit(this.prepareEmailTemplateData(this.currentTemplateId(), true));
    }
    ...
    private setValueIsCanSendEmail() {
        this.isCanSendEmail = (this.emailSettingForm &&
    ...
    private currentTemplateId() {
        return (this.currentTemplate && this.currentTemplate.id) ? this.currentTemplate.id : '';
    }
