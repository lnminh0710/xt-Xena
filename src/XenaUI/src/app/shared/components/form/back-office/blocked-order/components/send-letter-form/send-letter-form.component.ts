...
export class SendLetterFormComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    public loadDataForForm() {
        ...
        this.getListOfMandantsByIdSalesOrder(this.showDialogData.selectedItems[0]['IdSalesOrder']);
    }
    ...
    public getSaveData(previewIdBackOfficeLetters?: any) {
        ...
        for (let item of this.groupItems) {
            for (let control of item.Children) {
                let _data = {
                    'IdBackOfficeLetters': ((this.letterTypeCtr && this.letterTypeCtr.selectedValue) ? this.letterTypeCtr.selectedValue : previewIdBackOfficeLetters),
                    'IdBackOfficeLettersGroups': item.IdBackOfficeLettersGroups,
                    'IdBackOfficeLettersGroupsItems': control.IdBackOfficeLettersGroupsItems > 0 ? control.IdBackOfficeLettersGroupsItems : null
                };
                if ((control.TypeOfChoice == 0 ||
                    control.TypeOfChoice == 1) &&
                    control.Checked) {
    ...
    public getPreviewData() {
        ...
        for (let group of this.groupItems) {
            ...
            for (let i = 0; i < group.Children.length; i++) {
                if ((group.Children[i].TypeOfChoice == 0 ||
                    group.Children[i].TypeOfChoice == 1) &&
                    !group.Children[i].Checked) {
                    continue;
                }
                _itemValue += group.Children[i].GroupItemName + ((i === (group.Children.length - 1)) ? '' : '\n');
            }
            if (!_itemValue) {
                continue;
            }
    ...
    public clickRadio(item, groupItem) {
        for (let _item of groupItem.Children) {
            if (_item['TypeOfChoice'] != 0) {
                ...
                _item['Checked'] = (_item['IdBackOfficeLettersGroupsItems'] === item['IdBackOfficeLettersGroupsItems']);
            }
        }
    ...
    private addIdConnection(data: any): any {
        if (this.isSAVWidget) {
            data.BackOfficeLetter[this.showDialogData.data.key] = this.showDialogData.data.value;
            return data;
        }
        data.BackOfficeLetter['IdSalesOrder'] = this.showDialogData.selectedItems[0]['IdSalesOrder'];
        return data;
    }

    private getListOfMandantsByIdSalesOrder(idSalesOrder: any) {
        this._blockedOrderService.getListOfMandantsByIdSalesOrder(idSalesOrder)
    ...
    private getListOfMandantsByIdPerson(idPerson: any) {
        this._blockedOrderService.getListOfMandantsByIdPerson(idPerson)
            .subscribe((response: ApiResultResponse) => {
                this.getMandantDataSuccess(response);
            });
    }
    ...
    private getMandantDataSuccess(response: ApiResultResponse) {
        this._appErrorHandler.executeAction(() => {
            ...
            try { this.mandants = response.item.data[0].map(x => {
                return {
                    'textValue': x.Company,
    ...
    private getLetterType(mandants: string) {
        this._blockedOrderService.getLetterTypeByMandant(mandants)
        .subscribe((response: ApiResultResponse) => {
            this._appErrorHandler.executeAction(() => {
                let data: any = [];
                try {data = response.item.data[0];}catch(e){}
            ...
            return {
                ...
                'Children': x.JSONGroupItems.map(y => {
                    return {
                        ...
                        'IdBackOfficeLettersGroupsItems': y.IdBackOfficeLettersGroupsItems || Uti.getTempId()
                    };
                })
            };
        });
    ...
    private buildDataForReasonGroup(rawData: Array<any>) {
        ...
        for (let raw of rawData) {
            _group = this.groupItems.find(x => x['IdBackOfficeLettersGroups'] == raw.IdBackOfficeLettersGroups);
