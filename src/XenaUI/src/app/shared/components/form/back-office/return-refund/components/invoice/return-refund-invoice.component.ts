...
export class ReturnRefundInvoiceComponent
    ...
    implements OnInit, OnDestroy
{
    ...
    private subscribeSubmitForm() {
        this.resetAllSubscription = this.dispatcher
            .filter((action: CustomAction) => {
                return (
                    action.type === ReturnRefundActions.SUBMIT_FORM &&
                    action.module.idSettingsGUI == this.ofModule.idSettingsGUI
                );
            })
            .subscribe(() => {
    ...
    private initValueForCurrency() {
        if (
            ...
            this.invoiceForm.controls["currency"].setValue(
                this.listComboBox.currency[0].idValue
            );
        }
    }
    ...
    private createOutputModel() {
        ...
        outputData.isDirty =
            this.invoiceForm.dirty || !!this.articleSearchData.gridData.length;
        outputData.isValid =
            this.invoiceForm.valid || !!this.articleSearchData.gridData.length;
        if (this.invoiceForm) {
            outputData.formValue = this.invoiceForm.value;
        }
        outputData.formValue["totalAmount"] =
            this.articleSearchData.totalAmount;
        outputData.formValue["gridData"] = this.articleSearchData.gridData;
        this.store.dispatch(
            this.returnRefundActions.requestUpdateInvoiceNewData(
                outputData,
                this.ofModule
            )
        );
