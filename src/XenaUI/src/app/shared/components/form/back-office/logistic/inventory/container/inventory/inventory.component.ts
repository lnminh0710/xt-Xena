    ...
    implements OnInit, OnDestroy, AfterViewInit
{
        ...
        if (event && event.response && event.response.item) {
            if (event.response.item.message) {
                ...
                this.toasterService.pop(
                    "error",
                    "Failed",
                    event.response.item.message
                );
                return;
            } else if (
                !event.response.item.data ||
                !event.response.item.data.length
            ) {
                this.fileUpload.clearItem();
                this.toasterService.pop(
                    "error",
                    "Failed",
                    "There's no row to import"
                );
                return;
            } else {
                this.importData = event.response.item.data
                    ...
                    .map((item) => {
                        return {
                            ...
                            Warehouse:
                                this.warehouseCtrl.selectedItem.textValue,
                            ArticleNrId: null,
                            ArticleNr: item["articleNr"],
                            QtyWithColor: item["qty"],
                            Notes: null,
                            IdRepIsoCountryCode:
                                this.warehouseCtrl.selectedItem
                                    .idRepIsoCountryCode,
                        };
                    });
                this.close();
    ...
    private setOutputData(data) {
        this.outputData.emit({
            isValid:
                ...
                this.inventoryDataSource.data.length &&
                !this.xnAgGridComponent.hasError(),
    ...
    private initForm() {
        ...
        this.inventoryFormValueChangesSubscription =
            this.inventoryForm.valueChanges
                .debounceTime(this.consts.valueChangeDeboundTimeDefault)
                .subscribe((value) => {
                    this.appErrorHandler.executeAction(() => {
                        this.enabledAddButton = this.checkEnableddAddBtn();
                        this.enabledImportButton =
                            this.checkEnableddImportBtn();
                    });
                });
        this.setOutputData({});
    }
    ...
    private prepareSaveData(data) {
        ...
        data.forEach((item) => {
            result["StockCorrections"].push({
                ...
                ArticleNr: item.ArticleNr,
                AddOnStock:
                    item.QtyWithColor > 0 ? Math.abs(item.QtyWithColor) : 0,
                LessOnStock:
                    item.QtyWithColor < 0 ? Math.abs(item.QtyWithColor) : 0,
    ...
    private processSaveData(data) {
        const processFailed = () => {
            ...
            this.toasterService.pop(
                "error",
                "Failed",
                "Inventory Data cannot be saved"
            );
            this.setOutputData({ submitResult: false });
        };

        this.stockService.saveStockCorrection(data).subscribe(
            (response) => {
                this.appErrorHandler.executeAction(() => {
                    if (
                        response &&
                        ...
                        this.toasterService.pop(
                            "success",
                            "Success",
                            "Inventory Data is saved successfully"
                        );
                        this.setOutputData({
                            submitResult: true,
    ...
    public add() {
        ...
        const existingItem = this.inventoryDataSource.data.find(
            (item) =>
    ...
    private createNewGridItem() {
        ...
        if (
            ...
            item.ArticleNrId =
                this.searchArticleNumberCtl.selectedItem.IdArticle;
    ...
    public onTableEditSuccess(dataItem) {
        ...
        const filterDataItem = this.inventoryDataSource.data.find(
