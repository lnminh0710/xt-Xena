...
import {
    WidgetDetail, ReturnRefundInvoiceNumberModel, FormOutputModel,
    ReturnPaymentModel, ArticleOrder, WidgetKeyType
} from 'app/models';
import { Uti } from 'app/utilities';
import {
    WidgetUtils
} from 'app/shared/components/widget/utils';
...
export class ReturnRefundInvoiceNumberComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        this.refundPaymentDataState = store.select(state => returnRefundReducer.getReturnRefundState(state, this.ofModule.moduleNameTrim).refundPaymentData);
        this.returnPaymentDataState = store.select(state => returnRefundReducer.getReturnRefundState(state, this.ofModule.moduleNameTrim).returnPaymentData);
        this.keepArticleOrdersState = store.select(state => returnRefundReducer.getReturnRefundState(state, this.ofModule.moduleNameTrim).keepArticleOrders);
        this.keepArticleOrdersValidState = store.select(state => returnRefundReducer.getReturnRefundState(state, this.ofModule.moduleNameTrim).keepArticleOrdersValid);
        this.invoiceNewDataState = store.select(state => returnRefundReducer.getReturnRefundState(state, this.ofModule.moduleNameTrim).invoiceNewData);
        this.selectedBackOfficeEntityState = store.select(state => backofficeReducer.getBackofficeState(state, ModuleList.Backoffice.moduleNameTrim).selectedEntity);
    ...
    public ngOnInit() {
        ...
        this.setWidgetDataTypeValues(['IdPerson', 'IdSalesOrder', 'InvoiceNr', 'MEDIACODE', 'IdCountryLanguage']);
        this.store.dispatch(this.returnRefundActions.updateInvoiceNumberData(this.invoiceNumberData, this.ofModule));
        this.subscribe();
    }
    ...
    public ngOnDestroy() {
        ...
        this.store.dispatch(this.returnRefundActions.updateInvoiceNumberData(this.invoiceNumberData, this.ofModule));
        this.store.dispatch(this.widgetDetailActions.clearRequestReload(this.ofModule));
    ...
    private checkToEnableConfirmBtn() {
        ...
        const hasNewInvoice = !!(this.invoiceNewData && this.invoiceNewData.isDirty);

        // this.isEnableConfirmBtn = hasInvoiceNumberData && (hasReturnData || hasRefundData || hasKeepArticleOrders || hasNewInvoice);
        // Apply new behavior
        this.isEnableConfirmBtn = hasInvoiceNumberData && this.keepArticleOrdersValid && (hasKeepArticleOrders || hasNewInvoice);
    ...
    private subscribe() {
        this.refundPaymentDataStateSubscription = this.refundPaymentDataState.subscribe((data: any) => {
        ...
        this.returnPaymentDataStateSubscription = this.returnPaymentDataState.subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.keepArticleOrdersStateSubscription = this.keepArticleOrdersState.subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.keepArticleOrdersValidStateSubscription = this.keepArticleOrdersValidState.subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.invoiceNewDataStateSubscription = this.invoiceNewDataState.subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.requestConfirmSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === ReturnRefundActions.REQUEST_CONFIRM && action.module.idSettingsGUI == this.ofModule.idSettingsGUI;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                this.submit();
            });
        ...
        this.selectedBackOfficeEntityStateSubscription = this.selectedBackOfficeEntityState.subscribe((selectedBackOfficeEntityState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (selectedBackOfficeEntityState && (selectedBackOfficeEntityState['invoiceNr'] || selectedBackOfficeEntityState['InvoiceNr']) && this.invoiceForm) {
                    this.invoiceForm.controls['invoiceNumber'].setValue(selectedBackOfficeEntityState['invoiceNr'] || selectedBackOfficeEntityState['InvoiceNr']);
                    this.go();
                }
            })
        });
        ...
        this.selectedEntityStateSubscription = this.selectedEntityState.subscribe((selectedEntityState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (!this.invoiceForm || !this.invoiceForm.controls || !selectedEntityState)
                    return;
                const invoiceNr = selectedEntityState['invoiceNr'];
                this.invoiceForm.controls['invoiceNumber'].setValue(invoiceNr || '');
            });
        });
    ...
    private getInvoiceNumberDetail(callbackFunc?) {
        ...
        this.returnRefundServiceSubscription = this.returnRefundService.getWidgetInfoByIds(personNr, invoiceNr, mediaCode)
            .subscribe((data: any) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (callbackFunc) {
                        ...
                        if (!data || !data.data || !data.data.length || !data.data[0] || !data.data[0].length || !data.data[0][0]) {
                            this.dontExistSearchData = true;
                            this.resetWidgetIdData();
                        } else {
                            const _data = data.data[0][0];
                            this.dontExistSearchData = (!_data || !_data.IdPerson);
    ...
    public submit() {
        ...
        if (!this.invoiceNumberData || !this.invoiceNumberData.invoiceNr) {
            this.toasterService.pop('warning', 'Warning', 'Please input invoice number');
            return;
        }
        if (!this.isEnableConfirmBtn) {
            this.toasterService.pop('warning', 'Warning', 'Please complete data of each widget');
            return;
        }
        this.saveData();
    }
    ...
    private setWidgetIdData(data: any) {
        if (!data || !data.data || !data.data.length || !data.data[0] || !data.data[0].length || !data.data[0][0]) return;
        data = data.data[0][0];
        ...
        this.store.dispatch(this.widgetDetailActions.requestReload(this.ofModule));
    }
    ...
    private setWidgetDataTypeValue(propertyName: string, data: any) {
        this.widgetUtils.updateWidgetDataTypeValues(this.ofModule.moduleNameTrim, propertyName, data, WidgetKeyType.Main);
    ...
    private resetWidgetIdData() {
        ...
        for (let item of this.listWidgetIds) {
            widgetUtils.updateWidgetDataTypeValues(this.ofModule.moduleNameTrim, item, '', WidgetKeyType.Main);
        }
        WidgetUtils.widgetDataTypeValues[this.ofModule.moduleNameTrim].renderFor = null;
        this.store.dispatch(this.widgetDetailActions.requestReload(this.ofModule));
        this.listWidgetIds = [];
    ...
    private saveData() {
        ...
        this.returnRefundServiceSubscription = this.returnRefundService.saveWidgetData(this.prepareSaveData()).subscribe(response => {
            this.appErrorHandler.executeAction(() => {
                if (response) {
                    if (response.eventType == FormSaveEvenType.Successfully) {
                        this.toasterService.pop('success', 'Success', 'Invoice Data is saved successfully');
                        this.reset();
                        this.store.dispatch(this.returnRefundActions.updateInvoiceNumberData(null, this.ofModule));
                    } else {
                        // log warning for error message from return SQL
                        if (response.sqlStoredMessage) {
                            console.warn(response.sqlStoredMessage);
                        }
                        this.toasterService.pop('error', 'Failed', 'Invoice Data cannot be saved');
                    }
                }
            });
        })
    ...
    private valid(): boolean {
        if (this.refundPaymentData && !this.refundPaymentData.isValid && !this.refundPaymentData.formValue['dontRefund']) {
            this.toasterService.pop('warning', 'Warning', 'Please complete data of the payment form');
            return false;
        }

        if (!this.keepArticleOrdersValid) {
            this.toasterService.pop('warning', 'Warning', 'Please complete data of the Article widget');
            return false;
        }

        if (this.invoiceNewData && this.invoiceNewData.isDirty && !this.invoiceNewData.isValid) {
            this.toasterService.pop('warning', 'Warning', 'Please complete data of the New Invoice widget');
            return false;
        }
        return true;
    }
    ...
    private prepareSaveData() {
        ...
        if (this.returnPaymentData) {
            if (this.returnPaymentData.returnNotes) {
                result.Notes += 'Return Note: ' + this.returnPaymentData.returnNotes + '. ';
            }
            result.IdRepSalesOrderReturnReason = this.returnPaymentData.returnReason;
        }
                ...
                result.ReturnAndRefundOrderPayments = [
                    {
                        ...
                        "ChequeNr": null,
                        "ChequeType": null,
                        "ChequeCreditedDate": isNil(paymentType.year) && isNil(paymentType.month) && isNil(paymentType.day) ?
                            null :
                            paymentType.year + '.' + paymentType.month + '.' + paymentType.day,
        ...
        if (this.invoiceNewData && this.invoiceNewData.formValue && this.invoiceNewData.isDirty) {
            result['NewInvoiceAmount'] = this.invoiceNewData.formValue.amount;
            result['NewInvoiceTotalAmount'] = this.invoiceNewData.formValue.totalAmount;
            result['NewInvoiceCurrency'] = this.invoiceNewData.formValue.currency;
            result['NewInvoiceFreeShipping'] = this.invoiceNewData.formValue.isFreeShipping ? '1' : '0';
            result['NewInvoiceItemData'] = (this.invoiceNewData.formValue.gridData && this.invoiceNewData.formValue.gridData.length)
    ...
    public reset() {
        ...
        this.resetWidgetIdData();
        this.store.dispatch(this.returnRefundActions.updateInvoiceNumberData(null, this.ofModule));
        this.store.dispatch(this.returnRefundActions.resetAllEditableForm(this.ofModule));
        this.invoiceNumberData = new ReturnRefundInvoiceNumberModel();
        this.store.dispatch(this.returnRefundActions.updateInvoiceNumberData(this.invoiceNumberData, this.ofModule));
    }
