  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  private subscribeWarehouseMovementRequestConfirmAllState() {
    this.warehouseMovementRequestConfirmAllStateSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === WarehouseMovementActions.REQUEST_CONFIRM_ALL &&
          action.module.idSettingsGUI == this.ofModule.idSettingsGUI
        );
      })
  ...
  private subscribeWarehouseMovementSaveAndConfirmState() {
    this.warehouseMovementRequestConfirmAllStateSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === WarehouseMovementActions.REQUEST_SAVE_AND_CONFIRM &&
          action.module.idSettingsGUI == this.ofModule.idSettingsGUI
        );
      })
      .subscribe((simpleTabID: any) => {
  ...
  private isAllowQuantity(inputQuantity = 0) {
    ...
    const totalQuantiry = this.stockedArticleGrid.data
      .filter(
        (x) => x.IdWarehouseMovementGoodsIssue === idWarehouseMovementGoodsIssue
      )
      .reduce((acc, value) => {
        return (
    ...
    this.getSortingGoodSubscription = this.wareHouseMovementService
      ...
      .subscribe((response: ApiResultResponse) => {
        this.appErrorHandler.executeAction(() => {
          ...
          let gridData = this.datatableService.formatDataTableFromRawData(
            response.item.data
          );
    ...
    this.getStockedArticlesSubscription = this.wareHouseMovementService
      ...
      .subscribe((response: ApiResultResponse) => {
        this.appErrorHandler.executeAction(() => {
          ...
          let gridData: any = this.datatableService.formatDataTableFromRawData(
            response.item.data
          );
          gridData = this.datatableService.buildDataSource(gridData);
