import {
    Component, Input, Output, OnInit, OnDestroy, AfterViewInit, EventEmitter,ViewChild
} from '@angular/core';
...
export class PurchaseFormArticleComponent implements OnInit, OnDestroy, AfterViewInit {
    @Output() outputData: EventEmitter<any> = new EventEmitter();
    @Output() saveData: EventEmitter<any> = new EventEmitter();
    @Output() reloadData: EventEmitter<any> = new EventEmitter();

    @ViewChild('compositionGridLeft') private compositionGridLeft: WijmoGridComponent;
    @ViewChild('compositionGridRight') private compositionGridRight: WijmoGridComponent;
    ...
    private getArticle():void {
        this.articlesSubscription = this.campaignService.getCampaignArticle(null)
            .subscribe(
            (response) => {
                this.appErrorHandler.executeAction(() => {
                    let rawData = this.datatableService.formatDataTableFromRawData(response.item.data);                    
    ...
    private onSearchArticleComplete(response: ApiResultResponse):void {
        ...
        const notExistColumns = ['articlemanufacturersnr', 'idsalescampaignarticle', 'quantityset', 'idsalescampaignwizard'];
        const newData: Array<any> = [];
        for (const res of results) {
            const dataObj: any = {};            
            for (const col of this.compositionDataLeft.columns) {
                if (notExistColumns.indexOf(col.data.toString().toLowerCase()) > -1) {
                    dataObj[col.data] = (col.data.substring(0, 2) === 'id') ? Uti.getTempId() : '';
                } else {
    ...
    private addArticleItem(controlGridModel: ControlGridModel): ControlGridModel {
        controlGridModel.data.push(cloneDeep(this.selectItem));
        return Uti.cloneDataForGridItems(controlGridModel);
    }
    public removeArticleItem(controlGridModel: ControlGridModel): ControlGridModel {
        Uti.removeItemInArray(controlGridModel.data, this.selectItem, 'IdArticle');
    ...
    public addComposition(event?: any): void {
        this.compositionDataRight = this.addArticleItem(this.compositionDataRight);
        this.compositionDataLeft = this.removeArticleItem(this.compositionDataLeft);
        this.resetRowIdForGrid();
    }

    private addAllArticleItem(controlGridModel: ControlGridModel): ControlGridModel {
        controlGridModel.data = controlGridModel.data.concat(cloneDeep(this.selectItems));
        return Uti.cloneDataForGridItems(controlGridModel);
    }

    private removeAllArticleItem(controlGridModel: ControlGridModel): ControlGridModel {
    ...
    public addAllComposition(event?: any): void {
        this.compositionDataRight = this.addAllArticleItem(this.compositionDataRight);
        this.compositionDataLeft = this.removeAllArticleItem(this.compositionDataLeft);
        this.resetRowIdForGrid();
    }

    public removeComposition(event?: any): void {
        this.compositionDataLeft = this.addArticleItem(this.compositionDataLeft);
        this.compositionDataRight = this.removeArticleItem(this.compositionDataRight);
        this.resetRowIdForGrid();
    }

    public removeAllComposition(event?: any): void {
        this.compositionDataLeft = this.addAllArticleItem(this.compositionDataLeft);
        this.compositionDataRight = this.removeAllArticleItem(this.compositionDataRight);
        this.resetRowIdForGrid();
    }
    ...
    public onLeftTableSearch($event?: any):void {
        this.searchArticleSubscription = this.searchService.search('article', $event, null, 1, 9999)            
