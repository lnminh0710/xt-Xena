    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    private checkIsActiveArticle() {
        const isActiveArticle = this.dataSource.data.filter(
            (x) => x.IsActive == 1
        );
        let checkSum = true;
        ...
        for (const v of isActiveArticle) {
            ...
            if (!checkSum) {
                this.isEnableConfirmBtn =
                    isActiveArticle.length > 0 && checkSum;
    ...
    private disabledParent(rowData: any) {
        for (const item of this.dataSource.data) {
            if (
                item["IdSalesOrderArticles"] !=
                rowData["ParentIdSalesOrderArticles"]
            )
                continue;
            item["IsActive"] = 0;
            break;
    ...
    private lookingForChildren(rowData: any, keyData) {
        for (const item of this.dataSource.data) {
            if (
                item["ParentIdSalesOrderArticles"] !=
                rowData["IdSalesOrderArticles"]
            )
                continue;
            if (keyData === ArticlesInvoiceQuantity.QtyKeep) {
    ...
    private initArticlOrder(item) {
        if (item) {
            return new ArticleOrder({
                articleNumber:
                    item[ArticleOrdersComponent.ARTICLE_NR_COLUMN_NAME],
                description:
                    item[ArticleOrdersComponent.DESCRIPTION_COLUMN_NAME],
                ...
                idSalesOrder:
                    item[ArticleOrdersComponent.ID_SALE_ORDER_COLUMN_NAME],
                idSalesOrderArticles:
                    item[
                        ArticleOrdersComponent.ID_SALE_ORDER_ARTICLE_COLUMN_NAME
                    ],
            });
        }
        return null;
