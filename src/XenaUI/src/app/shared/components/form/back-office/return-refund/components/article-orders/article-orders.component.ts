import {
    ...
    OnDestroy, AfterViewInit, ElementRef, ViewChild, Output, EventEmitter, ChangeDetectorRef
} from '@angular/core';
import {
    ArticleOrder, Module, ReturnRefundInvoiceNumberModel
} from 'app/models';
...
export class ArticleOrdersComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
     ...
     */
    ngOnDestroy() {
        this.store.dispatch(this.returnRefundActions.clearKeepArticleOrders(this.ofModule));
        this.store.dispatch(this.returnRefundActions.clearAllEditArticleOrders(this.ofModule));
    }
    ...
    private checkIsActiveArticle() {
        ...
        for (const v of isActiveArticle) {
            const sum = toNumber(v[ArticlesInvoiceQuantity.QtyKeep]) + toNumber(v[ArticlesInvoiceQuantity.QtyBackToWareHouse]) + toNumber(v[ArticlesInvoiceQuantity.QtyDefect]);
            ...
            if (!checkSum) {
                ...
                if (this.invoiceNumberData) {
                    ...
                    this.store.dispatch(this.returnRefundActions.setKeepArticleOrdersValid(false, this.ofModule));
                    setTimeout(() => {
        ...
        if (this.invoiceNumberData) {
            this.invoiceNumberData.enableConfirmButton = this.isEnableConfirmBtn;
            this.store.dispatch(this.returnRefundActions.setKeepArticleOrdersValid(this.isEnableConfirmBtn, this.ofModule));
            setTimeout(() => {
                this.store.dispatch(this.returnRefundActions.updateInvoiceNumberData(this.invoiceNumberData, this.ofModule));
            });
        }
    ...
    private onDisabledForRow(rowData) {
        ...
        const dataHasChildren = this.dataSource.data.filter(x => x.children.length > 0);
    ...
    private findAllParentDisabled(dataHasChildren, rowData) {
        for (const v of dataHasChildren) {
            if ((v[ArticlesInvoiceQuantity.QtyKeep] || v[ArticlesInvoiceQuantity.QtyBackToWareHouse] || v[ArticlesInvoiceQuantity.QtyDefect])) {
                this.disabledAllChildren(rowData);
            }
            if ((!v[ArticlesInvoiceQuantity.QtyKeep] && !v[ArticlesInvoiceQuantity.QtyBackToWareHouse] && !v[ArticlesInvoiceQuantity.QtyDefect]) && v.children && v.children.length > 0) {
    ...
    private getChildListByParentId(parentId: any): any[] {
        const childList = [];
        let tempData = this.dataSource.data.filter(x => x['ParentIdSalesOrderArticles'] === parentId);
        while (!!tempData && tempData.length > 0) {
            ...
            tempData = this.dataSource.data.filter(x => childList.includes(x['ParentIdSalesOrderArticles']) && !childList.includes(x['IdSalesOrderArticles']));
        }
        return childList;
    }
    ...
    private disabledParent(rowData: any) {
        for (const item of this.dataSource.data) {
            if (item['IdSalesOrderArticles'] != rowData['ParentIdSalesOrderArticles']) continue;
            item['IsActive'] = 0;
            break;
        }
    }
    ...
    private enabledAllChildren(rowData: any) {
        ...
        for (const item of this.dataSource.data) {
            if (!childList.includes(item['IdSalesOrderArticles'])) continue;
            if (!rowData[ArticlesInvoiceQuantity.QtyKeep] && !rowData[ArticlesInvoiceQuantity.QtyBackToWareHouse] && !rowData[ArticlesInvoiceQuantity.QtyDefect])
                item['IsActive'] = 1;
        }
    }
    ...
    private lookingForChildren(rowData: any, keyData) {
        for (const item of this.dataSource.data) {
            if (item['ParentIdSalesOrderArticles'] != rowData['IdSalesOrderArticles']) continue;
            if (keyData === ArticlesInvoiceQuantity.QtyKeep) {
    ...
    private rowValidation(item: any) {
        ...
        const inputQuantity = toNumber(keepQuantity) + toNumber(backQuantity) + toNumber(defectQuantity);
    ...
    reset() {
        ...
        this.store.dispatch(this.returnRefundActions.setKeepArticleOrdersValid(true, this.ofModule));
    }
    ...
    private initArticlOrder(item) {
        if (item) {
            return new ArticleOrder({
                ...
                quantity: toSafeInteger(item[ArticleOrdersComponent.QUANTITY_COLUMN_NAME]),
                price: toSafeInteger(item[ArticleOrdersComponent.UNIT_PRICE_COLUMN_NAME]),
                total: toSafeInteger(item[ArticleOrdersComponent.TOTAL_PRICE_COLUMN_NAME]),
                ...
                idSalesOrderArticles: item[ArticleOrdersComponent.ID_SALE_ORDER_ARTICLE_COLUMN_NAME]
            });
        }
        return null;
    }
    ...
    private dispatchKeepOrders() {
        if (this.dataSource && this.dataSource.data && this.dataSource.data.length) {
            ...
            this.store.dispatch(this.returnRefundActions.setKeepArticleOrders(articleOrders, this.ofModule));
        }
    }
    ...
    private dispatchAllEditOrders() {
        if (this.dataSource && this.dataSource.data.length && this.dataSource.data.length) {
            const items: Array<any> = [];
            ...
            this.store.dispatch(this.returnRefundActions.setAllEditArticleOrders(articleOrders, this.ofModule));
        }
    }
}
