...
import { AppErrorHandler, BackOfficeService, CommonService } from 'app/services';
import { ControlFocusComponent, DataEntryPaymentTypeItemComponent } from 'app/shared/components/form';
...
export class RefundPaymentFormComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    public ngOnDestroy() {
        this.store.dispatch(this.returnRefundActions.clearRefundPayment(this.ofModule));
    ...
    public dontRefundChanged() {
        if (!this.refundPaymentForm ||
            !this.refundPaymentForm.value ||
            !this.refundPaymentForm.controls ||
            !this.paymentItem) return;
        if (this.refundPaymentForm.value['dontRefund']) {
            Uti.resetValueForForm(this.refundPaymentForm, ['dontRefund', 'reason'], ['leftCharacters']);
    ...
    private setOutputData(submitResult: any, data?: any) {
        ...
        this.store.dispatch(this.returnRefundActions.setRefundPayment(this.outputModel, this.ofModule));
    }
    ...
    public updateLeftCharacters(event) {
        setTimeout(() => {
            this.refundPaymentForm['leftCharacters'] = this.consts.noteLengthDefault - event.target.value.length;
        });
    }
    ...
    private getListComboBox(){
        ...
        this.commonService.getListComboBox('' + ComboBoxTypeConstant.currency)
