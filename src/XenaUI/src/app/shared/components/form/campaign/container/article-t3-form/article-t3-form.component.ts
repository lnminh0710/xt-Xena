  ...
  implements OnInit, OnDestroy
{
  ...
  private search() {
    ...
    this.searchServiceSubscription = this.searchService
      .search('article', this.keyword, null, this.pageIndex, this.pageSize)
      .finally(() => {
  ...
  private subscription() {
    ...
    this.translationSubscription = this.dispatcher
      .filter((action: CustomAction) => {
        return (
          action.type === ProcessDataActions.OPEN_TRANSLATION_DIALOG &&
          action.module.idSettingsGUI == this.ofModule.idSettingsGUI
        );
      })
      .subscribe(() => {
    ...
    this.campaignServiceSubscription = this.campaignService
      ...
      .subscribe((response) => {
        this.appErrorHandler.executeAction(() => {
          ...
          if (this.campaignGridBottom) this.campaignGridBottom.refresh();
        });
      });
  }
  ...
  private getDetailGrid($event: any) {
    ...
    if (this.articleSelectedWidget) this.articleSelectedWidget.rowData = $event;
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      if (translateEditedDatas && !translateEditedDatas.length) {
        ...
        translateEditedDatas.forEach((translateEditedData: Array<any>) => {
          this.prepareDataForColTranslateSaving(translateEditedData).subscribe(
            (saveData) => {
              this.appErrorHandler.executeAction(() => {
                if (
                  !saveData ||
