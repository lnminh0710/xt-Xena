    ...
    implements OnInit, OnDestroy
{
    ...
    public addCampaign() {
        ...
        this.showDialogConfimation(
            this.addCampaignAfterConfirmation.bind(this)
        );
    }
    ...
    public addCampaignAfterConfirmation() {
        ...
        if (this.campaignDialogModel.daysToWait) {
            currentSeletedItemLeft.DaysToWait =
        ...
        this.campaignServiceSubscription = this.campaignService
            ...
            .subscribe((response) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.campaignDataLeft.columns =
                        this.makeCampaignDataLeftColumn(
                            this.campaignDataBottom.columns
                        );
                    this.currentcampaignDataBottom = cloneDeep(
                        this.campaignDataBottom
                    );
                    this.campaignDataBottom = this.datatableService.appendRowId(
                        this.campaignDataBottom
                    ...
                    this.campaignDataLeft = {
                        data: [],
                        columns: this.datatableService.appendRowId(
                            this.campaignDataLeft
                        ).columns,
                    };
    ...
    private makeAddedAndEdittedBottomData(): Array<any> {
        ...
        for (let item of this.cachedCampaignDataBottom) {
            if (item.isNew) {
                ...
                result = [
                    ...result,
                    ...this.makeAddOrEditRightData(item, true),
                ];
            }
        }
        return result;
        ...
        for (let item of currentData.detailData.data) {
            if (item.BorderStatus != "1" && (!isEdited || item.isEdited)) {
                let updateData = {
                    ...
                    DaysToWait:
                        typeof item.DaysToWait == "object" &&
                        isEmpty(item.DaysToWait)
                            ? null
                            : item.DaysToWait,
                    IsActive:
                        typeof item.IsActive == "object" &&
                        isEmpty(item.IsActive)
                            ? false
                            : !!item.IsActive,
                };
                if (isEdited) {
    ...
    private updateIsChangedDataForBottomCachedItem(rightSelectedItem?: any) {
        rightSelectedItem =
            rightSelectedItem || this.currentSelectedItemBottom();
    ...
    private isDataCachedAndReGetCachedData(
        idSalesCampaignTracks: any
    ): boolean {
        let currentItem = this.cachedCampaignDataBottom.find(
            (x) =>
                x.itemData &&
                x.itemData.IdSalesCampaignTracks === idSalesCampaignTracks
        );
        if (
            currentItem &&
            currentItem.itemData &&
    ...
    public daysToWaitChange($event: any) {
        setTimeout(() => {
            this.daysToWaitValid =
                this.campaignDialogModel.daysToWait <= 9999999;
