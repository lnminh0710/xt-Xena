...
import {
    AppErrorHandler, SearchService, ModalService,
    DatatableService, CampaignService, PropertyPanelService
} from 'app/services';
import {
    SalesCampaignType,
    Configuration
...
export class CampaignInterReTakesFormComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    public addCampaignAfterConfirmation() {
        ...
        this.campaignDataBottom = this.addItemCampaign(this.campaignDataBottom, currentSeletedItemLeft);
        this.campaignDataLeft = this.removeItemCampaign(this.campaignDataLeft, this.currentSelectedItemLeft());
        this.resetRowIdForGrid();
        this.focusBottomLastItem();
    }

    public removeCampaign() {
        if (isEmpty(this.currentSelectedItemBottom())) { return; }
        this.removeItemInCached();
        // this.campaignDataLeft = this.addItemCampaign(this.campaignDataLeft, this.currentSelectedItemBottom());
        this.campaignDataBottom = this.removeItemCampaign(this.campaignDataBottom, this.currentSelectedItemBottom());
        this.resetRowIdForGrid();
        this.search();
    }
    ...
    public removeAllCampaign() {
        ...
        this.modalService.confirmMessageHtmlContent(new MessageModel({
            message: [{key: '<p>'}, {key: 'Modal_Message__Are_You_Sure_You_Want_To_Remove_All_Campaigns'},
                {key: '</p>'}],
    ...
    private getRightItem(leftSelectItem: any): any {
        let cachedItem = this.currentcampaignDataBottom.data.find(x => x.CampaignNr == leftSelectItem.CampaignNr);
    ...
    private resetRowIdForGrid() {
        this.campaignDataLeft = this.datatableService.appendRowId(this.campaignDataLeft);
        this.campaignDataBottom = this.datatableService.appendRowId(this.campaignDataBottom);
        this.campaignDataRight = this.datatableService.appendRowId(this.campaignDataRight);
        this.ref.detectChanges();
    }

    private removeItemInCached() {
        Uti.removeItemInArray(this.cachedCampaignDataBottom, {
            idSalesCampaignArticle: this.currentSelectedItemBottom()['IdSalesCampaignArticle']
        }, 'idSalesCampaignArticle')
    }

    private addItemCampaign(controlGridModel: ControlGridModel, selectItem: any): ControlGridModel {
        controlGridModel.data.push(cloneDeep(selectItem));
        return Uti.cloneDataForGridItems(controlGridModel);
    }

    private removeItemCampaign(controlGridModel: ControlGridModel, selectItem: any): ControlGridModel {
        Uti.removeItemInArray(controlGridModel.data, selectItem, 'IdSalesCampaignTracks');
    ...
    private focusBottomLastItem() {
        ...
        setTimeout(() => {
            if (this.campaignGridBottom) {
                this.campaignGridBottom.selectRowIndex(this.campaignDataBottom.data.length - 1, true);
            }
        });
    }
    ...
    private subcribtion() {
            ...
            this.appErrorHandler.executeAction(() => {
                if (selectedEntityState) {
                    ...
                    if (selectedEntityState.idSalesCampaignWizard) {
                        ...
                        if (!selectedEntityState.selectedParkedItem || !selectedEntityState.selectedParkedItem.campaignNr) return;
                        this.currentCampaignNumber = selectedEntityState.selectedParkedItem.campaignNr.value;
                    }
                }
            });
        });
        ...
        this.bottomRowClickSubscription = this.bottomRowClickSubject.debounceTime(250).subscribe(event => {
    ...
    private getCampaignForRight() {
        this.campaignServiceSubscription = this.campaignService.getCampaignTracks(this.idSalesCampaignWizard)
            .subscribe(
                (response) => {
                    this.appErrorHandler.executeAction(() => {
                        response = this.datatableService.formatDataTableFromRawData(response.item.data);
                        this.campaignDataBottom = this.datatableService.buildDataSource(response);
                        this.campaignDataLeft.columns = this.makeCampaignDataLeftColumn(this.campaignDataBottom.columns);
                        this.currentcampaignDataBottom = cloneDeep(this.campaignDataBottom);
                        this.campaignDataBottom = this.datatableService.appendRowId(this.campaignDataBottom);
                        this.campaignDataLeft = {
                            data: [],
                            columns: this.datatableService.appendRowId(this.campaignDataLeft).columns
                        };
    ...
    private makeCampaignDataLeftColumn(columns: any): any {
        ...
        const description = result.find(x => x.data.toLowerCase() === 'descriptions');
        description.setting.Setting = description.setting.Setting || [];
        description.setting.Setting[0] = description.setting.Setting[0] || {};
        description.setting.Setting[0] = description.setting.Setting[0] || {};
        description.setting.Setting[0].DisplayField = description.setting.Setting[0].DisplayField || {};
        description.setting.Setting[0].DisplayField.Hidden = '1';
        return result;
    }

    private getCampaignForDetail(idSalesCampaignTracks: any, idSalesCampaignWizard?: any, idSalesCampaignWizardTrack?: any) {
        this.isRenderCompleted = false;
        ...
        this.campaignServiceSubscription = this.campaignService.getCampaignTracksCountries(idSalesCampaignTracks < 0 ? 0 : idSalesCampaignTracks, idSalesCampaignWizard, idSalesCampaignWizardTrack)
            .subscribe(
                (response) => {
                    this.appErrorHandler.executeAction(() => {
                        response = this.datatableService.formatDataTableFromRawData(response.item.data);
                        ...
                        if (!idSalesCampaignTracks && !idSalesCampaignWizard && !idSalesCampaignWizardTrack) return;
    ...
    private makeTempIdForBottomGrid() {
        for (const item of this.campaignDataRight.data) {
            item.IdSalesCampaignTracksCountrys = item.IdSalesCampaignTracksCountrys ? item.IdSalesCampaignTracksCountrys : Uti.getTempId();
        }
    }

    private addBottomCachedItem() {
        this.isNewItem = this.currentSelectedItemBottom()['IdSalesCampaignTracks'] < 0;
    ...
    private hasDataAdded(): boolean {
        const addItems = Uti.getItemsDontExistItems(this.campaignDataBottom.data, this.currentcampaignDataBottom.data, 'IdSalesCampaignTracks');
        return !!addItems && !!addItems.length;
    }

    private makeDeleteData(): Array<any> {
        const deleteData = Uti.getItemsDontExistItems(this.currentcampaignDataBottom.data, this.campaignDataBottom.data, 'IdSalesCampaignTracks');
    ...
    private makeAddOrEditRightData(currentData: ChangingModel, isEdited?: boolean): Array<any> {
        let updateItem = {
            Descriptions: currentData.itemData.Descriptions || '',
            DaysToWait: currentData.itemData.DaysToWait,
            IsActive: true,
            MasterToIdSalesCampaignWizard: currentData.itemData.IdSalesCampaignWizard_Master,
            ChainToIdSalesCampaignWizard: currentData.itemData.IdSalesCampaignWizard_Chain,
            IdRepSalesCampaignType: currentData.itemData.IdRepSalesCampaignType,
            JsonCampagneTracksCountries: JSON.stringify({ CampagneTracksCountries: this.makeAddOrEditDetailData(currentData, isEdited) })
        };
        if (isEdited) {
            updateItem['IdSalesCampaignTracks'] = currentData.itemData.IdSalesCampaignTracks;
        }
        return [updateItem];
    }

    private makeAddOrEditDetailData(currentData: ChangingModel, isEdited?: boolean): Array<any> {
        let result: Array<any> = [];
        for (let item of currentData.detailData.data) {
            if (item.BorderStatus != '1' && (!isEdited || item.isEdited)) {
                let updateData = {
                    IdSalesCampaignWizardItems: (typeof item.IdSalesCampaignWizardItems == 'object' && isEmpty(item.IdSalesCampaignWizardItems)) ? null : item.IdSalesCampaignWizardItems,
                    DaysToWait: (typeof item.DaysToWait == 'object' && isEmpty(item.DaysToWait)) ? null : item.DaysToWait,
                    IsActive: (typeof item.IsActive == 'object' && isEmpty(item.IsActive)) ? false : !!item.IsActive,
                }
                if (isEdited) {
                    updateData['IdSalesCampaignTracksCountrys'] = (typeof item.IdSalesCampaignTracksCountrys == 'object' && isEmpty(item.IdSalesCampaignTracksCountrys)) ? null : item.IdSalesCampaignTracksCountrys;
                }
    ...
    private udpateCampaign() {
        this.campaignServiceSubscription = this.campaignService.saveCampaignTracks({ 'CampaignTracks': this.saveData })
            .subscribe(
                (data) => {
                    this.appErrorHandler.executeAction(() => {
                        this.setOutputData(null, {
    ...
    private updateCachedItemData() {
        let currentCachedItem = this.cachedCampaignDataBottom.find(x => x.itemData['IdSalesCampaignTracks'] === this.currentSelectedItemBottom()['IdSalesCampaignTracks']);
    ...
    private onSearchComplete(response: ApiResultResponse) {
        ...
        if (results) {
            for (const res of results) {
                ...
                if (res.campaignNr.toLowerCase() == ((this.currentCampaignNumber || '').toLowerCase())
        ...
        this.filterLeftDataOnSearchComplete(newData, { chainValue: this.chainValue });
        this.campaignGridLeft.isSearching = false;
    }

    private filterLeftDataOnSearchComplete(itemList: Array<any>, filterParam: any) {
        const newData = filter(itemList, function (item) {
            return ((filterParam.chainValue == 'all') || (item.DefaultValue && item.DefaultValue.toLowerCase() == filterParam.chainValue));
        });
        let leftData = {
            data: newData,
            columns: this.campaignDataLeft.columns,
    ...
    private makeGridSearchColumnTypeId(res: any): any {
        if ((typeof res.isMaster === 'string' && res.isMaster === 'True') || (typeof res.isMaster === 'boolean' && res.isMaster)) {
            return SalesCampaignType.Master;
        }
        if ((typeof res.isAsile === 'string' && res.isAsile === 'True') || (typeof res.isAsile === 'boolean' && res.isAsile)) {
            return SalesCampaignType.Asile;
        }
        if ((typeof res.isInter === 'string' && res.isInter === 'True') || (typeof res.isInter === 'boolean' && res.isInter)) {
            return SalesCampaignType.Inter;
        }
        if ((typeof res.isTrack === 'string' && res.isTrack === 'True') || (typeof res.isTrack === 'boolean' && res.isTrack)) {
    ...
    private checkExistPrimaryItem(campaignNr: any): boolean {
        if (!this.campaignDataBottom ||
            !this.campaignDataBottom.data || !this.campaignDataBottom.data.length)
            return false;
        const articleItem = this.campaignDataBottom.data.find(x => x.CampaignNr == campaignNr);
        return (articleItem && articleItem.CampaignNr);
    }

    private getDetailGrid($event: any) {
        if (!$event) return;
        this.getCampaignForDetail(Uti.getValueFromArrayByKey($event, 'IdSalesCampaignTracks'), Uti.getValueFromArrayByKey($event, 'IdSalesCampaignWizard_Master'), Uti.getValueFromArrayByKey($event, 'IdSalesCampaignWizard_Chain'));
    ...
    private updateIsChangedDataForBottomCachedItem(rightSelectedItem?: any) {
        ...
        const currentItem = this.cachedCampaignDataBottom.find(x => x.itemData && x.itemData.IdSalesCampaignTracks === rightSelectedItem['IdSalesCampaignTracks']);
        if (!currentItem || !currentItem.itemData || !currentItem.itemData.IdSalesCampaignTracks) return;
        currentItem.isChanged = true;

    }

    private isDataCachedAndReGetCachedData(idSalesCampaignTracks: any): boolean {
        let currentItem = this.cachedCampaignDataBottom.find(x => x.itemData && x.itemData.IdSalesCampaignTracks === idSalesCampaignTracks);
        if (currentItem && currentItem.itemData && currentItem.itemData.IdSalesCampaignTracks) {
    ...
    public dragEnd(event: any) {
        setTimeout(() => {
            if (this.splitContainerComponent && this.splitContainerComponent.areas && this.splitContainerComponent.areas.length >= 2) {
                if (this.splitContainerComponent.areas[0].size < 30) {
                    this.splitContainerComponent.updateArea(this.splitContainerComponent.areas[0].component, 0, 50, 50);
                }
            }
        }, 200);
    }
    ...
    public radioChecked(selectedValue: string): void {
        ...
        this.filterLeftDataOnSearchComplete(this.shadowCampaignDataLeft, { chainValue: this.chainValue });
    }
    ...
    private currentSelectedItemBottom(): any {
        return Uti.mapArrayToObject((this.campaignGridBottom.selectedItem() || []), true);
    }
}
