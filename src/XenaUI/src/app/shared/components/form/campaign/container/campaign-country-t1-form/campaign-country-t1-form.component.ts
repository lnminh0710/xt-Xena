...
export class CampaignCountryT1FormComponent
    ...
    implements OnInit, OnDestroy
{
    ...
    public itemFormatterShippingAddress =
        this.customItemFormatterShippingAddress.bind(this);
    ...
    @ViewChild("controlFocusComponent")
    ...
    constructor(
        ...
        this.supportCampaignNumberPrefix =
            Configuration.PublicSettings.supportCampaignNumberPrefix;
    }
    ...
    public campaignTypeChanged() {
        if (this.campaignCountryForm.value.campaignType === "master") {
            if (!this.campaignCountryForm.controls["postageDate"].errors) {
                this.campaignCountryForm.controls["postageDate"].setValidators(
                    Validators.required
                );
                if (!this.campaignCountryForm.controls["postageDate"].value)
                    this.campaignCountryForm.controls["postageDate"].setErrors({
                        required: true,
                    });
    ...
    private loadData() {
                    ...
                    this.appErrorHandler.executeAction(() => {
                        ...
                        if (response.item["collectionData"]) {
                            ...
                            if (this.isEditMode && !this.isClone) {
                                this.campaignCountryForm.controls[
                                    "campaignMode"
                                ].clearValidators();
                                this.campaignCountryForm.controls[
                                    "campaignMode"
        ...
        this.campaignServiceSubscription = this.campaignService
            .getCampaignWizardCountry(this.idSalesCampaignWizard)
            .subscribe((rs) => {
                this.appErrorHandler.executeAction(() => {
                    if (rs && rs.item) {
                        this.countryCheckListData = this.getValidCombobox(
                            rs.item,
                            ComboBoxTypeConstant.countryCode
                        );
                        if (this.idSalesCampaignWizard) {
                            this.countryEditListData =
                                this.countryCheckListData.filter(
                                    (p) => p.isActive == "1"
                                );
                            if (this.countryEditListData) {
                                this.countryEditListData =
                                    this.countryEditListData.map((p) => {
                                        return {
                                            IdCountrylanguage: +p.idValue,
                                            IsActive: true,
                                            IdSalesCampaignWizardItems:
                                                +p.idSalesCampaignWizardItems,
                                        };
                                    });
                            }
                        }
        ...
        this.commonServiceSubscription = this.commonService
            ...
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.mandantList = this.getValidCombobox(
                        response.item,
                        ComboBoxTypeConstant.allMandant
                    );
                    this.serviceProviderList = this.getValidCombobox(
                        response.item,
                        ComboBoxTypeConstant.serviceProvider
                    );
                    this.wareHouseList = this.getValidCombobox(
                        response.item,
                        ComboBoxTypeConstant.wareHouse
                    );
                    this.campaignGroupList = this.getValidCombobox(
                        response.item,
                        ComboBoxTypeConstant.campaignGroup
                    );

                    if (this.supportCampaignNumberPrefix) {
                        this.campaignNumberList = this.getValidCombobox(
                            response.item,
                            ComboBoxTypeConstant.campaignNamePrefix
                        );
                    }
    ...
    private createFormGroup() {
        ...
        if (this.supportCampaignNumberPrefix) {
            this.campaignCountryForm.setControl(
                "campaignNumberPrefix",
    ...
    private buildBooleanRadio(data, fieldName) {
        if (data[fieldName]) {
            if (data[fieldName].value == "True") return true;
            if (data[fieldName].value == "False") return false;
        }
    ...
    private createFormGroupWithData(data: any) {
        ...
        if (data.postageDate.value) {
            try {
                postageDate = Uti.getUTCDate(
                    parse(data.postageDate.value, "dd.MM.yyyy", new Date())
                );
            } catch (ex) {
                postageDate = "";
            }
        ...
        if (this.campaignCountryForm && this.campaignCountryForm.controls) {
            this.campaignCountryForm.controls["campaignMode"].setValue(
                "newCampaign"
            );
            this.campaignCountryForm.controls["campaignType"].setValue(
                campaignType
            );
            this.campaignCountryForm.controls["campaignName"].setValue(
                data.campaignName.value
            );
            this.campaignCountryForm.controls["campaignGroup"].setValue(
                data.idSharingTreeGroups.value
            );
            this.campaignCountryForm.controls["mandant"].setValue(
                data.idPersonToMandant.value
            );
            this.campaignCountryForm.controls["serviceProvider"].setValue(
                data.idPersonToServiceProvider.value
            );
            this.campaignCountryForm.controls["wareHouse"].setValue(
                data.idPersonToWarehouse.value
            );
            this.campaignCountryForm.controls["shippingAddress"].setValue(
                data.idPersonInterfaceContactAddressGWToShippingAddress.value
            );
            this.campaignCountryForm.controls["returnAddress"].setValue(
                data.idPersonInterfaceContactAddressGWToReturnAddress.value
            );
            this.campaignCountryForm.controls["postageDate"].setValue(
                postageDate
            );
            this.campaignCountryForm.controls["postageCost"].setValue(
                this.buildBooleanRadio(data, "isWithPostageCost")
            );
            this.campaignCountryForm.controls["selection"].setValue(
                this.buildBooleanRadio(data, "isWithSelection")
            );
            this.campaignCountryForm.controls["collate"].setValue(
                this.buildBooleanRadio(data, "isWithCollate")
            );
            this.campaignCountryForm.controls["gift"].setValue(
                this.buildBooleanRadio(data, "isWithGift")
            );
            this.campaignCountryForm.controls["mail"].setValue(
                this.buildBooleanRadio(data, "isWithWhitemail")
            );
            this.campaignCountryForm.controls["track"].setValue(
                this.buildBooleanRadio(data, "isWithTrack")
            );
            this.campaignCountryForm.controls["inter"].setValue(
                this.buildBooleanRadio(data, "isWithInter")
            );
            this.campaignCountryForm.controls["asile"].setValue(
                this.buildBooleanRadio(data, "isWithAsile")
            );
            this.campaignCountryForm.controls["catalog"].setValue(
                this.buildBooleanRadio(data, "isWithCatalog")
            );
            this.campaignCountryForm.controls["birthday"].setValue(
                this.buildBooleanRadio(data, "isWithBirthday")
            );
            if (
                this.supportCampaignNumberPrefix &&
                this.campaignCountryForm.controls["campaignNumberPrefix"]
            ) {
                this.campaignCountryForm.controls[
                    "campaignNumberPrefix"
                ].setValue(data.idRepSalesCampaignNamePrefix.value);
            }
            this.campaignCountryForm.controls["campaignNr1"].setValue(
                data.campaignNr1.value
            );
            this.campaignCountryForm.controls["campaignNr2"].setValue(
                data.campaignNr2.value
            );
            this.campaignCountryForm.controls["campaignNr3"].setValue(
                data.campaignNr3.value
            );
        } else {
            this.campaignCountryForm = new FormGroup({
                campaignMode: new FormControl(
                    "newCampaign",
                    Validators.required
                ),
                ...
                campaignGroup: new FormControl(
                    data.idSharingTreeGroups.value,
                    Validators.required
                ),
                mandant: new FormControl(
                    data.idPersonToMandant.value,
                    Validators.required
                ),
        ...
        this.commonServiceSubscription = this.commonService
            ...
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    this.campaignAddressList = this.getValidCombobox(
                        response.item,
                        ComboBoxTypeConstant.campaignWizardAddress,
                        data.idPersonToMandant.value
                    );
    ...
    private prepareSubmitCreateData() {
        ...
        if (this.idSalesCampaignWizard && !this.isClone) {
            data.SalesCampaignWizard[0]["IdSalesCampaignWizard"] =
    ...
    onSubmit(event?: any) {
        ...
        try {
            ...
            if (
                this.campaignCountryForm.invalid ||
                this.existedCampaignNumber ||
                !!errorMessage
            ) {
                this.outputModel = {
                    submitResult: false,
            ...
            this.campaignServiceSubscription = this.campaignService
                .saveCampaignWizard(this.prepareSubmitCreateData())
                .subscribe(
                    (data) => {
                        this.appErrorHandler.executeAction(() => {
    ...
    getCountryList(data: any) {
        ...
        if (countryList && countryList.length) {
            ...
            // Create new Mode
            if (!this.idSalesCampaignWizard || this.isClone) {
                this.campaignWizardCountries = activeCountryList.map(
                    (p: Country) => {
                        return {
                            IdCountrylanguage: +p.idValue,
                            IsActive: true,
                ...
                if (activeCountryList.length) {
                    const activeCountryDataMapList = activeCountryList.map(
                        (p: Country) => {
                            return {
                                IdCountrylanguage: +p.idValue,
                                IsActive: true,
                                IdSalesCampaignWizardItems:
                                    +p.idSalesCampaignWizardItems == 0
                                        ? null
                                        : +p.idSalesCampaignWizardItems,
                                IsDeleted: "0",
                            };
                    ...
                    if (
                        this.countryEditListData &&
                        this.countryEditListData.length
                    ) {
                        activeCountryDataMapList.forEach((activeCountry) => {
                            let isExists = false;
                            for (
                                let i = 0;
                                i < this.countryEditListData.length;
                                i++
                            ) {
                                if (
                                    this.countryEditListData[i]
                                        .IdCountrylanguage ==
                                    activeCountry.IdCountrylanguage
                                ) {
                                    isExists = true;
                                    break;
                                }
                            ...
                            if (!isExists) {
                                this.campaignWizardCountries.push(
                                    activeCountry
                                );
                            }
                        });
                        ...
                        this.campaignWizardCountries =
                            this.countryEditListData.concat(
                                activeCountryDataMapList
                            );
                    }
                }

                if (inActiveCountryList.length) {
                    const inActiveCountryDataMapList = inActiveCountryList.map(
                        (p: Country) => {
                            return {
                                IdCountrylanguage: +p.idValue,
                                IsActive: false,
                    ...
                    if (
                        ...
                        inActiveCountryDataMapList.forEach(
                            (inActiveCountry) => {
                                this.countryEditListData.forEach(
                                    (item, index, object) => {
                                        if (
                                            item.IdCountrylanguage ==
                                            inActiveCountry.IdCountrylanguage
                                        ) {
                                            if (
                                                object[index]
                                                    .IdSalesCampaignWizardItems
                                            ) {
                                                inActiveCountry["IsDeleted"] =
                                                    "1";
                                                this.campaignWizardCountries.push(
                                                    inActiveCountry
                                                );
    ...
    private subscribeFormValueChange() {
        if (this.mandantValueChangesSubscription)
            this.mandantValueChangesSubscription.unsubscribe();
        if (this.campaignCountryFormValueChangesSubscription)
            this.campaignCountryFormValueChangesSubscription.unsubscribe();
        if (this.idRepSalesCampaignNamePrefixValueChangesSubscription)
            this.idRepSalesCampaignNamePrefixValueChangesSubscription.unsubscribe();
        if (this.campaignNr1ValueChangesSubscription)
            this.campaignNr1ValueChangesSubscription.unsubscribe();
        if (this.campaignNr2ValueChangesSubscription)
            this.campaignNr2ValueChangesSubscription.unsubscribe();
        if (this.campaignNr3ValueChangesSubscription)
            this.campaignNr3ValueChangesSubscription.unsubscribe();
        ...
        setTimeout(() => {
            ...
            this.mandantValueChangesSubscription = this.campaignCountryForm
                .get("mandant")
            ...
            this.campaignCountryFormValueChangesSubscription =
                this.campaignCountryForm.valueChanges
                    .debounceTime(this.consts.valueChangeDeboundTimeDefault)
                    .subscribe((data) => {
                        this.appErrorHandler.executeAction(() => {
                            if (
                                this.campaignCountryForm.pristine !=
                                    undefined &&
                                !this.campaignCountryForm.pristine
                            ) {
            ...
            if (this.supportCampaignNumberPrefix) {
                this.idRepSalesCampaignNamePrefixValueChangesSubscription =
                    this.campaignCountryForm
                        .get("campaignNumberPrefix")
                        .valueChanges.debounceTime(
                            this.consts.valueChangeDeboundTimeDefault
                        )
                        .subscribe((data) => {
            ...
            this.campaignNr1ValueChangesSubscription = this.campaignCountryForm
                .get("campaignNr1")
                .valueChanges.debounceTime(
                    this.consts.valueChangeDeboundTimeDefault
                )
            ...
            this.campaignNr2ValueChangesSubscription = this.campaignCountryForm
                .get("campaignNr2")
                .valueChanges.debounceTime(
                    this.consts.valueChangeDeboundTimeDefault
                )
            ...
            this.campaignNr3ValueChangesSubscription = this.campaignCountryForm
                .get("campaignNr3")
                .valueChanges.debounceTime(
                    this.consts.valueChangeDeboundTimeDefault
                )
    ...
    private checkCampaignNumber() {
        const checkCampaignNumberModel = new CheckCampaignNumber({
            IdRepSalesCampaignNamePrefix: this.supportCampaignNumberPrefix
                ? this.campaignCountryForm.controls["campaignNumberPrefix"]
                      .value
                : "",
        ...
        this.checkCampaignNumberSubscription = this.campaignService
            .checkCampaignNumber(checkCampaignNumberModel)
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
    ...
    public selectCampaignToClone() {
        const campaignMode =
            this.campaignCountryForm.controls["campaignMode"].value;
    ...
    public customItemFormatterShippingAddress(index, content) {
        if (this.campaignAddressList && this.campaignAddressList.length) {
            ...
            const company = isEmpty(campaignAddress.company)
                ? ""
                : campaignAddress.company;
            const poboxLabel = isEmpty(campaignAddress.poboxLabel)
                ? ""
                : campaignAddress.poboxLabel;
            const zip = isEmpty(campaignAddress.zip) ? "" : campaignAddress.zip;
            const place = isEmpty(campaignAddress.place)
                ? ""
                : campaignAddress.place;
            const newContent =
