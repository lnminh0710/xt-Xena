          ...
          this.appErrorHandler.executeAction(() => {
            ...
            if (response.item['collectionData']) {
              ...
              if (this.isEditMode && !this.isClone) {
                ...
                this.campaignCountryForm.controls['campaignMode'].setErrors(
                  null
                );
                this.campaignCountryForm.controls['campaignMode'].disable();
              } else {
                this.campaignCountryForm.controls['campaignMode'].setValidators(
                  Validators.required
                );
                this.campaignCountryForm.controls['campaignMode'].setErrors({
                  required: true,
      ...
      .subscribe((rs) => {
        this.appErrorHandler.executeAction(() => {
          if (rs && rs.item) {
            ...
            if (this.idSalesCampaignWizard) {
              this.countryEditListData = this.countryCheckListData.filter(
                (p) => p.isActive == '1'
              );
  ...
  private createFormGroupWithData(data: any) {
    ...
    if (this.campaignCountryForm && this.campaignCountryForm.controls) {
      ...
      if (
        ...
        this.campaignCountryForm.controls['campaignNumberPrefix'].setValue(
          data.idRepSalesCampaignNamePrefix.value
        );
      }
          ...
          if (this.countryEditListData && this.countryEditListData.length) {
            activeCountryDataMapList.forEach((activeCountry) => {
              ...
              for (let i = 0; i < this.countryEditListData.length; i++) {
                if (
                  this.countryEditListData[i].IdCountrylanguage ==
            ...
            this.campaignWizardCountries = this.countryEditListData.concat(
          ...
          if (this.countryEditListData && this.countryEditListData.length) {
            inActiveCountryDataMapList.forEach((inActiveCountry) => {
              this.countryEditListData.forEach((item, index, object) => {
                if (
                  item.IdCountrylanguage == inActiveCountry.IdCountrylanguage
                ) {
  ...
  private subscribeFormValueChange() {
    ...
    setTimeout(() => {
      ...
      this.mandantValueChangesSubscription = this.campaignCountryForm
        .get('mandant')
        .valueChanges.debounceTime(this.consts.valueChangeDeboundTimeDefault)
        .subscribe((data) => {
          this.appErrorHandler.executeAction(() => {
      ...
      this.campaignCountryFormValueChangesSubscription =
        this.campaignCountryForm.valueChanges
          ...
          .subscribe((data) => {
            this.appErrorHandler.executeAction(() => {
              if (
                this.campaignCountryForm.pristine != undefined &&
  ...
  private checkCampaignNumber() {
    const checkCampaignNumberModel = new CheckCampaignNumber({
      IdRepSalesCampaignNamePrefix: this.supportCampaignNumberPrefix
        ? this.campaignCountryForm.controls['campaignNumberPrefix'].value
        : '',
