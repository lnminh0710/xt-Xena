  ...
  implements OnInit, OnDestroy
{
  ...
  public onCompleteImportItem(event) {
    ...
    if (
      this._fileUploadCounter < this.mediaCodeFileUpload.uploader.queue.length
    ) {
      return;
    }
    if (
      ...
      .subscribe((result) => {
        this.appErrorHandler.executeAction(() => {
          ...
          setTimeout(() => {
            ...
            this.countryLanguageForm.controls['countryLanguage'].setValue(
              this.currentIdSalesCampaignWizardItems
            );
            this.loadMediaCodeByCountry(this.currentIdSalesCampaignWizardItems);
  ...
  private buildDuplicateMessage(existData: any) {
    ...
    for (let item of this._fileUploadResponses) {
      ...
      mediaExistedItems = [
        ...mediaExistedItems,
        ...Uti.getItemsExistItems(item.response, nodeItems, 'MediaCode'),
      ];

      if (mediaExistedItems && mediaExistedItems.length) {
        this._messageDuplicationMediaCode +=
          '<li> File name: <strong>' + item.item.file.name + '</strong></li>';
        ...
        mediaExistedItems.forEach((x) => {
          this._messageDuplicationMediaCode +=
            '<li> Media code: <strong>' + x.MediaCode + '</strong></li>';
        });
        this._messageDuplicationMediaCode += '</ul>';
    ...
    this.campaignServiceSubscription = this.campaignService
      ...
      .subscribe(
        (inputData) => {
          this.appErrorHandler.executeAction(() => {
            if (
              inputData.item.eventType !== FormSaveEvenType.Successfully ||
    ...
    this.campaignService
      ...
      .subscribe((response: ApiResultResponse) => {
        this.appErrorHandler.executeAction(() => {
          ...
          const gridData = this.datatableService.formatDataTableFromRawData(
            response.item.data
          );
          this.dataSourceTable =
  ...
  private focusOnInvalidControl() {
    setTimeout(() => {
      const $allItems = $('[data-pageId=' + this.pageId + '] .ng-invalid').not(
        'form,div,span,a,p'
      );
      const $item = $allItems
