...
import { Component, EventEmitter, Input, Output, OnInit, ViewChild, OnDestroy, ElementRef } from '@angular/core';
...
export class MediaCodeT2FormComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.rowCampaignMediaMainDataState = store.select(state => widgetContentReducer.getWidgetContentDetailState(state, this.ofModule.moduleNameTrim).rowCampaignMediaMainData_T2);
    ...
    public downloadTeamplate($event) {
        this.downloadFileService.makeDownloadFile('Campaign-T2-media-code.zip', null, null, 4);
    }
    ...
    public onCompleteImportItem(event) {
        ...
        if (!this.checkCorrectImportData(mediaItems)) {
            this._messageIncorrectFile += '<li>' + event.item.file.name + '</li>';
            event.item.isSuccess = false;
            event.item.isError = true;
            this._fileUploadErrorCounter++;
            if (this._fileUploadCounter === this.mediaCodeFileUpload.uploader.queue.length
                && !!this._fileUploadErrorCounter) {
    ...
    itemFormatter(index, item) {
        let newContent = '<div class="col-md-12  col-lg-12 xn-wj-ddl-item"><div class="col-sm-5 no-padding-left" >{Country}</div><div class="col-sm-1" style="display:none" >-</div><div class="col-sm-5 no-padding-right border-left" >{Language}&nbsp;</div></div>';
            ...
            if (this.wjCountryLanguage.selectedItem) {
                ...
                if (this.checkDirtyAndGetSaveData()) {
                    this.modalService.unsavedWarningMessage({
                        ...
                        message: [{key: '<p>'}, {key: 'Modal_Message__You_Have_Unsaved_Mediacode_Saving_Change'},
                            {key: '</p>'}],
                        onModalSaveAndExit: () => {
                            this.allowToChangeCountry = false;
                            this.willChangeCountry = this.wjCountryLanguage.selectedValue;
                            this.submit();
                        },
                        onModalExit: () => {
                            this.currentIdSalesCampaignWizardItems = this.wjCountryLanguage.selectedValue;
                            this.loadMediaCodeByCountry(this.currentIdSalesCampaignWizardItems);
                        },
                        onModalCancel: () => {
                            this.allowToChangeCountry = false
                            this.wjCountryLanguage.selectedValue = this.currentIdSalesCampaignWizardItems;
                        }
                    });
                } else {
    ...
    private buildForm() {
        ...
        this.mediaCodeT2Form = this.formBuilder.group({
            MediaCode: ['', CustomValidators.required, this.validateArticleNr.bind(this)],
    ...
    private validateArticleNr(c: FormControl) {
        ...
        return this.campaignService.checkMediaCodeNumber(c.value).finally(() => {
    ...
    private buildComboboxDataSource(idSalesCampaignWizard): void {
        ...
        this.countryLanguageService.getCountryLanguage({ 'IdSalesCampaignWizard': idSalesCampaignWizard }).subscribe((result) => {
            this.appErrorHandler.executeAction(() => {
                ...
                setTimeout(() => {
                    if (!this.currentIdSalesCampaignWizardItems)
                        this.currentIdSalesCampaignWizardItems = this.countryLanguageList[0].IdSalesCampaignWizardItems;

                    this.countryLanguageForm.controls['countryLanguage'].setValue(this.currentIdSalesCampaignWizardItems);
                    this.loadMediaCodeByCountry(this.currentIdSalesCampaignWizardItems);

                    if (!this.countryLanguageList || !this.countryLanguageList.length)
    ...
    private showMesageOfUploadFile() {
        if (!!this._messageIncorrectFile) {
            this.messageIncorrectFile = [{key: '<p>'},
        ...
        if (this._messageDuplicationMediaCode) {
            this.messageDuplicationMediaCode = [{key: '<p>'},
        ...
        const message = [...this.messageIncorrectFile, ...this.messageDuplicationMediaCode];
    ...
    private checkExistingMediaCodeMulti() {
        ...
        this.checkExistingMediaCodeSubscription = this.campaignService.checkExistingMediaCodeMulti(this._mediaCodeNrs).subscribe((response: ApiResultResponse) => {
            this.appErrorHandler.executeAction(() => {
                let existData = [];
                if (Uti.isResquestSuccess(response)
                    && response.item && response.item.data && response.item.data.length) {
    ...
    private buildDuplicateMessage(existData: any) {
        const nodeItems = this.gridComponent && this.gridComponent.getCurrentNodeItems();
        for (let item of this._fileUploadResponses) {
            let mediaItems = Uti.getItemsDontExistItems(item.response, existData, 'MediaCode');
            mediaItems = Uti.getItemsDontExistItems(mediaItems, nodeItems, 'MediaCode');
            let mediaExistedItems = Uti.getItemsExistItems(item.response, existData, 'MediaCode');
            mediaExistedItems = [...mediaExistedItems, ...Uti.getItemsExistItems(item.response, nodeItems, 'MediaCode')];

            if (mediaExistedItems && mediaExistedItems.length) {
                this._messageDuplicationMediaCode += '<li> File name: <strong>' + item.item.file.name + '</strong></li>';
                this._messageDuplicationMediaCode += '<ul>';
                mediaExistedItems = uniqBy(mediaExistedItems, x => {
                    return x.MediaCode;
                });
                mediaExistedItems.forEach(x => {
                    this._messageDuplicationMediaCode += '<li> Media code: <strong>' + x.MediaCode + '</strong></li>'
                });
                this._messageDuplicationMediaCode += '</ul>';
    ...
    private isExistedInGrid(mediaCode: string): boolean {
        const items = this.gridComponent && this.gridComponent.getCurrentNodeItems();
    ...
    private subcribtion() {
        this.selectedEntityStateSubscription = this.selectedEntityState.subscribe((selectedEntityState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (selectedEntityState && selectedEntityState.idSalesCampaignWizard) {
                    this.buildComboboxDataSource(selectedEntityState['idSalesCampaignWizard']);
                }
            });
        });
        ...
        this.rowCampaignMediaMainDataStateSubscription = this.rowCampaignMediaMainDataState.subscribe((rowData: RowData) => {
            this.appErrorHandler.executeAction(() => {
                if (!rowData || !rowData.data || !!this.currentIdSalesCampaignWizardItems) return;
                if (!Uti.checkKeynameExistInArray(rowData.data, 'key', 'IdSalesCampaignWizardItems')) return;
                const primaryValue = Uti.getPrimaryValueFromKey(rowData);
                if (!primaryValue || !primaryValue.length) return;
                MediaCodeT2FormComponent.idSalesCampaignWizardItems = primaryValue[0].value;
                this.currentIdSalesCampaignWizardItems = primaryValue[0].value;
            });
        });
    }
    ...
    private checkCountryLanguage() {
        if (!this.wjCountryLanguage.selectedValue) {
            this.toasterService.pop('warning', 'Validation Failed', 'Please select Country + Language!');
            return false;
        }
        return true;
    }
    ...
    private buildEditedItems(currentItem: any) {
        const item = this.editedItems.find(x => x.DT_RowId == currentItem.DT_RowId);
    ...
    private checkDirtyAndGetSaveData(): boolean {
        ...
        const nodeItems = this.gridComponent && this.gridComponent.getCurrentNodeItems();
        // Add Items
        let addItems = Uti.getItemsDontExistItems(nodeItems, this.cachedGridData, 'DT_RowId');
        if (addItems && addItems.length) {
            addItems = addItems.filter(x => !x.deleted && x.IdSalesCampaignMediaCode < 0);
            addItems = addItems.map(x => {
                return {
                    'idSalesCampaignWizardItems': MediaCodeT2FormComponent.idSalesCampaignWizardItems,
                    'mediaCode': x.MediaCode,
        ...
        for (let x of this.editedItems) {
            if (!x.IdSalesCampaignMediaCode || x.IdSalesCampaignMediaCode < 0) continue;
            editItems.push({
                'idSalesCampaignWizardItems': MediaCodeT2FormComponent.idSalesCampaignWizardItems,
        ...
        let deleteItems = nodeItems.filter(x => x.IsDeleted && !!x.IdSalesCampaignMediaCode);
    ...
    private udpateCampaign(data: any) {
        this.campaignServiceSubscription = this.campaignService.createMediaCode(data).subscribe(
            (inputData) => {
                this.appErrorHandler.executeAction(() => {
                    if (inputData.item.eventType !== FormSaveEvenType.Successfully || !inputData.item.returnID) {
                    ...
                    if (this.willChangeCountry) {
                        this.toasterService.pop('success', 'Success', 'MediaCode saved successfully');
    ...
    private loadMediaCodeByCountry(idSalesCampaignWizardItems) {
        MediaCodeT2FormComponent.idSalesCampaignWizardItems = idSalesCampaignWizardItems;
        this.campaignService.getMediaCodeDetail(idSalesCampaignWizardItems).subscribe((response: ApiResultResponse) => {
            this.appErrorHandler.executeAction(() => {
                if (!Uti.isResquestSuccess(response) || !response.item || !response.item.data && !response.item.data.length) {
                    return;
                }

                const gridData = this.datatableService.formatDataTableFromRawData(response.item.data);
                this.dataSourceTable = this.datatableService.buildDataSource(gridData);
                this.cachedGridData = Object.assign([], this.dataSourceTable.data);

                if (this.willChangeCountry) {
                    this.allowToChangeCountry = true;
                    this.wjCountryLanguage.selectedValue = this.willChangeCountry;
                    this.willChangeCountry = null;
                }
            });
        });
    ...
    private focusOnInvalidControl() {
        setTimeout(() => {
            const $allItems = $('[data-pageId=' + this.pageId + '] .ng-invalid').not('form,div,span,a,p');
            const $item = $allItems.find('input:visible, select:visible, textarea:visible').first();
