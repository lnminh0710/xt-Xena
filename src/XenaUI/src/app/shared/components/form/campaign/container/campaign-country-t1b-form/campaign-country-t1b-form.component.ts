    ...
    implements OnInit, OnDestroy, AfterViewInit
{
            ...
            if (currentFocusedCell && selectedNodes && selectedNodes.length) {
                if (selectedNodes[0].rowIndex != currentFocusedCell.rowIndex) {
                    this.api.forEachNode(
                        (rowNode: RowNode, rowIndex: number) => {
        ...
        this.campaignServiceSubscription = this.campaignService
            ...
            .subscribe((rs) => {
                this.appErrorHandler.executeAction(() => {
                    if (!rs) return;
                    const data =
                        this.datatableService.buildTableDataFromCollection(rs);
                    this.initDataMap().subscribe((dataMaps) => {
                        this.appErrorHandler.executeAction(() => {
                            ...
                            this.gridOptions.columnDefs =
                                agGridDataSource.columnDefs;
                            this.gridOptions.rowData = agGridDataSource.rowData;
                            this.isShowPostageCostItem =
                                !this.isShowPostageCostItem;
    ...
    onSubmit(event?: any) {
        try {
            ...
            if (items.length) {
                this.campaignServiceSubscription = this.campaignService
                    .saveCampaignWizardCountriesT2(
                        this.prepareSubmitCreateData()
                    )
                    .subscribe(
                        (data) => {
                            this.appErrorHandler.executeAction(() => {
    ...
    private buildCampaignWizardCountriesT2sData(item) {
        ...
        let data = {
            ...
            IdSalesCampaignWizardOffice:
                +item.IdSalesCampaignWizardOffice.value,
    ...
    private buildCurrenciesData(item) {
        ...
        if (selectedCurrencyValues) {
            selectedCurrencyValues.forEach((valObj) => {
                const iRet = originalCurrencyValues.filter(
                    (p) => p.key == valObj.key
                );
                if (iRet.length) {
                    currencies.push({
        ...
        Object.keys(item).forEach((key) => {
            if (item[key]) {
                ...
                if (control.controlType == "dynamic") {
                    const dynamicControl: DynamicControl =
                        control as DynamicControl;
                ...
                if (col.length) {
                    ...
                    if (col[0].controlType == "dropdown") {
                        ...
                        if (dropdownCtrl.type == "multi-select") {
                            const values: Array<string> = keyValueObj.map(
                                (p) => {
                                    return p.value;
                                }
                            );
                            ...
                            const options: Array<any> =
                                this.comboBoxData[
                                    dropdownCtrl.identificationKey
                                ];
                            if (options) {
    ...
    private initDataMap(): Observable<any> {
        ...
        return this.commonService
            ...
            .map((response: ApiResultResponse) => {
                ...
                this.comboBoxData[ComboBoxTypeConstant.allMandant] =
                    mandantList;
                this.comboBoxData[ComboBoxTypeConstant.serviceProvider] =
                    serviceProviderList;
                this.comboBoxData[ComboBoxTypeConstant.wareHouse] =
                    wareHouseList;
    ...
    public onCellEditingStarted($event) {
        if (
            !$event.data["IsActive"].value &&
            $event.colDef.colId !== "IsActive"
        ) {
            ...
            this.commonServiceSubscription = this.commonService
                ...
                .subscribe((response: ApiResultResponse) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        if (
                            comboOptions &&
                            dropdownControl.type == "multi-select"
                        ) {
                                ...
                                comboOptions.forEach((option) => {
                                    keys.forEach((keyObj) => {
                                        if (keyObj.key) {
                                            ...
                                            option.idCountryCurrency ==
                                            keyObj.idCountryCurrency
                                        ) {
                                            option.selected = true;
                                        }
                                    });
    ...
    viewPostageCost() {
        ...
        const dynamicCols: Array<any> = this.gridOptions.columnDefs.filter(
            (col: ColDef) =>
                col.refData.controlType == "dynamic" &&
                col.field != "DynamicColumn"
        );
