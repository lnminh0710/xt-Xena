import {
    ...
    OnInit, ViewChild, OnDestroy, AfterViewInit, TemplateRef, ViewContainerRef
} from '@angular/core';
...
import {
    ...
    CheckboxControl, DateControl, DynamicControl,
    ApiResultResponse
} from 'app/models';
...
export class CampaignCountryT1BFormComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    // Core API of Ag Grid
    private api: GridApi;
    ...
    @ViewChild(WidgetModuleInfoTranslationComponent) widgetModuleInfoTranslationComponent: WidgetModuleInfoTranslationComponent;
    ...
    @ViewChild('checkBoxEditCell') checkBoxEditCellTemplateRef: TemplateRef<any>;
    @ViewChild('labelCell') labelCellTemplateRef: TemplateRef<any>;
    @ViewChild('labelDateCell') labelDateCellTemplateRef: TemplateRef<any>;
    @ViewChild('labelDropdownCell') labelDropdownCellTemplateRef: TemplateRef<any>;
    @ViewChild('multiSelectEditCell') multiSelectEditTemplateRef: TemplateRef<any>;
    @ViewChild('dropdownEditCell') dropdownEditTemplateRef: TemplateRef<any>;
    @ViewChild('datePickerEditCell') datePickerEditTemplateRef: TemplateRef<any>;
    @ViewChild('postageCostsEditCell') postageCostsEditCellTemplateRef: TemplateRef<any>;
    @ViewChild('labelPostageCostsCell') labelPostageCostsCellTemplateRef: TemplateRef<any>;
    ...
    private initGrid() {
        ...
        this.gridOptions.rowClassRules = {
            'in-active-cell': function (params) {
                return params.node.data && (params.node.data['IsActive'].value == false || params.node.data['IsActive'].value == 0);
            }
        };
     ...
     */
    private loadData() {
        this.getCampaignWizardT1().finally(() => {
            this.getCampaignWizardT2();
        }).subscribe(data => {
            this.appErrorHandler.executeAction(() => {
                this.isPostageCosts = this.buildBooleanRadio(data, 'isWithPostageCost');
    ...
    private getCampaignWizardT2() {
        this.campaignServiceSubscription = this.campaignService.getCampaignWizardT2(this.idSalesCampaignWizard).subscribe(rs => {
            this.appErrorHandler.executeAction(() => {
                if (!rs)
                    return;
                ...
                this.initDataMap().subscribe(dataMaps => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        const agGridDataSource = this.mapToAgGridDataSource(this.dataSource);
                        if (!this.gridOptions) {
                            this.initGrid();
                        }
            ...
            let colDef: ColDef = {
                ...
                cellClassRules: {
                    'edited-cell': function (cellClassParams: CellClassParams) {
                        return !isEqual(cellClassParams.value.value, cellClassParams.value.originalValue);
                    }
                },
            }
    ...
    private getCampaignWizardT1() {
        ...
        return this.campaignService.getCampaignWizardT1(this.idSalesCampaignWizard)
    ...
    onSubmit(event?: any) {
        try {
            ...
            if (items.length) {
                this.campaignServiceSubscription = this.campaignService.saveCampaignWizardCountriesT2(this.prepareSubmitCreateData())
                    .subscribe(
                        (data) => {
                            this.appErrorHandler.executeAction(() => {
                                this.outputModel = { submitResult: true, formValue: this.dataSource, isValid: true, isDirty: false, returnID: returnID };
                                this.saveData.emit(this.outputModel);
                            });
                        },
                        (err) => {
                            this.outputModel = { submitResult: true, formValue: this.dataSource, isValid: true, isDirty: true, returnID: null };
                            this.saveData.emit(this.outputModel);
                        });
            } else {
                this.outputModel = {
    ...
    private buildEditedItem() {
        if (!this.isPostageCosts || !this.itemsEdited)
            return;
    ...
    private prepareSubmitCreateData() {
        ...
        items.forEach(item => {
            campaignWizardCountriesT2s.push(this.buildCampaignWizardCountriesT2sData(item));
    ...
    private buildCampaignWizardCountriesT2sData(item) {
        ...
        if (item.PostageDate.value) {
            postageDate = this.uti.formatLocale(item.PostageDate.value, this.consts.dateFormatInDataBase);
        ...
        let data = {
            ...
            'IdPersonInterfaceContactAddressGWToShippingAddress': +item.IdPersonInterfaceContactAddressGWToShippingAddress.value,
            'IdPersonInterfaceContactAddressGWToReturnAddress': +item.IdPersonInterfaceContactAddressGWToReturnAddress.value,
    ...
    private buildCurrenciesData(item) {
        ...
        if (originalCurrencyValues) {
            originalCurrencyValues.forEach(orgValObj => {
                if (selectedCurrencyValues) {
                    const iRet = selectedCurrencyValues.filter(p => p.key == orgValObj.key);
                    if (!iRet.length) {
                        currencies.push({
                            'IdSalesCampaignWizardItems': +item.IdSalesCampaignWizardItems.value,
            ...
            if (item[key]) {
                ...
                if (control.controlType == 'dynamic') {
                    ...
                    if (!dynamicControl.isRawDynamicData) {
                        payments.push({
                            'IdSalesCampaignWizardItems': +item.IdSalesCampaignWizardItems.value,
                            'IdSalesCampaignWizardItemsPayments': dynamicControl.value.idSalesCampaignWizardItemsPayments,
                            'PostageCosts': dynamicControl.value.postageCosts ? dynamicControl.value.postageCosts : null,
    ...
    private buildDatasource(datasource: any) {
        ...
        if (dynamicCol.length && datasource.data.length) {
            const dynamicCols = this.buildDynamicHeaderColumn(datasource.data[0], 'DynamicColumn');
        ...
        columns.forEach(col => {
            ...
            switch (col.data) {
                ...
                case 'Currency':
                    ...
                    (control as DropdownControl).identificationKey = ComboBoxTypeConstant.currencyByWizardItems;
                ...
                case 'IdPersonInterfaceContactAddressGWToReturnAddress':
                    control = new DropdownControl(defaultConfig);
                    (control as DropdownControl).identificationKey = ComboBoxTypeConstant.campaignWizardAddress;
                ...
                case 'IdPersonToMandant':
                    control = new DropdownControl(defaultConfig);
                    (control as DropdownControl).identificationKey = ComboBoxTypeConstant.allMandant;
                    control.order = 8;
                    col = control;
                    break;
                ...
                case 'IdPersonToServiceProvider':
                    control = new DropdownControl(defaultConfig);
                    (control as DropdownControl).identificationKey = ComboBoxTypeConstant.serviceProvider;
                    control.order = 9;
                    col = control;
                    break;
                ...
                case 'IdPersonToWarehouse':
                    control = new DropdownControl(defaultConfig);
                    (control as DropdownControl).identificationKey = ComboBoxTypeConstant.wareHouse;
                    control.order = 10;
                    col = control;
                    break;
    ...
    private buildDynamicHeaderColumn(dataRow, key): Array<any> {
        ...
        if (dynamicColRaw) {
            ...
            if (dynamicColObj && dynamicColObj[Object.keys(dynamicColObj)[0]]) {
                const dynamicCols: Array<any> = dynamicColObj[Object.keys(dynamicColObj)[0]];
     ...
     * buildData
     */
    private buildData(data: Array<any>, columns: Array<ControlBase<any>>, dynamicKey) {
        data.forEach(dataRow => {
            ...
            Object.keys(dataRow).forEach(key => {
                ...
                if (col.length) {
                    ...
                    if (col[0].controlType == 'checkbox') {
                        ...
                        const checkboxCtrl: CheckboxControl = (Object.assign({}, col[0]) as CheckboxControl);
                    ...
                    if (col[0].controlType == 'textbox') {
                        const textboxCtrl: TextboxControl = (Object.assign({}, col[0]) as TextboxControl);
                        textboxCtrl.value = dataRow[key];
                        textboxCtrl.originalValue = dataRow[key];
                        dataRow[key] = textboxCtrl;
                    }
                    if (col[0].controlType == 'dynamic') {
                        const dynamicCtrl: DynamicControl = (Object.assign({}, col[0]) as DynamicControl);
                        dynamicCtrl.value = dataRow[key];
                        dataRow[key] = dynamicCtrl;
                    }
                    if (col[0].controlType == 'date') {
                        const dateCtrl: DateControl = (Object.assign({}, col[0]) as DateControl);
                        const date = dataRow[key] ? parse(dataRow[key], 'dd.MM.yyyy', new Date()) : null;
                        dateCtrl.value = date;
                        dateCtrl.originalValue = date;
                        dataRow[key] = dateCtrl;
                    }
                    if (col[0].controlType == 'dropdown') {
                        const dropdownCtrl: DropdownControl = (Object.assign({}, col[0]) as DropdownControl);
                        ...
                        if (dropdownCtrl.type == 'multi-select') {
                            ...
                            dataRow[key] = dropdownCtrl;
                            const options: Array<any> = this.comboBoxData[dropdownCtrl.identificationKey];
                            if (options) {
                                const option = options.filter(p => p.key == keyValueObj[0].key);
                                if (option.length) {
                                    dropdownCtrl.displayValue = option[0].value;
                                }
                                dropdownCtrl.options = this.comboBoxData[dropdownCtrl.identificationKey];
                                dataRow[key] = dropdownCtrl;
                            }
                        }
                    }
            ...
            if (dynamicColRaw) {
                ...
                if (dynamicColObj && dynamicColObj[Object.keys(dynamicColObj)[0]]) {
                    const dynamicCols: Array<any> = dynamicColObj[Object.keys(dynamicColObj)[0]];
                    for (let i = 0; i < dynamicCols.length; i++) {
                        const dynamicCtrl: DynamicControl = (Object.assign({}, dataRow[dynamicKey]) as DynamicControl);
                        const value = {
                            idSalesCampaignWizardItemsPayments: dynamicCols[i].IdSalesCampaignWizardItemsPayments,
                            postageCosts: dynamicCols[i].PostageCosts ? dynamicCols[i].PostageCosts : null,
    ...
    private initDataMap(): Observable<any> {
        ...
        return this.commonService.getListComboBox(keys.join(','))
            .map((response: ApiResultResponse) => {
                if (!Uti.isResquestSuccess(response)) {
                    return;
                }
                const mandantList = this.getValidCombobox(response.item, ComboBoxTypeConstant.allMandant);
                const serviceProviderList = this.getValidCombobox(response.item, ComboBoxTypeConstant.serviceProvider);
                const wareHouseList = this.getValidCombobox(response.item, ComboBoxTypeConstant.wareHouse);
                this.comboBoxData[ComboBoxTypeConstant.allMandant] = mandantList;
                this.comboBoxData[ComboBoxTypeConstant.serviceProvider] = serviceProviderList;
    ...
    public onCellEditingStarted($event) {
        ...
        if (control.controlType == 'dropdown' && control.filterBy) {
            ...
            this.commonServiceSubscription = this.commonService.getComboBoxDataByFilter(dropdownControl.identificationKey, id, null, true)
     ...
     */
    private getValidCombobox(listComboBox: any, identificationKey: number, filterBy?: string, mapFunc?: any) {
    ...
    onSelectedIndexChanged(cell) {
        ...
        if (dropdownControl.options && dropdownControl.options.length) {
            const option = dropdownControl.options.filter(p => p.key == dropdownControl.value);
            if (option.length) {
    ...
    viewPostageCost() {
        this.isShowPostageCostItem = !this.isShowPostageCostItem;
        const dynamicCols: Array<any> = this.gridOptions.columnDefs.filter((col: ColDef) =>
    ...
    private setOutputData(submitResult: any, data?: any) {
        if ((typeof data) !== 'undefined') {
            ...
            this.outputModel = { submitResult: false, formValue: this.dataSource, isValid: true, isDirty: true, returnID: this.idSalesCampaignWizard };
        }
     ...
     */
    public translate() {
        this.campaignService.getCampaignWizardT2(this.idSalesCampaignWizard, '-1').subscribe(rs => {
            this.appErrorHandler.executeAction(() => {
                if (!rs)
                    return;
                if (rs.collectionData && rs.collectionData.length) {
                    rs.collectionData[0].Currency = this.parseJsonValue(rs.collectionData[0].Currency);
                    rs.collectionData[0].IdPersonToMandant = this.parseJsonValue(rs.collectionData[0].IdPersonToMandant);
                    rs.collectionData[0].IdPersonToServiceProvider = this.parseJsonValue(rs.collectionData[0].IdPersonToServiceProvider);
                    rs.collectionData[0].IdPersonToWarehouse = this.parseJsonValue(rs.collectionData[0].IdPersonToWarehouse);
    ...
    public boolFormatter(params) {
        try {
            ...
            if (control.value == 'True' || control.value == 'true' || control.value == true || control.value == 1) {
                control.value = true;
            }
            else {
                control.value = false;
