...
export abstract class CustomerFormBase extends FormBase {
    ...
    protected initDataForAddressFG(model: any, updateData?: any, isMarkAsPristineForm?: boolean) {
        Uti.setValidatorForForm(this.formGroup, this.mandatoryData);
        Uti.setValidatorRegularExpresionForForm(this.formGroup, this.regularExpressionData);
        ...
        this.globalPropertiesStateSubscription = this.globalPropertiesState.subscribe((globalProperties: any) => {
            this.appErrorHandler.executeAction(() => {
                if (globalProperties) {
                    ...
                    let propDOBFormatByCountry: WidgetPropertyModel = this.propertyPanelService.getItemRecursive(globalProperties, "DOBFormatByCountry");
    ...
    protected buildDateOfBirthFormat() {
        ...
        if (!this.countrySelectedItem)
            return;
        if (this.supportDOBCountryFormat) {
            this.dobDateFormat = Uti.getDateFormatFromIsoCode(this.countrySelectedItem.isoCode);
        }
    }
}
