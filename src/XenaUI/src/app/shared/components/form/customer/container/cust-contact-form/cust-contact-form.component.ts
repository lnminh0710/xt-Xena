...
import { ContactModel, ApiResultResponse } from "app/models";
...
export class CustContactFormComponent
    ...
    implements OnInit, OnDestroy
{
    ...
    constructor(
        ...
        this.globalSubscription = this.globalPropertiesState.subscribe(
            (globalProperties) => {
    ...
    public onIdRepContactAddressTypeChanged() {
        if (!this.idRepContactAddressTypeCombobox.checkedItems.length) {
            this.formGroup.controls["idRepContactAddressType"].setValidators(
                Validators.required
            );
            this.formGroup.controls["idRepContactAddressType"].setErrors({
                required: true,
            });
        } else {
    ...
    private createContactAddressType(): Array<any> {
        ...
        if (
            ...
            if (
                ...
                this.originalContactTypeList.length
            ) {
                        ...
                        const rowDataItem = rowsData.find(
                            (p) =>
                                ...
                                p.widgetDetail.idRepWidgetApp ==
                                    RepWidgetAppIdEnum.ContactTable
                        );
        ...
        this.comService
            ...
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (!this.contactModel || !this.contactModel.firstName)
                        return;
                    this.setRenderForm();
                });
            });
    ...
    private getContactData() {
        this.personServ
            .getMandatoryField("CustomerContact")
                        ...
                        .subscribe((response: ApiResultResponse) => {
                            this.appErrorHandler.executeAction(() => {
                                ...
                                this.regularExpressionData =
                                    Uti.getRegularExpressionData(response.item);
                                data.idRepContactAddressType = {
    ...
    private createContact() {
        ...
        this.formValuesChangeSubscription = observable.subscribe(
