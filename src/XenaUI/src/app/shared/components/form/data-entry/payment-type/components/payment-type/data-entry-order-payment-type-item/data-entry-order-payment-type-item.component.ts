  ...
  implements OnInit, OnDestroy, AfterViewInit
{
    ...
    setTimeout(() => {
      ...
      if (
        ...
        if (!this._information.parentFormGroup.contains('paymentTypeForm'))
          ...
          this._information.parentFormGroup.controls['paymentTypeForm'] =
            this.dataEntryPaymentTypeItemForm;
            ...
            for (let index in formValue['amounts']) {
              if (index == '0') {
                if (formArray.controls.length) {
                  formArray.controls[0]['controls']['amount'].setValue(
                    formValue['amounts'][index]['amount']
                  );
                  formArray.controls[0]['controls']['chequeDate'].setValue(
          ...
          if (controlName === 'creditCardAmounts') {
            ...
            for (let index in formValue['creditCardAmounts']) {
              ...
              formGroup.controls['creditDate'].setValue(
                formValue['creditCardAmounts'][index]['creditDate']
              );
              formArray.push(formGroup);
            } //for
          }
    ...
    if (formValue.paymentTypeId) {
      ...
      switch (this.data.paymentTypeGroup) {
          ...
          if (formValue.amounts && formValue.amounts.length) {
            ...
            for (let i = chequeAmountsLength - 1; i >= 0; i--) {
              if (formValue.amounts[i].amount) {
                chequeAmount = Number(
                  parseFloat(formValue.amounts[i].amount).toFixed(2)
                );
              ...
              for (
                ...
                if (formValue.creditCardAmounts[i].amount) {
                  creditCardAmount = Number(
                    parseFloat(formValue.creditCardAmounts[i].amount).toFixed(2)
                  );
        ...
        case PaymentTypeGroupEnum.PostBank: // PostBank
          this.data.amount = formValue.amount;
          this.data.postBankNameText = this.postBankNameCtl.selectedItem
  ...
  private validatePaymentForm(): boolean {
    ...
    for (let i = controls.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        continue;
      if (
        (this.isOrderDataEntryModule() || this.isReturnRefundModule()) &&
    ...
    setTimeout(() => {
      ...
      if (this.dataEntryPaymentTypeItemForm) {
        this.data.currencyId =
          this.dataEntryPaymentTypeItemForm.controls['currency'].value;
      ...
      buttonList.unshift(
        new ButtonList({
          ...
          callBackFunc: () => {
            ...
            this.parentComponent['isCancelThreatSAV'] = this.parentComponent[
              'isCancelBadChequeSAV'
            ] = true;
            this.modalService.hideModal();
          },
        })
  ...
  private chequeProcessDateWhenEnterOrLostFocusAmount(index: number) {
    ...
    if (!dateControl.value && !amountControl.nativeElement.value) {
      ...
      this.dataEntryPaymentTypeItemForm.controls['amounts']['controls'][index][
        'controls'
      ]['chequeDate'].setValue(date);
    }
  }
  ...
  public creditCardOptionChanged(numofMonth?: number): void {
    ...
    let option =
      this.dataEntryPaymentTypeItemForm.controls['creditCardOption'].value;
    ...
    try {
      ...
      if (!numofMonths && !creditCardCustomMonth) {
        ...
        if (numofMonths > 1 && total) {
          ...
          firstAmount =
            totalWithoutFractional * percentOfFirstMonth + fractional;
          ...
          if (numofMonths > 2) {
            ...
            secondAmount =
              remainOfTotal - averageAmount * (remainNumofMonths - 1);
          } else {
