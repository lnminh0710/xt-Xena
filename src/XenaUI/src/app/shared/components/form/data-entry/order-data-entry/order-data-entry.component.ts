  ...
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.dataEntryOrderDataState = this.store.select(
      (state) => dataEntryReducer.getDataEntryState(state, this.tabID).orderData
    );
    ...
    this.communicationDataState = this.store.select(
      (state) =>
        dataEntryReducer.getDataEntryState(state, this.tabID).communicationData
    );
  ...
  private subcribeDataEntryState() {
    ...
    this.communicationDataStateSubscription =
      this.communicationDataState.subscribe((communicationDataState: any) => {
        this.appErrorHandler.executeAction(() => {
          if (
            communicationDataState &&
            !isEqual(this.communicationData, communicationDataState)
          ) {
        ...
        (orderTotalSummaryDataState: any) => {
          this.appErrorHandler.executeAction(() => {
            if (
              ...
              !isEqual(this.orderTotalSummaryData, orderTotalSummaryDataState)
            ) {
  ...
  private rebuildTooltip(formField: string, divId: string) {
    ...
    this[formField + 'ChangeSubscription'] = this.orderDataEntryForm.controls[
      formField
    ].valueChanges
  ...
  private getCustomMandatoryField(mediaCode: string) {
    ...
    this.personService.preLoadBusinessLogic(mediaCode).subscribe((response) => {
      this.appErrorHandler.executeAction(() => {
  ...
  public storeFormControlValue(): void {
    ...
    const formControlValueState = {
      ...
      idRepSalesOrderShipper:
        this.orderDataEntryForm.controls['idRepSalesOrderShipper'].value,
    };
            ...
            if (orderTypeValue !== '2') {
              ...
              if (
                ...
                (!this.articleGridExportData ||
                  ...
                  !this.articleGridExportData.formValue.length)
              )
                isFormValid = false;
  ...
  private getGlobalSettingsMandatoryCustomerNr() {
    ...
    const mandatoryCustomerNrSetting = settings.find(
      (x) => x.globalName === this.globalSettingConstant.mandatoryCustomerNr
    );
    ...
    this.formFields.forEach((field) => {
      if (!field.selected) {
        ...
        if (!this.orderDataEntryForm.controls[field.fieldName]) {
          ...
          if (requireValidate)
            this.orderDataEntryForm.controls[field.fieldName].setValidators(
              requireValidate
            );
  ...
  private submitFailed() {
    ...
    this.store.dispatch(
      this.processDataActions.saveOnlyMainTabResult(outputData, this.ofModule)
    );
  }
    ...
    try {
      ...
      let data = {
        ...
        OrderPayments:
          ...
          paymentTypeData.mappedData.length
            ? this.prepareOrderPaymentsData(paymentTypeData.mappedData)
            : null,
      ...
      if (customerData && customerData.mappedData) {
        ...
        if (customerData.originalCustomerData) {
          customerData.mappedData['Person']['IsActive'] = Uti.toBoolean(
            customerData.originalCustomerData.isActive.value || 'true'
          );
          customerData.mappedData['Person']['IsMatch'] = Uti.toBoolean(
            customerData.originalCustomerData.isMatch.value || 'false'
          );
        }
  ...
  private prepareOrderPaymentsData(mappedData: Array<any>) {
    if (
      ...
      mappedData.forEach((item) => {
        const paymentItem = this.orderTotalSummaryData.paymentDetails.find(
    ...
    this.dataEntryService
      ...
      .subscribe(
        (response: any) => {
          this.appErrorHandler.executeAction(() => {
            if (!response) return;
            if (response.returnID) this.saveDataEntrySuccess(response);
  ...
  private clearAllData(dontClearStore?: boolean) {
    ...
    if (dontClearStore != true) {
      this.store.dispatch(
        this.dataEntryActions.dataEntrySetArticleGridData({}, this.tabID)
      );
      this.store.dispatch(
  ...
  public mediacodeKeyup(event) {
    ...
    const isCtrlAorCorV =
      ...
      (event.keyCode === 17 || event.keyCode === 65 || event.keyCode === 67); // || event.keyCode === 86
    if (
      ...
      if (
        ...
        this.modalService.confirmMessageHtmlContent(
          new MessageModel({
            ...
            cancelText: 'No',
            callBack1: () => {
              let idx = this.validMediaCodeCampaignNrList.findIndex(
                (x) =>
                  x.mediaCode == this.previousValues.mediaCode &&
                  x.campaignNr == this.previousValues.campaignNumber
              );
              if (idx !== -1) {
                ...
                callBack: () => {
                  if (
                    ...
                    if (this.orderDataEntryForm) {
                      ...
                      this.orderDataEntryForm.controls[
                        'campaignNumber'
                      ].setValue(lastMediaCodeCampaignNr.campaignNr);
                    }
                    this.previousValues = {
                      mediaCode: lastMediaCodeCampaignNr.mediaCode,
                      campaignNumber: lastMediaCodeCampaignNr.campaignNr,
  ...
  private setIncorrectValidatorControl(controlName: string, isValid: boolean) {
    ...
    if (errorMessage) this.toasterService.pop('error', 'Error', errorMessage);
  ...
  private customerDoSearch() {
    ...
    if (value) {
      if (value !== this.customerNr) {
        ...
        this.searchService
          .search('customer', value, 2, 1, 100, null, null, null, null, true)
          .finally(() => {
              ...
              .subscribe((response) => {
                this.appErrorHandler.executeAction(() => {
                  if (
                    ...
                    this.dataEntryProcess.notApprovedPaymentsMethods = null;
                    this.store.dispatch(
                      this.dataEntryActions.rejectIdPayments([], this.tabID)
                    );
                  }
                });
              });
  ...
  private clearCustomerAndCommunicationData(dontClearStore?: boolean) {
    ...
    if (dontClearStore != true) {
      ...
      this.store.dispatch(
        this.dataEntryActions.dataEntryCustomerDataChanged(null, this.tabID)
      );
      this.store.dispatch(
        this.dataEntryActions.dataEntryCommunicationDataChanged(
          new FormModel({ formValue: null, rawData: [] }),
      ...
      if (value && (value as string).indexOf(';') > 0) {
        ...
        if (splits.length === 3) {
          ...
          if (mediacode && customer) {
            ...
            if (this.isGamerTab) this.scanBarcodeSaveAutomatically();
          }
        }
      }
  ...
  private scanBarcodeRequestSave() {
    ...
    this.store.dispatch(
      this.tabButtonActions.requestSaveOnlyWithoutControllingTab(this.ofModule)
    );
  }
  ...
  private focusOnControlOfWigdetInSimpleTab() {
    ...
    this.timeoutFocusOnControl = setTimeout(() => {
      ...
      if ($invalidInputs.length) {
        ...
        $element =
          this.idPerson || this.customerNr ? $inputs.last() : $inputs.first();
      }
  ...
  private saveOrderFailed() {
    ...
    if (
      ...
      this.orderFailedData.customerData.dateOfBirth.value = format(
        new Date(this.customerData.mappedData.PersonMasterData.DateOfBirth),
        'dd.MM.yyyy'
      );
  ...
  private execWidgetProperties(data: WidgetPropertyModel[]) {
    ...
    const orderType: WidgetPropertyModel =
      this.propertyPanelService.getItemRecursive(this._properties, 'OrderType');
    if (!orderType) return;
      ...
      .subscribe((response) => {
        this.appErrorHandler.executeAction(() => {
          ...
          if (response && response.data && response.data.length > 2) {
            this.orderDataEntryForm.controls['idSalesOrder'].setValue(
              this.idSalesOrder
            );
  ...
  private waitToLoadOrder(data: any, count?: number) {
    ...
    if (this.getErrorMessageWithRequiredWigdets()) {
      ...
      setTimeout(() => {
        ...
        this.store.dispatch(
          this.dataEntryActions.clearRequestChangeTabThenLoadData(this.tabID)
        );
      });
    }
  }
