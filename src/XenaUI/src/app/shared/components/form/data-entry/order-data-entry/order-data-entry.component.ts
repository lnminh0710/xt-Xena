import {
    ...
    ViewChild, OnInit, OnDestroy, ChangeDetectorRef, ElementRef
} from '@angular/core';
...
import {
    ...
    ModalService,
    DataEntryProcess, BaseService, SignalRService
} from 'app/services';
import { ComboBoxTypeConstant, Configuration, MessageModal, LocalStorageKey, OrderFailedDataEnum, OrderDataEntryTabEnum, GlobalSettingConstant } from 'app/app.constants';
...
import {
    FormModel, FieldFilter, ApiResultResponse,
    Currency, PaymentType, Module, MessageModel,
    FormOutputModel, OrderDataEntryModel, ParkedItemModel,
    WidgetPropertyModel,
    MessageModalModel,
...
export class OrderDataEntryFormComponent extends DataEntryFormBase implements OnInit, OnDestroy {
    ...
    @Input() set widgetProperties(data: WidgetPropertyModel[]) { this.execWidgetProperties(data); }
    ...
    @ViewChild('searchCustomerDialogModule') searchCustomerDialogModule: ModuleSearchDialogComponent;
    @ViewChild('searchCompaignDialogModule') searchCompaignDialogModule: ModuleSearchDialogComponent;
    ...
    @ViewChild('pDialogMediacode') set pDialogMediacodeInstance(pDialogMediacodeInstance: Dialog) {
    ...
    constructor(
        ...
        this.dataEntryOrderDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).orderData);
        this.customerDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).customerData);
        this.communicationDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).communicationData);
        this.paymentTypeDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).paymentTypeData);
        this.orderTotalSummaryDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).orderTotalSummaryData);
        this.articleGridExportDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).articleGridExportData);
        ...
        this.selectedSimpleTabChangedState = this.store.select(state => tabSummaryReducer.getTabSummaryState(state, this.ofModule.moduleNameTrim).selectedSimpleTab);
        this.selectedODETabState = this.store.select(state => tabSummaryReducer.getTabSummaryState(state, this.ofModule.moduleNameTrim).selectedODETab);
        this.requestChangeTabThenLoadDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).requestChangeTabThenLoadData);
        this.selectedParkedItemState = this.store.select(state => parkedItemReducer.getParkedItemState(state, ModuleList.OrderDataEntry.moduleNameTrim).selectedParkedItem);
        this.dataEntryCustomerMatchedDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).customerMatchedData);

        if (Configuration.PublicSettings.enableOrderFailed) {
            this.orderFailedReceiveDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).orderFailedReceiveData);
        }
    }

    public ngOnInit() {
        this.store.dispatch(this.dataEntryActions.dataEntryResetAllFormData(this.tabID));
        ...
        this.globalPropertiesStateSubscription = this.globalPropertiesState.subscribe((globalProperties: any) => {
            this.appErrorHandler.executeAction(() => {
                if (globalProperties.length) {
                    ...
                    this.additionSubtractionDay = this.propertyPanelService.getItemRecursive(globalProperties, 'AdditionSubtractionDay');
    ...
    private buildMinAndMaxOrderDate() {
        let additionSubtractionDay = ((this.additionSubtractionDay && this.additionSubtractionDay.value ? this.additionSubtractionDay.value : 0) * 1) + 1;
        const today = new Date();
        const subDate = subDays(today, additionSubtractionDay);
        const addDate = addDays(today, additionSubtractionDay);
        this.disableUntil = { year: subDate.getFullYear(), month: subDate.getMonth() + 1, day: subDate.getDate() };
        this.disableSince = { year: addDate.getFullYear(), month: addDate.getMonth() + 1, day: addDate.getDate() };
    }

    private subscribeDataEntryOrderDataState() {
        if (this.dataEntryOrderDataStateSubscription) this.dataEntryOrderDataStateSubscription.unsubscribe();

        this.dataEntryOrderDataStateSubscription = this.dataEntryOrderDataState.subscribe((orderData) => {
            this.appErrorHandler.executeAction(() => {
                if (orderData && orderData.formValue && this.orderDataEntryForm && orderData.formValue != this.orderDataEntryForm.value) {
                    this.orderDataEntryForm.setValue(orderData.formValue);
                    this.orderDataEntryForm.updateValueAndValidity();
                }
                if (!orderData || !orderData.formValue || !orderData.formValue.mediacode)
                    this.store.dispatch(this.dataEntryActions.dataEntrySetOrderDataMediaCode('', this.tabID));
                else
                    this.store.dispatch(this.dataEntryActions.dataEntrySetOrderDataMediaCode(orderData.formValue.mediacode, this.tabID));
            });
        })
    }
    ...
    private maximize() {
        ...
        if (this.pDialogMediacode) {
            this.preDialogW = this.pDialogMediacode.containerViewChild.nativeElement.style.width;
            this.preDialogH = this.pDialogMediacode.containerViewChild.nativeElement.style.height;
            this.preDialogLeft = this.pDialogMediacode.containerViewChild.nativeElement.style.left;
            this.preDialogTop = this.pDialogMediacode.containerViewChild.nativeElement.style.top;

            this.pDialogMediacode.containerViewChild.nativeElement.style.width = $(document).width() + 'px';
            this.pDialogMediacode.containerViewChild.nativeElement.style.height = $(document).height() + 'px';
            this.pDialogMediacode.containerViewChild.nativeElement.style.top = '0px';
            this.pDialogMediacode.containerViewChild.nativeElement.style.left = '0px';
            this.mediaCodeBodyHeight = { 'height': ($(document).height() - 120) + 'px' };
        }
    }
    ...
    private restore() {
        ...
        this.mediaCodeBodyHeight = { 'height': (this.mediaCodeModalHeight - 135) + 'px' };
        if (this.pDialogMediacode) {
            this.pDialogMediacode.containerViewChild.nativeElement.style.width = this.preDialogW;
            this.pDialogMediacode.containerViewChild.nativeElement.style.height = this.preDialogH;
            this.pDialogMediacode.containerViewChild.nativeElement.style.top = this.preDialogTop;
            this.pDialogMediacode.containerViewChild.nativeElement.style.left = this.preDialogLeft;
        }
    }

    private subscribeScanningStatusDataState() {
        if (this.scanningStatusDataStateSubscription) this.scanningStatusDataStateSubscription.unsubscribe();
        ...
        this.scanningStatusDataStateSubscription = this.scanningStatusDataState.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (response == null || response == undefined || !this.orderDataEntryForm) return;

                const item: any = response.length ? response[0] : null;
                const message = item ? ' getDataByMediaCode & getCustomerData: ' + item.mediaCode + ' - ' + item.customerNr : '';
                ...
                if (item) {
                    ...
                    if (item.customerNr && item.customerNr.length) {
                        this.orderDataEntryForm.controls['customer'].setValue(item.customerNr);
                        this.orderDataEntryForm.controls['customer'].updateValueAndValidity();

                        this.getCustomerData(item.customerNr, item.mediaCode);
                    }

                    if (item.mediaCode && item.mediaCode.length) {
                        this.orderDataEntryForm.controls['mediacode'].setValue(item.mediaCode);
                        this.orderDataEntryForm.controls['mediacode'].updateValueAndValidity();
                        this.orderDataEntryForm.controls['campaignNumber'].setValue(item.campaignNr);
                        this.orderDataEntryForm.controls['campaignNumber'].updateValueAndValidity();
    ...
    private subcribeDataEntryState() {
        this.customerDataStateSubscription = this.customerDataState.subscribe((customerDataState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (customerDataState && !isEqual(this.customerData, customerDataState)) {
        ...
        this.communicationDataStateSubscription = this.communicationDataState.subscribe((communicationDataState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (communicationDataState && !isEqual(this.communicationData, communicationDataState)) {
        ...
        this.articleGridExportDataStateSubscription = this.articleGridExportDataState.subscribe((response: any) => {
            this.appErrorHandler.executeAction(() => {
                if (response && !isEqual(this.articleGridExportData, response)) {
        ...
        this.orderTotalSummaryDataStateSubscription = this.orderTotalSummaryDataState.subscribe((orderTotalSummaryDataState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (orderTotalSummaryDataState && !isEqual(this.orderTotalSummaryData, orderTotalSummaryDataState)) {
                    this.orderTotalSummaryData = orderTotalSummaryDataState;
                }
            });
        });
        ...
        this.selectedSimpleTabChangedStateSubscription = this.selectedSimpleTabChangedState.subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
                if (!data || !data.TabID || this.dataEntryProcess.selectedODETab.TabID != this.tabID) return;

                this.focusOnControlOfWigdetInSimpleTab();
            });
        });

        this.selectedODETabStateSubscription = this.selectedODETabState.subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
                if (!data || !data.TabID || this.dataEntryProcess.selectedODETab.TabID != this.tabID) return;
    ...
    private reInitTootltip() {
        ...
        this.rebuildTooltip('campaignNumber', 'order-data-entry-campaign-number');
        this.rebuildTooltip('customer', 'order-data-entry-customer-number');
    }

    private rebuildTooltip(formField: string, divId: string) {
        if (this[formField + 'ChangeSubscription']) this[formField + 'ChangeSubscription'].unsubscribe();

        this[formField + 'ChangeSubscription'] = this.orderDataEntryForm.controls[formField].valueChanges
            .debounceTime(this.consts.valueChangeDeboundTimeDefault)
            .subscribe((data) => {
                this.appErrorHandler.executeAction(() => {
                    const txt = $('#txt-' + divId, this.elementRef.nativeElement);
                    let fontSize: any = txt.css('font-size');
                    fontSize = isNaN(parseInt(fontSize)) ? 12 : parseInt(fontSize);
                    const textWidth = Uti.getTextWidth(txt.val(), fontSize);
                    const div: any = $('#' + divId, this.elementRef.nativeElement);
                    ...
                    switch (formField) {
                        case 'campaignNumber':
                            this.store.dispatch(this.dataEntryActions.dataEntrySelectedCampaignNumberData(this.orderDataEntryForm.controls['campaignNumber'].value, this.tabID));
                            break;

                        case 'customer':
                            this.store.dispatch(this.dataEntryActions.dataEntryCustomerIdChanged(this.orderDataEntryForm.controls['customer'].value, this.tabID));
                            break;
                    }
                });
            });
    ...
    private getCustomMandatoryField(mediaCode: string) {
        if (!mediaCode) {
            this.store.dispatch(this.dataEntryActions.dataEntrySetCustomerMandatoryField(null, this.tabID));
            return;
        }

        this.personService.preLoadBusinessLogic(mediaCode)
            .subscribe((response) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (Uti.isResquestSuccess(response) &&
                        ...
                        && typeof response.item[0][0].GetMandatoryField === 'string') {
                        mandatoryFields = JSON.parse(response.item[0][0].GetMandatoryField);
                        if (mandatoryFields && mandatoryFields.MandatoryParameter) {
                            mandatoryFields = mandatoryFields.MandatoryParameter;
                        }
                    }
                    this.store.dispatch(this.dataEntryActions.dataEntrySetCustomerMandatoryField(mandatoryFields, this.tabID));
                });
            });
    }
    ...
    private getInitDropdownlistData() {
        this.comService.getListComboBox(
            ComboBoxTypeConstant.orderBy + ',' + ComboBoxTypeConstant.principal + ',' + ComboBoxTypeConstant.orderType + ',' + ComboBoxTypeConstant.getShipper)
            .subscribe((response: ApiResultResponse) => {
                this.appErrorHandler.executeAction(() => {
                    if (!Uti.isResquestSuccess(response)) return;

                    if (!response.item.getShipper) response.item.getShipper = [];
    ...
    public storeFormControlValue(): void {
        ...
        if (this.orderDataEntryForm.controls['orderDate'].value) {
            orderDate = new Date(this.orderDataEntryForm.controls['orderDate'].value);
            if (this.isValidOrderDate(orderDate))
                orderDate.setHours(7);
            else
                orderDate = '';
        }
        const formControlValueState = {
            isDisplayBarCode: this.orderDataEntryForm.controls['isDisplayBarCode'].value,
            ...
            idRepSalesOrderShipper: this.orderDataEntryForm.controls['idRepSalesOrderShipper'].value,
        };
        this.callUpdateOrderTypeProperty(formControlValueState.orderType);
        LocalStorageHelper.toInstance(LocalStorageProvider).setItem(LocalStorageKey.buildKey(LocalStorageKey.OrderDataEntry_FormControlValue, this.tabID), formControlValueState);
    }
    private subscribeCustomerMatchedData() {
        this.dataEntryCustomerMatchedDataStateSubcription = this.dataEntryCustomerMatchedDataState.subscribe((customerMatchedData: any) => {
            this.appErrorHandler.executeAction(() => {
    ...
    private subscribeRequestChangeTabThenLoadDataState() {
        this.requestChangeTabThenLoadDataStateSubcription = this.requestChangeTabThenLoadDataState.subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
                if (!data) return;
        ...
        this.selectedParkedItemStateSubscription = this.selectedParkedItemState.subscribe((selectedParkedItemState: ParkedItemModel) => {
            this.appErrorHandler.executeAction(() => {
                if (selectedParkedItemState) {
                    this.reGetOrderFailed(selectedParkedItemState.id.value);
                }
            });
        });
    }
    ...
    private bindFormControlValue(): void {
        ...
        this.orderDataEntryForm.controls['isDisplayBarCode'].setValue(formControlValueState['isDisplayBarCode']);

        if (formControlValueState['orderBy']) {
            this.orderDataEntryForm.controls['orderBy'].setValue(formControlValueState['orderBy']);
        }
        if (!this.orderTypeValueFromWidgetProperty && formControlValueState['orderType']) {
            this.orderTypeValueFromWidgetProperty = formControlValueState['orderType'];
        }
        this.orderDataEntryForm.controls['orderType'].setValue(this.orderTypeValueFromWidgetProperty || '');
        this.orderDataEntryForm.controls['orderDate'].setValue(formControlValueState['orderDate'] ? new Date(formControlValueState['orderDate']) : new Date());
        this.orderDataEntryForm.controls['orderDate'].markAsPristine();
        this.orderDataEntryForm.controls['orderDay'].setValue(formControlValueState['orderDay']);
        this.orderDataEntryForm.controls['orderMonth'].setValue(formControlValueState['orderMonth']);
        this.orderDataEntryForm.controls['orderYear'].setValue(formControlValueState['orderYear']);
        if (formControlValueState['idRepSalesOrderShipper']) {
            this.orderDataEntryForm.controls['idRepSalesOrderShipper'].setValue(formControlValueState['idRepSalesOrderShipper']);
        }
    }
    ...
    private subcribeRequestSaveState() {
        this.dispatcherSubscription = this.dispatcher.filter((action: CustomAction) => {
            ...
            this.appErrorHandler.executeAction(() => {
                ...
                if (isFormValid) {
                    const orderTypeValue = this.orderDataEntryForm.controls['orderType'].value;
                    //If order type is not GAMMER
                    if (orderTypeValue !== "2") {
                        //orderTotalSummary
                        if (!this.orderTotalSummaryData || !this.orderTotalSummaryData.paymentDetails || !this.orderTotalSummaryData.paymentDetails.length)
                            isFormValid = false;

                        //articleGrid
                        if (isFormValid && (!this.articleGridExportData || !this.articleGridExportData.isValid || !this.articleGridExportData.formValue || !this.articleGridExportData.formValue.length))
                            isFormValid = false;

                        if (isFormValid && (!this.paymentTypeData || !this.paymentTypeData.isValid || !this.paymentTypeData.mappedData || !this.paymentTypeData.mappedData.length))
                            isFormValid = false;
                    }
                }
                ...
                if (isFormValid) {
                    ...
                    if (Configuration.PublicSettings.callODEDoublet && this.isCallDoubletCheckBeforeSavingODE())
    ...
    private onSubmitSaveODE() {
        ...
        this.onSubmit(this.customerData, this.communicationData, this.paymentTypeData, this.articleGridExportData);
    }

    private getGlobalSettingsMandatoryCustomerNr() {
        const settings = BaseService.cacheService.getValue('getAllGlobalSettings:-1');
        const mandatoryCustomerNrSetting = settings.find(x => x.globalName === this.globalSettingConstant.mandatoryCustomerNr);

        if (!mandatoryCustomerNrSetting || mandatoryCustomerNrSetting.jsonSettings != '1')
            return false;
    ...
    private initEmptyData(): any {
        ...
        return {
            ...
            campaignNumber: null,
            packageNr: null,
            idRepSalesOrderShipper: formControlValueState['idRepSalesOrderShipper'],
            barcode: null,
            customer: null,
            customerId: null,
    ...
    private manageDisplayFormFields() {
        ...
        this.formFields.forEach((field) => {
            if (!field.selected) {
                ...
                if (!this.orderDataEntryForm.controls[field.fieldName]) {
                    const requireValidate = field.fieldName == 'barcode' || field.fieldName == 'customer' || field.fieldName == 'mediacode' ?
                        Validators.required : null;
                    this.orderDataEntryForm.addControl(field.fieldName, new FormControl());

                    if (requireValidate)
                        this.orderDataEntryForm.controls[field.fieldName].setValidators(requireValidate);
    ...
    private initFormData() {
        ...
        const isMandatoryCustomerNr = this.getGlobalSettingsMandatoryCustomerNr();
        ...
        this.orderDataEntryForm = this.formBuilder.group({
            ...
            customer: isMandatoryCustomerNr ? [_data.customer, Validators.required, this.incorrectValidator] : [_data.customer, null, this.incorrectValidator],
            customerId: null,
            idSalesCampaignMediaCode: null,
            mediacode: [_data.mediacode, Validators.required, this.incorrectValidator],
            orderBy: [_data.orderBy || '', Validators.required],
            orderType: [(_data.orderType || this.orderTypeValueFromWidgetProperty) || '', Validators.required],
        ...
        this.formChangeSubscription = this.orderDataEntryForm.valueChanges
            ...
            .subscribe((data) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.store.dispatch(this.dataEntryActions.dataEntryOrderDataChanged(
                        new FormModel({
                            formValue: formValue,
                            isValid: isFormValid,
                    ...
                    if (!(this.orderDataEntryForm.pristine && this.orderDataEntryForm.untouched)) {
    ...
    private getErrorMessageWithRequiredWigdets() {
        ...
        if (this.orderDataEntryForm && this.orderDataEntryForm.controls['orderType'].value != "2") {
        ...
        if (arrayObjects.length) {
            ...
            return arrayObjects.length == 1 ?
                ...
                `The ` + wigdetNames + ` widgets are required for saving data!`;
        }
    ...
    private submitFailed() {
        ...
        this.store.dispatch(this.processDataActions.formValid(outputData.isValid, this.ofModule));
        this.store.dispatch(this.processDataActions.formDirty(outputData.isDirty, this.ofModule));
        this.store.dispatch(this.processDataActions.saveOnlyMainTabResult(outputData, this.ofModule));
    }

    public onSubmit(customerData: FormModel, communicationData: FormModel, paymentTypeData, articleGridData) {
        try {
            ...
            let data = {
                ...
                CustomerData: null,
                Communications: communicationData && communicationData.mappedData && communicationData.mappedData.length ?
                    communicationData.mappedData : null,
                Orders: [this.prepareOrderData()],
                OrderArticles: articleGridData && articleGridData.mappedData && articleGridData.mappedData.length ?
                    articleGridData.mappedData : null,
                OrderPayments: paymentTypeData && paymentTypeData.mappedData && paymentTypeData.mappedData.length ?
                    this.prepareOrderPaymentsData(paymentTypeData.mappedData) : null,
                PaymentConfirm: paymentTypeData ? paymentTypeData.paymentConfirm : null
            };

            if (customerData && customerData.mappedData) {
                customerData.mappedData['PersonAlias'] = null;
                if (customerData.originalCustomerData) {
                    customerData.mappedData['Person']['IsActive'] = Uti.toBoolean(customerData.originalCustomerData.isActive.value || 'true');
                    customerData.mappedData['Person']['IsMatch'] = Uti.toBoolean(customerData.originalCustomerData.isMatch.value || 'false');
                }
    ...
    private prepareOrderPaymentsData(mappedData: Array<any>) {
        if (this.orderTotalSummaryData && this.orderTotalSummaryData.paymentDetails.length) {
            mappedData.forEach(item => {
                const paymentItem = this.orderTotalSummaryData.paymentDetails.find(p => p.paymentId == item['PaymentId']);
                if (paymentItem) {
                    let findItem = null;
                    if (paymentItem.amounts)
                        findItem = paymentItem.amounts.find(p => p.index == item['AmountIndex']);
                    ...
                    if (findItem) {
                        ...
                        if (findItem.exchangeRate) {
                            ...
                            item['SystemConversionValue'] = findItem.systemConversionValue;
                        }
                    }
                }
            });
    ...
    private prepareOrderData() {
        ...
        let resultData = {
            ...
            "IsGift": true,
            "GiftType": null,
            "IdRepCurrencyCode": this.mainCurrency ? this.mainCurrency.idRepCurrencyCode : null,
            "IdScansContainerItems": this.currentScanningStatus ? this.currentScanningStatus.idScansContainerItems : null
        };
        ...
        if (this.orderTotalSummaryData) {
            ...
            resultData['IsFreeShipping'] = this.orderTotalSummaryData.isFreeShipping;
            if (!this.orderTotalSummaryData.isFreeShipping)
                resultData['PostageCost'] = this.orderTotalSummaryData.deliveryCharges;
        }
        if (this.articleGridExportData && this.articleGridExportData.giftInfo) {
            ...
            resultData['GiftType'] = this.articleGridExportData.giftInfo.giftType;
        }
        return resultData;
    }
    ...
    private saveDataEntry(data) {
        ...
        this.dataEntryService.saveOrderDataEntry(data)
            .finally(() => {
    ...
    private saveDataEntrySuccess(saveResult: any) {
        this.dataEntryProcess.paymentTypeData = {
            paymentType: this.paymentTypeData ? cloneDeep(this.paymentTypeData.formValue) : null,
        ...
        this.store.dispatch(this.dataEntryActions.dataEntrySaveResult(new FormOutputModel({
            isDirty: false,
            isValid: true,
            formValue: this.orderDataEntryForm.value,
            submitResult: true,
        ...
        setTimeout(() => {
            this.store.dispatch(this.dataEntryActions.dataEntryScanningStatusCallReload(true, this.tabID));
    ...
    private saveDataEntryForScanBarcode(data) {
        ...
        this.dataEntryService.saveOrderDataEntry(data, true)
            .subscribe((response: any) => {
                if (!response) return;

                this.toasterService.pop('success', 'Success', 'Save added successfully');
                if (response.returnID && response.idGenerateLetter) {
                    this.saveSAVFile(response.returnID, response.idGenerateLetter);
                }
            }, (err) => {
                this.toasterService.pop('error', 'Failed', 'Save operation is not successful.');
            });
    ...
    private checkValidStatusForSaving() {
        ...
        if (orderTypeValue == "2") {
            ...
            isValid = orderDataEntryFormStatus && paymentTypeStatus && articleGridExportStatus && customerStatus;
        }

        if (isValid)
            isValid = this.customerHasCountryOfCurrentCampaign();

        this.store.dispatch(this.dataEntryActions.dataEntryDisabledSaveButton(isValid, orderTypeValue, this.tabID));

        return isValid;
    }

    private checkDirtyStatusForForm() {
        const isDirty = this.orderDataEntryForm && this.orderDataEntryForm.dirty ||
            this.customerData && this.customerData.isDirty ||
            this.paymentTypeData && this.paymentTypeData.isDirty ||
            this.articleGridExportData && this.articleGridExportData.isDirty ||
            this.communicationData && this.communicationData.isDirty;

        this.store.dispatch(this.processDataActions.formDirty(isDirty, this.ofModule));
    }
    ...
    private clearAllData(dontClearStore?: boolean) {
        ...
        if (dontClearStore != true) {
            this.store.dispatch(this.dataEntryActions.dataEntrySetArticleGridData({}, this.tabID));
            this.store.dispatch(this.dataEntryActions.dataEntrySetMainCurrencyAndPaymentTypeList(null, [], this.tabID));
        }
        this.resetWidgetData();
        this.storeFormControlValue();
        Uti.resetValueForForm(this.orderDataEntryForm);
        ...
        this.store.dispatch(this.tabSummaryActions.requestSelectSimpleTab(0, this.ofModule));
    ...
    public orderTypeSelectedIndexChanged() {
        if (this.orderTypeCtrl && this.orderTypeCtrl.selectedValue &&
            ...
            this.orderTypeValueFromWidgetProperty = this.orderDataEntryForm.controls['orderType'].value;
            this.storeFormControlValue();
        }
    }
    //#endregion
    ...
    public mediacodeKeyup(event) {
        ...
        const isCtrlAorCorV = (event.composed || event.ctrlKey) && (event.keyCode === 17 || event.keyCode === 65 || event.keyCode === 67);// || event.keyCode === 86
        if (this.inputMediaCodeFirst && !isCtrlAorCorV && this.orderDataEntryForm.controls['mediacode'].value != this.previousValues.mediaCode)
    ...
    private mediacodeDoSearch() {
        ...
        const campaignNumber = this.orderDataEntryForm.controls['campaignNumber'].value;

        if (mediaCode) {
            let allowSearch: boolean = this.previousValues.mediaCode !== mediaCode || this.previousValues.campaignNumber !== campaignNumber || (mediaCode && !campaignNumber);
            if (allowSearch) {
                this.previousValues = {
                    mediaCode: mediaCode,
            ...
            if (this.isShowDialogConfirmForChangingMediaCode() && this.validMediaCodeCampaignNrList && this.validMediaCodeCampaignNrList.length) {

                const resetDataWhenMediaCodeChanged = () => {
                    const lastMediaCodeCampaignNr = this.validMediaCodeCampaignNrList[this.validMediaCodeCampaignNrList.length - 1];
                    this.orderDataEntryForm.controls['mediacode'].setValue(lastMediaCodeCampaignNr.mediaCode);
                    this.orderDataEntryForm.controls['campaignNumber'].setValue(lastMediaCodeCampaignNr.campaignNr);
                ...
                this.modalService.confirmMessageHtmlContent(new MessageModel({
                    ...
                    messageType: MessageModal.MessageType.warning,
                    message: [{ key: '<p>' }, { key: 'Modal_Message__Do_You_Want_To_Reset_Current_Payment_Article_Data' },
                    { key: '</p>' }],
                    buttonType1: MessageModal.ButtonType.warning,
                    okText: 'Yes',
                    cancelText: 'No',
                    callBack1: () => {
                        let idx = this.validMediaCodeCampaignNrList.findIndex(x => x.mediaCode == this.previousValues.mediaCode && x.campaignNr == this.previousValues.campaignNumber);
                        if (idx !== -1) {
                            //At position Index, remove 1 item
                            this.validMediaCodeCampaignNrList.splice(idx, 1);
                        }
                        ...
                        this.store.dispatch(this.dataEntryActions.dataEntrySetArticleGridData({}, this.tabID));
                        this.store.dispatch(this.dataEntryActions.dataEntrySetMainCurrencyAndPaymentTypeList(this.mainCurrency, [], this.tabID));
        ...
        this.store.dispatch(this.dataEntryActions.dataEntrySetOrderDataMediaCode(mediaCode, this.tabID));
    }
    ...
    private isShowDialogConfirmForChangingMediaCode() {
        if (this.validMediaCodeCampaignNrList && this.validMediaCodeCampaignNrList.length &&
            (
                (this.paymentTypeData && this.paymentTypeData.isDirty) ||
                (this.articleGridExportData && this.articleGridExportData.isDirty)
            ))
    ...
    private checkShowDialogConfirmForChangingMediaCode(mediacode, campaign, setNewPaymentType, setNewArticleGrid) {
        let setNewMediaCodeCampaignNr = () => {
            //CampaignNr
            if (this.orderDataEntryForm.controls) {
                this.orderDataEntryForm.controls['campaignNumber'].setValue(campaign.CampaignNr);
        ...
        // Edited case
        if (this.isShowDialogConfirmForChangingMediaCode() && this.isSkipAction === false &&
            this.validMediaCodeCampaignNrList && this.validMediaCodeCampaignNrList.length) {

            const lastMediaCampaign = this.validMediaCodeCampaignNrList[this.validMediaCodeCampaignNrList.length - 1];
            if (!this.mediaCodeShowDialog && (lastMediaCampaign.mediaCode != mediacode || lastMediaCampaign.campaignNr != campaign.CampaignNr)) {
                // Display confirm message here
                this.modalService.confirmMessageHtmlContent(new MessageModel({
                    headerText: 'Change MediaCode',
                    messageType: MessageModal.MessageType.warning,
                    message: [{ key: '<p>' }, { key: 'Modal_Message__Mediacode_CampaignNr_Changed_Do_You_Want_To_Reset_Current_Payment_Article_Data' },
                    { key: '</p>' }],
                    buttonType1: MessageModal.ButtonType.warning,
                    callBack1: () => {
            ...
            if (lastMediaCampaign.mediaCode == mediacode && lastMediaCampaign.campaignNr == campaign.CampaignNr && !this.orderDataEntryForm.controls['campaignNumber'].value) {
                this.orderDataEntryForm.controls['campaignNumber'].setValue(lastMediaCampaign.campaignNr);
                this.focusOnControl();
            }
        }
        // Not edited yet.
    ...
    private getDataByMediaCode(mediacode: string) {
        ...
        if (this.orderDataEntryForm) {
            campaignNr = this.orderDataEntryForm.controls['campaignNumber'].value;
        }
        forkJoin(this.dataEntryService.getArticleDataByMediacodeNr(mediacode, this.isScanningTab), this.dataEntryService.getMainCurrencyAndPaymentType(mediacode, campaignNr, this.isScanningTab))
            .finally(() => {
                ...
                this.appErrorHandler.executeAction(() => {
                    ...
                    // For Article Data
                    if (response1 && response1.data && response1.data.length && response1.data[1].length)
                        isValidArticleData = true;

                    // For CampaignNr
                    if (response2 && response2.data && response2.data.length) {
                        if (response2.data[currencyIndex] && response2.data[currencyIndex].length && response2.data[currencyIndex][0].CampaignNr) {
                            ...
                            this.countryListByCampaign = JSON.parse(item.IsoCountries);

                            isValidCampaignNr = true;
                            this.store.dispatch(this.dataEntryActions.dataEntrySelectedCampaignNumberData(item.CampaignNr, this.tabID));

                            this.customerHasCountryOfCurrentCampaign();
                        }

                        // For Payment Type
                        if (response2.data[paymentTypeIndex] && response2.data[paymentTypeIndex].length)
                            isValidPaymentType = true;
                    }

                    this.setIncorrectValidatorControl('mediacode', isValidCampaignNr && isValidPaymentType);//true: valid, false: error
                    ...
                    if (isValidCampaignNr) {
                        ...
                        let setNewArticleGrid = () => {
                            if (isValidArticleData)
                                this.store.dispatch(this.dataEntryActions.dataEntrySetArticleGridData(response1, this.tabID));
                            else
                                this.store.dispatch(this.dataEntryActions.dataEntrySetArticleGridData({}, this.tabID));
                        };
                        ...
                        let setNewPaymentType = () => {
                            ...
                            if (isValidPaymentType) {
                                const array = (response2.data[paymentTypeIndex] as Array<any>);
                                let mainPaymenTypeList: Array<PaymentType> = array.map(p => {
                                ...
                                this.orderFailedData.mainCurrency = this.mainCurrency;
                                this.orderFailedData.mainPaymenTypeList = mainPaymenTypeList;
                            } else {
                                this.orderFailedData.mainPaymenTypeList = [];
                            }
                            this.store.dispatch(this.dataEntryActions.dataEntrySetMainCurrencyAndPaymentTypeList(this.mainCurrency, this.orderFailedData.mainPaymenTypeList, this.tabID));
                        };
                        this.checkShowDialogConfirmForChangingMediaCode(mediacode, campaign, setNewPaymentType, setNewArticleGrid);
                    }
                    // Not Found Campagin Nr
                    else {
                        if (this.isShowDialogConfirmForChangingMediaCode()) {
                            this.modalService.warningMessageHtmlContent({
                                message: [{ key: 'Modal_Message__The_Mediacode_And_CampaignNr_Are_Not_Consistent' }],
                                closeText: 'Ok',
                                callBack: () => {
                                    if (this.validMediaCodeCampaignNrList && this.validMediaCodeCampaignNrList.length) {
                                        const lastMediaCodeCampaignNr = this.validMediaCodeCampaignNrList[this.validMediaCodeCampaignNrList.length - 1];
    ...
    public mediaCodeClickOkDialog() {
        ...
        this.orderDataEntryForm.controls['mediacode'].setValue(this.mediaCodeResult.mediaCode);
        this.orderDataEntryForm.controls['idSalesCampaignMediaCode'].setValue(this.mediaCodeResult.idSalesCampaignMediaCode);

        if (this.inputMediaCodeFirst) {
            this.orderDataEntryForm.controls['campaignNumber'].setValue(this.mediaCodeResult.campaignNr);
        }

        this.getDataByMediaCode(this.orderDataEntryForm.controls['mediacode'].value);
        this.getCustomMandatoryField(this.orderDataEntryForm.controls['mediacode'].value);

        this.orderDataEntryForm.updateValueAndValidity();

        this.store.dispatch(this.dataEntryActions.dataEntrySetOrderDataMediaCode(this.mediaCodeResult.mediaCode, this.tabID));
        ...
        if (this.mediaCodeResult.mediaCode) {
            this.setOutputData(null, new FormOutputModel({
                isValid: this.orderDataEntryForm.valid,
                isDirty: true,
                formValue: this.orderDataEntryForm.value,
                submitResult: null
    ...
    private setIncorrectValidatorControl(controlName: string, isValid: boolean) {
        if (!isValid)
            this.scanBarcodeProcess.preventFocus = false;
        ...
        switch (controlName) {
            ...
            default:
                if (!isValid)
                    errorMessage = controlName.toUpperCase() + ' does not exist';
                break;
        }//switch
    ...
    private campaignNumberDoSearch() {
        ...
        this.store.dispatch(this.dataEntryActions.dataEntrySelectedCampaignNumberData(campaignNumber, this.tabID));
    }

    public campaignNumberShowSearchDialog() {
        if (!this.searchCompaignDialogModule) return;

        this.searchCompaignDialogModule.open(this.orderDataEntryForm.controls.campaignNumber.value);
    }
    ...
    public campaignNumberItemSelect(data) {
        ...
        this.orderDataEntryForm.controls['campaignNumber'].setValue(campaignNumber);
        this.orderDataEntryForm.controls['idSalesCampaignWizard'].setValue(data['idSalesCampaignWizard']);
        this.orderDataEntryForm.updateValueAndValidity();
        this.previousValues = {
            mediaCode: mediaCode,
            campaignNumber: campaignNumber
    ...
    public customerResetValue() {
        ...
        if (this.idPerson || this.customerNr) {
            this.modalService.confirmMessageHtmlContent(new MessageModel({
                ...
                messageType: MessageModal.MessageType.warning,
                message: [{ key: '<p>' }, { key: 'Modal_Message__Do_You_Want_To_Reset_Customer' },
                { key: '</p>' }],
                buttonType1: MessageModal.ButtonType.warning,
                callBack1: () => {
                    this.clearCustomerAndCommunicationData();
    ...
    private customerDoSearch() {
        ...
        if (value) {
            if (value !== this.customerNr) {
                ...
                this.searchService.search('customer', value, 2, 1, 100, null, null, null, null, true)
                    .finally(() => {
        ...
        if (data) {
            if (data.total == 1) {
                ...
                if (this.idPerson || this.customerNr) {
                    this.modalService.confirmMessageHtmlContent(new MessageModel({
                        headerText: 'Change Customer',
                        messageType: MessageModal.MessageType.warning,
                        message: [{ key: '<p>' }, { key: 'Modal_Message__Customer_Number_Changed_Would_You_Like_Change_Current_Customer_Communication_Data' },
                        { key: '</p>' }],
                        buttonType1: MessageModal.ButtonType.warning,
                        okText: 'Yes',
                        cancelText: 'No',
                ...
                const customerNr = this.orderDataEntryForm.controls['customer'].value;
            ...
            if (this.idPerson || this.customerNr) {
                this.modalService.confirmMessageHtmlContent(new MessageModel({
                    headerText: 'Change Customer',
                    messageType: MessageModal.MessageType.warning,
                    message: [{ key: '<p>' }, { key: 'Modal_Message__Customer_Number_Changed_Would_You_Like_Reset_Customer_Communication_Data' },
                    { key: '</p>' }],
                    buttonType1: MessageModal.ButtonType.warning,
                    callBack1: () => {
                        this.clearCustomerAndCommunicationData();
    ...
    private getCustomerData(custNr: string, mdCode?: string) {
        ...
        let mediaCode = mdCode || this.orderDataEntryForm.controls['mediacode'].value;
        this.isDataProcessing = true;
        this.dataEntryService.getCustomerDataByCustomerNr(custNr, this.isScanningTab, mediaCode)
            .finally(() => {
            ...
            .subscribe((response) => {
                this.appErrorHandler.executeAction(() => {
                    if (!response || !this.orderDataEntryForm || !this.orderDataEntryForm.controls) return;

                    //has data
                    if (response.customerData && response.customerData['personNr'] && response.customerData['personNr'].value.length) {

                        this.store.dispatch(this.dataEntryActions.dataEntryCustomerDataChanged(new FormModel({ rawData: response.customerData }), this.tabID));

                        this.idPerson = response.customerData['idPerson'].value;
                        this.customerNr = response.customerData['personNr'].value;
                        this.orderDataEntryForm.controls['customerId'].setValue(this.idPerson);
                        this.orderDataEntryForm.controls['customerId'].updateValueAndValidity();
                        this.orderDataEntryForm.controls['customer'].setValue(this.customerNr);
                        this.setIncorrectValidatorControl('customer', true);//valid
                        ...
                        this.dataEntryProcess.notApprovedPaymentsMethods = null;
                        this.dataEntryService.getRejectPayments(this.idPerson)
                            .finally(() => {
                                if (!this.idSalesOrder) {
                                    this.isDataProcessing = false;
                                }
                                ...
                                this.appErrorHandler.executeAction(() => {
                                    if (response && response.data && response.data.length && response.data[0].length) {
                                        this.dataEntryProcess.notApprovedPaymentsMethods = response.data[0];
                                        this.store.dispatch(this.dataEntryActions.rejectIdPayments(response.data[0], this.tabID));
                                        this.store.dispatch(this.dataEntryActions.updateIsCancelSAV(false, this.tabID, this.ofModule));
                                    } else {
                                        this.dataEntryProcess.notApprovedPaymentsMethods = null;
                                        this.store.dispatch(this.dataEntryActions.rejectIdPayments([], this.tabID));
                                    }
                                });
                            });
                        ...
                        setTimeout(() => {
                            ...
                            if (!this.idPerson && this.customerNr) {
                                if (Configuration.PublicSettings.callODEDoublet) {
                                    if (this.customerData) {
                                        this.customerData.needToCheckMatchingData = true;
                                    }
                    ...
                    if (response.communicationData) {
                        ...
                        this.store.dispatch(this.dataEntryActions.dataEntryCommunicationDataChanged(new FormModel({ formValue: formValue, rawData: response.communicationData }), this.tabID));
                    }
                    else {
                        //reset
                        this.communicationDataRaw = null;
                        this.store.dispatch(this.dataEntryActions.dataEntryCommunicationDataChanged(new FormModel({ formValue: null, rawData: [] }), this.tabID));
                    }
    ...
    private restoreCustomerAndCommunicationData() {
        ...
        if (this.communicationDataRaw)
            this.store.dispatch(this.dataEntryActions.dataEntryCommunicationDataChanged(new FormModel({ formValue: this.communicationDataRaw[1], rawData: this.communicationDataRaw }), this.tabID));
    ...
    private clearCustomerAndCommunicationData(dontClearStore?: boolean) {
        ...
        this.communicationDataRaw = null;
        if (dontClearStore != true) {
            this.store.dispatch(this.dataEntryActions.rejectIdPayments(null, this.tabID));
            this.store.dispatch(this.dataEntryActions.dataEntryCustomerDataChanged(null, this.tabID));
            this.store.dispatch(this.dataEntryActions.dataEntryCommunicationDataChanged(new FormModel({ formValue: null, rawData: [] }), this.tabID));
        }
    }

    public customerShowSearchDialog() {
        this.searchCustomerDialogModule.open(this.orderDataEntryForm.controls.customer.value);
    }

    public customerItemSelect(data) {
        if (!data) return;

        if ((this.idPerson || this.customerNr) && ((this.customerData && this.customerData.isDirty) || (this.communicationData && this.communicationData.isDirty))) {
            this.modalService.confirmMessageHtmlContent(new MessageModel({
                headerText: 'Change Customer',
                messageType: MessageModal.MessageType.warning,
                message: [{ key: '<p>' }, { key: 'Modal_Message__Customer_Number_Changed_Would_You_Like_Change_Current_Customer_Communication_Data' },
    ...
    private customerHasCountryOfCurrentCampaign(caseChecking?: number) {
        ...
        */
        ...
        if (this.customerData && this.customerData.formValue && this.customerData.formValue.idRepIsoCountryCode &&
            this.countryListByCampaign && this.countryListByCampaign.length &&
            this.orderDataEntryForm.controls['campaignNumber'].value) {

            //[{"IsoCode":"FR"},{"IsoCode":"DE"},{"IsoCode":"GB"},{"IsoCode":"AU"},{"IsoCode":"US"}]
            const findItem = this.countryListByCampaign.find(item => item.IdRepIsoCountryCode == this.customerData.formValue.idRepIsoCountryCode);

            if (findItem != null) { this.isShowingAlertCustomerHasCountry = false; return true; }
            ...
            if (caseChecking === 1) {
                ...
                this.modalService.alertMessage(new MessageModel({
                    ...
                    message: [{ key: 'Modal_Message__The_Country_Not_Support_Campaign' }],
                    okText: 'OK',
                    callBack: () => {
                ...
                this.toasterService.pop('error', 'Unsupported Order', 'The country does not support on this Campaign');
            }
        }
        return false;
    }
    ...
    private callDoubletCheck(forceSaveODE?: boolean) {
        this.store.dispatch(this.dataEntryActions.callDoubletCheck({ isCheck: true, forceSaveODE: forceSaveODE }, this.tabID));
    }

    private isCallDoubletCheckBeforeSavingODE() {
        return !this.idPerson || this.customerData && (this.customerData.isDirty && this.customerData.needToCheckMatchingData);
    }
    //#endregion
        ...
        if (event.keyCode == 13) {
            ...
            if (value && (value as string).indexOf(';') > 0) {
                ...
                if (splits.length === 3) {
                    ...
                    this.orderDataEntryForm.controls['customer'].setValue(customer);
                    this.orderDataEntryForm.controls['mediacode'].setValue(mediacode);
                    this.orderDataEntryForm.updateValueAndValidity();

                    if (mediacode && customer) {
                        if (this.isGamerTab) {
                            this.scanBarcodePreventFocus();
                            this.scanBarcodeProcess.needToSaveAutomatically = true;
                            this.scanBarcodeProcess.isGetCustomerFinish = false;
                            this.scanBarcodeProcess.isGetMediaCodeFinish = false;
                        }
    ...
    private scanBarcodeSaveAutomatically() {
        ...
        this.scanBarcodeSaveAutomaticallyTimeout = setTimeout(() => {
            ...
            if (!this.scanBarcodeProcess.isGetCustomerFinish || !this.scanBarcodeProcess.isGetMediaCodeFinish) {
    ...
    private scanBarcodeRequestSave() {
        ...
        this.store.dispatch(this.tabButtonActions.requestSaveOnlyWithoutControllingTab(this.ofModule))
    }
    ...
    public shipperSelectedIndexChanged() {
        ...
        if (this.shipperCtrl.selectedValue) {
            if (!this.shipperCtrl.isDroppedDown && this.orderDataEntryForm.controls['idRepSalesOrderShipper'].value && !this._isPreventSaveOrderTypeFromProperties) {
                this.storeFormControlValue();
            }
            if (ctrlPackageNr) {
    ...
    private focusOnElement($element) {
        ...
        if (!$element.is(':visible'))
            this.controlFocus.selectChildAndParentTab($element.closest('.tab-pane'));
        else
            this.getRealElementToFocus($element).focus();
    }
    ...
    private focusOnControl() {
        ...
        this.timeoutFocusOnControl = setTimeout(() => {
            if (!this.orderDataEntryForm || !this.orderDataEntryForm.controls ||
                !this.mediacodeCtrl || this.mediaCodeShowDialog || !this.barcodeCtrl || this.scanBarcodeProcess.saveEnabled) return;
            ...
            else if (!this.orderDataEntryForm.controls['idRepSalesOrderShipper'].value)
            ...
            if (elementToFocus) {
                ...
                const tabId = 'order-data-entry-tab #' + this.tabID + ' ';
                const selector = tabId + 'app-customer-data-entry-form form input, ' + tabId + 'app-customer-data-entry-form form select';
    ...
    private focusOnControlOfWigdetInSimpleTab() {
        ...
        this.timeoutFocusOnControl = setTimeout(() => {
            if (!this.mediacodeCtrl || this.mediaCodeShowDialog || this.scanBarcodeProcess.saveEnabled) return;
            ...
            const tabId = 'order-data-entry-tab #' + this.tabID + ' .simple-tab-content-container .tab-pane.active ';
            let $element: any;
            const invalidSelector = tabId + 'app-customer-data-entry-form form .ng-invalid, ' + tabId + 'data-entry-order-payment-type form .ng-invalid';
            ...
            if ($invalidInputs.length) {
                ...
                const selector = tabId + 'app-customer-data-entry-form form input:visible, ' + tabId + 'app-customer-data-entry-form form select:visible, ' +
                    tabId + 'data-entry-order-payment-type form input:visible, ' + tabId + 'data-entry-order-payment-type form select:visible';
                let $inputs = $(selector);

                if (!$inputs.length) return;

                $element = this.idPerson || this.customerNr ? $inputs.last() : $inputs.first();
            }
    ...
    private focusOnBarcode(timeout?: number) {
        if (this.orderDataEntryForm.controls['isDisplayBarCode'].value && !this.orderDataEntryForm.controls['barcode'].value)
            setTimeout(() => {
                this.barcodeCtrl.nativeElement.focus();
            }, timeout || 300);
        else
    ...
    private orderFailedDataCheckSaving: any = this.createOrderFailedDataCheckSaving();
    ...
    private clearOrderFailed(dontClearStore?: boolean) {
        ...
        this.orderFailedDataCheckSaving = this.createOrderFailedDataCheckSaving();
        //clear store
        if (dontClearStore != true) {
            this.store.dispatch(this.dataEntryActions.cachedFailedData(null, this.tabID));
        }
    }
    ...
    private subscribeOrderFailed() {
        ...
        this.orderFailedReceiveDataSubcription = this.orderFailedReceiveDataState.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                ...
                if (response && response.data) {
                    switch (response.key) {
                        case OrderFailedDataEnum.CustomerData:
                            this.orderFailedData.customerData = response.data;
                            this.orderFailedDataCheckSaving.hasCustomerData = true;
                            break;
                        case OrderFailedDataEnum.PaymentType:
                            this.orderFailedData.paymentType = response.data;
                            this.orderFailedDataCheckSaving.hasPaymentType = true;
                            break;
                        case OrderFailedDataEnum.ArticleGrid:
                            if (response.data) {
                                this.orderFailedData.articleGridExportData = response.data.articleGridExportData;
                                this.orderFailedData.articleGridCampaignData = response.data.articleGridCampaignData;
                                this.orderFailedDataCheckSaving.hasArticleGrid = true;
                            }
                            break;
                        case OrderFailedDataEnum.CommunicationData:
                            this.orderFailedData.communicationData = response.data;
                            this.orderFailedDataCheckSaving.hasCommunicationData = true;
                            break;
                    }

                    //check if there is full data -> will save local storage and save file
                    if (this.orderFailedDataCheckSaving.hasCustomerData
    ...
    private processForOrderFailed() {
        ...
        this.store.dispatch(this.dataEntryActions.orderFailedRequestData({ isNotify: true }, this.tabID));
    }
    ...
    private saveOrderFailed() {
        ...
        if (this.orderFailedData.customerData.dateOfBirth && this.customerData.mappedData && this.customerData.mappedData.PersonMasterData)
            this.orderFailedData.customerData.dateOfBirth.value = format((new Date(this.customerData.mappedData.PersonMasterData.DateOfBirth)), 'dd.MM.yyyy');
        ...
        if (this.currentScanningStatus) {
            data.IdScansContainerItems = this.currentScanningStatus.idScansContainerItems;
        }
        else {
            const fileName = data.Mediacode + '_' + data.CampaignNr + '_' + data.CustomerNr;
            //Clean FileName: remove Invalid FileName Chars
            data.IdScansContainerItems = fileName.replace(/[^a-z0-9-_]/gi, '-').replace(/-{2,}/gi, '-');
        }

        this.idFileOrderFailed = data.IdScansContainerItems;

        this.dataEntryService.saveOrderFailed(data)
            .finally(() => {
                this.isDataProcessing = false;
                this.isSaveOrderFailedProcessing = false;
    ...
    private deleteOrderFailed() {
        if (!this.dataEntryProcess.isProcessForOrderFailed || !this.idFileOrderFailed) return;

        this.dataEntryService.deleteOrderFailed(this.tabID, this.idFileOrderFailed)
            .finally(() => {
                this.isDataProcessing = false;
    ...
    private reGetOrderFailed(idScansContainerItems) {
        ...
        this.reGetOrderFailedTimeout = setTimeout(() => {
            this.isDataProcessing = true;
            this.dataEntryService.getOrderFailed(this.tabID, idScansContainerItems)
                .finally(() => {
                    this.isDataProcessing = false;
                })
                .subscribe((response: any) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        setTimeout(() => {
                            ...
                            this.store.dispatch(this.dataEntryActions.cachedFailedData(this.orderFailedData, this.tabID));
                        }, 500);
    ...
    private syncOrderFailed() {
        if (!Configuration.PublicSettings.enableOrderFailed ||
            this.dataEntryProcess.selectedODETab.TabID != this.tabID || !this.dataEntryService.hasOrderFailedInLocalStorage()) return;

        this.dataEntryService.syncOrderFailedFromLocalStorageToFiles()
            .finally(() => {
            }).subscribe(response => {
                this.appErrorHandler.executeAction(() => {
                    this.store.dispatch(this.parkedItemActions.requestReloadList(this.ofModule));
    ...
    private execWidgetProperties(data: WidgetPropertyModel[]) {
        ...
        const orderType: WidgetPropertyModel = this.propertyPanelService.getItemRecursive(this._properties, 'OrderType');
        if (!orderType) return;
    ...
    private callUpdateOrderTypeProperty(orderType: any) {
        if (!orderType || this._isPreventSaveOrderTypeFromProperties || this.orderTypeValueFromWidgetProperty == orderType) return;

        let orderTypeItem: WidgetPropertyModel = this.propertyPanelService.getItemRecursive(this._properties, 'OrderType');
        if (orderTypeItem && orderTypeItem.value && orderTypeItem.value != orderType) {
    ...
    private loadOrder() {
        if (!this.idSalesOrder || !this.editOrderCheckLoading.loadedMediaCode || !this.editOrderCheckLoading.loadedCustomer) return;

        this.dataEntryProcess.isProcessingLoadEditOrder = true;
        this.dataEntryService.getSalesOrderById(this.idSalesOrder)
            .finally(() => {
                ...
                this.appErrorHandler.executeAction(() => {
                    ...
                    if (response && response.data && response.data.length > 2) {
                        this.orderDataEntryForm.controls['idSalesOrder'].setValue(this.idSalesOrder);
                        const editOrderData = this.dataEntryProcess.buildEditOrderModel(response.data, this.mainCurrency, this.orderFailedData.mainPaymenTypeList);
                        this.restorePreviousFormValue(editOrderData.order, true);
    ...
    private waitToLoadOrder(data: any, count?: number) {
        ...
        if (this.getErrorMessageWithRequiredWigdets()) {
            ...
            setTimeout(() => {
                ...
                this.orderDataEntryForm.controls['mediacode'].setValue(data.mediacode);
                this.orderDataEntryForm.controls['customer'].setValue(data.customerNr);
                this.orderDataEntryForm.controls['campaignNumber'].setValue(data.campaignNr);
                this.getDataByMediaCode(data.mediacode);
                this.getCustomerData(data.customerNr);

                this.store.dispatch(this.dataEntryActions.clearRequestChangeTabThenLoadData(this.tabID));
            });
        }
    }
    //#endregion
