...
import { FormModel, FieldFilter, FormOutputModel, OrderDataEntryCustomerModel } from 'app/models';
...
export class CustomerDataEntryFormComponent extends DataEntryFormBase implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.dataEntryCustomerState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).customerData);
    ...
    private subscribeDataEntrySate() {
        this.dataEntryStateCustomerSubscription = this.dataEntryCustomerState.subscribe((response: FormModel) => {
        ...
        this.orderFailedRequestDataStateSubcription = this.orderFailedRequestDataState.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (response && response.isNotify) {
                    this.store.dispatch(this.dataEntryActions.orderFailedReceiveData({ key: OrderFailedDataEnum.CustomerData, data: this.customerData }, this.tabID));
                }
            });
        });
    ...
    public outputDataCustomerForm(evenData) {
        if (evenData.customData) {
            this.store.dispatch(this.dataEntryActions.dataEntryCustomerDataChanged(evenData.customData, this.tabID));
        }
