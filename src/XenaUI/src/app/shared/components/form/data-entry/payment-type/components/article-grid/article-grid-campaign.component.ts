...
export class ArticleGridCampaignComponent extends DataEntryFormBase implements OnInit, OnDestroy, AfterViewInit {
    ...
    private autoAddItemToTopGrid() {
        if (!this.dataResult2 || !this.dataResult2.data || !this.dataResult2.data.length || this.dataResult2.data.length > 1) return;
    ...
    public addArticleToGrid(articleData: any, quantity?: number, doNotShowMessage?: boolean, autoAdd?: boolean) {
        this.store.dispatch(this.dataEntryActions.addArticleDataFromArticleCampaignGrid({
            articleData,
            quantity
        }, this.tabID));
    }     
