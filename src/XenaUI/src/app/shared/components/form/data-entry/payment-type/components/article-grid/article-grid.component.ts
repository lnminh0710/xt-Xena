    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    private subscription() {
        this.articleGridDataStateSubscription =
            this.articleGridDataState.subscribe((response) => {
        ...
        if (item) {
            ...
            if (!item.deleted) {
                ...
                gridData = gridData.filter(
                    (p) => p.ArticleNr != articleData.ArticleNr
                );
            }
        }
    ...
    private setValidationForForm() {
        ...
        if (!this.checkResult1HasData()) {
            this.formGroup.controls["articleNr"].setValidators(
                Validators.required
            );
    ...
    private calcualteArticleTotal(): void {
        ...
        if (isFinite(this.total)) {
            this.store.dispatch(
                this.dataEntryActions.dataEntrySetArticleTotal(
                    this.total,
                    this.tabID
                )
            );
    ...
    private buildOutputModel(data) {
        let model = new FormModel({
            ...
            isValid: false,
            giftInfo: {
                isGift: this.formGroup
                    ? this.formGroup.controls["isGift"].value
                    : null,
                giftType: this.formGroup
                    ? this.formGroup.controls["gift"].value
                    : null,
            },
        });
    ...
    onShowDialog() {
        setTimeout(() => {
            if (this.wijQuantNum) {
                $(this.wijQuantNum.nativeElement).val(
                    this.selectedArticleItemQuantity
                );
    ...
    public deleteDataEntryArticle($event: any) {
        const remainItems =
            this.articleOrderGrid.gridOptions.rowData.filter(
                (x) => !x.IsDeleted
            ) || [];
        this.dataResult1 = {
            data: remainItems,
