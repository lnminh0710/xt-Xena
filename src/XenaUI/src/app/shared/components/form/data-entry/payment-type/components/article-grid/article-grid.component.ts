...
import {
    ...
    ModalService,
    HotKeySettingService, DataEntryProcess
} from 'app/services';
import isNil from 'lodash-es/isNil';
import isEmpty from 'lodash-es/isEmpty';
import cloneDeep from 'lodash-es/cloneDeep';
import {
    DataEntryActions, CustomAction
...
import {
    DataEntryFormBase
} from 'app/shared/components/form/data-entry/data-entry-form-base';
import {
...
export class ArticleGridComponent extends DataEntryFormBase implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('articleUsedInCampaign') articleUsedInCampaign: XnAgGridComponent;
    ...
    constructor(
        ...
        this.articleGridDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).articleGridData);
    ...
    private subscription() {
        this.articleGridDataStateSubscription = this.articleGridDataState.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (!response || (isEmpty(response))) {
                    this.createEmptyDataForGrid();
                    this.resetFormData();
                    this.store.dispatch(this.dataEntryActions.dataEntrySetArticleGridExportData(null, this.tabID));
                ...
                if (this.dataResult2 && this.dataResult2.data && this.dataResult2.data.length == 1) {
        ...
        this.orderDataMediaCodeSubscription = this.orderDataMediaCode.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (!response) return;
        ...
        this.selectedSimpleTabChangedStateSubscription = this.selectedSimpleTabChangedState.subscribe((selectedSimpleTab: any) => {
            this.appErrorHandler.executeAction(() => {
        ...
        this.addArticleDispatcherSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === DataEntryActions.DATA_ENTRY_ADD_ARTICLE_DATA_FROM_ARTICLE_CAMPAIGN_GRID && action.area == this.tabID;
        }).subscribe((data: any) => {
            this.appErrorHandler.executeAction(() => {
                if (data && data.payload) {
                    this.addArticleToGrid(data.payload.articleData, data.payload.quantity);
                }
            });
        });
    ...
    public addArticleToGrid(articleData: any, quantity?: number, doNotShowMessage?: boolean, autoAdd?: boolean) {
        ...
        const salePrice = Uti.hasNotValue(articleData.SalesPrice) ? 0 : articleData.SalesPrice;
        const dataRow = {
            IdArticle: articleData.IdArticle,
            ArticleNr: articleData.ArticleNr,
            ArticleNameShort: articleData.ArticleNameShort,
            Quantity: Uti.hasNotValue(quantity) ? (this.formGroup.value.quantity || 0) : quantity,
            SalesPrice: salePrice,
            SalesPriceOrigin: salePrice,
            TotalPrice: (salePrice * (Uti.hasNotValue(quantity) ? (this.formGroup.value.quantity || 0) : quantity)).toFixed(2),
            IsAllArticle: (!articleData.IsAllArticle) ? false : articleData.IsAllArticle
        };
        ...
        this.store.dispatch(this.dataEntryActions.dataEntrySetArticleGridExportData(this.buildOutputModel(this.dataResult1.data), this.tabID));
    ...
    private checkResult1HasData(): boolean {
        if (!this.dataResult1 || !this.dataResult1.data || !this.dataResult1.data.length)
            return false;
    ...
    private calcualteArticleTotal(): void {
        ...
        if (isFinite(this.total)) {
            this.store.dispatch(this.dataEntryActions.dataEntrySetArticleTotal(this.total, this.tabID));
        }
    }
    ...
    private findArticleNumberExistInBottomGrid() {
        if (!this.dataResult2 || !this.dataResult2.data || !this.dataResult2.data.length) return null;
        const resutl = this.dataResult2.data.find(x => x.ArticleNr === this.formGroup.value.articleNr);
        return resutl;
    }
    private findArticleAndAddToGrid() {
        this.articleServiceSubscription = this.articleService.searchArticleByNr(ArticleGridComponent.orderDataMediaCodeNr, this.formGroup.value.articleNr).subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (!response || !response.item.data || !response.item.data.length || !response.item.data[1] || !response.item.data[1].length) {
                    this.showArticleNrError = true;
                    this.focusControls('articleNr');
                    this.isAdding = false;
                    return;
    ...
    onShowDialog() {
        setTimeout(() => {
            if (this.wijQuantNum) {
                ...
                setTimeout(() => {
                    $(this.wijQuantNum.nativeElement).unbind("keyup").bind("keyup", (event) => {
                        if (event.keyCode === 13) { // means Enter key
    ...
    public deleteDataEntryArticle($event: any) {
        const remainItems = this.articleOrderGrid.gridOptions.rowData.filter(x => !x.IsDeleted) || [];
        this.dataResult1 = {
            data: remainItems,
            columns: this.dataResult1.columns
        };
    ...
    public onArticleOrderGridCellEditEndedHandler(e) {
        ...
        if (willCalculateTotal) {
            e.data.TotalPrice = Uti.parFloatFromObject((e.data.SalesPrice * e.data.Quantity).toFixed(2), 0);
        }
        ...
        this.orderFailedRequestDataStateSubcription = this.orderFailedRequestDataState.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (response && response.isNotify) {
                    this.store.dispatch(this.dataEntryActions.orderFailedReceiveData({
                        key: OrderFailedDataEnum.ArticleGrid, data: {
    ...
    private subscribeEditOrder() {
        this.editOrderDataStateSubcription = this.editOrderDataState.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (!response || !response.articleData || !this.dataResult1 || !this.dataResult1.data) return;
