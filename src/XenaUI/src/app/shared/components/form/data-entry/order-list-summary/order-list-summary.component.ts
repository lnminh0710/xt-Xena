...
import {
    PaperworkComponent
} from 'app/shared/components/widget/components/paperwork';
import {
...
import { FilterModeEnum, MessageModal, OrderDataEntryTabEnum } from 'app/app.constants';
...
export class DataEntryOrderListSummaryComponent extends DataEntryFormBase implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.selectSummaryFilterState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).selectSummaryFilter);
        this.clearSummaryFilterDataState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).clearSummaryFilterData);
    }
    public ngOnInit() {
        this.selectSummaryFilterStateSubscription = this.selectSummaryFilterState.subscribe((selectSummaryFilterState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (selectSummaryFilterState) {
                    this.dataEntryService.getOrderDataEntryByLogin(selectSummaryFilterState.mode, selectSummaryFilterState.dateFrom, selectSummaryFilterState.dateTo).subscribe((response) => {
                        this.appErrorHandler.executeAction(() => {
                            if (!uti.Uti.isResquestSuccess(response)) {
                                return;
                            }
                            let tableData: any = this.datatableService.formatDataTableFromRawData(response.item.data);
                            this.contentDetail = cloneDeep(tableData);
                            this.onHeaderColsUpdated.emit(Object['values'](this.contentDetail.columnSettings).map(x => {
                                return {
                            ...
                            this.store.dispatch(this.dataEntryActions.orderSummaryDataLoaded({
                                mode: selectSummaryFilterState.mode,
                                dataCount: this.orderListSummaryDataSource.data.length
                            }, this.tabID));
                        });
                    });
                }
        ...
        this.clearSummaryFilterDataStateSubscription = this.clearSummaryFilterDataState.subscribe((clearSummaryFilterDataState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (clearSummaryFilterDataState) {
                    if (this.orderListSummaryDataSource && this.orderListSummaryDataSource.columns) {
                        let currentColumns = this.orderListSummaryDataSource.columns;
    ...
    public ngOnDestroy() {
        this.store.dispatch(this.dataEntryActions.clearSelectedOrderSummaryItem(this.tabID));

        uti.Uti.unsubscribe(this);
    }
    public rebuildTranslateText() {
        this.rebuildTranslateTextSelf();
        uti.Uti.rebuildColumnHeaderForGrid(this.orderListSummaryDataSource, this.transferTranslate);
    }
    ...
    private subscribePrintState() {
        this.printStateSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === DataEntryActions.DATA_ENTRY_PRINT && action.area == this.tabID;
        }).subscribe(() => {
            this.appErrorHandler.executeAction(() => {
                this.printWidget();
            });
    ...
    private rebuldColumnsSettingFromFilter() {
        ...
        let tableData = this.datatableService.buildDataSource(cloneDeep(contentDetail));
        tableData = this.datatableService.buildWijmoDataSource(tableData);
        this.orderListSummaryDataSource = tableData;
    }
        ...
        if (item) {
            ...
            this.modalService.confirmMessageHtmlContent(new MessageModel({
                ...
                message: [{key: '<p>'},
                    ...
                    {key: '</p>'}],
                callBack1: () => {
                    this.store.dispatch(this.dataEntryActions.requestChangeTab(willChangeTabId));

                    this.store.dispatch(this.dataEntryActions.requestChangeTabThenLoadData({
    ...
    public onChangeSelectedItems($event) {
        if ($event && $event.length) {
            let selectedItem = {
                IdSalesOrder: $event.find(x => x.key === 'IdSalesOrder').value
            }
            this.store.dispatch(this.dataEntryActions.selectOrderSummaryItem(selectedItem, this.tabID));
        }
    }
    ...
    private saveUnblockOrder(eventData: any, isDelete?: boolean) {
        this.backOfficeService.saveUnblockOrder(eventData.IdSalesOrder, isDelete)
