  ...
  implements OnInit, OnDestroy
{
                ...
                .subscribe((response) => {
                  this.appErrorHandler.executeAction(() => {
                    ...
                    this.onHeaderColsUpdated.emit(
                      Object['values'](this.contentDetail.columnSettings).map(
                        (x) => {
                          return {
                      ...
                      this.dataEntryActions.orderSummaryDataLoaded(
                        {
                          ...
                          dataCount:
                            this.orderListSummaryDataSource.data.length,
                        },
                        this.tabID
                      )
                    );
    ...
    if (item) {
      ...
      this.modalService.confirmMessageHtmlContent(
        new MessageModel({
          ...
          callBack1: () => {
            this.store.dispatch(
              this.dataEntryActions.requestChangeTab(willChangeTabId)
            );
  ...
  public onChangeSelectedItems($event) {
    if ($event && $event.length) {
      ...
      this.store.dispatch(
        this.dataEntryActions.selectOrderSummaryItem(selectedItem, this.tabID)
      );
    }
  }
