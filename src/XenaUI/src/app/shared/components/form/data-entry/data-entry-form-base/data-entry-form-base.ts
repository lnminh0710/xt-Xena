...
export abstract class DataEntryFormBase extends BaseComponent {
    ...
    set translatedSource(source: Array<any>) {
        if (source && source.length) {
            this.currentTranslatedSource = Uti.mergeLocalizer(this.defaultTranslateText, source);
        } else {
            this.currentTranslatedSource = this.defaultTranslateText;
        }
        this.localizer = Uti.buildLocalizer(this.localizer, this.currentTranslatedSource);
    ...
    private initLoccalize(data: any) {
        ...
        this.defaultTranslateText = this.translateInitData[data.defaultTranslateText];
