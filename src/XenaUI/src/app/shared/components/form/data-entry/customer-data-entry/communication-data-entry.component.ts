import {
    Component, Input, Output, ViewChild, OnInit, OnDestroy, EventEmitter
} from '@angular/core';
import { WidgetTemplateSettingService, AppErrorHandler, DataEntryProcess } from 'app/services';
...
import { FormModel, WidgetDetail, FormOutputModel, OrderDataEntryCommunicationModel } from 'app/models';
...
export class CommunicationDataEntryComponent extends DataEntryFormBase implements OnInit, OnDestroy {
    ...
    private REQUEST_STRING = {
        'Request':
        {
            ...
            'Data':
                '{"MethodName" : "SpAppWg002GetGridCommunication", "CrudType"  : null, "Object" : null,"Mode" :  null, "WidgetTitle" : "Begin new World...", "IsDisplayHiddenFieldWithMsg" : "1",<<LoginInformation>>,<<InputParameter>>}'
        }
    };
    ...
    constructor(
        ...
        this.dataEntryCommunicationState = this.store.select(state => dataEntryReducer.getDataEntryState(state, this.tabID).communicationData);
    ...
    private subscribeDataEntryCustomerIdState() {
        ...
        this.dataEntryCustomerIdStateSubscription = this.dataEntryCustomerIdState.subscribe((customerId) => {
    ...
    private subscribeDataEntryCommunicationState() {
        ...
        this.dataEntryCommunicationStateSubscription = this.dataEntryCommunicationState.subscribe((data) => {
            this.appErrorHandler.executeAction(() => {
                if (data && data.rawData && data.rawData.length > 0) {
                    this.dataSourceTable = cloneDeep(data.rawData[1]);
                }
                else if (!this.isUpdatedInside && (!data || !data.mappedData)) {
    ...
    private getCommunicationByCustomerId() {
        ...
        this.widgetTemplateSettingServiceSubscription = this.widgetTemplateSettingService.getWidgetDetailByRequestString(widgetDetail, { IdPersonInterface: this.customerId })
            .subscribe((response: WidgetDetail) => {
                this.appErrorHandler.executeAction(() => {
                    this.dataSourceTable = response.contentDetail.collectionData;
                });
            });
    }
    ...
    public getCommOutputData(eventData) {
        ...
        this.store.dispatch(this.dataEntryActions.dataEntryCommunicationDataChanged(eventData, this.tabID));
            ...
            this.appErrorHandler.executeAction(() => {
                if (response && response.isNotify) {
                    this.store.dispatch(this.dataEntryActions.orderFailedReceiveData({
                        key: OrderFailedDataEnum.CommunicationData, data: this.formvalue || {}
                    }, this.tabID));
                }
            });
        });
        ...
        this.cachedFailedDataStateSubcription = this.cachedFailedDataState.subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (response && response.communicationData) {
                    ...
                    this.dataSourceTable = Object.assign([], response.communicationData);
                }
            });
        });
    }
