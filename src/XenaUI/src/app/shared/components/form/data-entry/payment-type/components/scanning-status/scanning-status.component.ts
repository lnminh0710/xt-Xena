  ...
  implements OnInit, OnDestroy
{
  ...
  public ngOnDestroy() {
    ...
    this.store.dispatch(
      this.dataEntryAction.dataEntryScanningStatusCallReload(false, this.tabID)
    );
    this.store.dispatch(
      this.dataEntryAction.dataEntryScanningStatusCallSkip(false, this.tabID)
    );
  ...
  private loadScanningLot(isReload?: boolean) {
    this.commonService
      .getListComboBox('' + ComboBoxTypeConstant.dataEntryLots, null, true)
      .subscribe((response: any) => {
  ...
  private processForLoadFromBuffer() {
    ...
    // save data to Store
    this.store.dispatch(
      this.dataEntryAction.dataEntryScanningStatusSummary(tempData, this.tabID)
    );
  ...
  private makeData(data: any) {
    ...
    // save data to Store
    this.store.dispatch(
      this.dataEntryAction.dataEntryScanningStatusSummary(newData, this.tabID)
    );
  ...
  private makeTotalData(data) {
    return {
      ...
      qtyCapture: Uti.isNullUndefinedEmptyObject(data['QtyCapture'])
        ? 0
        : data['QtyCapture'],
      remaningOrder: Uti.isNullUndefinedEmptyObject(data['Remaning Order'])
        ? 0
        : data['Remaning Order'],
  ...
  private loadBufferForItem(item: any) {
    ...
    if (item.CustomerNr)
      this.dataEntryService
        .getCustomerDataByCustomerNr(item.CustomerNr, true, item.MediaCode)
        .subscribe();
    ...
    if (item.MediaCode) {
      this.dataEntryService
        ...
        .subscribe();
      this.dataEntryService
        .getMainCurrencyAndPaymentType(item.MediaCode, item.CampaignNr, true)
        .subscribe();
    }
  }
  ...
  private getLastestLotIndex() {
    if (!this.lotNames.length || !this.dataEntryProcess.currentScanningLotId)
      return null;
  ...
  private loadDataByLotId() {
    ...
    if (this.autoSelectLot || !selectedValue || this.preventLoadScanningData)
      return;
