...
export class DataEntryNumberOrderSummaryComponent extends DataEntryFormBase implements OnInit, OnDestroy {
    ...
    public ngOnInit() {
        ...
        this.store.dispatch(this.dataEntryActions.dataEntrySelectSummaryFilter(this.currentFilterType, this.tabID));

        this.dataEntryService.getTotalDataEntryByLogin().subscribe((response) => {
            this.appErrorHandler.executeAction(() => {
                if (!Uti.isResquestSuccess(response) || !response.item.data || !response.item.data[0] || !response.item.data[0][0]) {
                    return;
                }
    ...
    private subscribeOrderSummaryDataLoadedState() {
        this.onOrderSummaryDataLoadedSubscription = this.dispatcher.filter((action: CustomAction) => {
            return action.type === DataEntryActions.DATA_ENTRY_ORDER_SUMMARY_DATA_LOADED && action.area == this.tabID;
        }).map((action: CustomAction) => {
            return action.payload;
        }).subscribe((orderSummaryDataLoadedState: any) => {
            this.appErrorHandler.executeAction(() => {
                let filter = this.uiFilterData.find(f => f.type == orderSummaryDataLoadedState.mode);
                if (filter) {
    ...
    private subscribeSelectedODETabState() {
        this.selectedODETabStateSubscription = this.selectedODETabState.subscribe((selectedODETabState: any) => {
            this.appErrorHandler.executeAction(() => {
                if (selectedODETabState && selectedODETabState.TabID == 'OrderSummary') {
                    this.dataEntryService.getTotalDataEntryByLogin().subscribe((response) => {
                        this.appErrorHandler.executeAction(() => {
                            if (!Uti.isResquestSuccess(response) || !response.item.data || !response.item.data[0] || !response.item.data[0][0]) {
                                return;
                            ...
                            if (this.uiFilterData[0].count != response.item.data[0][0].Day) {
                                this.uiFilterData[0].count = response.item.data[0][0].Day;

                                if (this.currentFilterType === OrderSummaryFilterConst.DAY) {
                                    currentFilterTypeIndex = 0;
                                }
                            }

                            if (this.uiFilterData[1].count != response.item.data[0][0].Week) {
                                this.uiFilterData[1].count = response.item.data[0][0].Week;

                                if (this.currentFilterType === OrderSummaryFilterConst.WEEK) {
                                    currentFilterTypeIndex = 1;
                                }
                            }

                            if (this.uiFilterData[2].count != response.item.data[0][0].Month) {
                                this.uiFilterData[2].count = response.item.data[0][0].Month;

                                if (this.currentFilterType === OrderSummaryFilterConst.MONTH) {
                                    currentFilterTypeIndex = 2;
                                }
                            }

                            if (this.uiFilterData[4].count != response.item.data[0][0].SendToAdmin) {
                                this.uiFilterData[4].count = response.item.data[0][0].SendToAdmin;

                                if (this.currentFilterType === OrderSummaryFilterConst.SEND_TO_ADMIN) {
    ...
    public filterTypeClicked($event, index): void {
        ...
        if (this.currentFilterType) {
            switch (this.currentFilterType) {
                ...
                default:
                    this.store.dispatch(this.dataEntryActions.dataEntryClearSummaryFilterData(this.tabID));
                    break;
            }
        }
    }
    ...
    public filterDateRangeEvent($event, dateFromControl: wjcInput.InputDate, dateToControl: wjcInput.InputDate, isAdminFilter?: boolean): void {
