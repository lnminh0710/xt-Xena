  ...
  implements OnInit, OnDestroy
{
    ...
    this.rowsDataStateSubscription = this.rowsDataState.subscribe(
      (rowsData: any) => {
        this.appErrorHandler.executeAction(() => {
          ...
          if (!Uti.checkKeynameExistInArray(rowData, 'key', 'IdLogin')) return;
  ...
  private updateUserProfileAfterCheck() {
    if (this.fileData && this.fileData.name) {
      this.uploadAvatar(this.updateUserProfileAfterUpload);
      return;
    }
    this.updateUserProfileAfterUpload();
  }
  ...
  private updateUserProfileSuccess(response: any) {
    this.appErrorHandler.executeAction(() => {
      this.setFormOutputData(true, response.returnID, null, true);
  ...
  private reInitControlMessage() {
    if (!this.controlMessagePasswords || !this.controlMessagePasswords.length)
      return;
                  ...
                  this.appErrorHandler.executeAction(() => {
                    ...
                    this.userRoleCached.forEach((checkedRole) => {
                      let found = this.roleDatasource.data.find(
                        (role) => role.IdLoginRoles == checkedRole.IdLoginRoles
                      );
                      if (found) {
