...
export class ElasticSearchSyncMainComponent
    ...
    implements OnInit, OnDestroy
{
    ...
    @ViewChild("searchSyncCommand")
    searchSyncCommand: ElasticSearchSyncCommandComponent;
    ...
    public onCallStart() {
        ...
        if (
            !this.syncItem ||
            !this.syncItem.checkedItems ||
            !this.syncItem.checkedItems.length
        ) {
    ...
    public onCallStop() {
        this._modalService.confirmMessageHtmlContent(
            new MessageModel({
                ...
                message: [
                    { key: "<p>" },
                    { key: "Modal_Message__Do_You_Want_To_Stop_Synchronizing" },
                    { key: "</p>" },
                ],
                    ...
                    this._commonService
                        ...
                        .subscribe((response: any) => {
                            this._appErrorHandler.executeAction(() => {
                                ...
                                this.sendMessage(
                                    SignalRActionEnum.ES_ReIndex_Stop
                                );
                            });
                        });
                },
    ...
    private createQueueAndStart() {
        ...
        this._commonService
            .createQueue(this.buildDataForSave())
            .finally(() => {
    ...
    private listenSignalRMessage() {
        if (this.messageReIndexElasticSearchSubscription)
            this.messageReIndexElasticSearchSubscription.unsubscribe();

        this.messageReIndexElasticSearchSubscription =
            this._signalRService.messageReIndexElasticSearch.subscribe(
                (message: SignalRNotifyModel) => {
                    this._appErrorHandler.executeAction(() => {
                        ...
                        if (
                            isDevMode() &&
                            message.Action !=
                                SignalRActionEnum.ES_ReIndex_SyncProcessState
                        ) {
    ...
    private updateDataForGrid(
        ...
        let row = this.dataSource.data.find(
            (x) => x.id == data.IdAppSystemScheduleQueue
        );
        if (!row || !row.id) return;

        if (isFinished) {
            row.syncStatus = data.IsSuccess
                ? "fa-check  green-color"
                : "fa-times  red-color";
            row.syncRecords =
                data.TotalRecords + " (" + data.Percentage + " %)";
        } else {
            row.syncStatus = "fa-spinner  fa-spin  orange-color";
            row.syncRecords =
                data.SynchronizedRecords + " (" + data.Percentage + " %)";
        }
    ...
    private setDataForGrid(message: SignalRNotifyModel) {
        ...
        for (let x of message.Data) {
            data.push({
                ...
                time: x.TotalSeconds,
                syncRecords:
                    x.SynchronizedRecords + " (" + x.Percentage + " %)",
    ...
    private calculatePercentValue(message: SignalRNotifyModel) {
        const data =
            message.Data && message.Data.length ? message.Data[0] : null;
        if (!data) return;
        this.percentValue = Math.floor(
            (data.SynchronizedRecords * 100) / data.TotalRecords
        );
    }
    ...
    private buildDataForSave() {
        ...
        const idRepAppSystemScheduleServiceName =
            this.syncMode === SyncModeConstant.byId
