  ...
  public createGroup(callBackFunc?: Function): void {
    ...
    setTimeout(() => {
      ...
      if (this.isAutoMatching) {
        this.autoMatchingGridData = this.pushRowDataForMatchingGroupGridData(
          this.autoMatchingGridData
        );
      } else {
        this.manualMatchingGridData = this.pushRowDataForMatchingGroupGridData(
    ...
    this.matchingConfigurationSubscription = this.toolsService
      ...
      .subscribe((response: any) => {
        this.appErrorHandler.executeAction(() => {
          ...
          try {
            cacheAllConfigurationData = this.makeConfigurationStringData(
              response.item.data[1][0].Value
            );
          } catch (e) {
