    ...
    implements OnInit, OnDestroy
{
    ...
    private changeQuantitySettingInUserTable(): void {
        for (
            var i = this.dataSourceTableForUsers.columns.length - 1;
            i >= 0;
            i--
        ) {
            ...
            .subscribe((response) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    for (
                        ...
                        this.dataSourceTableForPoolsDetail.data[i][
                            "Available"
                        ] = this.dataSourceTableForPoolsDetail.data[i]["Qty"];
                    }
            ...
            if (this.totalPoolQuantity >= this.selectedUsers.length) {
                ...
                for (
                    ...
                    if (existedItem) {
                        ...
                        this.selectedUsers[index].Quantity =
                ...
                if (mod > 0) {
                    for (let i = 0; i < mod; ++i) {
                        ...
                        this.dataSourceTableForUsers.data[index][
                            "ValidationRangeTo"
                ...
                for (
                    let i = 0;
                    i < this.dataSourceTableForUsers.data.length;
                    ++i
                ) {
                    if (
                        this.dataSourceTableForUsers.data[i][
    ...
    onTableForUsersEditSuccess(outputData) {
        ...
        if (outputData.Qty > 0)
            this.dataSourceTableForUsers.data[index][ColHeaderKey.SelectAll] =
                true;
    ...
    private mapScanAssignmentUserLoginModel(
        data: any
    ): ScanAssignmentUserLogin {
    ...
    private emptyValueUncheckedUserItems(
        oldCollection: any,
        newCollection: any
    ) {
        ...
        oldCollection.forEach((item) => {
            if (!newCollection || !newCollection.length) item.Qty = null;
                ...
                const filteredItem = newCollection.find(
