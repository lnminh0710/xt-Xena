...
import {
  ScanAssignmentAssignPoolsToUsers,
  ScanAssignmentPool,
  ScanAssignmentUserLogin,
} from 'app/models/apimodel';
...
export class ScanAssignmentStep2Component
  ...
  implements OnInit, OnDestroy
{
  ...
  private changeQuantitySettingInUserTable(): void {
    for (var i = this.dataSourceTableForUsers.columns.length - 1; i >= 0; i--) {
      if (this.dataSourceTableForUsers.columns[i].data == 'Qty') {
        this.dataSourceTableForUsers.columns[i].readOnly =
          this.isSetQtyReadOnly;
      }
    }
  }
  ...
  private reUpdateAvailableForPoolDetailGrid(value: any) {
    ...
    for (let i = 0; i < this.dataSourceTableForPoolsDetail.data.length; i++) {
      this.dataSourceTableForPoolsDetail.data[i]['Available'] =
        this.dataSourceTableForPoolsDetail.data[i]['Qty'];
    }
  }
  ...
  private createGridColumnsForPoolsDetail() {
    ...
    result.push(
      this.makeColumn('Type', 'Type', true, '35', 'nvarchar', 'Type', false)
    );
    result.push(
      this.makeColumn('Qty', 'Qty', true, '0', 'int', 'Qty', false, null, 'D')
    );
  ...
  private registerService() {
    const model = new ScanAssignmentUserLanguageCountry({
      ...
      IdScansContainer: this._inputData.gridItemData
        ? this._inputData.gridItemData.IdScansContainer || ''
        : '',
      IdScansContainerDispatchers: this._inputData.gridItemData
        ? this._inputData.gridItemData.IdScansContainerDispatchers || ''
        : '',
    });
    this.toolsService
      .getScanAssignmentUserLanguageAndCountry(model)
      .subscribe((response) => {
        this.appErrorHandler.executeAction(() => {
          ...
          this.dataSourceTableForPoolsDetail.columns.splice(
            0,
            0,
            this.makeColumn(
              'UniqueId',
          ...
          this.dataSourceTableForPoolsDetail.columns.push(
            this.makeColumn(
              'Available',
              'Available',
              true,
          ...
          for (
            let i = 0;
            i < this.dataSourceTableForPoolsDetail.data.length;
            i++
          ) {
            this.dataSourceTableForPoolsDetail.data[i]['UniqueId'] =
    ...
    this.toolsService.getScanAssignmentUsers().subscribe((response) => {
      this.appErrorHandler.executeAction(() => {
        if (!response) return;
        response = this.datatableService.formatDataTableFromRawData(
          response.data
  ...
  private calculateQuantityPerUser(): void {
    if (
      ...
      if (this.totalPoolQuantity >= this.selectedUsers.length) {
        ...
        const qtyPerSelectedUser: number =
          this.totalPoolQuantity / this.selectedUsers.length;

        // Loop all and assign value
        for (
          let i = this.dataSourceTableForUsers.data.length - 1;
          i >= 0;
          i--
        ) {
          const existedItem = find(this.selectedUsers, {
            IdLogin: this.dataSourceTableForUsers.data[i].IdLogin,
          });
          if (existedItem) {
            const index = findIndex(this.selectedUsers, {
              IdLogin: existedItem.IdLogin,
            });
            this.selectedUsers[index].Quantity = Math.floor(qtyPerSelectedUser);
            this.selectedUsers[index].InvalidQuantity = false;
            this.dataSourceTableForUsers.data[i].Qty =
              Math.floor(qtyPerSelectedUser);
            this.dataSourceTableForUsers.data[i][ColHeaderKey.SelectAll] = true;
          } else {
            this.dataSourceTableForUsers.data[i].Qty = 0;
            this.dataSourceTableForUsers.data[i][ColHeaderKey.SelectAll] =
              false;
          }
          this.dataSourceTableForUsers.data[i]['ValidationRangeTo'] =
            this.dataSourceTableForUsers.data[i].Qty;
        }
        ...
        if (mod > 0) {
          for (let i = 0; i < mod; ++i) {
            ...
            const index = findIndex(this.dataSourceTableForUsers.data, {
              IdLogin: this.selectedUsers[i].IdLogin,
            });
            this.dataSourceTableForUsers.data[index].Qty += 1;
            this.dataSourceTableForUsers.data[index]['ValidationRangeTo'] =
              this.dataSourceTableForUsers.data[index].Qty;
          }
        }
        ...
        for (let i = 0; i < this.dataSourceTableForUsers.data.length; ++i) {
          if (
            ...
            this.selectedUsers.push(
              this.mapScanAssignmentUserLoginModel(
                this.dataSourceTableForUsers.data[i]
              )
            );
          } else {
            this.dataSourceTableForUsers.data[i]['Qty'] = 0;
    ...
    if (this.totalUserQuantity > 0) {
      ...
      if (this.totalUserQuantity < this.totalPoolQuantity) {
        for (
          ...
          if (
            this.dataSourceTableForPoolsDetail.data[i][ColHeaderKey.SelectAll]
          ) {
            if (
  ...
  private updateValidateRange() {
    const totalAvailableQuantity = sumBy(
      this.dataSourceTableForPoolsDetail.data,
      function (item, index) {
  ...
  onChangeSelectedPoolItems(outputData) {
    ...
    this.totalPoolQuantity = sumBy(outputData, function (item, index) {
      return item['Qty'] > 0 && item[ColHeaderKey.SelectAll] === true
        ? item['Qty']
        : 0;
    });
  ...
  onChangeSelectedUserItems(outputData) {
    ...
    // Reset Quantity
    for (let i = 0; i < this.dataSourceTableForUsers.data.length; i++) {
      const foundItem = find(outputData, {
        IdLogin: this.dataSourceTableForUsers.data[i]['IdLogin'],
      });
      this.dataSourceTableForUsers.data[i]['Qty'] = foundItem
        ? foundItem['Qty']
        : 0;
    }
  ...
  onTableForUsersEditSuccess(outputData) {
    let index = findIndex(this.dataSourceTableForUsers.data, {
      IdLogin: outputData.IdLogin,
    });
    if (outputData.Qty > 0)
      this.dataSourceTableForUsers.data[index][ColHeaderKey.SelectAll] = true;

    this.totalUserQuantity = sumBy(
      this.dataSourceTableForUsers.data,
      function (item, index) {
    ...
    if (outputData.Qty > 0) {
      index = findIndex(this.selectedUsers, {
        IdLogin: outputData.IdLogin,
      });
      // Exsited
      if (index >= 0) {
        ...
        this.selectedUsers.push(
          this.mapScanAssignmentUserLoginModel(outputData)
        );
        index = findIndex(this.selectedUsers, {
          IdLogin: outputData.IdLogin,
        });
      ...
      if (this.totalPoolQuantity < this.totalUserQuantity) {
        ...
        this.selectedUsers[index].InvalidQuantity = false;
        index = findIndex(this.dataSourceTableForUsers.data, {
          IdLogin: outputData.IdLogin,
        });
        this.dataSourceTableForUsers.data[index]['ValidationRangeTo'] =
          outputData.Qty;
    ...
    setTimeout(() => {
      ...
      if (this.selectedUsers.length == 0) {
        this.toasterService.pop(
          'warning',
          'Validation Failed',
          'Please select at least one user item.'
        );
        return;
      }
      if (this.selectedPools.length == 0) {
        this.toasterService.pop(
          'warning',
          'Validation Failed',
          'Please select at least one pool detail item.'
        );
        return;
      }
      if (!this.queueMode) {
      ...
      this.toolsService
        .scanAssignmentAssignPoolsToUsers(model)
        .subscribe((response) => {
          this.appErrorHandler.executeAction(() => {
            if (!response) return;
            // Show Msgs
  ...
  private mapScanAssignmentAssignPoolsToUsersModel(): ScanAssignmentAssignPoolsToUsers {
    ...
    if (this.queueMode == 1) {
      scanAssignmentUserLogin = this.selectedUsers.filter(
        (r) => r.Quantity > 0
      );
    } else {
  ...
  private checkTotalAssign(saveData: ScanAssignmentAssignPoolsToUsers) {
    if (
      !saveData ||
      !saveData.ScanAssignmentUserLogins ||
      !saveData.ScanAssignmentUserLogins.length
    ) {
      this.isTotalAssign = false;
      return;
    }
  ...
  private mapScanAssignmentPoolModel(data: any): ScanAssignmentPool {
    return new ScanAssignmentPool({
      ...
      IdScansContainerAssignment: this._inputData.gridItemData
        ? this._inputData.gridItemData.IdScansContainer || ''
        : '',
      IdScansContainerDispatchers: this._inputData.gridItemData
        ? this._inputData.gridItemData.IdScansContainerDispatchers || ''
  ...
  private emptyValueUncheckedUserItems(oldCollection: any, newCollection: any) {
    if (!oldCollection || !oldCollection.length) return;
    oldCollection.forEach((item) => {
      if (!newCollection || !newCollection.length) item.Qty = null;
      else {
        const filteredItem = newCollection.find((_item) => _item.id == item.Id);
        if (!filteredItem) item.Qty = null;
      }
    });
    this.wjgridUsersTable.refresh();
  }
  ...
  private showWarningDialog() {
    this.toasterService.pop(
      'warning',
      'Validation Failed',
      'The total quantity is not enough to assign users.'
    );
  }

  private applySuggestQtyValue() {
    if (!this.invalidUserItem) return;
    this.selectedUserItems.forEach((_item) => {
      if (_item.Id !== this.invalidUserItem.item.Id) _item.Qty = 0;
      else _item.Qty = this.invalidUserItem.suggestQty;
    });
    if (this.wjgridUsersTable) this.wjgridUsersTable.refresh();
  ...
  private createGridColumnsForUsers(isQtyReadOnly?: boolean) {
    const result: any = [];
    result.push(
      this.makeColumn('Id', 'Id', false, '35', 'nvarchar', 'Id', false)
    );
    result.push(
      this.makeColumn('User', 'User', true, '35', 'nvarchar', 'User', false)
    );
    if (!isNil(isQtyReadOnly) && !isQtyReadOnly)
      result.push(
        this.makeColumn('Qty', 'Qty', true, '0', 'int', 'Qty', true, null, 'D')
      );
    else
      result.push(
        this.makeColumn('Qty', 'Qty', true, '0', 'int', 'Qty', false, null, 'D')
