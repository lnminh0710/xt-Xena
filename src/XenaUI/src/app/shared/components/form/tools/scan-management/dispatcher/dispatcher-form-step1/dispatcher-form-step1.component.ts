...
export class DispatcherFromStep1Component
    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    public onDispatchClick() {
        ...
        if (
            !this.currentDataEntryCenterValue ||
            !this.currentDataEntryCenterValue.idValue
        ) {
            this.toasterService.pop(
                "warning",
                "Validation Failed",
                "Please select Data entry center!"
            );
            return;
        }
    ...
    private currentSelectedItemLeft(): any {
        return Uti.mapArrayToObject(
            this.poolLeftGrid.selectedItem() || [],
            true
        );
    }
    ...
    private callSaveDispatcherData() {
        this.toolsServiceSubscription = this.toolsService
            .saveScanDispatcherPool({ ScanDispatcherPools: this.saveData })
    ...
    private loadLeftGridData() {
        this.toolsServiceSubscription = this.toolsService
            .getAllScanCenters(DispatcherMode.Pool)
            .subscribe((response: any) => {
                this.appErrorHandler.executeAction(() => {
    ...
    private loadRightGridData(scanCenterId: any) {
        ...
        this.toolsServiceSubscription = this.toolsService
            .getScanCenterPools(scanCenterId)
            .subscribe((response: any) => {
                this.appErrorHandler.executeAction(() => {
                    if (!response || !response.data) {
                        this.resetDataPoolRightGrid();
                    ...
                    response.columns = this.remakePoolGridColumn(
                        response.columns
                    );
    ...
    private isDataCachedAndReGetCachedData(): boolean {
        let currentItem = this.cachedPoolDataValues.find(
            (x) =>
                x.itemData &&
                x.itemData.IdPerson ===
                    this.currentSelectedItemLeft()["IdPerson"]
        );
        if (
            currentItem &&
            currentItem.itemData &&
            currentItem.itemData.IdPerson
        ) {
        ...
        for (let item of this.poolRightData.data) {
            if (item.select && item.IsActive) {
                ...
                item.dataEntryCenter =
                    this.currentDataEntryCenterValue.textValue;
            } else {
                item.dataEntryCenterId = "";
                item.dataEntryCenter = "";
            }
