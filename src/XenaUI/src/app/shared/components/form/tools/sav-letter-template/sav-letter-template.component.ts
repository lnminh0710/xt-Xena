...
import {ComboBoxTypeConstant, MessageModal, UploadFileMode, Configuration} from 'app/app.constants';
...
export class SavLetterTemplateComponent extends BaseComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private showPreviewDialogData: { main: any, template: any, reason: Array<any> } = {
        main: {},
        template: {},
        reason: []
    };
    ...
    constructor(
        ...
        this.subsGroupReason.add(_dragulaService.dropModel.asObservable().takeUntil(this.destroy$).subscribe((value) => {
    ...
    private onDropModel(args: any) {
        ...
        const itemArray = (<FormArray>this.formSav.controls['groupReason']).controls;
    ...
    requireCheckboxesToBeCheckedValidator(activeWidget, propName, minRequired = 1) {
        let checked = 0;
    ...
    public submit(callBack?: Function) {
        ...
        if (!this.letterTypeCtr.selectedValue) {
            this._toasterService.pop('warning', 'Validation Failed', 'Please select letter type.');
            return;
        }
        const isActiveWidget = (<FormArray>this.formSav.controls['assignmentWidget']).controls;
        ...
        if (this.requireOneCheckboxToBeChecked) {
            if (this.isNewAutoTemplate) {
                this._toasterService.pop('warning', 'Validation Failed', 'Please choose type of auto letter.');
            } else {
                this._toasterService.pop('warning', 'Validation Failed', 'Please choose Assignment Widget.');
            }
            return;
        }
        if (this.requireOneGroupReason && !this.isNewAutoTemplate) {
            this._toasterService.pop('warning', 'Validation Failed', 'Please add Reason Group.');
            return;
        }
        if (!this.dataTable.data.length) {
            this._toasterService.pop('warning', 'Validation Failed', 'Please upload Template File.');
            return;
        }
        if (!this.countryCheckList.hasCheckedItems()) {
            this._toasterService.pop('warning', 'Validation Failed', 'Please choose Country.');
            return;
        }
        if (this.hasDuplicatePlaceholderName && !this.isNewAutoTemplate) {
            this._toasterService.pop('warning', 'Validation Failed', 'Please don\'t input duplicate Placeholder Name');
            return;
        }
        if (this.formSav.invalid && this.formSav.dirty) {
            for (const v of group) {
                if (v['controls'].GroupName.invalid) {
                    this._toasterService.pop('warning', 'Validation Failed', 'GroupName is required');
                    return;
                }
                if (v['controls'].PlaceHolderName.invalid) {
                    this._toasterService.pop('warning', 'Validation Failed', 'PlaceHolderName is required');
                    return;
                }
                if (v['controls']['JSONGroupItems'] && v['controls']['JSONGroupItems'].length) {
                    for (const data of v['controls']['JSONGroupItems'].controls) {
                        if (data.controls.GroupItemName.invalid && data.controls.TypeOfChoice.value !== 2) {
                            this._toasterService.pop('warning', 'Validation Failed', 'Item Name is required');
                            return;
                        }
                        if (data.controls.GroupItemName.invalid && data.controls.TypeOfChoice.value === 2) {
    ...
    buildAssignWidget(data?: any): FormGroup {
        return this.fb.group({
            ...
            IdBackOfficeLettersAssignWidget: data && data.IdBackOfficeLettersAssignWidget,
    ...
    createItemReason(data?: any): FormGroup {
        ...
        return this.fb.group({
            TypeOfChoice: new FormControl(+data.TypeOfChoice || +data.TypeOfChoice == 0 ? +data.TypeOfChoice : 1),
            ItemOrderBy: new FormControl(data.ItemOrderBy || 1),
            IdBackOfficeLettersGroupsItems: new FormControl(data.IdBackOfficeLettersGroupsItems),
            IsActive: new FormControl(1),
            IsSoftDelete: new FormControl(0),
            GroupItemName: new FormControl(data.GroupItemName || '', Validators.required)
        })
    }

    onChangeTypeOfChoices(indexGroup, indexItem, data) {
        const control = (<FormArray>this.formSav.controls['groupReason']).at(indexGroup).get('JSONGroupItems') as FormArray;
    ...
    deleteReasonGroup(indexGroup) {
        this._modalService.confirmMessageHtmlContent(new MessageModel({
            ...
            message: [{key: '<p>'}, {key: 'Modal_Message__Do_You_Want_To_Delete_Group_Reason'},
                {key: '</p>'}],
    ...
    onCollapseAndExpand(event: any, index?: any) {
        const elementCollapse = this.elementCollapsed.nativeElement.querySelector(`#collapsed${index}`);
    ...
    public validateDuplicatePlaceHolderName() {
        ...
        let isFound = false;
        for (let i = 0; i < this.formSav.controls['groupReason']['controls'].length; i++) {
            isFound = false;
            for (let j = 0; j < this.formSav.controls['groupReason']['controls'].length; j++) {
                if (j === i) continue;
    ...
    private isEqualValue(i: number, j: number) {
        try {
            return this.formSav.controls['groupReason']['controls'][i].controls['PlaceHolderName']['value']
                && this.formSav.controls['groupReason']['controls'][j].controls['PlaceHolderName']['value']
                && this.formSav.controls['groupReason']['controls'][i].controls['PlaceHolderName']['value'].toLowerCase() === this.formSav.controls['groupReason']['controls'][j].controls['PlaceHolderName']['value'].toLowerCase();
        } catch(e){return false;}
    }

    private setInvalidMessage(index: number, isValid: boolean) {
        this.formSav.controls['groupReason']['controls'][index].controls['PlaceHolderName']['ErrorMessage'] = isValid ? '' : this.existedMessage;
    }

    private addDeleteGroupData(controlGroup) {
        controlGroup['IsSoftDelete'] = 1;
        Uti.removeItemInArray(this.cloneGroupReason, controlGroup, 'IdBackOfficeLettersGroups');
        this.cloneGroupReason.push(controlGroup);
    }

    addItemReason(index) {
        const control = (<FormArray>this.formSav.controls['groupReason']).at(index).get('JSONGroupItems') as FormArray;
    ...
    deleteItemReason(indexGroup, indexItem) {
        this._modalService.confirmMessageHtmlContent(new MessageModel({
            ...
            messageType: MessageModal.MessageType.error,
            message: [{key: '<p>'}, {key: 'Modal_Message__Do_You_Want_To_Delete_Item_Reason'},
                {key: '</p>'}],
            buttonType1: MessageModal.ButtonType.danger,
            callBack1: () => {
                const controlItem = (<FormArray>this.formSav.controls['groupReason']).at(indexGroup).get('JSONGroupItems') as FormArray;
    ...
    private addDeleteGroupItemData(controlGroup, indexGroup) {
        ...
        const _controlItem = _controlGroup.JSONGroupItems.find(x => x['IsSoftDelete']);
        if (!_controlItem.IdBackOfficeLettersGroupsItems) return;
        let isFound = false;
        for (let item of this.cloneGroupReason) {
            if (item.IdBackOfficeLettersGroups != _controlGroup.IdBackOfficeLettersGroups) continue;
            isFound = true;
    ...
    public onLetterTypeChanged() {
        if (this.currentLetterTypeIndex === this.letterTypeCtr.selectedIndex) return;
    ...
    private setAutoTemplate() {
        let item = this.letter.find(x => x.idValue === this.letterTypeCtr.selectedValue);
        if (!item || !item.idValue) return;
        this.isNewAutoTemplate = !!item.IsAutoGeneratedLetter;
        this.formSav.controls['typeOfAutoLetterActive'].setValue(item['IdRepTypeOfAutoLetter']);
    ...
    private getListOfTemplate(idBackOfficeLetters: string) {
        this._blockedOrderService.getListOfTemplate(idBackOfficeLetters).subscribe((response: ApiResultResponse) => {
            this._appErrorHandler.executeAction(() => {
                if (!Uti.isResquestSuccess(response)) {
                    return;
                }
                const rawData = this._datatableService.formatDataTableFromRawData(response.item.data);
    ...
    private buildFormData(response) {
        ...
        const parseResponse = JSON.parse(response.item.data[0][0].SettingColumnName);

        setTimeout(() => {
                this.formData = cloneDeep(response);
                this.formData.contentDetail = { data: [[[]], []] };
                this.formData.contentDetail.data[1] = this.widgetUtils.buildReadonlyGridFormColumns(parseResponse[1].ColumnsName, this.formData.contentDetail.data[1]);
                this.formData.contentDetail.data[1] = this.widgetUtils.buildReadonlyGridFormColumnsValue(this.dataTable.data, this.formData.contentDetail.data[1]);
        }, 200);
    }
    ...
    private makeDataForTemplateGrid(file: any) {
        if (this.dataTable.data.length) {
            ...
            row['IdBackOfficeLettersDocTemplate'] = row['IdBackOfficeLettersDocTemplate'] || this.templateDeleteRow['IdBackOfficeLettersDocTemplate'];
            row['IdSharingTreeMedia'] = row['IdSharingTreeMedia'] || this.templateDeleteRow['IdSharingTreeMedia'];
    ...
    private getGroupAndItemsByLetterType() {
        this.isLoading = true;
        this._blockedOrderService.getGroupAndItemsByLetterType(this.letterTypeCtr.selectedValue)
    ...
    changeAutoLetterActiveCheckbox(item) {
        this.formSav.controls['typeOfAutoLetterActive'].setValue(item.value.IdRepTypeOfAutoLetter);
        this.requireOneCheckboxToBeChecked = false;
    }
    ...
    buildDataForReasonGroup(data) {
        ...
        for (const value of data) {
            const indexItem = control.value.findIndex(v => v.IdBackOfficeLettersGroups === value.IdBackOfficeLettersGroups);
            if (indexItem !== -1) {
                const controlItem = (<FormArray>this.formSav.controls['groupReason']).at(indexItem).get('JSONGroupItems') as FormArray;
    ...
    public saveAllSavLetterTemplate() {
        this._blockedOrderService.saveBackOfficeLetters(this.buildSavLetterTemplate())
            .subscribe((response: ApiResultResponse) => {
                this._appErrorHandler.executeAction(() => {
                    if (!Uti.isResquestSuccess(response)) {
                        return;
    ...
    public saveTemplateName(mode: string) {
        ...
        this._blockedOrderService.saveBackOfficeLetters(this.buildSavingDataForLetterType(mode))
            .subscribe((response: ApiResultResponse) => {
                this._appErrorHandler.executeAction(() => {
                    if (!Uti.isResquestSuccess(response)) {
                        return;
    ...
    public previewClickHandler(rowData: any) {
        if (!this.letterTypeCtr || !this.letterTypeCtr.selectedValue) {
            this._modalService.warningMessage([{key: 'Modal_Message__Please_Select_Letter_Type'}]);
            return;
        }
        if (!this.isNewAutoTemplate && (!this.formSav.controls['groupReason'].value || !this.formSav.controls['groupReason'].value.length)) {
            this._modalService.warningMessage([{key: 'Modal_Message__Please_Input_Reason'}]);
            return;
        }
        if (this.hasDuplicatePlaceholderName) {
            this._modalService.warningMessage([{key: 'Modal_Message__Please_Dont_Input_Duplicate_Placeholder_Name'}]);
            return;
        }
        if (this.isNewAutoTemplate) {
        ...
        Uti.executeFunctionWithTimeout(
            () => {
                this.sendLetterDialogComponent.callShowPreviewDialog(this.showPreviewDialogData);
            },
            () => {
    ...
    public downloadClickHandler(rowData: any) {
        ...
        const relativePath = (() => {
            try {
                return (rowData['MediaRelativePath'].indexOf(uploadFolder) > -1) ? rowData['MediaRelativePath'] : (uploadFolder + '\\' + rowData['MediaRelativePath']);
            } catch(e) {return '';}
        })();

        this._downloadFileService.makeDownloadFile(relativePath + '\\' + rowData['MediaName'],
    ...
    public refreshLetterType(func?: Function) {
        setTimeout(() => {
            const _temp = this.cachedLetter.filter(x => (
                (this.formSav.value['savChecked'] && this.formSav.value['autoChecked']) ? true
                : ((this.formSav.value['savChecked'] && !this.formSav.value['autoChecked']) ? !x.IsAutoGeneratedLetter
                : ((!this.formSav.value['savChecked'] && this.formSav.value['autoChecked']) ? x.IsAutoGeneratedLetter
                : false))
            ));
    ...
    private getAssignmentWidgetByLetterTypeId(idBackOfficeLetter?: string) {
        this._blockedOrderService.getAssignWidgetByLetterTypeId(idBackOfficeLetter).take(1)
            .subscribe((response: ApiResultResponse) => {
                this._appErrorHandler.executeAction(() => {
                    try {
                        ...
                        for (const value of this.assignmentWidget) {
                            const controlItem = (<FormArray>this.formSav.controls['assignmentWidget']) as FormArray;
    ...
    private getAllTypeOfAutoLetter() {
        this._blockedOrderService.getAllTypeOfAutoLetter()
            .subscribe((response: ApiResultResponse) => {
                this._appErrorHandler.executeAction(() => {
                    try {
    ...
    private getLetterType(mandants: string) {
        this._blockedOrderService.getLetterTypeByMandant(mandants).take(1)
            .subscribe((response: ApiResultResponse) => {
                this._appErrorHandler.executeAction(() => {
                    let data: any = [];
                    try {
    ...
    private getSaveCountriesCountries(newCountries: any[], oldCountries: any[]) {
        ...
        const newCountriesActive = newCountries.filter(x => x.IsActive || x.isActive);
        const oldCountriesActive = oldCountries.filter(x => x.IsActive || x.isActive).map(x => {
            return {
        ...
        const newItems = Uti.getItemsDontExistItems(newCountriesActive, oldCountriesActive, 'idValue').map(x => {
            return {
        ...
        const deleteItems = Uti.getItemsDontExistItems(oldCountriesActive, newCountriesActive, 'idValue').map(x => {
            return {
                IdBackOfficeLettersCountries: x.idBackOfficeLettersCountries,
        ...
        for (let cloneItem of cloneArrayObj) {
            ...
            for (let groupItem of groupReasons) {
                if (cloneItem.PlaceHolderName === groupItem.PlaceHolderName) {
                    groupItem.JSONGroupItems = [...cloneItem.JSONGroupItems, ...groupItem.JSONGroupItems];
                    isFound = true;
                }
            }
            if (isFound) continue;
    ...
    private buildSavLetterTemplate() {
        ...
        const result: any = {
            BackOfficeTemplate: {
                LetterName: (this.templateName || this.letterTypeCtr.text).replace('[Auto Letter] ', ''),
                IsActive: 1,
                IdBackOfficeLetters: this.letterTypeCtr.selectedValue,
                IsDeleted: 0
            },
            Mandant: mandant,
            Countries: this.getSaveCountriesCountries(this._countriesOutput, this.countryCheckListData),
            Groups: this.cloneGroupReason && this.cloneGroupReason.length > 0 ? this.mergeGroupReason(this.cloneGroupReason) : this.formSav.value.groupReason,
        ...
        if (!this.isNewAutoTemplate) {
            ...
            let item = this.typeOfAutoLetter.find(x => x.IdRepTypeOfAutoLetter == this.formSav.value['typeOfAutoLetterActive']);
            if (item && item.IdRepTypeOfAutoLetter) {
                result.BackOfficeTemplate['IdRepTypeOfAutoLetter'] = item.IdRepTypeOfAutoLetter;
    ...
    private getCountries(idBackOfficeLetter?: any) {
        this.isLoading = true;
        this._blockedOrderService.getCountriesLanguageByLetterTypeId(idBackOfficeLetter).take(1)
            .subscribe(response => {
                this._appErrorHandler.executeAction(() => {
    ...
    private makeTemplateDataWhenAddNew(savedResponse: any) {
        ...
        temp.push({
            ...
            textValue: (this.isNewAutoTemplate ? '[Auto Letter] ' : '') + this.templateName,
            IsAutoGeneratedLetter: this.isNewAutoTemplate,
            IdRepTypeOfAutoLetter: this.formSav.controls['typeOfAutoLetterActive'].value,
        });
    ...
    private makeTemplateDataWhenAddEdit() {
        const temp = cloneDeep(this.cachedLetter);
        const currentItem = temp.find(x => x.idValue == this.letterTypeCtr.selectedValue);
        if (!currentItem || !currentItem.idValue) return;
        currentItem.textValue = (this.isNewAutoTemplate ? '[Auto Letter] ' : '') + this.templateName;
        const currentIndex = Uti.getIndexOfItemInArray(temp, this.letterTypeCtr.selectedItem, 'idValue');
        this.letter.length = 0;
        this.cachedLetter = cloneDeep(temp);
        this.refreshLetterType(() => {
            setTimeout(() => {
    ...
    private toggleSaveButtonMode(isToggle: boolean, isEnable?: boolean) {
        if (this.parentInstance.widgetMenuStatusComponent && this.parentInstance.widgetMenuStatusComponent.settings) {
            ...
            this.parentInstance.widgetMenuStatusComponent.settings.btnSaveSAVTemplateForm.enable = isEnable;
        }
    }
