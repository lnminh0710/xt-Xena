    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    private reSetOrderNumber(bagName: string, arr: Array<any>) {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            if (
                bagName !== "group" &&
                arr[i].get("JSONGroupItems").length > 1
            ) {
    ...
    public submit(callBack?: Function) {
        ...
        const group = (<FormArray>this.formSav.controls["groupReason"])
            .controls;
        ...
        if (this.formSav.invalid && this.formSav.dirty) {
            for (const v of group) {
                ...
                if (
                    ...
                    for (const data of v["controls"]["JSONGroupItems"]
                        .controls) {
                        if (
    ...
    createItemReason(data?: any): FormGroup {
        ...
        return this.fb.group({
            TypeOfChoice: new FormControl(
                +data.TypeOfChoice || +data.TypeOfChoice == 0
                    ? +data.TypeOfChoice
                    : 1
            ),
    ...
    addReasonGroup() {
        const control = <FormArray>this.formSav.controls["groupReason"];
        control.push(
            this.createGroupReason({ GroupOrderBy: control.length + 1 })
        );
        const group = (<FormArray>this.formSav.controls["groupReason"])
            .controls;
        this.requireGroupReasonAdd(group);
            ...
            new MessageModel({
                ...
                callBack1: () => {
                    const control = <FormArray>(
                        this.formSav.controls["groupReason"]
                    );
                    const controlGroup = cloneDeep(
                        control.controls[indexGroup].value
                    );
    ...
    onCollapseAndExpand(event: any, index?: any) {
        const elementCollapse =
            this.elementCollapsed.nativeElement.querySelector(
                `#collapsed${index}`
            );
        ...
        try {
            return (
                ...
                this.formSav.controls["groupReason"]["controls"][i].controls[
                    ...
                    this.formSav.controls["groupReason"]["controls"][
                        j
                    ].controls["PlaceHolderName"]["value"].toLowerCase()
            );
        } catch (e) {
            return false;
        }
    ...
    addItemReason(index) {
        const control = (<FormArray>this.formSav.controls["groupReason"])
            ...
            .get("JSONGroupItems") as FormArray;
        control.push(
            this.createItemReason({ ItemOrderBy: control.length + 1 })
        );
    }
            ...
            new MessageModel({
                ...
                callBack1: () => {
                    const controlItem = (<FormArray>(
                        ...
                        .at(indexGroup)
                        .get("JSONGroupItems") as FormArray;
                    controlItem.controls[indexItem]
                        .get("IsSoftDelete")
                        .setValue(1);
                    const controlGroup = <FormArray>(
                        this.formSav.controls["groupReason"]
                    );
    ...
    private addDeleteGroupItemData(controlGroup, indexGroup) {
        const _controlGroup = cloneDeep(
            controlGroup.controls[indexGroup].value
        );
        ...
        this._blockedOrderService
            ...
            .subscribe((response: ApiResultResponse) => {
                this._appErrorHandler.executeAction(() => {
                    ...
                    const rawData =
                        this._datatableService.formatDataTableFromRawData(
                            response.item.data
                        );
                    this.templateDeleteRow = {};
                    this.dataTable =
                        this._datatableService.buildDataSource(rawData);
    ...
    buildDataForReasonGroup(data) {
        ...
        for (const value of data) {
            const indexItem = control.value.findIndex(
                (v) =>
                    v.IdBackOfficeLettersGroups ===
                    value.IdBackOfficeLettersGroups
            );
            if (indexItem !== -1) {
                const controlItem = (<FormArray>(
                    this.formSav.controls["groupReason"]
    ...
    public onMandantChange() {
        ...
        const mandants = this.mandant.checkedItems
            .map((v) => v.idValue)
            .join(",");
        if (!mandants) {
            this.letter = [];
        ...
        if (this._countriesOutput.length) {
            ...
            for (let i = 0; i < this.countryCheckListData.length; i++) {
                if (this.countryCheckListData[i]["IsActive"]) {
                    result +=
                        this.countryCheckListData[i]["IdCountrylanguage"] + ",";
                }
            }
        }
        if (!!result) {
    ...
    public deleteClickHandler(rowData: any) {
        this._modalService.confirmDeleteMessageHtmlContent({
            ...
            message: [
                { key: "Modal_Message__Are_You_Sure_You_Want_To_Delete_File" },
            ],
            callBack1: () => {
    ...
    private getSaveCountriesCountries(
        newCountries: any[],
        oldCountries: any[]
    ) {
            ...
            .map((x) => {
                return {
                    idBackOfficeLettersCountries:
                        x.IdBackOfficeLettersCountries,
    ...
    private buildSavLetterTemplate() {
        ...
        const result: any = {
            BackOfficeTemplate: {
                LetterName: (
                    this.templateName || this.letterTypeCtr.text
                ).replace("[Auto Letter] ", ""),
                IsActive: 1,
                IdBackOfficeLetters: this.letterTypeCtr.selectedValue,
                IsDeleted: 0,
        ...
        if (!this.isNewAutoTemplate) {
            const assignmentWidget = this.formSav.value.assignmentWidget.map(
                (v) => {
                    return {
                        IdBackOfficeLettersAssignWidgetRep: v.idBackOffice,
                        IdBackOfficeLettersAssignWidget:
                            v.IdBackOfficeLettersAssignWidget,
    ...
    private renameTemplate() {
        ...
        if (this.letterTypeCtr) {
            this.templateName = this.letterTypeCtr.text.replace(
                "[Auto Letter] ",
                ""
            );
        } else {
    ...
    private makeTemplateDataWhenAddNew(savedResponse: any) {
        ...
        temp.push({
            ...
            textValue:
                (this.isNewAutoTemplate ? "[Auto Letter] " : "") +
                this.templateName,
    ...
    private buildSavingDataForLetterType(mode: string) {
        ...
        switch (mode) {
            case "New":
                result = {
                    BackOfficeTemplate: {
                        LetterName: this.templateName.replace(
                            "[Auto Letter] ",
                            ""
                        ),
                        IsActive: 1,
            ...
            case "Edit":
                result = {
                    BackOfficeTemplate: {
                        ...
                        IsDeleted: 0,
                        IdRepTypeOfAutoLetter:
                            this.formSav.controls["typeOfAutoLetterActive"]
                                .value,
                    },
                };
                break;
