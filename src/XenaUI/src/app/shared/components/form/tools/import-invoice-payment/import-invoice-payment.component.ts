...
import {
    ...
    MessageModal, UploadFileMode, ComboBoxTypeConstant, Configuration
} from 'app/app.constants';
import { SignalRNotifyModel, MessageModel, ApiResultResponse, Module } from 'app/models';
...
export class ImportInvoicePaymentComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    @ViewChild('importInvoicePaymentGrid') importInvoicePaymentGrid: XnAgGridComponent;
    ...
    private getInitDropdownlistData() {
        this.commonService.getListComboBox(ComboBoxTypeConstant.paymentProvider)
    ...
    private signalRGetData() {
        this.sendMessage(SignalRActionEnum.ImportInvoicePayment_GetProcessingList);
    }

    private signalRListenMessage() {
        if (this.messageImportSub) this.messageImportSub.unsubscribe();

        this.messageImportSub = this.signalRService.messageImportInvoicePayment
                    ...
                    switch (message.Action) {
                        case SignalRActionEnum.ImportInvoicePayment_ServiceAlive:
                            if (message.JobIsProcessing) {
                                ...
                                this.toasterService.pop('warning', 'Notification', 'Data is processing please wait for a moment...');
                            }
                            else {
    ...
    private signalRGetProcessingList(message: SignalRNotifyModel) {
        ...
        for (let item of procesingList) {
            let row = this.dataSource.data.find(x => x.IdSharingTreeMedia && x.IdSharingTreeMedia == item.IdSharingTreeMedia && x.ProcessStatus != this.processStatusEnum.Success);
            if (row) {
    ...
    private signalRProcessingFile(messageModel: SignalRNotifyModel) {
        if (!messageModel || !messageModel.Data || !messageModel.Data.length) return;

        const fileSaved = messageModel.Data.filter(x => x.ImportFileStatus == this.importFileStatusEnum.Saved);
        const fileUnSaved = messageModel.Data.filter(x => x.ImportFileStatus == this.importFileStatusEnum.UnSaved);
        const fileExist = messageModel.Data.filter(x => x.ImportFileStatus == this.importFileStatusEnum.Exist);
        const fileIncorrect = messageModel.Data.filter(x => x.ImportFileStatus == this.importFileStatusEnum.Incorrect);

        const message: Array<any> = [];
        this.addMessageForFile(message, fileUnSaved, 'Modal_Message__File_Action_Files_Correct_UnSaved');
        this.addMessageForFile(message, fileSaved, 'Modal_Message__File_Action_Files_Correct_Saved');
        this.addMessageForFile(message, fileExist, 'Modal_Message__File_Action_Files_Exist');
        this.addMessageForFile(message, fileIncorrect, 'Modal_Message__File_Action_Files_Incorrect');
    ...
    private addMessageForFile(message: Array<any>, files: Array<any>, messageKey: string) {
    ...
    //#region Grid
    private getImportInvoiceFiles(message?: SignalRNotifyModel, callback?: Function) {
        this.toolsService.getImportInvoiceFiles()
            .subscribe((response: any) => {
                this.appErrorHandler.executeAction(() => {
                    if (response.data && response.data.length > 1) {
                        const data = this.datatableService.formatDataTableFromRawData(response.data);
                        this.dataSource = this.datatableService.buildDataSource(data);
    ...
    private updateStatusForGrid(data: any) {
        let row = this.dataSource.data.find(x => x.IdSharingTreeMedia && x.IdSharingTreeMedia == data.IdSharingTreeMedia);
        if (!row) return;
    ...
    public startStop() {
        if (!this.importStatus) {
            ...
            this.modalService.confirmMessageHtmlContent(new MessageModel({
                ...
                message: [{ key: '<p>' }, { key: 'Modal_Message__Do_You_Want_Stop_Import_Invoice_Payment' }, { key: '</p>' }],
    ...
    private start() {
        if (this.isCallStart) {
            this.toasterService.pop('info', 'SignalR', 'Bus connection is connecting...');
            return;
        }

        //Only check for the Import Action
        if (!this.isCallStartSavingFiles) {
            const items = this.dataSource.data.filter(x => x.IdSharingTreeMedia && x.ProcessStatus == this.processStatusEnum.Idle).slice(0, this.maxNumOfFiles);
            if (!items || !items.length) {
                this.modalService.warningText('Modal_Message__Upload_Files_Process');
                return;
            }
        }
    ...
    private createQueueAndStart() {
        ...
        if (this.isCallStartSavingFiles) {
            this.sendMessage(SignalRActionEnum.ImportInvoicePayment_ImportFile, this.tempUploadedItems);
            ...
            const items = this.dataSource.data.filter(x => x.IdSharingTreeMedia && x.ProcessStatus == this.processStatusEnum.Idle).slice(0, this.maxNumOfFiles);;
            if (items && items.length) {
                this.sendMessage(SignalRActionEnum.ImportInvoicePayment_Start, items);
            }
        }
    }
    ...
    public onProviderChanged($event) {
        ...
        this.uploadFileIdFolder = Uti.replaceSpecialCharacterOfFile($event.textValue) + '/' + new Date().getFullYear() + '/' + (('0' + (new Date().getMonth() + 1)).slice(-2));
    }

    public uploadFileOpenDialog() {
        if (!this.providerNameCtrl && !this.providerNameCtrl.selectedValue) {
            this.modalService.warningText('Import_Invoice_Payment__Choose_ProviderName');
            return;
        }
    ...
    public uploadFileCloseDialog() {
        ...
        // -> will call 'start'
        if (this.tempUploadedItems && this.tempUploadedItems.length && !this.importStatus) {
            this.start();
        }
    }
