...
import {
    ...
    ToolsService, ModalService, DownloadFileService
} from 'app/services';
import { ToasterService, BodyOutputType } from 'angular2-toaster/angular2-toaster';
import {
    ComboBoxTypeConstant, UploadFileMode, Configuration, MessageModal, FormSaveEvenType
...
export class MailingReturnComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    private getInitDropdownlistData() {
        this.commonService.getListComboBox(ComboBoxTypeConstant.mailingReturnReason)
    ...
    private initForm() {
        ...
        this.mailingReturnFormValueChangesSubscription = this.mailingReturnForm.valueChanges
            .debounceTime(this.consts.valueChangeDeboundTimeDefault)
            .subscribe((value) => {
                this.appErrorHandler.executeAction(() => {
    ...
    public downloadTemplateFile() {
        this.downloadFileService.makeDownloadFile('ImportMailingReturnTemplate.xlsx', null, null, 4);
    }
    ...
    public uploadFileOnComplete(event) {
        ...
        if (event && event.response && event.response.item) {
            ...
            if (resItem.messageException) {
                this.toasterService.pop('error', 'Import Failed with exception', resItem.messageException);
                return;
            }
            ...
            else if (resItem.numofRowsValid && resItem.saveResult && resItem.saveResult.isSuccess) {
                if (resItem.numofRowsInvalid) {
                    //save successfully hower there are 'x' items failed
                    this.toasterService.pop({
                        type: 'success',
                        title: 'Partial Success',
                        body: 'Import Mailing Return successfully with ' + resItem.numofRowsValid + ' valid item(s) and ' + resItem.numofRowsInvalid + ' invalid item(s)',
                        bodyOutputType: BodyOutputType.TrustedHtml,
                        timeout: 10000
                    });
                }
                ...
                this.toasterService.pop('error', 'Import Failed', 'There is no row to import.');
                return;
            }
        }
        ...
        this.toasterService.pop('error', 'Import Failed', 'Import failed, please try again.');
    }
    //#endregion
    ...
    public barcodeProcess(): void {
        ...
        if (value) {
            ...
            if (splits.length === 3) {
                ...
                this.mailingReturnForm.controls['customerNr'].setValue(customer);
                this.mailingReturnForm.controls['mediaCode'].setValue(mediacode);
                this.mailingReturnForm.updateValueAndValidity();
    ...
    private processSaveData(data) {
        ...
        this.toolsService.saveMailingReturn(data)
            .finally(() => {
            ...
            .subscribe(response => {
                this.appErrorHandler.executeAction(() => {
                    if (response && response.isSuccess) {
                        ...
                        this.toasterService.pop('success', 'Success', 'Mailing Return is saved successfully');
                        this.setOutputData({
                            submitResult: true,
                        ...
                        this.setIncorrectValidatorControl('barcode', true);//valid
                        Uti.resetValueForForm(this.mailingReturnForm, ['reason']);
                        this.mailingReturnForm.markAsPristine();
                        this.mailingReturnForm.markAsUntouched();
                        this.barcodeFocus();
                    }
    ...
    private saveDataFailed() {
        this.toasterService.pop('error', 'Save Failed', 'The information of CustomerNr or Mediacode is wrong, please correct it before save?');
    ...
    private prepareSaveData() {
        ...
        if (this.reasonCtrl.selectedItem.statusType == 'IdRepPersonBusinessStatus') {
            item.IdRepPersonBusinessStatus = this.reasonCtrl.selectedItem.idValue;
        }
        else if (this.reasonCtrl.selectedItem.statusType == 'IdRepPersonStatus') {
    ...
    private setIncorrectValidatorControl(controlName: string, isValid: boolean) {
        if (!isValid)
            if (!this.mailingReturnForm) return;
