...
export class DispatcherFromStep2Component
    ...
    implements OnInit, OnDestroy, AfterViewInit
{
    ...
    public unDispatch() {
        ...
        if (!unDispatchData || !unDispatchData.length) {
            this.toasterService.pop(
                "warning",
                "Validation Failed",
                "Please select at least one item to undispatch!"
            );
            return;
        }
        const saveData = {
    ...
    private loadData() {
        this.toolsServiceSubscription = this.toolsService
            .getAllScanCenters(DispatcherMode.Dispatcher)
    ...
    private reloadPoolData(scanCenterId: any) {
        this.toolsServiceSubscription = this.toolsService
            .getScanCenterDispatcher(scanCenterId)
            .subscribe((response: any) => {
                this.appErrorHandler.executeAction(() => {
                    if (!response || !response.data) {
                        this.resetDataPoolGrid();
    ...
    private updateDataGridData() {
        const newData = this.poolRightData.data.filter((x) => {
            return !x.select;
        });
        this.poolRightData = {
            data: newData,
