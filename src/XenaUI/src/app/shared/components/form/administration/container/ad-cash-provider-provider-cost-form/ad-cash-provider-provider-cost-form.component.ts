...
import {PersonService, PropertyPanelService, AppErrorHandler, WidgetTemplateSettingService} from 'app/services';
...
export class AdCashProviderProviderCostFormComponent extends BaseComponent implements OnInit, OnDestroy {
        ...
        'Request':
            {
                ...
                'Data':
                    '{"MethodName" : "SpAppWg002GetGridProviderCosts", "CrudType"  : null, "Object" : null,"Mode" :  null, "WidgetTitle" : "ProviderCosts List...", "IsDisplayHiddenFieldWithMsg" : "1",<<LoginInformation>>,<<InputParameter>>}'
            }
    };
        ...
        this.widgetTemplateSettingServiceSubscription = this.widgetTemplateSettingService.getWidgetDetailByRequestString(widgetDetail, {IdPerson: this.idPerson})
            .subscribe((response: WidgetDetail) => {
                this.appErrorHandler.executeAction(() => {
                    ...
                    this.paymentCostTypeLabel = data['__DefaultValue'].ColumnHeader;
                    this.paymentTypeLabel = data['_DefaultValue'].ColumnHeader;
    ...
    private initDataForForm() {
        if (!this.data || this.isRenderForm || !this.listComboBox) { return; }
    ...
    private updateFormMainValue() {
        setTimeout(() => {
            if (this.adCashProviderPCFormValueChangesSubscription) this.adCashProviderPCFormValueChangesSubscription.unsubscribe();

            this.adCashProviderPCFormValueChangesSubscription = this.adCashProviderPCForm.valueChanges
    ...
    private getDropdownlistData() {
        this.xnComStateSubcription = this.xnComState.subscribe((xnComState: SubCommonState) => {
            this.appErrorHandler.executeAction(() => {
                if (!xnComState.listComboBox || !xnComState.listComboBox.currency) { return; }
    ...
    private regisValueChange() {
        if (this.formGroupSubscriptionPaymentType) this.formGroupSubscriptionPaymentType.unsubscribe();
        if (this.formGroupSubscriptionPaymentCostType) this.formGroupSubscriptionPaymentCostType.unsubscribe();
        if (this.formGroupSubscriptionCurrency) this.formGroupSubscriptionCurrency.unsubscribe();

        this.formGroupSubscriptionPaymentType = Uti.updateFormComboboxValue(this.adCashProviderPCForm, this.listComboBox.paymentType,
            (this.adCashProviderPCForm), 'paymentType', 'paymentTypeText');

        this.formGroupSubscriptionPaymentCostType = Uti.updateFormComboboxValue(this.adCashProviderPCForm, this.listComboBox.paymentCostType,
            (this.adCashProviderPCForm), 'paymentCostType', 'paymentCostTypeText');

        this.formGroupSubscriptionCurrency = Uti.updateFormComboboxValue(this.adCashProviderPCForm, this.listComboBox.currency,
            (this.adCashProviderPCForm), 'currency', 'currencyText');
    }

    public addToGrid($event: any) {
        this.adCashProviderPCForm['submitted'] = true;
        this.adCashProviderPCForm.updateValueAndValidity();
        if (!this.adCashProviderPCForm.valid) { return; }
        ...
        currentGridData.push({
            ...
            'providerTypeText': this.adCashProviderPCForm.value.paymentCostTypeText,
        ...
        this.paymentCostTypeData = { data: cloneDeep(currentGridData), columns: this.createGridColumns() };
        currentGridData = {};
    ...
    private setCurrentPayemtType() {
        if (!isEmpty(this.currentPayemtType)) { return; }
        this.adCashProviderPCForm.updateValueAndValidity();
    ...
    public setChosenValueForPaymentType() {
        (<any>(<any>this.adCashProviderPCForm.controls).paymentType).setValue(this.currentPayemtType.paymentType);
    }

    public setDefaultValueForActive() {
        (<any>(<any>this.adCashProviderPCForm.controls).isActive).setValue(true);
    }
    ...
    private createProviderCost() {
        this.personServiceSubscription = this.personService.createCostProvider(this.prepareSubmitCreateData())
            .subscribe(
                (data) => {
                    this.appErrorHandler.executeAction(() => {
                        this.setOutputData(false, {
                            submitResult: true, formValue: this.adCashProviderPCForm.value,
    ...
    private setOutputData(submitResult: any, data?: any) {
        if ((typeof data) !== 'undefined') {
            ...
            this.outputModel = {
                submitResult: submitResult, formValue: this.adCashProviderPCForm.value,
                isValid: this.adCashProviderPCForm.valid, isDirty: this.adCashProviderPCForm.dirty
            };
        }
        this.outputData.emit(this.outputModel);
    }
