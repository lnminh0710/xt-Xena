import { Component, Output, Input, EventEmitter, OnDestroy, ViewChild } from '@angular/core';
...
export class AdContactMainFieldComponent implements OnDestroy {
    ...
    set initInformation(information: any) {
        if (information && information.data && information.listComboBox && information.parentFormGroup
            ...
            if (!information.parentFormGroup.contains('adminContactMainFieldForm'))
                information.parentFormGroup.addControl('adminContactMainFieldForm', this.adminContactMainFieldForm);
            else
                information.parentFormGroup.controls['adminContactMainFieldForm'] = this.adminContactMainFieldForm;

            this.initFormGroup.emit({ form: this.adminContactMainFieldForm, name: 'adminContactMainFieldForm' });
            Uti.setValidatorForForm(this.adminContactMainFieldForm, this.mandatoryCheckData);
            this.isRenderForm = true;
        }
    }

    @Output() initFormGroup: EventEmitter<FormGroupChild> = new EventEmitter();
    @Output() contactAddressChanged: EventEmitter<FormGroupChild> = new EventEmitter();
    ...
    constructor(
        ...
        this.adminContactMainFieldForm.controls['idRepContactAddressType'].markAsPristine();
    ...
    public createContactAddressType(originalContactTypeList: Array<any>): Array<any> {
    ...
    public onIdRepContactAddressTypeChanged() {
        if (!this.idRepContactAddressTypeCombobox.checkedItems.length) {
            this.adminContactMainFieldForm.controls['idRepContactAddressType'].setValidators(Validators.required);
            this.adminContactMainFieldForm.controls['idRepContactAddressType'].setErrors({ 'required': true });
        } else {
            this.adminContactMainFieldForm.controls['idRepContactAddressType'].clearValidators();
            this.adminContactMainFieldForm.controls['idRepContactAddressType'].setErrors(null);
        }
