...
import { ApiResultResponse, WidgetDetail } from "app/models";
...
export class PaymentAccountFormComponent
    ...
    implements OnInit, OnDestroy
{
    ...
    private REQUEST_STRING = {
        Request: {
            ...
            Data: '{"MethodName" : "SpAppWg002CashProviderGrid", "CrudType"  : null, "Object" : null,"Mode" :  null, "WidgetTitle" : "Cash Provider", "IsDisplayHiddenFieldWithMsg" : "1",<<LoginInformation>>,<<InputParameter>>}',
        },
    };
    ...
    public ngOnInit() {
        ...
        this.paymentAccountFormValueChangesSubscription =
            this.paymentAccountForm.valueChanges
                    ...
                    this.appErrorHandler.executeAction(() => {
                        setTimeout(() => {
                            if (!this.paymentAccountForm.pristine) {
                                this.outputModel = {
                                    submitResult: null,
                                    formValue: this.paymentAccountForm.value,
                                    isValid: this.paymentAccountForm.valid,
                                    isDirty: this.paymentAccountForm.dirty,
                                };
            ...
            this.widgetTemplateSettingService
                ...
                .subscribe((response: WidgetDetail) => {
                    this.appErrorHandler.executeAction(() => {
                        ...
                        this.paymentsTermsType =
                            data["DefaultValue"].ColumnHeader;
                        this.countryCode = data["__DefaultValue"].ColumnHeader;
                        this.paymentsTermsName =
                            data.PaymentsTermsName.ColumnHeader;
                    });
                });
    }
    ...
    public onSubmit(event?: any) {
        ...
        try {
            ...
            this.personServSubscription = this.personServ
                .createPaymentAccount(this.prepareSubmitCreateData())
                .subscribe(
                    (data) => {
                        this.appErrorHandler.executeAction(() => {
                            this.outputModel = {
    ...
    private onDeletedRowsSuccess($event: any) {
        ...
        this.outputModel = {
            submitResult: null,
            formValue: this.paymentAccountForm.value,
            isValid: this.paymentAccountForm.valid && !this.gridError,
            isDirty: isDirty,
            returnID: null,
        };
        this.paymentAccountForm.markAsDirty();
    ...
    private mapGridItemToSaveData(): any {
        ...
        const addedItem = updatedItems.itemsAdded.filter(
            (v) =>
                v.DT_RowId.includes("newrow") &&
                !v.IsDeleted &&
                !v[ColHeaderKey.Delete]
        );
        const editedItem = updatedItems.itemsEdited.filter(
            (v) =>
                (!v.DT_RowId || v.DT_RowId.includes("newrow")) &&
                !v.IsDeleted &&
                !v[ColHeaderKey.Delete]
        );
        const deletedItem = updatedItems.itemsRemoved.filter(
            (v) => !v.DT_RowId || v.DT_RowId.includes("newrow")
        );
        const identiferCodes = [...addedItem, ...editedItem, ...deletedItem];
        return identiferCodes.map((x) => {
            return {
                IdRepCashProviderPaymentTermsColumns:
                    x.IdRepCashProviderPaymentTermsColumns.key,
