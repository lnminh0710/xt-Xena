  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  public prepareSubmitData() {
    ...
    const result: any = {
      ...
      PersonTypeGw: {
        IdRepPersonType:
          this.mapMenuIdToPersonTypeId[this.getSubModuleId(this.subModule)],
  ...
  private getDropdownlistData() {
    this.xnComStateSubcription = this.xnComState.subscribe(
      (xnComState: SubCommonState) => {
        this.appErrorHandler.executeAction(() => {
          if (!xnComState.listComboBox || !xnComState.listComboBox.title) {
            return;
          }
  ...
  private updateFormValue() {
    ...
    setTimeout(() => {
      ...
      this.formGroupSubscriptionAdMainFieldForm = Uti.updateFormComboboxValue(
        this.formGroup,
