...
import { DialogResourceTranslationComponent } from "app/shared/components/widget";
...
export class ResourceTranslationDirective implements OnDestroy, OnInit {
    private translationStatus: boolean;
    ...
    private gridEventHandler() {
        ...
        $(document).on(
            "DOMNodeInserted",
            ".ag-theme-balham, wj-popup, .xn-translate-text-html",
            function (e) {
                // console.log('DOMNodeInserted');
    ...
    private subscribeTranslationStatus() {
        this.enableTranslationSubscription =
            this.resourceTranslationService.translationStatus$.subscribe(
                (status) => {
        ...
        this.rebindEventSubscription =
            this.resourceTranslationService.bindEventMessage$
                .debounceTime(250)
                .subscribe((status) => {
                    if (status) {
                        this.unbindEvent();
    ...
    appendComponentToBody(evt, isMenu: boolean) {
        const languageIcon =
            '<i class="fa fa-language" style="font-size: 17px; color: #4775c7; cursor: pointer"></i>';
        ...
        domElem.style.top = isMenu
            ? domRect.top + "px"
            : domRect.top - this.popHeight - 2 + "px"; // - this.popHeight - 2 + 'px';
        ...
        domElem
            .querySelector(".fa-language")
            .addEventListener("click", this.onClick.bind(this, keyword));
    }
    ...
    private getKeyword(element: HTMLElement) {
        ...
        if (!keyword) {
            ...
            const keyTranslationElement =
                element.querySelector(".key-translation");
    ...
    public onClick(keyword, event) {
        ...
        const popoverComponent: DialogResourceTranslationComponent =
            this.componentRef.instance;
        popoverComponent.showDialog = true;
        popoverComponent.keyword = keyword;
        let value = defaultLanguage;
        popoverComponent.defaultValue =
            defaultLanguage && defaultLanguage[keyword]
        ...
        this.successSavedEventSubscription =
            popoverComponent.onSuccessSaved.subscribe(() => {
