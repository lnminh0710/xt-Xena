import { Input, Directive, EventEmitter, Output, OnDestroy, OnInit, ComponentRef, ComponentFactoryResolver, ViewContainerRef, ElementRef, Renderer2, Injector, ApplicationRef, HostListener } from '@angular/core';
...
export class ResourceTranslationDirective implements OnDestroy, OnInit {
    ...
    private gridEventHandler() {
        let that = this;
        $(document).on('DOMNodeInserted', '.ag-theme-balham, wj-popup, .xn-translate-text-html', function (e) {
            // console.log('DOMNodeInserted');
    ...
    private subscribeTranslationStatus() {
        ...
        this.rebindEventSubscription = this.resourceTranslationService.bindEventMessage$.debounceTime(250).subscribe(status => {
            if (status) {
                this.unbindEvent();
                this.registerHoverEvent();
                this.formatLabelStyle(status);
