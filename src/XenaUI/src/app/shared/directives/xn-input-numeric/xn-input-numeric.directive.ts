...
export class XnInputNumericDirective
  implements OnDestroy, ControlValueAccessor
{
  @Input() typeNumber = InputTypeNumber.Decimal; // 3 type: integer, decimal, Positive
  ...
  constructor(
    ...
    renderer.setElementStyle(elementRef.nativeElement, 'text-align', 'left');
    renderer.setElementStyle(elementRef.nativeElement, 'padding-right', '15px');
  }
  ...
  @HostListener('keydown.out-zone', ['$event']) onKeyDown(
    event: KeyboardEvent
  ) {
    ...
    const numberNotComma = this.elementRef.nativeElement.value.replace(
      /,/g,
      ''
    );
    this.previousValue =
      this.checkRegexExpression(numberNotComma) &&
      numberNotComma.length <= this.maxLength
  ...
  @HostListener('keyup.out-zone', ['$event']) onKeyUp(event: any) {
    ...
    if (value) {
      ...
      let raw = this.reverseFormatNumber(
        this.elementRef.nativeElement.value,
        'en'
      );
      this.onChange(raw);
    ...
    try {
      if (value) {
        ...
        if (
          ...
          formatted = this.formatDecimalNumber(
            numberNotComma,
            cursorPositionStart,
            value,
            true
  ...
  private formatDecimalNumber(
    numberNotComma: number,
    cusorPositionStart: number,
    value: any,
    isValid: boolean
  ) {
    let formatted: any;
    ...
    if (parts[1]) {
      formatted =
        this.format === 'N' &&
        this.isExistDotOrMinus(this.elementRef.nativeElement.value)
          ? parts[1].slice(-1) == '0'
      ...
      parts[0] =
        this.format === 'N' && this.isExistDotOrMinus(parts[0])
  ...
  private checkMinMaxNumber(numberNotComma: number): number {
    if (numberNotComma + '' === '') return numberNotComma;
    numberNotComma =
      !isNaN(Number(this.min)) && numberNotComma < this.min
        ? this.min
        : numberNotComma;
    numberNotComma =
      !isNaN(Number(this.max)) && numberNotComma > this.max
        ? this.max
        : numberNotComma;
    return numberNotComma;
  }

  private setTextCursorPosition(
    oldValue: any,
    ...
    if (isValid) {
      const quantityCommaBeforeCursor = this.countQuantityComma(
        oldValue,
        currentPosition
      );
      const currentQuantityCommaBeforeCursor = this.countQuantityComma(
        this.elementRef.nativeElement.value,
        currentPosition
      );
      newPosition +=
        currentQuantityCommaBeforeCursor - quantityCommaBeforeCursor;
    } else {
      newPosition -=
        oldValue.length - this.elementRef.nativeElement.value.length;
    }
  ...
  private countQuantityComma(value: string, cusorPosition: number): number {
    return value.substring(0, cusorPosition).match(/,/g)
      ? value.substring(0, cusorPosition).match(/,/g).length
      : 0;
  }

  private checkRegexExpression(text: string): boolean {
    const regExp =
      this.typeNumber === InputTypeNumber.Decimal
        ? this.regexDecimal
        : this.regexInteger;
  ...
  private allowControlKey(e: any): boolean {
    ...
    const listAllowKeyCode =
      this.typeNumber === InputTypeNumber.Decimal
        ? [46, 8, 9, 27, 13, 110, 190, 109, 189]
        : this.typeNumber === InputTypeNumber.Integer
        ? [46, 8, 9, 27, 13, 109, 189]
        : [46, 8, 9, 27, 13, 110, 190, 189];
    return (
      listAllowKeyCode.indexOf(e.keyCode) !== -1 ||
