import { Directive, HostListener, Input, ElementRef, OnDestroy, Renderer, Output, EventEmitter, Self, Optional, forwardRef, Renderer2 } from '@angular/core';
import { DecimalPipe } from '@angular/common';
import { InputTypeNumber } from 'app/models/input-numeric-type.enum';
import { NgControl, ControlValueAccessor, NG_VALUE_ACCESSOR, NgModel } from '@angular/forms';
...
export class XnInputNumericDirective implements OnDestroy, ControlValueAccessor {
    ...
    @Input() set textAlign(textAlign: string) {
        ...
        this.renderer.setElementStyle(this.elementRef.nativeElement, 'text-align', value);
    }
    ...
    constructor(
        ...
        renderer.setElementStyle(elementRef.nativeElement, 'border', 'solid 1px #d2d6de');
    ...
    private checkRegexExpression(text: string): boolean {
        const regExp = this.typeNumber === InputTypeNumber.Decimal ? this.regexDecimal : this.regexInteger
    ...
    private allowControlKey(e: any): boolean {
        ...
        const listAllowKeyCode = this.typeNumber === InputTypeNumber.Decimal
            ...
            :this.typeNumber === InputTypeNumber.Integer ? [46, 8, 9, 27, 13, 109, 189]:
