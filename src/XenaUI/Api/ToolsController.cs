...
namespace XenaUI.Api
{
    ...
    public class ToolsController : BaseController
    {
        ...
        public ToolsController(IToolsBusiness toolsBusiness, ICommonBusiness commonBusiness)
        {
        ...
        #region MailingReturn
        [HttpPost]
        [Route("SaveMailingReturn")]
        public async Task<object> SaveMailingReturn([FromBody] IList<MailingReturnSaveItem> model)
        {
            return await _toolsBusiness.SaveMailingReturn(model);
        }

        [HttpPost]
        [Route("ResetMailingReturn")]
        public async Task<object> ResetMailingReturn()
        {
            return await _toolsBusiness.ResetMailingReturn();
        }

        [HttpGet]
        [Route("GetMailingReturnSummary")]
        public async Task<object> GetMailingReturnSummary()
        {
            return await _toolsBusiness.GetMailingReturnSummary();
        }

        [HttpGet]
        [Route("ExportCustomerAndBusinessStatus")]
        public async Task<object> ExportCustomerAndBusinessStatus(string customerStatusIds, string businessStatusIds)
        {
            return await _toolsBusiness.ExportCustomerAndBusinessStatus(customerStatusIds, businessStatusIds);
        }
        #endregion

        #region ImportInvoicePayment
        [HttpGet]
        [Route("GetImportInvoiceFiles")]
        public async Task<object> GetImportInvoiceFiles()
        {
            return await _toolsBusiness.GetImportInvoiceFiles();
        }
        #endregion

        [HttpPost]
        [Route("CallbackSP")]
        public async Task<object> CallbackSP([FromBody] Dictionary<string, object> model)
        {
            if (model == null) return StatusCode(400);
            //if (string.IsNullOrEmpty(model.SessionId)) return StatusCode(400);
            
            var result = _commonBusiness.CallBackToDBByDynamicSP(model);
