...
namespace XenaUI.Business
{
    public interface IToolsBusiness
    {
        ...
        Task<WSEditReturn> SaveMailingReturn(IList<MailingReturnSaveItem> items, string importFileName = null);
        Task<object> ResetMailingReturn();
        Task<object> GetMailingReturnSummary();
        Task<MailingReturnImportFileResult> ImportFileMailingReturn(MailingReturnImportFileModel model);

        Task<object> ExportCustomerAndBusinessStatus(string customerStatusIds, string businessStatusIds);
        #endregion

        #region ImportInvoicePayment
        Task<object> GetImportInvoiceFiles();
        #endregion
