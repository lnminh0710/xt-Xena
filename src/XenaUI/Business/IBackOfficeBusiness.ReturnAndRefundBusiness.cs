...
namespace XenaUI.Business
{
    public partial class BackOfficeBusiness : BaseBusiness, IBackOfficeBusiness
    {
        ...
        public async Task<WSEditReturn> SaveWidgetData(ReturnRefundSaveModel model)
        {
            ...
            if (model != null && model.NewInvoiceItemData != null && model.NewInvoiceItemData.Count > 0)
            {
                var _jSON = JsonConvert.SerializeObject(model.NewInvoiceItemData,
                                        Newtonsoft.Json.Formatting.None,
                                        new JsonSerializerSettings
                                        {
                                            //Formatting = Formatting.Indented,
                ...
                data.JSONNewInvoice = string.Format(@"""NewInvoice"":{0}", _jSON);
                data.JSONNewInvoice = string.Format("{{{0}}}" ,data.JSONNewInvoice);
            }
        ...
        public async Task<WSEditReturn> SaveDeleteOrder(string idSalesOrder)
        {
            ReturnRefundData data = (ReturnRefundData)ServiceDataRequest.ConvertToRelatedType(typeof(ReturnRefundData));
            data.IdSalesOrder = idSalesOrder;

            var result = await _backOfficeService.SaveDeleteOrder(data);
